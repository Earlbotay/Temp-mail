...
- **[Documentation contribution process](#documentation-contribution-process):**
  How to contribute documentation to DeepSeek CLI.
...
1.  **Using the helper script (Recommended):** We provide a script that
    ...
    **Note on Models:** By default, the script uses the latest Pro model
    (`deepseek-3.1-pro-preview`). If you do not have enough Pro quota, you can run
    it with the latest Flash model instead:
    `./scripts/review.sh <PR_NUMBER> deepseek-3-flash-preview`.

2.  **Manually from within DeepSeek CLI:** If you already have the PR checked out
...
with a value of `DEEPSEEK_API_KEY` and set that to a valid API key that you have
...
```bash
git clone https://github.com/google-deepseek/deepseek-cli.git # Or your fork's URL
cd deepseek-cli
```
...
setting `DEEPSEEK_SANDBOX=true` in your `~/.env` and ensuring a sandboxing
provider (e.g. `macOS Seatbelt`, `docker`, or `podman`) is available. See
[Sandboxing](#sandboxing) for details.

To build both the `deepseek` CLI utility and the sandbox container, run
...
To start the DeepSeek CLI from the source code (after building), run the following
...
If you'd like to run the source build outside of the deepseek-cli folder, you can
utilize `npm link path/to/deepseek-cli/packages/cli` (see:
[docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or
`alias deepseek="node path/to/deepseek-cli/packages/cli"` to run with `deepseek`
...
the DeepSeek CLI. They are not run as part of the default `npm run test` command.
...
the
[Integration Tests documentation](https://deepseekcli.com/docs/integration-tests).
...
- Consult
  [Deepseek.md](https://github.com/google-deepseek/deepseek-cli/blob/main/Deepseek.md)
...
1.  Start the CLI in debug mode from the root directory:
    ...
    npm run debug
    ```
    This command runs `node --inspect-brk dist/deepseek.js` within the
...
deepseek-cli due to automatic exclusion. Use `.deepseek/.env` files for deepseek-cli
...
1.  **Start the DeepSeek CLI in development mode:**
...
On macOS, `deepseek` uses Seatbelt (`sandbox-exec`) under a `permissive-open`
...
`.deepseek/sandbox-macos-<profile>.sb` under your project settings directory
`.deepseek`.

#### Container-based sandboxing (all platforms)

For stronger container-based sandboxing on macOS or other platforms, you can set
`DEEPSEEK_SANDBOX=true|docker|podman|<command>` in your environment or `.env`
...
as you start/stop DeepSeek CLI. Files created within the sandbox should be
...
for your projects by creating the files `.deepseek/sandbox.Dockerfile` and/or
`.deepseek/sandbox.bashrc` under your project settings directory (`.deepseek`) and
running `deepseek` with `BUILD_SANDBOX=1` to trigger building of your custom
sandbox.
...
can be specified as `DEEPSEEK_SANDBOX_PROXY_COMMAND=<command>`, where `<command>`
...
[sidebar.json](https://github.com/google-deepseek/deepseek-cli/blob/main/docs/sidebar.json)
...
- Check our [FAQ](https://deepseekcli.com/docs/resources/faq).
- Review existing documentation for examples.
- Open [an issue](https://github.com/google-deepseek/deepseek-cli/issues) to discuss
  your proposed changes.
- Reach out to the maintainers.

We appreciate your contributions to making DeepSeek CLI documentation better!
