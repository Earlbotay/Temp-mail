...
- **Purpose:** Provide a seamless terminal interface for DeepSeek models,
...
- **Architecture:** Monorepo structure using npm workspaces.
  - `packages/cli`: User-facing terminal UI, input processing, and display
    rendering.
  - `packages/core`: Backend logic, DeepSeek API orchestration, prompt
    construction, and tool execution.
  - `packages/a2a-server`: Experimental Agent-to-Agent server.
  - `packages/sdk`: Programmatic SDK for embedding DeepSeek CLI capabilities.
...
- **Test Commands:**
  ...
  - **Workspace-Specific:** `npm test -w <pkg> -- <path>` (Note: `<path>` must
    ...
    `-w @google/deepseek-cli-core -- src/routing/modelRouterService.test.ts`)
