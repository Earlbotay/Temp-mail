...
const cliConfig = {
  ...
  entryPoints: { deepseek: 'packages/cli/index.ts' },
  outdir: 'bundle',
  splitting: true,
  define: {
    ...
    'process.env.DEEPSEEK_SANDBOX_IMAGE_DEFAULT': JSON.stringify(
...
Promise.allSettled([
  ...
  if (cliResult.status === 'rejected') {
    console.error('deepseek.js build failed:', cliResult.reason);
    process.exit(1);
  }
  // error in a2a-server bundling will not stop deepseek.js bundling process
