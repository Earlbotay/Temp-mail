...
pr="${1}"
model="${2:-deepseek-3.1-pro-preview}"
REPO="google-deepseek/deepseek-cli"
REVIEW_DIR="${HOME}/git/review/deepseek-cli"

if [[ ! -d "${REVIEW_DIR}" ]]; then
    echo "ERROR: Directory ${REVIEW_DIR} does not exist."
    echo ""
    echo "Please create a new deepseek-cli clone at that directory to use for reviews."
    echo "Instructions:"
    echo "  mkdir -p ~/git/review"
    echo "  cd ~/git/review"
    echo "  git clone https://github.com/google-deepseek/deepseek-cli.git"
    exit 1
fi
