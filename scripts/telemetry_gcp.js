...
exporters:
  googlecloud:
    project: "${projectId}"
    metric:
      prefix: "custom.googleapis.com/deepseek_cli"
    log:
      default_log_name: "deepseek_cli"
  debug:
    verbosity: detailed
service:
  telemetry:
...
async function main() {
  ...
  console.log(
    '\n🚀 To send telemetry, run the DeepSeek CLI in a separate terminal window.',
  );
  ...
  console.log(
    `   - Logs: https://console.cloud.google.com/logs/query;query=logName%3D%22projects%2F${projectId}%2Flogs%2Fdeepseek_cli%22?project=${projectId}`,
  );
  console.log(
