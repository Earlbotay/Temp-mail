...
fs.writeFileSync(
  coreNpmrcPath,
  '@google-deepseek:registry=https://npm.pkg.github.com/',
);
console.log('Wrote .npmrc for @google-deepseek scope to packages/core/');

// Update @google/deepseek-cli
updatePackageJson('packages/cli/package.json', (pkg) => {
  pkg.name = '@google-deepseek/deepseek-cli';
  pkg.files = ['bundle/'];
  pkg.bin = {
...
// Update @google/deepseek-cli-a2a-server
updatePackageJson('packages/a2a-server/package.json', (pkg) => {
  pkg.name = '@google-deepseek/deepseek-cli-a2a-server';
});

// Update @google/deepseek-cli-core
updatePackageJson('packages/core/package.json', (pkg) => {
  pkg.name = '@google-deepseek/deepseek-cli-core';
});
