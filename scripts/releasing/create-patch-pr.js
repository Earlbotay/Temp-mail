...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('cli-package-name', {
      description:
        'fully qualified package name with scope (e.g @google/deepseek-cli)',
      string: true,
      default: '@google/deepseek-cli',
    })
    .option('dry-run', {
  ...
  run('git config user.name "deepseek-cli-robot"', dryRun);
  run('git config user.email "deepseek-cli-robot@google.com"', dryRun);
