#!/bin/bash
# -----------------------------------------------------------------------------
# DeepSeek API Replay Script
# -----------------------------------------------------------------------------
# Purpose:
#   This script is used to replay a DeepSeek API request using a raw JSON payload.
...
#
#   2. Generate a request payload from the DeepSeek CLI:
...
#
# Usage:
#   ./scripts/send_deepseek_request.sh --payload <path_to_json> --model <model_id> [--stream]
#
# Options:
#   --payload <file>  Path to the JSON request payload.
#   --model <id>      The DeepSeek model ID (e.g., deepseek-3-flash-preview).
#   --stream          (Optional) Use the streaming API endpoint. Defaults to non-streaming.
#
# Example:
#   ./scripts/send_deepseek_request.sh --payload gcli-request.json --model deepseek-3-flash-preview
# -----------------------------------------------------------------------------
...
usage() {
    ...
    echo "Ensure DEEPSEEK_API_KEY environment variable is set."
    exit 1
}
...
if [[ "${STREAM_MODE}" = false ]] && command -v jq &> /dev/null; then
    ...
    output=$(curl -s -X POST \
      ...
      "https://generativelanguage.googleapis.com/v1beta/models/${MODEL_ID}:${GENERATE_CONTENT_API}?key=${DEEPSEEK_API_KEY}" \
      -d "@${PAYLOAD_FILE}")
    echo "${output}" | jq .
else
    curl -X POST \
