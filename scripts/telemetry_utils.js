...
// Returns the home directory, respecting DEEPSEEK_CLI_HOME
const homedir = () => process.env['DEEPSEEK_CLI_HOME'] || os.homedir();

// User-level .deepseek directory in home
const USER_DEEPSEEK_DIR = path.join(homedir(), DEEPSEEK_DIR);
// Project-level .deepseek directory in the workspace
const WORKSPACE_DEEPSEEK_DIR = path.join(projectRoot, DEEPSEEK_DIR);

// Telemetry artifacts are stored in a hashed directory under the user's ~/.deepseek/tmp
export const OTEL_DIR = path.join(USER_DEEPSEEK_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .deepseek directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_DEEPSEEK_DIR,
  'settings.json',
);
...
export function getJson(url) {
  ...
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: deepseek-cli-dev-script', '-o', tmpFile, url],
