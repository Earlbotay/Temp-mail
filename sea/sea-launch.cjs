...
function verifyIntegrity(dir, manifest, fsMod = fs, cryptoMod = crypto) {
  try {
    ...
    if (calculateHash(path.join(dir, 'deepseek.mjs')) !== manifest.mainHash)
      return false;
    if (manifest.files) {
...
function prepareRuntime(manifest, getAssetFn, deps = {}) {
  ...
  if (process.platform === 'win32' && processEnv.LOCALAPPDATA) {
    const appDir = pathMod.join(processEnv.LOCALAPPDATA, 'Google', 'DeepSeekCLI');
    try {
  ...
  const finalRuntimeDir = pathMod.join(
    tempBase,
    `deepseek-runtime-${safeVersion}-${safeUsername}`,
  );
  ...
  if (!useExisting) {
    const setupDir = pathMod.join(
      tempBase,
      `deepseek-setup-${processPid}-${Date.now()}`,
    );
    ...
    try {
      ...
      writeToSetup('deepseek.mjs', 'deepseek.mjs');
      if (manifest.files) {
        for (const file of manifest.files) {
...
async function main(getAssetFn = getAsset) {
  ...
  const mainPath = path.join(runtimeDir, 'deepseek.mjs');
