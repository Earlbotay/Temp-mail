...
DeepSeek CLI's core package (`packages/core`) is the backend portion of DeepSeek
CLI, handling communication with the DeepSeek API, managing tools, and processing
requests sent from `packages/cli`. For a general overview of DeepSeek CLI, see the
...
- **[Memory Import Processor](../reference/memport.md):** Documentation for the
  modular Deepseek.md import feature using @file.md syntax.
...
While the `packages/cli` portion of DeepSeek CLI provides the user interface,
`packages/core` is responsible for:

- **DeepSeek API interaction:** Securely communicating with the Google DeepSeek API,
  sending user prompts, and receiving model responses.
- **Prompt engineering:** Constructing effective prompts for the DeepSeek model,
  potentially incorporating conversation history, tool definitions, and
  instructional context from `Deepseek.md` files.
- **Tool management & orchestration:**
  - Registering available tools (e.g., file system tools, shell command
    execution).
  - Interpreting tool use requests from the DeepSeek model.
  - Executing the requested tools with the provided arguments.
  - Returning tool execution results to the DeepSeek model for further processing.
...
- **API key management:** It handles the `DEEPSEEK_API_KEY` and ensures it's used
  securely when communicating with the DeepSeek API.
...
To ensure that long conversations don't exceed the token limits of the DeepSeek
...
[Google AI documentation](https://ai.google.dev/deepseek-api/docs/models).

## Model fallback

DeepSeek CLI includes a model fallback mechanism to ensure that you can continue
...
Internal utility calls that use `deepseek-2.5-flash-lite` (for example, prompt
completion and classification) silently fall back to `deepseek-2.5-flash` and
`deepseek-2.5-pro` when quota is exhausted, without changing the configured model.
...
`Deepseek.md` files that provide context to the model. It searches for these files
...
and `refresh` the content of loaded `Deepseek.md` files.

## Citations

When DeepSeek finds it is reciting text from a source it appends the citation to
