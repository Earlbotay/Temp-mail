...
DeepSeek CLI supports connecting to remote subagents using the Agent-to-Agent
(A2A) protocol. This allows DeepSeek CLI to interact with other agents, expanding
its capabilities by delegating tasks to remote services.

DeepSeek CLI can connect to any compliant A2A agent. You can find samples of A2A
...
DeepSeek CLI routes traffic to remote agents through an HTTP/HTTPS proxy if one is
...
Many remote agents require authentication. DeepSeek CLI supports several
...
DeepSeek CLI supports the following authentication types:
...
use, DeepSeek CLI opens your browser for sign-in and persists the resulting tokens
...
When DeepSeek CLI loads a remote agent, it validates your auth configuration
...
When connecting to a remote agent, DeepSeek CLI first fetches the agent card
...
Users can manage subagents using the following commands within the DeepSeek CLI:
...
<!-- prettier-ignore -->
> [!TIP]
> You can use the `@cli_help` agent within DeepSeek CLI for assistance
