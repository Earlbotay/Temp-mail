...
Subagents are specialized agents that operate within your main DeepSeek CLI
...
Subagents are "specialists" that the main DeepSeek agent can hire for a specific
job.
...
DeepSeek CLI's main agent is instructed to use specialized subagents when a task
...
DeepSeek CLI comes with the following built-in subagents:
    ...
    "agents": {
      "overrides": {
        "codebase_investigator": {
          "modelConfig": { "model": "deepseek-3-flash-preview" },
...
- **Name:** `cli_help`
- **Purpose:** Get expert knowledge about DeepSeek CLI itself, its commands,
...
server is bundled with DeepSeek CLI and launched automatically — no separate
...
| Mode         | Description                                                                                                                                                                                 |
| :----------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `persistent` | **(Default)** Launches Chrome with a persistent profile stored at `~/.deepseek/cli-browser-profile/`. Cookies, history, and settings are preserved between sessions.                          |
| `isolated`   | Launches Chrome with a temporary profile that is deleted after each session. Use this for clean-state automation.                                                                           |
| `existing`   | Attaches to an already-running Chrome instance. You must enable remote debugging first by navigating to `chrome://inspect/#remote-debugging` in Chrome. No new browser process is launched. |

#### First-run consent

The first time the browser agent is invoked, DeepSeek CLI displays a consent
...
```json
{
  "agents": {
    ...
    "browser": {
      "visualModel": "deepseek-2.5-computer-use-preview-10-2025"
    }
  }
}
```
...
3. Launch the CLI with port forwarding:
   ...
   ```bash
   DEEPSEEK_SANDBOX=docker SANDBOX_PORTS=9222 deepseek
   ```
...
1.  **Project-level:** `.deepseek/agents/*.md` (Shared with your team)
2.  **User-level:** `~/.deepseek/agents/*.md` (Personal agents)
...
**Example: `.deepseek/agents/security-auditor.md`**
...
| `model`        | string | No       | Specific model to use (e.g., `deepseek-3-preview`). Defaults to `inherit` (uses the main session model).                                                                                                        |
...
Subagent tool isolation moves DeepSeek CLI away from a single global tool
...
DeepSeek CLI can also delegate tasks to remote subagents using the Agent-to-Agent
(A2A) protocol.
