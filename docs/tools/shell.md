...
The `run_shell_command` tool allows the DeepSeek model to execute commands
...
`settings.json` file or by using the `/settings` command in the DeepSeek CLI.
...
When an interactive command is running, you can send input to it from the DeepSeek
...
When `run_shell_command` executes a command, it sets the `DEEPSEEK_CLI=1`
environment variable in the subprocess's environment. This allows scripts or
tools to detect if they are being run from within the DeepSeek CLI.
