...
The `get_internal_docs` tool lets DeepSeek CLI access its own technical
documentation to provide more accurate answers about its capabilities and usage.

## Description

This tool is used when DeepSeek CLI needs to verify specific details about DeepSeek
...
The `get_internal_docs` tool is used exclusively by DeepSeek CLI. You cannot
invoke this tool manually.

When DeepSeek CLI uses this tool, it retrieves the content of the requested
...
DeepSeek CLI uses this tool to ensure technical accuracy:

- **Capability discovery:** If DeepSeek CLI is unsure how a feature works, it can
  lookup the corresponding documentation.
- **Reference lookup:** DeepSeek CLI can verify slash command sub-commands or
  specific setting names.
- **Self-correction:** DeepSeek CLI can use the documentation to correct its
  understanding of DeepSeek CLI's system logic.
