# MCP servers with the DeepSeek CLI
...
An MCP server is an application that exposes tools and resources to the DeepSeek
CLI through the Model Context Protocol, allowing it to interact with external
systems and data sources. MCP servers act as a bridge between the DeepSeek model
and your local environment or other services like APIs.

An MCP server enables the DeepSeek CLI to:
...
With an MCP server, you can extend the DeepSeek CLI's capabilities to perform
actions beyond its built-in features, such as interacting with databases, APIs,
custom scripts, or specialized workflows.

## Core integration architecture

The DeepSeek CLI integrates with MCP servers through a sophisticated discovery and
...
4. **Sanitizes and validates** tool schemas for compatibility with the DeepSeek
   API
...
The DeepSeek CLI supports three MCP transport types:
...
prompts. DeepSeek CLI discovers these automatically and gives you the possibility
...
The DeepSeek CLI uses the `mcpServers` configuration in your `settings.json` file
...
DeepSeek CLI automatically expands environment variables in the `env` block of
...
To protect your credentials, DeepSeek CLI performs environment sanitization when
...
- Core project keys: `DEEPSEEK_API_KEY`, `GOOGLE_API_KEY`, etc.
...
The DeepSeek CLI supports OAuth 2.0 authentication for remote MCP servers using
...
- **Stored securely** in `~/.deepseek/mcp-oauth-tokens.json`
...
When the DeepSeek CLI starts, it performs MCP server discovery through the
...
4. **Name sanitization:** Tool names are cleaned to meet DeepSeek API
   requirements:
...
Tool parameter schemas undergo sanitization for DeepSeek API compatibility:
...
When the DeepSeek model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the DeepSeek model like built-in
...
`settings.json`. DeepSeek CLI merges your local configuration with the extension's
defaults:
...
- **Access tokens:** Be security-aware when configuring environment variables
  ...
  for details on how DeepSeek CLI protects your credentials.
...
- **Property stripping:** The system automatically removes certain schema
  properties (`$schema`, `additionalProperties`) for DeepSeek API compatibility
...
DeepSeek CLI's capabilities while maintaining security, reliability, and ease of
use.
...
The DeepSeek CLI will correctly process this array, separating text from binary
...
When the DeepSeek CLI receives this response, it will:
...
context to the DeepSeek model.

## MCP prompts as slash commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within the DeepSeek CLI. This allows you to create
...
```bash
/poem-writer --title="DeepSeek CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "DeepSeek CLI" reverent
```

When you run this command, the DeepSeek CLI executes the `prompts/get` method on
...
## Managing MCP servers with `deepseek mcp`

While you can always configure MCP servers by manually editing your
`settings.json` file, the DeepSeek CLI provides a convenient set of commands to
...
### Adding a server (`deepseek mcp add`)
...
`~/.deepseek/settings.json` or the project config `.deepseek/settings.json` file.

**Command:**

```bash
deepseek mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
deepseek mcp add [options] <name> <command> [args...]

# Example: Adding a local server
deepseek mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
deepseek mcp add python-server python server.py -- --server-arg my-value
```
...
```bash
# Basic syntax
deepseek mcp add --transport http <name> <url>

# Example: Adding an HTTP server
deepseek mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
deepseek mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
```bash
# Basic syntax
deepseek mcp add --transport sse <name> <url>

# Example: Adding an SSE server
deepseek mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
deepseek mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing servers (`deepseek mcp list`)
...
> "Disconnected". Use `deepseek trust` to trust the current folder.
...
You can also use `deepseek mcp list` from your shell to see connection errors for
all configured servers.

### Removing a server (`deepseek mcp remove`)
...
### Enabling/disabling a server (`deepseek mcp enable`, `deepseek mcp disable`)
...
```bash
deepseek mcp enable <name> [--session]
deepseek mcp disable <name> [--session]
```
...
`~/.deepseek/mcp-server-enablement.json`.
