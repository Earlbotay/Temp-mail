...
This monorepo contains two main packages: `@google/deepseek-cli` and
`@google/deepseek-cli-core`.

## `@google/deepseek-cli`

This is the main package for the DeepSeek CLI. It is responsible for the user
...
`@google/deepseek-cli-core`. This means that whether a user installs the package
with `npm install -g @google/deepseek-cli` or runs it directly with
`npx @google/deepseek-cli`, they are using this single, self-contained executable.

## `@google/deepseek-cli-core`

This package contains the core logic for interacting with the DeepSeek API. It is
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @google/deepseek-cli`.
