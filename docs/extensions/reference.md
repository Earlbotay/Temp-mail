...
This guide covers the `deepseek extensions` commands and the structure of the
`deepseek-extension.json` configuration file.

## Manage extensions

Use the `deepseek extensions` command group to manage your extensions from the
terminal.

Note that commands like `deepseek extensions install` are not supported within the
...
DeepSeek CLI creates a copy of the extension during installation. You must run
`deepseek extensions update` to pull changes from the source. To install from
GitHub, you must have `git` installed on your machine.

```bash
deepseek extensions install <source> [--ref <ref>] [--auto-update] [--pre-release] [--consent] [--skip-settings]
```
...
```bash
deepseek extensions uninstall <name...>
```
...
```bash
deepseek extensions disable <name> [--scope <scope>]
```
...
```bash
deepseek extensions enable <name> [--scope <scope>]
```
...
Update an extension to the version specified in its `deepseek-extension.json`
file.

```bash
deepseek extensions update <name>
```

To update all installed extensions at once:

```bash
deepseek extensions update --all
```
...
```bash
deepseek extensions new <path> [template]
```
...
Create a symbolic link between your development directory and the DeepSeek CLI
extensions directory. This lets you test changes immediately without
reinstalling.

```bash
deepseek extensions link <path>
```

## Extension format

DeepSeek CLI loads extensions from `<home>/.deepseek/extensions`. Each extension
must have a `deepseek-extension.json` file in its root directory.

### `deepseek-extension.json`
...
- `description`: A short description of the extension. This will be displayed on
  [deepseekcli.com/extensions](https://deepseekcli.com/extensions).
...
- `contextFileName`: The name of the file that contains the context for the
  ...
  If this property is not used but a `Deepseek.md` file is present in your
...
- `plan`: Planning features configuration.
  - `directory`: The directory where planning artifacts are stored. This serves
    ...
    is `~/.deepseek/tmp/<project>/<session-id>/plans/`.

When DeepSeek CLI starts, it loads all the extensions and merges their
...
```bash
deepseek extensions config <name> [setting] [--scope <scope>]
```

### Custom commands

Provide [custom commands](../cli/custom-commands.md) by placing TOML files in a
`commands/` subdirectory. DeepSeek CLI uses the directory structure to determine
the command name.
...
hooks are not defined in the `deepseek-extension.json` manifest.
...
Extensions can contribute policy rules and safety checkers to the DeepSeek CLI
...
place your `.toml` policy files inside it. DeepSeek CLI automatically loads all
...
<!-- prettier-ignore -->
> [!WARNING]
> For security, DeepSeek CLI ignores any `allow` decisions or `yolo`
...
defined in the `themes` array in `deepseek-extension.json`.
...
DeepSeek CLI supports variable substitution in `deepseek-extension.json` and
`hooks/hooks.json`.
