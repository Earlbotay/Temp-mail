...
Release DeepSeek CLI extensions to your users through a Git repository or GitHub
Releases.
...
The [DeepSeek CLI extension gallery](https://deepseekcli.com/extensions/browse/)
...
1.  **Use a public repository:** Ensure your extension is hosted in a public
    GitHub repository.
2.  **Add the GitHub topic:** Add the `deepseek-cli-extension` topic to your
    repository's **About** section. Our crawler uses this topic to find new
    extensions.
3.  **Place the manifest at the root:** Ensure your `deepseek-extension.json` file
...
extension using `deepseek extensions install <your-repo-uri>`.

Users can optionally depend on a specific branch, tag, or commit using the
`--ref` argument. For example:

```bash
deepseek extensions install <your-repo-uri> --ref=stable
```
...
DeepSeek CLI checks for updates by looking for the **Latest** release on GitHub.
...
To let DeepSeek CLI find the correct asset for a user's platform, use the
...
Archives must be fully contained extensions. The `deepseek-extension.json` file
...
in your `deepseek-extension.json` file to seamlessly transition your users.
...
2. **Update the old repository**: In your original repository, update the
   `deepseek-extension.json` file to include the `migratedTo` property, pointing
...
When users check for updates, the DeepSeek CLI will detect the `migratedTo` field,
