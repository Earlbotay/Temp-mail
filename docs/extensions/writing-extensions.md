# Build DeepSeek CLI extensions

DeepSeek CLI extensions let you expand the capabilities of DeepSeek CLI by adding
...
Before you start, ensure you have the DeepSeek CLI installed and a basic
understanding of Node.js.

## Extension features

Extensions offer several ways to customize DeepSeek CLI. Use this table to decide
...
```bash
deepseek extensions new my-first-extension mcp-server
```
...
The manifest file tells DeepSeek CLI how to load and use your extension.
...
1.  Open `deepseek-extension.json`.
...
When a user installs this extension, DeepSeek CLI will prompt them to enter the
...
Link your extension to your DeepSeek CLI installation for local development.
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the DeepSeek CLI extensions
    directory to your development directory. Changes you make are reflected
    immediately.

    ```bash
    deepseek extensions link .
    ```

Restart your DeepSeek CLI session to use the new `fetch_posts` tool. Test it by
asking: "fetch posts".
...
After saving the file, restart DeepSeek CLI. Run `/fs:grep-code "some pattern"` to
use your new command.

## Step 6: Add a custom `Deepseek.md`

Provide persistent context to the model by adding a `Deepseek.md` file to your
extension. This is useful for setting behavior or providing essential tool
information.

1.  Create a file named `Deepseek.md` in the root of your extension directory:
...
2.  Update your `deepseek-extension.json` to load this file:
...
Restart DeepSeek CLI. The model now has the context from your `Deepseek.md` file in
...
DeepSeek CLI automatically discovers skills bundled with your extension. The model
