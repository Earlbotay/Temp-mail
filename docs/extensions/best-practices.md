# DeepSeek CLI extension best practices
...
Developing extensions for DeepSeek CLI is a lightweight, iterative process. Use
...
Use the `deepseek extensions link` command to develop locally without reinstalling
...
### Use `Deepseek.md` effectively

Your `Deepseek.md` file provides essential context to the model.
...
your `deepseek-extension.json` file:
...
deepseek extensions install github.com/user/repo

# Install the development version
deepseek extensions install github.com/user/repo --ref dev
```

### Clean artifacts

When using GitHub Releases, ensure your archives only contain necessary files
(such as `dist/`, `deepseek-extension.json`, and `package.json`). Exclude
...
- **Manual verification:** Use `deepseek extensions link` to test your extension
...
- **Check the manifest:** Ensure `deepseek-extension.json` is in the root
...
- **Restart the CLI:** Extensions are loaded at the start of a session. Restart
  DeepSeek CLI after making changes to the manifest or linking a new extension.
...
- **Test the command:** Run the server's `command` and `args` directly in your
  terminal to ensure it starts correctly outside of DeepSeek CLI.
