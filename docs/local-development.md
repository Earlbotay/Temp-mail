...
features for DeepSeek CLI.

## Tracing

DeepSeek CLI uses OpenTelemetry (OTel) to record traces that help you debug agent
...
2.  **Run DeepSeek CLI:**
    ...
    In a separate terminal, run your DeepSeek CLI command:
...
1.  **Start the telemetry collector:**
    ...
    - **Collector logs:** `~/.deepseek/tmp/<projectHash>/otel/collector.log`
...
1.  **Configure `.deepseek/settings.json`:**
...
2.  **Start the telemetry collector:**
    ...
    Cloud Console.
    - **Collector logs:** `~/.deepseek/tmp/<projectHash>/otel/collector-gcp.log`
...
```typescript
import { runInDevTraceSpan } from '@google/deepseek-cli-core';
import { DeepSeekCliOperation } from '@google/deepseek-cli-core/lib/telemetry/constants.js';

await runInDevTraceSpan(
  {
    operation: DeepSeekCliOperation.ToolCall,
    attributes: {
      [GEN_AI_AGENT_NAME]: 'deepseek-cli',
    },
  },
  async ({ metadata }) => {
