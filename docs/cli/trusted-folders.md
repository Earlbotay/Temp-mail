...
which projects can use the full capabilities of the DeepSeek CLI. It prevents
...
Once the feature is enabled, the first time you run the DeepSeek CLI from a
...
Your choice is saved in a central file (`~/.deepseek/trustedFolders.json`), so you
will only be asked once per folder.

## Understanding folder contents: The discovery phase

Before you make a choice, the DeepSeek CLI performs a **discovery phase** to scan
...
When a folder is **untrusted**, the DeepSeek CLI runs in a restricted "safe mode"
...
1.  **Workspace settings are ignored**: The CLI will **not** load the
    `.deepseek/settings.json` file from the project. This prevents the loading of
...
Granting trust to a folder unlocks the full functionality of the DeepSeek CLI for
that workspace.
...
- **View all trust rules**: To see a complete list of all your trusted and
  ...
  `~/.deepseek/trustedFolders.json` file in your home directory.
...
2.  **Local trust file**: If the IDE is not connected, the CLI checks the
    central `~/.deepseek/trustedFolders.json` file.
