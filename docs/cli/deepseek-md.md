# Provide context with Deepseek.md files

Context files, which use the default name `Deepseek.md`, are a powerful feature
for providing instructional context to the DeepSeek model. You can use these files
...
1.  **Global context file:**
    - **Location:** `~/.deepseek/Deepseek.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**
    - **Location:** The CLI searches for `Deepseek.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `Deepseek.md` files in that directory and its
...
### Example `Deepseek.md` file

Here is an example of what you can include in a `Deepseek.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `Deepseek.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.deepseek/Deepseek.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `Deepseek.md` files into smaller, more manageable
...
**Example `Deepseek.md` with imports:**

```markdown
# Main Deepseek.md file
...
While `Deepseek.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "Deepseek.md"]
  }
}
```
...
- Learn about [Ignoring files](./deepseek-ignore.md) to exclude content from the
  context system.
