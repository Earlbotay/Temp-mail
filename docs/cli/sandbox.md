# Sandboxing in the DeepSeek CLI

This document provides a guide to sandboxing in the DeepSeek CLI, including
prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the DeepSeek CLI:
...
When you set `sandbox: "runsc"`, DeepSeek CLI runs
...
`DEEPSEEK_SANDBOX=runsc` or `sandbox: "runsc"`).
...
- A container must be created and running before starting DeepSeek CLI. DeepSeek
...
lxc launch ubuntu:24.04 deepseek-sandbox

# Enable LXC sandboxing
export DEEPSEEK_SANDBOX=lxc
deepseek -p "build the project"
```
...
```bash
export DEEPSEEK_SANDBOX=lxc
export DEEPSEEK_SANDBOX_IMAGE=my-snapcraft-container
deepseek -p "build the snap"
```
...
(like `shell_exec` and `write_file`) instead of sandboxing the entire DeepSeek CLI
process.
...
<!-- prettier-ignore -->
> [!NOTE]
> Changing the `security.toolSandboxing` setting requires a restart of DeepSeek
> CLI to take effect.

## Sandbox expansion

Sandbox expansion is a dynamic permission system that lets DeepSeek CLI request
...
requiring extra permissions (like `npm install`), DeepSeek CLI will present you
with a "Sandbox Expansion Request."

### How sandbox expansion works

1.  **Detection**: DeepSeek CLI detects a sandbox denial or proactively identifies
...
deepseek -s -p "analyze the code structure"
```
...
```bash
export DEEPSEEK_SANDBOX=true
deepseek -p "run the test suite"
```

**Windows (PowerShell)**

```powershell
$env:DEEPSEEK_SANDBOX="true"
deepseek -p "run the test suite"
```
...
2. **Environment variable**:
   `DEEPSEEK_SANDBOX=true|docker|podman|sandbox-exec|runsc|lxc`
...
```bash
DEBUG=1 deepseek -s -p "debug command"
```

<!-- prettier-ignore -->
> [!NOTE]
> If you have `DEBUG=true` in a project's `.env` file, it won't affect
> deepseek-cli due to automatic exclusion. Use `.deepseek/.env` files for
> deepseek-cli specific debug settings.

### Inspect sandbox

```bash
# Check environment
deepseek -s -p "run shell command: env | grep SANDBOX"

# List mounts
deepseek -s -p "run shell command: mount | grep workspace"
```
