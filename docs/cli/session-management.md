...
- **Location:** Sessions are stored in `~/.deepseek/tmp/<project_hash>/chats/`,
...
When starting DeepSeek CLI, use the `--resume` (or `-r`) flag to load existing
sessions.
...
- **Resume by ID:** You can also provide the full session UUID:
  ```bash
  deepseek --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890
  ```
...
[Git worktrees](./git-worktrees.md) to give each DeepSeek session its own copy of
...
You can configure how DeepSeek CLI manages your session history in your
`settings.json` file. These settings let you control retention policies and
session lengths.

### Session retention

By default, DeepSeek CLI automatically cleans up old session data to prevent your
history from growing indefinitely. When a session is deleted, DeepSeek CLI also
