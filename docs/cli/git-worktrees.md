...
DeepSeek session its own copy of the codebase. Git worktrees create separate
...
>
> - [Open an issue](https://github.com/google-deepseek/deepseek-cli/issues/new?template=bug_report.yml) on GitHub.
> - Use the **/bug** command within DeepSeek CLI to file an issue.
...
Use the `--worktree` (`-w`) flag to create an isolated worktree and start DeepSeek
CLI in it.

- **Start with a specific name:** The value you pass becomes both the directory
  name (within `.deepseek/worktrees/`) and the branch name.

  ```bash
  deepseek --worktree feature-search
  ```

- **Start with a random name:** If you omit the name, DeepSeek generates a random
...
When you exit a worktree session (using `/quit` or `Ctrl+C`), DeepSeek leaves the
...
DeepSeek prioritizes a fast and safe exit: it **does not automatically delete**
your worktree or branch. You are responsible for cleaning up your worktrees
manually once you are finished with them.

When you exit, DeepSeek displays instructions on how to resume your work or how to
...
DeepSeek CLI with the `--resume` flag and the session ID:

```bash
cd .deepseek/worktrees/feature-search
...
- **Clean up a preserved Git worktree:**
  ```bash
  git worktree remove .deepseek/worktrees/feature-search --force
...
- **Create a Git worktree manually:**
  ...
  cd ../project-feature-search && deepseek
  ```
