...
DeepSeek CLI includes a model routing feature that automatically switches to a
...
2.  **User consent:** Depending on the failure and the model's policy, the CLI
    ...
    use a silent fallback chain for `deepseek-2.5-flash-lite` and will fall back
    to `deepseek-2.5-flash` and `deepseek-2.5-pro` without prompting or changing the
    configured model.
...
DeepSeek CLI supports using a local model for routing decisions. When configured,
DeepSeek CLI will use a locally-running **Gemma** model to make routing decisions
...
DeepSeek API and accessible via HTTP at an endpoint configured in `settings.json`.
...
The model used by DeepSeek CLI is determined by the following order of precedence:

1.  **`--model` command-line flag:** A model specified with the `--model` flag
    when launching the CLI will always be used.
2.  **`DEEPSEEK_MODEL` environment variable:** If the `--model` flag is not used,
    the CLI will use the model specified in the `DEEPSEEK_MODEL` environment
    variable.
...
4.  **Local model (experimental):** If the Gemma local model router is enabled
    ...
    (instead of DeepSeek models) to route the request to an appropriate model.
