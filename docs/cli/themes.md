...
DeepSeek CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

DeepSeek CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within DeepSeek CLI:
...
1.  Enter `/theme` into DeepSeek CLI.
...
Selected themes are saved in DeepSeek CLI's
...
DeepSeek CLI lets you create your own custom color themes by specifying them in
...
<!-- prettier-ignore -->
> [!WARNING]
> For your safety, DeepSeek CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in DeepSeek CLI. Your custom
