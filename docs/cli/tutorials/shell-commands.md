...
DeepSeek CLI also records the command and its output in the current session
...
You can automate tasks using a combination of DeepSeek CLI and shell commands.
...
1.  DeepSeek calls `run_shell_command('npm test')`.
2.  You see a confirmation prompt: `Allow command 'npm test'? [y/N]`.
3.  You press `y`.
4.  The tests run. If they fail, DeepSeek reads the error output.
5.  DeepSeek uses `read_file` to inspect the failing test.
6.  DeepSeek uses `replace` to fix the bug.
7.  DeepSeek runs `npm test` again to verify the fix.

This loop lets DeepSeek work autonomously.

## How to manage background processes

You can ask DeepSeek to start long-running tasks, like development servers or file
watchers.

**Prompt:** `Start the React dev server in the background.`

DeepSeek will run the command (e.g., `npm run dev`) and detach it.
...
DeepSeek CLI attempts to handle interactive commands (like `git add -p` or
...
Giving an AI access to your shell is powerful but risky. DeepSeek CLI includes
