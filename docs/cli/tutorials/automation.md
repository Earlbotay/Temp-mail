...
Automate tasks with DeepSeek CLI. Learn how to use headless mode, pipe data into
DeepSeek CLI, automate workflows with shell scripts, and generate structured JSON
output for other applications.

## Prerequisites

- DeepSeek CLI installed and authenticated.
- Familiarity with shell scripting (Bash/Zsh).

## Why headless mode?

Headless mode runs DeepSeek CLI once and exits. It's perfect for:
...
Run DeepSeek CLI in headless mode by providing a prompt with the `-p` (or
...
```bash
deepseek -p "Write a poem about TypeScript"
```

## How to pipe input to DeepSeek CLI

Feed data into DeepSeek using the standard Unix pipe `|`. DeepSeek reads the
...
```bash
cat error.log | deepseek -p "Explain why this failed"
```

**Windows (PowerShell)**

```powershell
Get-Content error.log | deepseek -p "Explain why this failed"
```

Pipe a command:

```bash
git diff | deepseek -p "Write a commit message for these changes"
```

## Use DeepSeek CLI output in scripts

Because DeepSeek prints to stdout, you can chain it with other tools or save the
results to a file.
...
1.  Save the following code as `generate_docs.sh` (or `generate_docs.ps1` for
    ...
    **macOS/Linux (`generate_docs.sh`)**
    ...
    for file in *.py; do
      ...
      # Ask DeepSeek CLI to generate the documentation and print it to stdout
      deepseek -p "Generate a Markdown documentation summary for @$file. Print the
    ...
    **Windows PowerShell (`generate_docs.ps1`)**
    ...
    Get-ChildItem -Filter *.py | ForEach-Object {
      ...
      # Ask DeepSeek CLI to generate the documentation and print it to stdout
      deepseek -p "Generate a Markdown documentation summary for @$($_.Name). Print the result to standard output." | Out-File -FilePath $newName -Encoding utf8
    }
    ```
...
1.  Save the following script as `generate_json.sh` (or `generate_json.ps1` for
    ...
    **macOS/Linux (`generate_json.sh`)**
    ...
    # Extract data
    deepseek --output-format json "Return a raw JSON object with keys 'version' and 'deps' from @package.json" | jq -r '.response' > data.json
    ```
    ...
    **Windows PowerShell (`generate_json.ps1`)**
    ...
    $output = deepseek --output-format json "Return a raw JSON object with keys 'version' and 'deps' from @package.json" | ConvertFrom-Json
...
2.  Scroll to the very bottom of the file and paste this code:
    ...
    function gcommit() {
      ...
      # Ask DeepSeek to write the message
      echo "Generating commit message..."
      msg=$(echo "$diff" | deepseek -p "Write a concise Conventional Commit message for this diff. Output ONLY the message.")
...
2.  Scroll to the very bottom of the file and paste this code:
    ...
    function gcommit {
      ...
      $msg = $diff | deepseek -p "Write a concise Conventional Commit message for this diff. Output ONLY the message."
...
4.  Use your new command:
    ...
    DeepSeek CLI will analyze your staged changes and commit them with a generated
    message.
