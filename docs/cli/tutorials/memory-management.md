...
Control what DeepSeek CLI knows about you and your projects. In this guide, you'll
learn how to define project-wide rules with `Deepseek.md`, teach the agent
...
DeepSeek CLI is powerful but general. It doesn't know your preferred testing
...
## How to define project-wide rules (Deepseek.md)

The most powerful way to control the agent's behavior is through `Deepseek.md`
...
1.  In the root of your project, create a file named `Deepseek.md`.
...
3.  Start a new session. DeepSeek CLI will now know these rules automatically.
...
1.  **Global:** `~/.deepseek/Deepseek.md` (Rules for _every_ project you work on).
2.  **Project Root:** `./Deepseek.md` (Rules for the current repository).
3.  **Subdirectory:** `./src/Deepseek.md` (Rules specific to the `src` folder).
...
`Deepseek.md` files and saved memories), use the `/memory show` command.
...
If you edit a `Deepseek.md` file while a session is running, the agent won't know
...
- **Keep it focused:** Avoid adding excessive content to `Deepseek.md`. Keep
...
- **Review often:** Periodically check your `Deepseek.md` files to remove outdated
  rules.
...
- Explore the [Command reference](../../reference/commands.md) for more
  `/memory` options.
- Read the technical spec for [Project context](../../cli/deepseek-md.md).
