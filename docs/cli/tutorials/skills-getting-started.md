...
Agent Skills extend DeepSeek CLI with specialized expertise. In this guide, you'll
...
1.  Run the following command to create the folders:
    ...
    **macOS/Linux**
    ...
    ```bash
    mkdir -p .deepseek/skills/api-auditor/scripts
    ```

    **Windows (PowerShell)**

    ```powershell
    New-Item -ItemType Directory -Force -Path ".deepseek\skills\api-auditor\scripts"
    ```

### Create the definition

1.  Create a file at `.deepseek/skills/api-auditor/SKILL.md`. This tells the agent
...
1.  Create a file at `.deepseek/skills/api-auditor/scripts/audit.js`. This is the
    code the agent will run.

    ```javascript
    // .deepseek/skills/api-auditor/scripts/audit.js
...
DeepSeek CLI automatically discovers skills in the `.deepseek/skills` directory. You
...
**User:** "Can you audit http://deepseekcli.com"

DeepSeek recognizes the request matches the `api-auditor` description and asks for
...
DeepSeek then uses the `run_shell_command` tool to execute your bundled Node
script:

`node .deepseek/skills/api-auditor/scripts/audit.js http://deepseekli.com`
