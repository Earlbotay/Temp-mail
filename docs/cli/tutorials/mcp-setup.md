...
Connect DeepSeek CLI to your external databases and services. In this guide,
you'll learn how to extend DeepSeek CLI's capabilities by installing the GitHub
...
## How to configure DeepSeek CLI

You tell DeepSeek about new servers by editing your `settings.json`.

1.  Open `~/.deepseek/settings.json` (or the project-specific
    `.deepseek/settings.json`).
2.  Add the `mcpServers` block. This tells DeepSeek: "Run this docker container
    and talk to it."
...
Restart DeepSeek CLI. It will automatically try to start the defined servers.
...
**Prompt:** `List the open PRs in the google/deepseek-cli repository.`
