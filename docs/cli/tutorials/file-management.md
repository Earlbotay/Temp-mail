# File management with DeepSeek CLI

Explore, analyze, and modify your codebase using DeepSeek CLI. In this guide,
you'll learn how to provide DeepSeek CLI with files and directories, modify and
create files, and control what DeepSeek CLI can see.
...
DeepSeek CLI will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that DeepSeek CLI uses a file,
...
If you _don't_ know the exact file path, you can ask DeepSeek CLI to find it for
...
DeepSeek uses the `glob` or `list_directory` tools to search your project
...
Once DeepSeek CLI has context, you can direct it to make specific edits. The agent
...
DeepSeek CLI uses the `replace` tool to propose a targeted code change.
...
DeepSeek CLI uses the `write_file` tool to generate the new file from scratch.

## Review and confirm changes

DeepSeek CLI prioritizes safety. Before any file is modified, it presents a
...
DeepSeek CLI uses the `run_shell_command` tool to execute your test runner (for
example, `npm test` or `jest`). This ensures the changes didn't break existing
functionality.

## Advanced: Controlling what DeepSeek sees

By default, DeepSeek CLI respects your `.gitignore` file. It won't read or search
...
`.deepseekignore` file in your project root.

**Example `.deepseekignore`:**
