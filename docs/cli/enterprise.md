# DeepSeek CLI for the enterprise

This document outlines configuration patterns and best practices for deploying
and managing DeepSeek CLI in an enterprise environment. By leveraging system-level
...
> DeepSeek CLI. However, they should not be considered a foolproof security
...
2. User Settings (`~/.deepseek/settings.json`)
3. Workspace Settings (`<project>/.deepseek/settings.json`)
...
- **System defaults `system-defaults.json`:**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/deepseek-cli/common-context"]
    }
  }
  ```

- **User `settings.json` (`~/.deepseek/settings.json`):**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["~/deepseek-context"]
    }
  }
  ```

- **Workspace `settings.json` (`<project>/.deepseek/settings.json`):**
...
- **System overrides `settings.json`:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/deepseek-cli/global-context"]
    }
  }
  ```
...
- **Final merged configuration:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": [
        "/etc/deepseek-cli/common-context",
        "~/deepseek-context",
        "./project-context",
        "/etc/deepseek-cli/global-context"
      ]
    }
  }
  ```
...
- **Location**:
  - **Linux**: `/etc/deepseek-cli/settings.json`
  - **Windows**: `C:\ProgramData\deepseek-cli\settings.json`
  - **macOS**: `/Library/Application Support/DeepSeekCli/settings.json`
  - The path can be overridden using the `DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH`
...
While the `DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH` environment variable provides
...
This approach ensures that no matter how the user calls the `deepseek` command,
the enterprise settings are always loaded with the highest precedence.

**Example wrapper script:**

Administrators can create a script named `deepseek` and place it in a directory
that appears earlier in the user's `PATH` than the actual DeepSeek CLI binary
(e.g., `/usr/local/bin/deepseek`).
...
export DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH="/etc/deepseek-cli/settings.json"

# Find the original deepseek executable.
# This is a simple example; a more robust solution might be needed
# depending on the installation method.
REAL_DEEPSEEK_PATH=$(type -aP deepseek | grep -v "^$(type -P deepseek)$" | head -n 1)

if [ -z "$REAL_DEEPSEEK_PATH" ]; then
  echo "Error: The original 'deepseek' executable was not found." >&2
  exit 1
fi

# Pass all arguments to the real DeepSeek CLI executable.
exec "$REAL_DEEPSEEK_PATH" "$@"
```

By deploying this script, the `DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH` is set within
the script's environment, and the `exec` command replaces the script process
with the actual DeepSeek CLI process, which inherits the environment variable.
...
```powershell
Add-Content -Path $PROFILE -Value '$env:DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH="C:\ProgramData\deepseek-cli\settings.json"'
```

## User isolation in shared environments

In shared compute environments (like ML experiment runners or shared build
servers), you can isolate DeepSeek CLI state by overriding the user's home
directory.

By default, DeepSeek CLI stores configuration and history in `~/.deepseek`. You can
use the `DEEPSEEK_CLI_HOME` environment variable to point to a unique directory
for a specific user or job. The CLI will create a `.deepseek` folder inside the
specified path.
...
export DEEPSEEK_CLI_HOME="/tmp/deepseek-job-123"
deepseek
```
...
```powershell
# Isolate state for a specific job
$env:DEEPSEEK_CLI_HOME="C:\temp\deepseek-job-123"
deepseek
```

## Restricting tool access

You can significantly enhance security by controlling which tools the DeepSeek
...
DeepSeek CLI loads `settings.json` files from three levels: System, Workspace, and
...
In corporate environments with strict network policies, you can configure DeepSeek
...
For auditing and monitoring purposes, you can configure DeepSeek CLI to send
...
control that is configured on a proxy server, not within DeepSeek CLI itself. It
...
patterns discussed above to create a secure, controlled environment for DeepSeek
CLI.
...
```json
{
  ...
  "mcpServers": {
    "corp-tools": {
      "command": "/opt/deepseek-tools/start.sh",
      "timeout": 5000
    }
  },
  "telemetry": {
