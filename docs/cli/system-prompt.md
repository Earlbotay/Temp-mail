# System Prompt Override (DEEPSEEK_SYSTEM_MD)

The core system instructions that guide DeepSeek CLI can be completely replaced
with your own Markdown file. This feature is controlled via the
`DEEPSEEK_SYSTEM_MD` environment variable.

## Overview

The `DEEPSEEK_SYSTEM_MD` variable instructs the CLI to use an external Markdown
...
via a `.deepseek/.env` file. See
[Persisting Environment Variables](../get-started/authentication.md#persisting-environment-variables).

- Use the project default path (`.deepseek/system.md`):
  - `DEEPSEEK_SYSTEM_MD=true` or `DEEPSEEK_SYSTEM_MD=1`
  - The CLI reads `./.deepseek/system.md` (relative to your current project
    directory).

- Use a custom file path:
  - `DEEPSEEK_SYSTEM_MD=/absolute/path/to/my-system.md`
...
- Disable the override (use built‑in prompt):
  - `DEEPSEEK_SYSTEM_MD=false` or `DEEPSEEK_SYSTEM_MD=0` or unset the variable.
...
- One‑off session using a project file:
  - `DEEPSEEK_SYSTEM_MD=1 deepseek`
- Persist for a project using `.deepseek/.env`:
  - Create `.deepseek/system.md`, then add to `.deepseek/.env`:
    - `DEEPSEEK_SYSTEM_MD=1`
- Use a custom file under your home directory:
  - `DEEPSEEK_SYSTEM_MD=~/prompts/SYSTEM.md deepseek`

## UI indicator

When `DEEPSEEK_SYSTEM_MD` is active, the CLI shows a `|⌐■_■|` indicator in the UI
...
- Write the built‑in prompt to the project default path:
  - `DEEPSEEK_WRITE_SYSTEM_MD=1 deepseek`
- Or write to a custom path:
  - `DEEPSEEK_WRITE_SYSTEM_MD=~/prompts/DEFAULT_SYSTEM.md deepseek`

This creates the file and writes the current built‑in system prompt to it.

## Best practices: SYSTEM.md vs Deepseek.md
...
Deepseek.md focused on high‑level guidance and project specifics.

## Troubleshooting

- Error: `missing system prompt file '…'`
  - Ensure the referenced path exists and is readable.
  - For `DEEPSEEK_SYSTEM_MD=1|true`, create `./.deepseek/system.md` in your project.
- Override not taking effect
  - Confirm the variable is loaded (use `.deepseek/.env` or export in your shell).
  - Paths are resolved from the current working directory; try an absolute path.
- Restore defaults
  - Unset `DEEPSEEK_SYSTEM_MD` or set it to `0`/`false`.
