...
prompts as personal shortcuts within DeepSeek CLI. You can create commands that
...
DeepSeek CLI discovers commands from two locations, loaded in a specific order:

1.  **User commands (global):** Located in `~/.deepseek/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.deepseek/commands/`. These commands are specific to the
...
- A file at `~/.deepseek/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.deepseek/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
- `prompt` (String): The prompt that will be sent to the DeepSeek model when the
...
```toml
# In: <project>/.deepseek/commands/changelog.toml
...
When a custom command attempts to execute a shell command, DeepSeek CLI will now
...
````toml
# In: <project>/.deepseek/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.deepseekignore` if enabled.
...
```toml
# In: <project>/.deepseek/commands/review.toml
...
```bash
mkdir -p ~/.deepseek/commands/refactor
touch ~/.deepseek/commands/refactor/pure.toml
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.deepseek\commands\refactor"
New-Item -ItemType File -Force -Path "$env:USERPROFILE\.deepseek\commands\refactor\pure.toml"
```

**2. Add the content to the file:**

Open `~/.deepseek/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.deepseek/commands/refactor/pure.toml
...
DeepSeek CLI will then execute the multi-line prompt defined in your TOML file.
