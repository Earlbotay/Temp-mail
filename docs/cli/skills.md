...
Agent Skills allow you to extend DeepSeek CLI with specialized expertise,
...
Unlike general context files ([`Deepseek.md`](./deepseek-md.md)), which provide
persistent workspace-wide background, Skills represent **on-demand expertise**.
This allows DeepSeek to maintain a vast library of specialized capabilities—such
as security auditing, cloud deployments, or codebase migrations—without
cluttering the model's immediate context window.

DeepSeek autonomously decides when to employ a skill based on your request and the
...
DeepSeek CLI discovers skills from three primary locations:

1.  **Workspace Skills**: Located in `.deepseek/skills/` or the `.agents/skills/`
    alias. Workspace skills are typically committed to version control and
    shared with the team.
2.  **User Skills**: Located in `~/.deepseek/skills/` or the `~/.agents/skills/`
...
precedence over the `.deepseek/skills/` directory. This generic alias provides an
...
The `deepseek skills` command provides management utilities:
...
# Discovers skills (SKILL.md or */SKILL.md) and creates symlinks in ~/.deepseek/skills
# (or ~/.agents/skills)
deepseek skills link /path/to/my-skills-repo

# Link to the workspace scope (.deepseek/skills or .agents/skills)
deepseek skills link /path/to/my-skills-repo --scope workspace

# Install a skill from a Git repository, local directory, or zipped skill file (.skill)
# Uses the user scope by default (~/.deepseek/skills or ~/.agents/skills)
deepseek skills install https://github.com/user/repo.git
deepseek skills install /path/to/local/skill
deepseek skills install /path/to/local/my-expertise.skill

# Install a specific skill from a monorepo or subdirectory using --path
deepseek skills install https://github.com/my-org/my-skills.git --path skills/frontend-design

# Install to the workspace scope (.deepseek/skills or .agents/skills)
deepseek skills install /path/to/skill --scope workspace

# Uninstall a skill by name
deepseek skills uninstall my-expertise --scope workspace

# Enable a skill (globally)
deepseek skills enable my-expertise

# Disable a skill. Can use --scope to specify workspace or user (defaults to workspace)
deepseek skills disable my-expertise --scope workspace
```

## How it Works

1.  **Discovery**: At the start of a session, DeepSeek CLI scans the discovery
    tiers and injects the name and description of all enabled skills into the
    system prompt.
2.  **Activation**: When DeepSeek identifies a task matching a skill's
...
Once a skill is activated (typically by DeepSeek identifying a task that matches
