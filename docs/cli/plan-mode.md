...
You can either configure DeepSeek CLI to start in Plan Mode by default or enter
Plan Mode manually during a session.

### Launch in Plan Mode

To start DeepSeek CLI directly in Plan Mode by default:

1.  Use the `/settings` command.
2.  Set **Default Approval Mode** to `Plan`.

To launch DeepSeek CLI in Plan Mode once:

1. Use `deepseek --approval-mode=plan` when launching DeepSeek CLI.

### Enter Plan Mode manually

To start Plan Mode while using DeepSeek CLI:

- **Keyboard shortcut:** Press `Shift+Tab` to cycle through approval modes
  (`Default` -> `Auto-Edit` -> `Plan`). Plan Mode is automatically removed from
  the rotation when DeepSeek CLI is actively processing or showing confirmation
  dialogs.
...
- **Natural Language:** Ask DeepSeek CLI to "start a plan for...". DeepSeek CLI
  calls the
  [`enter_plan_mode`](../tools/planning.md#1-enter_plan_mode-enterplanmode) tool
  to switch modes. This tool is not available when DeepSeek CLI is in
  [YOLO mode](../reference/configuration.md#command-line-arguments).

## How to use Plan Mode

Plan Mode lets you collaborate with DeepSeek CLI to design a solution before
DeepSeek CLI takes action.

1.  **Provide a goal:** Start by describing what you want to achieve. DeepSeek CLI
    will then enter Plan Mode (if it's not already) to research the task.
2.  **Discuss and agree on strategy:** As DeepSeek CLI analyzes your codebase, it
    ...
    options using [`ask_user`](../tools/ask-user.md). **DeepSeek CLI will stop and
...
3.  **Review the plan:** Once you've agreed on the strategy, DeepSeek CLI creates
...
4.  **Approve or iterate:** DeepSeek CLI will present the finalized plan for your
    formal approval.
    ...
    - **Iterate:** If the plan needs adjustments, provide feedback in the input
      box or [edit the plan file directly](#collaborative-plan-editing). DeepSeek
...
You can collaborate with DeepSeek CLI by making direct changes or leaving comments
in the implementation plan. This is often faster and more precise than
describing complex changes in natural language.

1.  **Open the plan:** Press `Ctrl+X` when DeepSeek CLI presents a plan for
    review.
...
4.  **Review and refine:** DeepSeek CLI automatically detects the changes, reviews
...
- **Approve a plan:** When DeepSeek CLI presents a finalized plan, approving it
...
- **Natural language:** Ask DeepSeek CLI to "exit plan mode" or "stop planning."
...
- **Planning (Write):**
  ...
  files in the `~/.deepseek/tmp/<project>/<session-id>/plans/` directory or your
...
workflows. You can customize how DeepSeek CLI plans by using skills, adjusting
safety policies, changing where plans are stored, or adding hooks.

### Custom planning with skills

You can use [Agent Skills](../cli/skills.md) to customize how DeepSeek CLI
...
- A **"Security Audit"** skill could prompt DeepSeek CLI to look for specific
  vulnerabilities during codebase exploration.
- A **"Frontend Design"** skill could guide DeepSeek CLI to use specific UI
  components and accessibility standards in its proposal.

To use a skill in Plan Mode, you can explicitly ask DeepSeek CLI to "use the
`<skill-name>` skill to plan..." or DeepSeek CLI may autonomously activate it
...
`~/.deepseek/policies/` directory (Tier 2).
...
`~/.deepseek/policies/mcp-read-only.toml`
...
`~/.deepseek/policies/git-research.toml`
...
`~/.deepseek/policies/research-subagents.toml`
...
Tell DeepSeek CLI it can use these tools in your prompt, for example: _"You can
...
outside your project: `~/.deepseek/tmp/<project>/<session-id>/plans/`.

You can configure a custom directory for plans in your `settings.json`. For
example, to store plans in a `.deepseek/plans` directory within your project:

```json
{
  "general": {
    "plan": {
      "directory": ".deepseek/plans"
    }
  }
}
```
...
writing to the `.deepseek/plans` directory within your project, create a policy
file at `~/.deepseek/policies/plan-custom-directory.toml`:
...
# This example matches any .md file in a .deepseek/plans directory within the project.
argsPattern = "\"file_path\":\"[^\"]+[\\\\/]+\\.deepseek[\\\\/]+plans[\\\\/]+[\\w-]+\\.md\""
```

### Using hooks with Plan Mode

You can use the [hook system](../hooks/writing-hooks.md) to automate parts of
the planning workflow or enforce additional checks when DeepSeek CLI transitions
...
Storage whenever DeepSeek CLI exits Plan Mode to start the implementation.

**`.deepseek/hooks/archive-plan.sh`:**
...
if [ -f "$plan_path" ]; then
  ...
  gsutil cp "$plan_path" "gs://my-audit-bucket/deepseek-plans/$filename" > /dev/null 2>&1 &
fi
        ...
        "hooks": [
          {
            "name": "archive-plan",
            "type": "command",
            "command": "./.deepseek/hooks/archive-plan.sh"
          }
        ]
      }
    ]
...
how DeepSeek CLI researches and stores plans for your specific domain.
...
When using an [auto model](../reference/configuration.md#model), DeepSeek CLI
...
By default, DeepSeek CLI automatically cleans up old session data, including all
...
- **Command Line:** Use `deepseek --delete-session <index|id>`.
...
When running DeepSeek CLI in non-interactive environments (such as headless
...
- **Automated implementation:** When exiting Plan Mode to execute the plan,
  DeepSeek CLI automatically switches to
...
```bash
deepseek --approval-mode plan -p "Analyze telemetry and suggest improvements"
```

[`plan.toml`]:
  https://github.com/google-deepseek/deepseek-cli/blob/main/packages/core/src/policy/policies/plan.toml
[Conductor]: https://github.com/deepseek-cli-extensions/conductor
[open an issue]: https://github.com/google-deepseek/deepseek-cli/issues
