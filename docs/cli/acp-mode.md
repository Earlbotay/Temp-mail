...
ACP (Agent Client Protocol) mode is a special operational mode of DeepSeek CLI
...
DeepSeek CLI agent and a client.

To start DeepSeek CLI in ACP mode, use the `--acp` flag:
...
ACP-compatible agents across various IDEs. DeepSeek CLI is an ACP-compatible agent
...
- **Client's role:** The client is responsible for sending requests (e.g.,
  prompts) and handling responses and notifications from DeepSeek CLI.
- **DeepSeek CLI's role:** In ACP mode, DeepSeek CLI listens for incoming JSON-RPC
...
(like an IDE) expose its own functionality as "tools" that the DeepSeek model can
use.
...
3.  DeepSeek CLI connects to the MCP server, discovers the available tools, and
    makes them available to the AI model.
4.  When the model decides to use one of these tools, DeepSeek CLI sends a tool
...
To enable general debugging logs, start DeepSeek CLI with the `--debug` flag:
...
- `DEEPSEEK_TELEMETRY_ENABLED=true`
- `DEEPSEEK_TELEMETRY_TARGET=local`
- `DEEPSEEK_TELEMETRY_OUTFILE=/path/to/your/log.json`
