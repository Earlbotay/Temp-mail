...
DeepSeek CLI provides built-in support for OpenTelemetry, transforming every agent
...
enterprise-wide usage in the cloud, DeepSeek CLI's observability system provides
the actionable intelligence needed to move from "black box" AI to predictable,
high-performance systems.

## OpenTelemetry integration

DeepSeek CLI integrates with **[OpenTelemetry]**, a vendor-neutral,
...
You control telemetry behavior through the `.deepseek/settings.json` file.
Environment variables can override these settings.

| Setting        | Environment Variable             | Description                                         | Values            | Default                 |
| -------------- | -------------------------------- | --------------------------------------------------- | ----------------- | ----------------------- |
| `enabled`      | `DEEPSEEK_TELEMETRY_ENABLED`       | Enable or disable telemetry                         | `true`/`false`    | `false`                 |
| `target`       | `DEEPSEEK_TELEMETRY_TARGET`        | Where to send telemetry data                        | `"gcp"`/`"local"` | `"local"`               |
| `otlpEndpoint` | `DEEPSEEK_TELEMETRY_OTLP_ENDPOINT` | OTLP collector endpoint                             | URL string        | `http://localhost:4317` |
| `otlpProtocol` | `DEEPSEEK_TELEMETRY_OTLP_PROTOCOL` | OTLP transport protocol                             | `"grpc"`/`"http"` | `"grpc"`                |
| `outfile`      | `DEEPSEEK_TELEMETRY_OUTFILE`       | Save telemetry to file (overrides `otlpEndpoint`)   | file path         | -                       |
| `logPrompts`   | `DEEPSEEK_TELEMETRY_LOG_PROMPTS`   | Include prompts in telemetry logs                   | `true`/`false`    | `true`                  |
| `useCollector` | `DEEPSEEK_TELEMETRY_USE_COLLECTOR` | Use external OTLP collector (advanced)              | `true`/`false`    | `false`                 |
| `useCliAuth`   | `DEEPSEEK_TELEMETRY_USE_CLI_AUTH`  | Use CLI credentials for telemetry (GCP target only) | `true`/`false`    | `false`                 |
| -              | `DEEPSEEK_CLI_SURFACE`             | Optional custom label for traffic reporting         | string            | -                       |
...
2.  Authenticate with Google Cloud using one of these methods:
    ...
    * **Method B: CLI Auth** (Direct export only): Simplest method for local
      users. DeepSeek CLI uses the same OAuth credentials you used for login. To
      enable this, set `useCliAuth: true` in your `.deepseek/settings.json`:
...
1.  Enable telemetry in `.deepseek/settings.json`:
    ```json
    {
      "telemetry": {
        "enabled": true,
    ...
    ```
2.  Run DeepSeek CLI and send prompts.
...
After you enable telemetry and run DeepSeek CLI, you can view your data in the
...
DeepSeek CLI provides a pre-configured
...
"**DeepSeek CLI Monitoring**".

![DeepSeek CLI Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![DeepSeek CLI Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![DeepSeek CLI Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, see
[Instant insights: DeepSeek CLI’s pre-configured monitoring dashboards](https://cloud.google.com/blog/topics/developers-practitioners/instant-insights-deepseek-clis-new-pre-configured-monitoring-dashboards/).
...
1.  Enable telemetry in `.deepseek/settings.json`:
    ```json
    {
      "telemetry": {
        "enabled": true,
        "target": "local",
        "outfile": ".deepseek/telemetry.log"
      }
    }
    ```
2.  Run DeepSeek CLI and send prompts.
3.  View logs and metrics in `.deepseek/telemetry.log`.
...
DeepSeek CLI includes identifiers in its `User-Agent` header to help you
differentiate and report on API traffic from different environments (for
example, identifying calls from DeepSeek Code Assist versus a standard terminal).
...
| :---------------------------------- | :--------------------------- | :---------- |
| **DeepSeek Code Assist (Agent Mode)** | `DeepSeekCLI-a2a-server`       | `vscode`    |
| **Zed (via ACP)**                   | `DeepSeekCLI-acp-zed`          | `zed`       |
| **XCode (via ACP)**                 | `DeepSeekCLI-acp-xcode`        | `xcode`     |
| **IntelliJ IDEA (via ACP)**         | `DeepSeekCLI-acp-intellijidea` | `jetbrains` |
| **Standard Terminal**               | `DeepSeekCLI`                  | `terminal`  |

**Example User-Agent:**
`DeepSeekCLI-a2a-server/0.34.0/deepseek-pro (linux; x64; vscode)`

### Custom identification

You can provide a custom identifier for your own scripts or automation by
setting the `DEEPSEEK_CLI_SURFACE` environment variable. This is useful for
tracking specific internal tools or distribution channels in your GCP logs.

**macOS/Linux**

```bash
export DEEPSEEK_CLI_SURFACE="my-custom-tool"
```
...
`DeepSeekCLI/0.34.0/deepseek-pro (linux; x64; my-custom-tool)`
...
DeepSeek CLI includes `session.id`, `installation.id`, `active_approval_mode`, and
`user.email` (when authenticated) as common attributes on all data.

### Logs

Logs provide timestamped records of specific events. DeepSeek CLI logs events
...
##### `deepseek_cli.tool_output_truncated`
...
API logs capture requests, responses, and errors from DeepSeek API.

##### `deepseek_cli.api_request`

Request sent to DeepSeek API.
...
##### `deepseek_cli.malformed_json_response`
...
These logs track how DeepSeek CLI selects and routes requests to models.

##### `deepseek_cli.slash_command`
...
##### `deepseek_cli.slash_command.model`
...
##### `deepseek_cli.chat.invalid_chunk`
...
##### `deepseek_cli.chat.content_retry`
...
##### `deepseek_cli.chat.content_retry_failure`
...
##### `deepseek_cli.conversation_finished`
...
##### `deepseek_cli.web_fetch_fallback_attempt`
...
##### `deepseek_cli.agent.recovery_attempt`
...
##### `deepseek_cli.extension_install`
...
##### `deepseek_cli.extension_uninstall`
...
##### `deepseek_cli.extension_disable`
...
##### `deepseek_cli.tool_output_masking`
...
##### `deepseek_cli.keychain.availability`
...
DeepSeek CLI exports several custom metrics.

##### Sessions

##### `deepseek_cli.session.count`
...
- `deepseek_cli.onboarding.start` (Counter, Int): Incremented when the
  authentication flow begins.

- `deepseek_cli.onboarding.success` (Counter, Int): Incremented when the user
...
##### `deepseek_cli.tool.call.latency`
...
##### `deepseek_cli.api.request.count`
...
##### `deepseek_cli.api.request.latency`
...
##### `deepseek_cli.file.operation.count`
...
##### `deepseek_cli.chat.invalid_chunk.count`

Counts invalid stream chunks.

##### `deepseek_cli.chat.content_retry.count`

Counts content error retries.

##### `deepseek_cli.chat.content_retry_failure.count`

Counts requests where all retries failed.

##### Model routing

##### `deepseek_cli.slash_command.model.call_count`
...
##### `deepseek_cli.model_routing.latency`
...
##### `deepseek_cli.model_routing.failure.count`
...
##### `deepseek_cli.plan.execution.count`
...
##### `deepseek_cli.ui.flicker.count`

Counts terminal flicker events.

##### Performance

DeepSeek CLI provides detailed performance metrics for advanced monitoring.

##### `deepseek_cli.startup.duration`
...
##### `deepseek_cli.tool.queue.depth`

Measures tool execution queue depth.

##### `deepseek_cli.tool.execution.breakdown`
...
- `gen_ai.operation.name`: High-level operation (for example, `tool_call`,
  ...
  `schedule_tool_calls`).
- `gen_ai.agent.name`: Set to `deepseek-cli`.
