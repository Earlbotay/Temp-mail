# Writing hooks for DeepSeek CLI

This guide will walk you through creating hooks for DeepSeek CLI, from a simple
...
- DeepSeek CLI installed and configured
...
```bash
mkdir -p .deepseek/hooks
cat > .deepseek/hooks/log-tools.sh << 'EOF'
#!/usr/bin/env bash
...
# Log to file
echo "[$(date)] Tool executed: $tool_name" >> .deepseek/tool-log.txt
...
chmod +x .deepseek/hooks/log-tools.sh
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path ".deepseek\hooks"
@"
# Read hook input from stdin
...
# Log to file
"[`$(Get-Date -Format 'o')] Tool executed: `$toolName" | Out-File -FilePath ".deepseek\tool-log.txt" -Append -Encoding utf8

# Return success with empty JSON
"{}"
"@ | Out-File -FilePath ".deepseek\hooks\log-tools.ps1" -Encoding utf8
```

## Exit Code Strategies

There are two ways to control or block an action in DeepSeek CLI:
...
**`.deepseek/hooks/block-secrets.sh`:**
...
**`.deepseek/hooks/inject-context.sh`:**
...
**`.deepseek/hooks/filter-tools.js`:**
        ...
        "hooks": [
          {
            "name": "intent-filter",
            "type": "command",
            "command": "node .deepseek/hooks/filter-tools.js"
          }
        ]
      }
    ]
...
### Configuration (`.deepseek/settings.json`)
        ...
        "hooks": [
          {
            "name": "init",
            "type": "command",
            "command": "node .deepseek/hooks/init.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "memory",
            "type": "command",
            "command": "node .deepseek/hooks/inject-memories.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "filter",
            "type": "command",
            "command": "node .deepseek/hooks/rag-filter.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "security",
            "type": "command",
            "command": "node .deepseek/hooks/security.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "record",
            "type": "command",
            "command": "node .deepseek/hooks/record.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "validate",
            "type": "command",
            "command": "node .deepseek/hooks/validate.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "save",
            "type": "command",
            "command": "node .deepseek/hooks/consolidate.js"
          }
        ]
      }
    ]
...
const logFile = path.join(
  process.env.DEEPSEEK_PROJECT_DIR,
...
[DeepSeek CLI extension](../extensions/index.md). This provides version control,
