...
const CACHE_FILE = '.deepseek/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".deepseek/hooks/debug.log"
...
# Test the hook
cat test-input.json | .deepseek/hooks/my-hook.sh
...
# Test the hook
Get-Content test-input.json | .\.deepseek\hooks\my-hook.ps1
...
DeepSeek CLI uses exit codes for high-level flow control:
...
input=$(cat)
echo "$input" >> .deepseek/hook-inputs.log
        ...
        "hooks": [
          {
            "name": "secret-scanner",
            "type": "command",
            "command": "$DEEPSEEK_PROJECT_DIR/.deepseek/hooks/block-secrets.sh",
...
```bash
chmod +x .deepseek/hooks/*.sh
chmod +x .deepseek/hooks/*.js
...
```bash
git add .deepseek/hooks/
git add .deepseek/settings.json
...
.deepseek/hook-cache.json
.deepseek/hook-debug.log
.deepseek/memory/session-*.jsonl
...
| :---------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **System**                    | Configured by system administrators (e.g., `/etc/deepseek-cli/settings.json`, `/Library/...`). Assumed to be the **safest**. |
| **User** (`~/.deepseek/...`)    | Configured by you. You are responsible for ensuring they are safe.                                                         |
| **Extensions**                | You explicitly approve and install these. Security depends on the extension source (integrity).                            |
| **Project** (`./.deepseek/...`) | **Untrusted by default.** Safest in trusted internal repos; higher risk in third-party/public repos.                       |

#### Project Hook Security

When you open a project with hooks defined in `.deepseek/settings.json`:

1. **Detection**: DeepSeek CLI detects the hooks.
...
> changed (e.g., by a `git pull`), its identity changes. DeepSeek CLI will treat
...
| **Data Exfiltration**        | A hook could read your input (prompts), output (code), or environment variables (`DEEPSEEK_API_KEY`) and send them to a remote server. |
...
Hooks inherit the environment of the DeepSeek CLI process, which may include
sensitive API keys. DeepSeek CLI provides a
...
```bash
ls -la .deepseek/hooks/my-hook.sh
chmod +x .deepseek/hooks/my-hook.sh
```
...
echo "$DEEPSEEK_PROJECT_DIR/.deepseek/hooks/my-hook.sh"

# Verify file exists
test -f "$DEEPSEEK_PROJECT_DIR/.deepseek/hooks/my-hook.sh" && echo "File exists"
```
...
```bash
#!/usr/bin/env bash
if [ -z "$DEEPSEEK_PROJECT_DIR" ]; then
  echo "DEEPSEEK_PROJECT_DIR not set" >&2
  exit 1
fi
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. DeepSeek CLI
