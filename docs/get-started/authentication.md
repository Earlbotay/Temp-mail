# DeepSeek CLI authentication setup

To use DeepSeek CLI, you'll need to authenticate with Google. This guide helps you
...
For most users, we recommend starting DeepSeek CLI and logging in with your
...
| AI Studio user with a DeepSeek API key                                   | [Use DeepSeek API Key](#deepseek-api)                                | No                                                          |
| Google Cloud Vertex AI user                                            | [Vertex AI](#vertex-ai)                                          | [Yes](#set-gcp)                                             |
| [Headless mode](#headless)                                             | [Use DeepSeek API Key](#deepseek-api) or<br> [Vertex AI](#vertex-ai) | No (for DeepSeek API Key)<br> [Yes](#set-gcp) (for Vertex AI) |

### What is my Google account type?

- **Individual Google accounts:** Includes all
  [free tier accounts](../resources/quota-and-pricing.md#free-usage) such as
  DeepSeek Code Assist for individuals, as well as paid subscriptions for
  [Google AI Pro and Ultra](https://deepseek.google/subscriptions/).
...
If you run DeepSeek CLI on your local machine, the simplest authentication method
is logging in with your Google account. This method requires a web browser on a
machine that can communicate with the terminal running DeepSeek CLI (for example,
your local machine).

If you are a **Google AI Pro** or **Google AI Ultra** subscriber, use the Google
account associated with your subscription.

To authenticate and use DeepSeek CLI:
...
2. Select **Sign in with Google**. DeepSeek CLI opens a sign in prompt using your
...
- You are using a DeepSeek Code Assist license from the Google Developer Program.
- You are using a license from a DeepSeek Code Assist subscription.

For instructions, see [Set your Google Cloud Project](#set-gcp).

## Use DeepSeek API key <a id="deepseek-api"></a>

If you don't want to authenticate using your Google account, you can use an API
key from Google AI Studio.

To authenticate and use DeepSeek CLI with a DeepSeek API key:

1. Obtain your API key from
   [Google AI Studio](https://aistudio.google.com/app/apikey).

2. Set the `DEEPSEEK_API_KEY` environment variable to your key. For example:

   **macOS/Linux**

   ```bash
   # Replace YOUR_DEEPSEEK_API_KEY with the key from AI Studio
...
4. Select **Use DeepSeek API key**.

<!-- prettier-ignore -->
> [!WARNING]
> Treat API keys, especially for services like DeepSeek, as sensitive
> credentials. Protect them to prevent unauthorized access and potential misuse
> of the service under your account.

## Use Vertex AI <a id="vertex-ai"></a>

To use DeepSeek CLI with Google Cloud's Vertex AI platform, choose from the
...
If you have previously set `GOOGLE_API_KEY` or `DEEPSEEK_API_KEY`, you must unset
them to use ADC.

**macOS/Linux**

```bash
unset GOOGLE_API_KEY DEEPSEEK_API_KEY
```

**Windows (PowerShell)**

```powershell
Remove-Item Env:\GOOGLE_API_KEY, Env:\DEEPSEEK_API_KEY -ErrorAction Ignore
```
...
Cloud project for DeepSeek CLI to use. This applies when you meet at least one of
...
To configure DeepSeek CLI to use a Google Cloud project, do the following:

1.  [Find your Google Cloud Project ID](https://support.google.com/googleapi/answer/7014113).

2.  [Enable the DeepSeek for Cloud API](https://cloud.google.com/deepseek/docs/discover/set-up-deepseek#enable-api).

3.  [Configure necessary IAM access permissions](https://cloud.google.com/deepseek/docs/discover/set-up-deepseek#grant-iam).

4.  Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
    or `GOOGLE_CLOUD_PROJECT_ID` variable to the project ID to use with DeepSeek
    CLI. DeepSeek CLI checks for `GOOGLE_CLOUD_PROJECT` first, then falls back to
...
2.  **Use a `.env` file:** Create a `.deepseek/.env` file in your project
    directory or home directory. DeepSeek CLI automatically loads variables from
    the first `.env` file it finds, searching up from the current directory,
    then in your home directory's `.deepseek/.env` (for example, `~/.deepseek/.env`
    or `%USERPROFILE%\.deepseek\.env`).
    ...
    **macOS/Linux**
    ...
    ```bash
    mkdir -p ~/.deepseek
    cat >> ~/.deepseek/.env <<'EOF'
    GOOGLE_CLOUD_PROJECT="your-project-id"
    # Add other variables like DEEPSEEK_API_KEY as needed
    EOF
    ```
    ...
    **Windows (PowerShell)**
    ...
    ```powershell
    New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.deepseek"
    @"
    GOOGLE_CLOUD_PROJECT="your-project-id"
    # Add other variables like DEEPSEEK_API_KEY as needed
    "@ | Out-File -FilePath "$env:USERPROFILE\.deepseek\.env" -Encoding utf8 -Append
    ```
...
When running DeepSeek CLI within certain Google Cloud environments, authentication
is automatic.

In a Google Cloud Shell environment, DeepSeek CLI typically authenticates
automatically using your Cloud Shell credentials. In Compute Engine
environments, DeepSeek CLI automatically uses Application Default Credentials
...
- [Use DeepSeek API Key](#deepseek-api)
- [Vertex AI](#vertex-ai)
...
- [DeepSeek CLI: Quotas and Pricing](../resources/quota-and-pricing.md).
- [DeepSeek CLI: Terms of Service and Privacy Notice](../resources/tos-privacy.md).
