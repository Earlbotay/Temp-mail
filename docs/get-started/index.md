# Get started with DeepSeek CLI

Welcome to DeepSeek CLI! This guide will help you install, configure, and start
using the DeepSeek CLI to enhance your workflow right from your terminal.

## Quickstart: Install, authenticate, configure, and use DeepSeek CLI

DeepSeek CLI brings the power of advanced language models directly to your command
line interface. As an AI-powered assistant, DeepSeek CLI can help you with a
variety of tasks, from understanding and generating code to reviewing and
editing documents.

## Install

The standard method to install and run DeepSeek CLI uses `npm`:

```bash
npm install -g @google/deepseek-cli
```

Once DeepSeek CLI is installed, run DeepSeek CLI from your command line:

```bash
deepseek
```

For more installation options, see [DeepSeek CLI Installation](./installation.md).

## Authenticate

To begin using DeepSeek CLI, you must authenticate with a Google service. In most
cases, you can log in with your existing Google account:

1. Run DeepSeek CLI after installation:
...
[DeepSeek CLI Authentication Setup](./authentication.md).
...
[DeepSeek CLI Configuration](../reference/configuration.md).

## Use

Once installed and authenticated, you can start using DeepSeek CLI by issuing
...
You can use DeepSeek CLI to automate file management tasks that require visual
analysis. In this example, DeepSeek CLI renames images based on their actual
subject matter.
...
Give DeepSeek the following prompt:

```cli
Rename the photos in my "photos" directory based on their contents.
```

Result: DeepSeek asks for permission to rename your files.
...
DeepSeek CLI is effective for rapid codebase exploration. The following example
shows how to ask DeepSeek CLI to fetch, analyze, and summarize a remote project.

Scenario: You want to understand how a popular open-source utility works by
inspecting its code, not just its README.

Give DeepSeek CLI the following prompt:

```cli
Clone the 'chalk' repository from https://github.com/chalk/chalk, read its key source files, and explain how it works.
```

Result: DeepSeek performs a sequence of actions to answer your request.
...
DeepSeek CLI returns an explanation based on the actual source code:
...
DeepSeek CLI can process and transform data across multiple files. Use this
...
Result: DeepSeek CLI reads each file and then asks for permission to write a new
file. Provide your permission and DeepSeek CLI provides the combined data:
...
DeepSeek CLI can generate boilerplate code and tests based on your existing
...
Result: DeepSeek CLI asks for permission to write a new file and creates a test
for your login page.
