...
the DeepSeek CLI. It serves as a checklist and quality gate for release manager to
...
- **Command:** `npx -y @google/deepseek-cli@<tag> --version` must return the
...
- **Setup:**
  - [ ] Uninstall any existing global version:
        `npm uninstall -g @google/deepseek-cli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g @google/deepseek-cli@preview`
  - [ ] Verify version: `deepseek --version`
...
- **Basic prompting:**
  - [ ] Run `deepseek "Tell me a joke"` and verify a sensible response.
  - [ ] Run in interactive mode: `deepseek`. Ask a follow-up question to test
        context.

- **Piped input:**
  - [ ] Run `echo "Summarize this" | deepseek` and verify it processes stdin.
...
- **Function calling:**
  - [ ] In interactive mode, ask deepseek to "create a file named hello.md with
...
- [ ] Navigate to `go/deepseek-cli-offline-evals-dash`.
