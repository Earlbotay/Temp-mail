# DeepSeek CLI companion plugin: Interface specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
DeepSeek CLI's IDE mode. For VS Code, these features (native diffing, context
awareness) are provided by the official extension
([marketplace](https://marketplace.visualstudio.com/items?itemName=Google.deepseek-cli-vscode-ide-companion)).
...
DeepSeek CLI and the IDE plugin communicate through a local communication channel.
...
For DeepSeek CLI to connect, it needs to discover which IDE instance it's running
...
- **File location:** The file must be created in a specific directory:
  `os.tmpdir()/deepseek/ide/`. Your plugin must create this directory if it
  doesn't exist.
- **File naming convention:** The filename is critical and **MUST** follow the
  pattern: `deepseek-ide-server-${PID}-${PORT}.json`
...
- **Tie-breaking with environment variables (recommended):** For the most
  ...
  set the `DEEPSEEK_CLI_IDE_SERVER_PORT` environment variable in the integrated
  ...
  `DEEPSEEK_CLI_IDE_SERVER_PORT` variable to identify and connect to the correct
  window's server.
