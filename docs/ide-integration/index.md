...
DeepSeek CLI can integrate with your IDE to provide a more seamless and
...
There are two primary ways to integrate DeepSeek CLI with an IDE:

1.  **VS Code companion extension**: Install the "DeepSeek CLI Companion"
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like DeepSeek CLI.

## VS Code companion extension

The **DeepSeek CLI Companion extension** grants DeepSeek CLI direct access to your
...
- **Native diffing:** When DeepSeek suggests code modifications, you can view the
...
- **VS Code commands:** You can access DeepSeek CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `DeepSeek CLI: Run`: Starts a new DeepSeek CLI session in the integrated
    terminal.
  - `DeepSeek CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `DeepSeek CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `DeepSeek CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run DeepSeek CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside DeepSeek CLI:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.deepseek-cli-vscode-ide-companion).
- **For VS Code forks:** To support forks of VS Code, the extension is also
  published on the
  [Open VSX Registry](https://open-vsx.org/extension/google/deepseek-cli-vscode-ide-companion).
  Follow your editor's instructions for installing extensions from this
  registry.

<!-- prettier-ignore -->
> [!NOTE]
> The "DeepSeek CLI Companion" extension may appear towards the bottom of
...
When enabled, DeepSeek CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask DeepSeek to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **DeepSeek CLI: Accept Diff**.
...
- Open the Command Palette and run **DeepSeek CLI: Close Diff Editor**.
...
DeepSeek CLI is officially available in the **ACP Agent Registry**. This allows
you to install and update DeepSeek CLI directly within supporting IDEs and
...
DeepSeek CLI is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install DeepSeek CLI
directly through their in-built registry features.

## Using with sandboxing

If you are using DeepSeek CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run DeepSeek CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** DeepSeek CLI could not find the necessary environment variables
    (`DEEPSEEK_CLI_IDE_WORKSPACE_PATH` or `DEEPSEEK_CLI_IDE_SERVER_PORT`) to connect
    to the IDE. This usually means the IDE companion extension is not running or
    did not initialize correctly.
  - **Solution:**
    1.  Make sure you have installed the **DeepSeek CLI Companion** extension in
...
If automatic IDE detection fails, or if you are running DeepSeek CLI in a
standalone terminal and want to manually associate it with a specific IDE
instance, you can set the `DEEPSEEK_CLI_IDE_PID` environment variable to the
process ID (PID) of your IDE.

**macOS/Linux**

```bash
export DEEPSEEK_CLI_IDE_PID=12345
```

**Windows (PowerShell)**

```powershell
$env:DEEPSEEK_CLI_IDE_PID=12345
```

When this variable is set, DeepSeek CLI will skip automatic detection and attempt
to connect using the provided PID.

#### Configuration errors

- **Message:**
  `🔴 Disconnected: Directory mismatch. DeepSeek CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run DeepSeek CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running DeepSeek CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run DeepSeek CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install DeepSeek CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "DeepSeek CLI
    Companion", and
