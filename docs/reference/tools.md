...
DeepSeek CLI uses tools to interact with your local environment, access
...
## How to use DeepSeek CLI's tools

Tools are generally invoked automatically by DeepSeek CLI when it needs to perform
an action. However, you can also trigger specific tools manually using shorthand
syntax.

### Automatic execution and security

When the model wants to use a tool, DeepSeek CLI evaluates the request against its
security policies.
...
| Tool                                             | Kind    | Description                                                                          |
| :----------------------------------------------- | :------ | :----------------------------------------------------------------------------------- |
| [`activate_skill`](../tools/activate-skill.md)   | `Other` | Loads specialized procedural expertise from the `.deepseek/skills` directory.          |
| [`get_internal_docs`](../tools/internal-docs.md) | `Think` | Accesses DeepSeek CLI's own documentation for accurate answers about its capabilities. |
| [`save_memory`](../tools/memory.md)              | `Think` | Persists specific facts and project details to your `Deepseek.md` file.                |
...
You can extend DeepSeek CLI with custom tools by configuring
