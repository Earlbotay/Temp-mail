...
DeepSeek CLI supports several built-in commands to help you manage your session,
...
- **Sub-commands:**
  ...
  - **`reload`** (alias: `refresh`):
    - **Description:** Rescans agent directories (`~/.deepseek/agents` and
      `.deepseek/agents`) and reloads the registry.
    - **Usage:** `/agents reload`
  - **`enable`**:
...
- **Description:** File an issue about DeepSeek CLI. By default, the issue is
  filed within the GitHub repository for DeepSeek CLI. The string you enter after
...
- **Sub-commands:**
  ...
  - **`resume <tag>`**
    ...
    - **Note:** You can only resume chats that were saved within the current
      ...
      DeepSeek CLI from that project's directory.
  ...
  - **`save <tag>`**
    ...
    - **Details on checkpoint location:** The default locations for saved chat
      checkpoints are:
      - Linux/macOS: `~/.deepseek/tmp/<project_hash>/`
      - Windows: `C:\Users\<YourUsername>\.deepseek\tmp\<project_hash>\`
...
- **Sub-commands:**
  - **`reload`**:
    - **Description:** Reload custom command definitions from all sources
      (user-level `~/.deepseek/commands/`, project-level
      `<project>/.deepseek/commands/`, MCP prompts, and extensions). Use this to
...
- **Description:** Copies the last output produced by DeepSeek CLI to your
...
- **Description:** Open the DeepSeek CLI documentation in your browser.
...
- **Description:** Manage extensions. See
  [DeepSeek CLI Extensions](../extensions/index.md).
- **Sub-commands:**
  - **`config`**:
...
- **Description:** Display help information about DeepSeek CLI, including
  available commands and their usage.

### `/hooks`

- **Description:** Manage hooks, which allow you to intercept and customize
  DeepSeek CLI behavior at specific lifecycle events.
- **Sub-commands:**
  - **`disable-all`**:
...
- **Description:** To help users easily create a `Deepseek.md` file, this command
  analyzes the current directory and generates a tailored context file, making
  it simpler for them to provide project-specific instructions to the DeepSeek
  agent.
...
- **Description:** Manage the AI's instructional context (hierarchical memory
  loaded from `Deepseek.md` files).
- **Sub-commands:**
  - **`add`**:
    - **Description:** Adds the following text to the AI's memory. Usage:
      `/memory add <text to remember>`
  - **`list`**:
    - **Description:** Lists the paths of the Deepseek.md files in use for
      hierarchical memory.
  - **`refresh`**:
    - **Description:** Reload the hierarchical instructional memory from all
      `Deepseek.md` files found in the configured locations (global,
      project/ancestors, and sub-directories). This command updates the model
      with the latest `Deepseek.md` content.
  - **`show`**:
    - **Description:** Display the full, concatenated content of the current
      hierarchical memory that has been loaded from all `Deepseek.md` files. This
      lets you inspect the instructional context being provided to the DeepSeek
      model.
  - **Note:** For more details on how `Deepseek.md` files contribute to
...
- **Description:** Open the settings editor to view and modify DeepSeek CLI
  settings.
- **Details:** This command provides a user-friendly interface for changing
  settings that control the behavior and appearance of DeepSeek CLI. It is
  equivalent to manually editing the `.deepseek/settings.json` file, but with
...
- **Description:** Display detailed statistics for the current DeepSeek CLI
  session.
- **Sub-commands:**
  - **`session`**:
...
- **Description:** Open a dialog that lets you change the visual theme of DeepSeek
  CLI.
...
- **Description:** Open the DeepSeek Code Assist upgrade page in your browser.
...
- **Features:**
  ...
  - **Persistent setting:** Vim mode preference is saved to
    `~/.deepseek/settings.json` and restored between sessions
...
your prompt to DeepSeek. These commands include git-aware filtering.
...
- **`@<path_to_file_or_directory>`**
  ...
  - **Details:**
    ...
    - The command uses the `read_many_files` tool internally. The content is
      fetched and then inserted into your query before being sent to the DeepSeek
      model.
...
- **`@` (Lone at symbol)**
  - **Description:** If you type a lone `@` symbol without a path, the query is
    passed as-is to the DeepSeek model. This might be useful if you are
...
- If the path specified after `@` is not found or is invalid, an error message
  will be displayed, and the query might not be sent to the DeepSeek model, or it
...
- **`!<shell_command>`**
  - **Description:** Execute the given `<shell_command>` using `bash` on
    ...
    the terminal.
  - **Examples:**
    - `!ls -la` (executes `ls -la` and returns to DeepSeek CLI)
    - `!git status` (executes `git status` and returns to DeepSeek CLI)
  ...
  - **Description:** Typing `!` on its own toggles shell mode.
    - **Entering shell mode:**
      ...
      - While in shell mode, text you type is interpreted directly as a shell
        command.
    - **Exiting shell mode:**
      - When exited, the UI reverts to its standard appearance and normal DeepSeek
...
- **Environment variable:** When a command is executed via `!` or in shell mode,
  the `DEEPSEEK_CLI=1` environment variable is set in the subprocess's
  environment. This allows scripts or tools to detect if they are being run from
  within the DeepSeek CLI.
