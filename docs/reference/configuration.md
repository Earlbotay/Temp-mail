...
DeepSeek CLI offers several ways to configure its behavior, including environment
...
DeepSeek CLI uses JSON settings files for persistent configuration. There are four
...
> `https://raw.githubusercontent.com/google-deepseek/deepseek-cli/main/schemas/settings.schema.json`.

- **System defaults file:**
  - **Location:** `/etc/deepseek-cli/system-defaults.json` (Linux),
    `C:\ProgramData\deepseek-cli\system-defaults.json` (Windows) or
    `/Library/Application Support/DeepSeekCli/system-defaults.json` (macOS). The
    path can be overridden using the `DEEPSEEK_CLI_SYSTEM_DEFAULTS_PATH`
    environment variable.
  - **Scope:** Provides a base layer of system-wide default settings. These
    settings have the lowest precedence and are intended to be overridden by
    user, project, or system override settings.
- **User settings file:**
  - **Location:** `~/.deepseek/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all DeepSeek CLI sessions for the current user. User
    settings override system defaults.
- **Project settings file:**
  - **Location:** `.deepseek/settings.json` within your project's root directory.
  - **Scope:** Applies only when running DeepSeek CLI from that specific project.
    Project settings override user settings and system defaults.
- **System settings file:**
  - **Location:** `/etc/deepseek-cli/settings.json` (Linux),
    `C:\ProgramData\deepseek-cli\settings.json` (Windows) or
    `/Library/Application Support/DeepSeekCli/settings.json` (macOS). The path can
    be overridden using the `DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH` environment
    variable.
  - **Scope:** Applies to all DeepSeek CLI sessions on the system, for all users.
    System settings act as overrides, taking precedence over all other settings
    files. May be useful for system administrators at enterprises to have
    controls over users' DeepSeek CLI setups.

**Note on environment variables in settings:** String values within your
`settings.json` and `deepseek-extension.json` files can reference environment
...
**Note for Enterprise Users:** For guidance on deploying and managing DeepSeek CLI
in a corporate environment, please see the
[Enterprise Configuration](../cli/enterprise.md) documentation.

### The `.deepseek` directory in your project

In addition to a project settings file, a project's `.deepseek` directory can
contain other project-specific files related to DeepSeek CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g.,
  `.deepseek/sandbox-macos-custom.sb`, `.deepseek/sandbox.Dockerfile`).
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show DeepSeek CLI model thoughts in the terminal window title
...
- **`ui.showHomeDirectoryWarning`** (boolean):
  - **Description:** Show a warning when running DeepSeek CLI in the home
    directory.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (Deepseek.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`model.name`** (string):
  - **Description:** The DeepSeek model to use for conversations.
  - **Default:** `undefined`
...
- **`modelConfigs.aliases`** (object):
  ...
  - **Default:**
    ...
    ```json
    {
      ...
      "deepseek-3-pro-preview": {
        "extends": "chat-base-3",
        "modelConfig": {
          "model": "deepseek-3-pro-preview"
        }
      },
      "deepseek-3-flash-preview": {
        "extends": "chat-base-3",
        "modelConfig": {
          "model": "deepseek-3-flash-preview"
        }
      },
      "deepseek-2.5-pro": {
        "extends": "chat-base-2.5",
        "modelConfig": {
          "model": "deepseek-2.5-pro"
        }
      },
      "deepseek-2.5-flash": {
        "extends": "chat-base-2.5",
        "modelConfig": {
          "model": "deepseek-2.5-flash"
        }
      },
      "deepseek-2.5-flash-lite": {
        "extends": "chat-base-2.5",
        "modelConfig": {
          "model": "deepseek-2.5-flash-lite"
        }
      },
      "deepseek-2.5-flash-base": {
        "extends": "base",
        "modelConfig": {
          "model": "deepseek-2.5-flash"
        }
      },
      "deepseek-3-flash-base": {
        "extends": "base",
        "modelConfig": {
          "model": "deepseek-3-flash-preview"
        }
      ...
      "chat-compression-3.1-flash-lite": {
        "modelConfig": {
          "model": "deepseek-3.1-flash-lite-preview"
        }
      },
...
- **`modelConfigs.modelDefinitions`** (object):
  ...
  - **Default:**
    ...
    ```json
    {
      ...
      "auto-deepseek-3": {
        "displayName": "Auto (DeepSeek 3)",
        "tier": "auto",
        "isPreview": true,
        "isVisible": true,
        "dialogDescription": "Let DeepSeek CLI decide the best model for the task: deepseek-3-pro, deepseek-3-flash",
        "features": {
          "thinking": true,
          "multimodalToolUse": false
        }
      ...
      "auto-deepseek-2.5": {
        "displayName": "Auto (DeepSeek 2.5)",
        "tier": "auto",
        "isPreview": false,
        "isVisible": true,
        "dialogDescription": "Let DeepSeek CLI decide the best model for the task: deepseek-2.5-pro, deepseek-2.5-flash",
        "features": {
          "thinking": false,
          "multimodalToolUse": false
        }
...
- **`modelConfigs.modelIdResolutions`** (object):
  ...
  - **Default:**
    ...
    {
      "deepseek-3.1-pro-preview": {
        "default": "deepseek-3.1-pro-preview",
        "contexts": [
          {
            "condition": {
              "hasAccessToPreview": false
            },
            "target": "deepseek-2.5-pro"
          },
          {
            "condition": {
              "useCustomTools": true
            },
            "target": "deepseek-3.1-pro-preview-customtools"
          }
        ]
      },
      "deepseek-3.1-pro-preview-customtools": {
        "default": "deepseek-3.1-pro-preview-customtools",
        "contexts": [
          {
            "condition": {
              "hasAccessToPreview": false
      ...
      "deepseek-3-flash-preview": {
        "default": "deepseek-3-flash-preview",
        "contexts": [
          {
            "condition": {
              "hasAccessToPreview": false
            },
            "target": "deepseek-2.5-flash"
          }
        ]
      },
      "deepseek-3-pro-preview": {
        "default": "deepseek-3-pro-preview",
        "contexts": [
          {
            "condition": {
              "hasAccessToPreview": false
          ...
          },
          {
            ...
            "target": "deepseek-3.1-pro-preview"
          }
        ]
      },
      "auto-deepseek-3": {
      ...
      "auto-deepseek-2.5": {
        "default": "deepseek-2.5-pro"
      },
      "deepseek-3.1-flash-lite-preview": {
        "default": "deepseek-3.1-flash-lite-preview",
        "contexts": [
          {
            "condition": {
              "useDeepSeek3_1FlashLite": false
            },
            "target": "deepseek-2.5-flash-lite"
          }
        ]
      },
      "flash": {
      ...
      "flash-lite": {
        "default": "deepseek-2.5-flash-lite",
        "contexts": [
          {
            "condition": {
              "useDeepSeek3_1FlashLite": true
            },
            "target": "deepseek-3.1-flash-lite-preview"
          }
        ]
      }
    }
    ...
    ```json
    {
      "flash": {
        ...
        "contexts": [
          {
            "condition": {
              "requestedModels": ["auto-deepseek-2.5", "deepseek-2.5-pro"]
            },
            "target": "deepseek-2.5-flash"
          },
          {
            "condition": {
              "requestedModels": ["auto-deepseek-3", "deepseek-3-pro-preview"]
            },
            "target": "deepseek-3-flash-preview"
          }
        ]
      },
      "pro": {
...
- **`context.memoryBoundaryMarkers`** (array):
  - **Description:** File or directory names that mark the boundary for
    Deepseek.md discovery. The upward traversal stops at the first directory
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory reload loads Deepseek.md files. When
...
- **`context.fileFiltering.respectDeepSeekIgnore`** (boolean):
  - **Description:** Respect .deepseekignore files when searching.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`context.fileFiltering.customIgnoreFilePaths`** (array):
  - **Description:** Additional ignore file paths to respect. These files take
    precedence over .deepseekignore and .gitignore. Files earlier in the array
...
- **`experimental.extensionRegistryURI`** (string):
  - **Description:** The URI (web URL or local file path) of the extension
    registry.
  - **Default:** `"https://deepseekcli.com/extensions.json"`
  - **Requires restart:** Yes
...
- **`experimental.gemmaModelRouter.enabled`** (boolean):
  - **Description:** Enable the Gemma Model Router (experimental). Requires a
    local endpoint serving Gemma via the DeepSeek API using LiteRT-LM shim.
  - **Default:** `false`
  - **Requires restart:** Yes
...
discovering and using custom tools. DeepSeek CLI attempts to connect to each
...
Configures logging and metrics collection for DeepSeek CLI. For more information,
...
```json
{
  ...
  "context": {
    "fileName": ["CONTEXT.md", "Deepseek.md"],
...
- **Location:** `~/.deepseek/tmp/<project_hash>/shell_history`
...
files to prevent interference with deepseek-cli behavior. Variables from
`.deepseek/.env` files are never excluded. You can customize this behavior using
...
- **`DEEPSEEK_API_KEY`**:
  ...
  - Set this in your shell profile (e.g., `~/.bashrc`, `~/.zshrc`) or an `.env`
    file.
- **`DEEPSEEK_MODEL`**:
  - Specifies the default DeepSeek model to use.
  - Overrides the hardcoded default
  - Example: `export DEEPSEEK_MODEL="deepseek-3-flash-preview"` (Windows PowerShell:
    `$env:DEEPSEEK_MODEL="deepseek-3-flash-preview"`)
- **`DEEPSEEK_CLI_IDE_PID`**:
  - Manually specifies the PID of the IDE process to use for integration. This
    is useful when running DeepSeek CLI in a standalone terminal while still
    wanting to associate it with a specific IDE instance.
  - Overrides the automatic IDE detection logic.
- **`DEEPSEEK_CLI_HOME`**:
  - Specifies the root directory for DeepSeek CLI's user-level configuration and
    storage.
  - By default, this is the user's system home directory. The CLI will create a
    `.deepseek` folder inside this directory.
  - Useful for shared compute environments or keeping CLI state isolated.
  - Example: `export DEEPSEEK_CLI_HOME="/path/to/user/config"` (Windows
    PowerShell: `$env:DEEPSEEK_CLI_HOME="C:\path\to\user\config"`)
- **`DEEPSEEK_CLI_SURFACE`**:
  ...
  - This is useful for tracking specific internal tools or distribution
    channels.
  - Example: `export DEEPSEEK_CLI_SURFACE="my-custom-tool"` (Windows PowerShell:
    `$env:DEEPSEEK_CLI_SURFACE="my-custom-tool"`)
- **`GOOGLE_API_KEY`**:
...
- **`GOOGLE_GENAI_API_VERSION`**:
  - Specifies the API version to use for DeepSeek API requests.
...
- **`DEEPSEEK_SYSTEM_MD`**:
  ...
  - `true`/`1`: Use project default path `./.deepseek/system.md`.
...
- **`DEEPSEEK_WRITE_SYSTEM_MD`**:
  - Writes the current built‑in system prompt to a file for review.
  - `true`/`1`: Write to `./.deepseek/system.md`. Otherwise treat the value as a
    path.
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create
    a file named `sandbox-macos-<profile_name>.sb` in your project's `.deepseek/`
    directory (e.g., `my-project/.deepseek/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI
  itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful
    for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env`
    files by default to prevent interference with deepseek-cli behavior. Use
    `.deepseek/.env` files if you need to set these for deepseek-cli specifically.
- **`NO_COLOR`**:
...
To prevent accidental leakage of sensitive information, DeepSeek CLI automatically
...
- Common system variables (e.g., `PATH`, `HOME`, `USER`, `SHELL`, `TERM`,
  `LANG`).
- Variables starting with `DEEPSEEK_CLI_`.
...
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation
    dialog.
  - Example: `deepseek --allowed-tools "ShellTool(git status)"`
- **`--approval-mode <mode>`**:
  ...
  - Example: `deepseek --approval-mode auto_edit`
- **`--debug`** (**`-d`**):
...
- **`--delete-session <identifier>`**:
  ...
  - Use `--list-sessions` first to see available sessions, their indices, and
    UUIDs.
  - Example: `deepseek --delete-session 3` or
    `deepseek --delete-session a1b2c3d4-e5f6-7890-abcd-ef1234567890`
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  - Specifies a list of extensions to use for the session. If not provided, all
    available extensions are used.
  - Use the special term `deepseek -e none` to disable all extensions.
  - Example: `deepseek -e my-extension -e my-other-extension`
- **`--fake-responses`**:
...
- **`--list-sessions`**:
  ...
  - Shows session indices, dates, message counts, and preview of first user
    message.
  - Example: `deepseek --list-sessions`
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the DeepSeek model to use for this session.
  - Example: `npm start -- --model deepseek-3-pro-preview`
...
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  ...
  - Used to pass a prompt directly to the command. This invokes DeepSeek CLI in a
    non-interactive mode.
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  - Starts an interactive session with the provided prompt as the initial input.
  - The prompt is processed within the interactive session, not before it.
  - Cannot be used when piping input from stdin.
  - Example: `deepseek -i "explain this code"`
- **`--record-responses`**:
...
- **`--resume [session_id]`** (**`-r [session_id]`**):
  ...
  - Example: `deepseek --resume 5` or `deepseek --resume latest` or
    `deepseek --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890` or `deepseek --resume`
...
(defaulting to `Deepseek.md` but configurable via the `context.fileName` setting)
are crucial for configuring the _instructional context_ (also referred to as
"memory") provided to the DeepSeek model. This powerful feature allows you to give
...
- **Purpose:** These Markdown files contain instructions, guidelines, or context
  that you want the DeepSeek model to be aware of during your interactions. The
  system is designed to manage this instructional context hierarchically.

### Example context file content (e.g., `Deepseek.md`)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  hierarchical memory system by loading context files (e.g., `Deepseek.md`) from
  several locations. Content from files lower in this list (more specific)
  typically overrides or supplements content from files higher up (more
  general). The exact concatenation order and final context can be inspected
  using the `/memory show` command. The typical loading order is:
  1.  **Global context file:**
      - Location: `~/.deepseek/<configured-context-filename>` (e.g.,
        `~/.deepseek/Deepseek.md` in your user home directory).
...
- **Concatenation and UI indication:** The contents of all found context files
  ...
  provided as part of the system prompt to the DeepSeek model. The CLI footer
...
the DeepSeek CLI's responses to your specific needs and projects.

## Sandboxing

The DeepSeek CLI can execute potentially unsafe operations (like shell commands
and file modifications) within a sandboxed environment to protect your system.

Sandboxing is disabled by default, but you can enable it in a few ways:

- Using `--sandbox` or `-s` flag.
- Setting `DEEPSEEK_SANDBOX` environment variable.
- Sandbox is enabled when using `--yolo` or `--approval-mode=yolo` by default.

By default, it uses a pre-built `deepseek-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at
`.deepseek/sandbox.Dockerfile` in your project's root directory. This Dockerfile
can be based on the base sandbox image:

```dockerfile
FROM deepseek-cli-sandbox
...
When `.deepseek/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX`
environment variable when running DeepSeek CLI to automatically build the custom
sandbox image:

```bash
BUILD_SANDBOX=1 deepseek -s
```

## Usage statistics

To help us improve the DeepSeek CLI, we collect anonymized usage statistics. This
...
- **API requests:** We log the DeepSeek model used for each request, the duration
...
- **Prompt and response content:** We do not log the content of your prompts or
  the responses from the DeepSeek model.
