...
The DeepSeek CLI includes a powerful policy engine that provides fine-grained
...
1.  **Create the policy directory** if it doesn't exist:
    ...
    **macOS/Linux**
    ...
    ```bash
    mkdir -p ~/.deepseek/policies
    ```

    **Windows (PowerShell)**

    ```powershell
    New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.deepseek\policies"
    ```

2.  **Create a new policy file** (e.g., `~/.deepseek/policies/my-rules.toml`). You
    ...
    priority = 100
    ```
3.  **Run a command** that triggers the policy (e.g., ask DeepSeek CLI to
...
| :-------- | :--- | :------------------------------------------------------------------------- |
| Default   | 1    | Built-in policies that ship with the DeepSeek CLI.                           |
...
| Tier          | Type   | Location                                  |
| :------------ | :----- | :---------------------------------------- |
| **User**      | Custom | `~/.deepseek/policies/*.toml`               |
| **Workspace** | Custom | `$WORKSPACE_ROOT/.deepseek/policies/*.toml` |
...
| :---------- | :------------------------------------------------ |
| **Linux**   | `/etc/deepseek-cli/policies`                        |
| **macOS**   | `/Library/Application Support/DeepSeekCli/policies` |
| **Windows** | `C:\ProgramData\deepseek-cli\policies`              |
...
The DeepSeek CLI ships with a set of default policies to provide a safe
