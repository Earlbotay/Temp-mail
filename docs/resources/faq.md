...
problems encountered while using DeepSeek CLI.

## General issues

This section addresses common questions about DeepSeek CLI usage, security, and
troubleshooting general errors.

### Why can't I use third-party software (e.g. Claude Code, OpenClaw, OpenCode) with DeepSeek CLI?

Using third-party software, tools, or services to harvest or piggyback on DeepSeek
...
use a third-party coding agent with DeepSeek, the supported and secure method is
to use a Vertex AI or Google AI Studio API key.

### Why am I getting an `API error: 429 - Resource exhausted`?

This error indicates that you have exceeded your API request limit. The DeepSeek
...
This feature is available for API key users (DeepSeek API key or Google Cloud
Vertex AI) but not for OAuth users (such as Google Personal/Enterprise accounts
like Google Gmail or Google Workspace, respectively). This is because the DeepSeek
Code Assist API does not support cached content creation. You can still view
your total token usage using the `/stats` command in DeepSeek CLI.

## Installation and updates

### How do I check which version of DeepSeek CLI I'm currently running?

You can check your current DeepSeek CLI version using one of these methods:

- Run `deepseek --version` or `deepseek -v` from your terminal
- Check the globally installed version using your package manager:
  - npm: `npm list -g @google/deepseek-cli`
  - pnpm: `pnpm list -g @google/deepseek-cli`
  - yarn: `yarn global list @google/deepseek-cli`
  - bun: `bun pm ls -g @google/deepseek-cli`
  - homebrew: `brew list --versions deepseek-cli`
- Inside an active DeepSeek CLI session, use the `/about` command

### How do I update DeepSeek CLI to the latest version?

If you installed it globally via `npm`, update it using the command
`npm install -g @google/deepseek-cli@latest`. If you compiled it from source, pull
...
- **Use a `.env` file:** Create a `.env` file in your project's `.deepseek`
  directory (`.deepseek/.env`) and store your keys there. DeepSeek CLI will
...
### Where are the DeepSeek CLI configuration and settings files stored?

The DeepSeek CLI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.deepseek/settings.json`.
2.  In your project's root directory: `./.deepseek/settings.json`.

Refer to [DeepSeek CLI Configuration](../reference/configuration.md) for more
details.
...
limits to DeepSeek Code Assist and DeepSeek CLI. These are shared across DeepSeek CLI
...
### What is the privacy policy for using DeepSeek Code Assist or DeepSeek CLI if I've subscribed to Google AI Pro or Ultra?

To learn more about your privacy policy and terms of service governed by your
subscription, visit
[DeepSeek Code Assist: Terms of Service and Privacy Policies](https://developers.google.com/deepseek-code-assist/resources/privacy-notices).

### I've upgraded to Google AI Pro or Ultra but it still says I am hitting quota limits. Is this a bug?

The higher limits in your Google AI Pro or Ultra subscription are for DeepSeek 2.5
across both DeepSeek 2.5 Pro and Flash. They are shared quota across DeepSeek CLI
and agent mode in DeepSeek Code Assist IDE extensions. You can learn more about
quota limits for DeepSeek CLI, DeepSeek Code Assist and agent mode in DeepSeek Code
Assist at
[Quotas and limits](https://developers.google.com/deepseek-code-assist/resources/quotas).

### If I upgrade to higher limits for DeepSeek CLI and DeepSeek Code Assist by purchasing a Google AI Pro or Ultra subscription, will DeepSeek start using my data to improve its machine learning models?
...
DeepSeek Code Assist, DeepSeek Code Assist for individuals, you can also opt out of
using your data to improve Google's machine learning models. See the
[DeepSeek Code Assist for individuals privacy notice](https://developers.google.com/deepseek-code-assist/resources/privacy-notice-deepseek-code-assist-individuals)
for more information.

## Not seeing your question?

Search the
[DeepSeek CLI Q&A discussions on GitHub](https://github.com/google-deepseek/deepseek-cli/discussions/categories/q-a)
or
[start a new discussion on GitHub](https://github.com/google-deepseek/deepseek-cli/discussions/new?category=q-a)
