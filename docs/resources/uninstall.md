...
"uninstall" the CLI, you must clear this cache, which will remove deepseek-cli and
...
If you installed the CLI globally (e.g., `npm install -g @google/deepseek-cli`),
...
`brew install deepseek-cli`), use the `brew uninstall` command to remove it.

```bash
brew uninstall deepseek-cli
```

## Method 4: MacPorts

If you installed the CLI globally using MacPorts (e.g.,
`sudo port install deepseek-cli`), use the `port uninstall` command to remove it.

```bash
sudo port uninstall deepseek-cli
```
