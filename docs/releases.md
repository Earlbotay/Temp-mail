...
package names beginning with `@google-deepseek/**` instead of `@google/**`.
...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | @google/deepseek-cli            | @google-deepseek/deepseek-cli                 |
| Core       | @google/deepseek-cli-core       | @google-deepseek/deepseek-cli-core A2A Server |
| A2A Server | @google/deepseek-cli-a2a-server | @google-deepseek/deepseek-cli-a2a-server      |
...
The workflow will then run `npm dist-tag add` for the appropriate `deepseek-cli`,
`deepseek-cli-core` and `deepseek-cli-a2a-server` packages, pointing the specified
...
- `npx -y @google/deepseek-cli@latest --version` to validate the push worked as
  expected if you were not doing a rc or dev tag
- `npx -y @google/deepseek-cli@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall @google/deepseek-cli && npm uninstall -g @google/deepseek-cli && npm cache clean --force &&  npm install @google/deepseek-cli@<version>`
...
1.  Go to the
    [Actions tab](https://github.com/google-deepseek/deepseek-cli/actions/workflows/release-manual.yml)
    of the repository.
...
`packages/cli/google-deepseek-cli-0.1.6.tgz`).
...
- **What happens:** The `npm publish` command is run for the
  `@google/deepseek-cli-core` and `@google/deepseek-cli` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g @google/deepseek-cli` will download these packages, and
  `npm` will handle installing the `@google/deepseek-cli-core` dependency
...
1.  **The JavaScript bundle is created:**
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      `deepseek.js`). The `node-pty` library is excluded from this bundle as it
...
2.  **The `bundle` directory is assembled:**
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `deepseek.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `deepseek.js` (from esbuild) -> `bundle/deepseek.js`
...
3.  **The GitHub release is created:**
    - **What happens:** The contents of the `bundle` directory, including the
      `deepseek.js` executable, are attached as assets to a new GitHub Release.
    - **Why:** This makes the single-file version of the CLI available for
      direct download and enables the
      `npx https://github.com/google-deepseek/deepseek-cli` command, which downloads
...
- **GitHub release:** Publishes a single, bundled `deepseek.js` file that contains
