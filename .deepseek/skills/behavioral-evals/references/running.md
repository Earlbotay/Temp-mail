...
```bash
export DEEPSEEK_API_KEY=$(grep '^DEEPSEEK_API_KEY=' .env | cut -d '=' -f2) && RUN_EVALS=1 npx vitest run --config evals/vitest.config.ts <file_name>
```
...
- **Verbose Reasoning**: Capture raw buffer traces by setting
  `DEEPSEEK_DEBUG_LOG_FILE`:
  ```bash
  export DEEPSEEK_DEBUG_LOG_FILE="debug.log"
  ```
...
```bash
deepseek /fix-behavioral-eval [optional-run-uri]
```
...
```bash
deepseek /promote-behavioral-eval
```
