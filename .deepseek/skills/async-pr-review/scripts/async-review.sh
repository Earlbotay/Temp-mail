...
# Use the repository's local .deepseek/tmp directory for ephemeral worktrees and logs
pr_dir="${base_dir}/.deepseek/tmp/async-reviews/pr-${pr_number}"
...
git branch -D "deepseek-async-pr-${pr_number}" >> "${log_dir}/setup.log" 2>&1 || true
git worktree prune >> "${log_dir}/setup.log" 2>&1 || true

echo "📡 Fetching PR #${pr_number}..." | tee -a "${log_dir}/setup.log"
if ! git fetch origin -f "pull/${pr_number}/head:deepseek-async-pr-${pr_number}" >> "${log_dir}/setup.log" 2>&1; then
...
if [[ ! -d "${target_dir}" ]]; then
  ...
  if ! git worktree add "${target_dir}" "deepseek-async-pr-${pr_number}" >> "${log_dir}/setup.log" 2>&1; then
    echo 1 > "${log_dir}/setup.exit"
...
# Dynamically resolve deepseek binary (fallback to your nightly path)
DEEPSEEK_CMD="$(command -v deepseek || echo "${HOME}/.gcli/nightly/node_modules/.bin/deepseek")"
# shellcheck disable=SC2312
POLICY_PATH="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/policy.toml"

echo "  ↳ [3/5] Starting DeepSeek code review..."
rm -f "${log_dir}/review.exit"
{ "${DEEPSEEK_CMD}" --policy "${POLICY_PATH}" -p "/review-frontend ${pr_number}" > "${log_dir}/review.md" 2>&1; echo $? > "${log_dir}/review.exit"; } &
...
if ! "${DEEPSEEK_CMD}" --policy "${POLICY_PATH}" -p "Read the review at ${log_dir}/review.md, the automated test logs at ${log_dir}/npm-test.log, and the manual test execution logs at ${log_dir}/test-execution.log. Summarize the results, state whether the build and tests passed based on ${log_dir}/build-and-lint.exit and ${log_dir}/npm-test.exit, and give a final recommendation for PR ${pr_number}." > "${log_dir}/final-assessment.md" 2>&1; then
