...
import { TestRig } from '@google/deepseek-cli-test-utils';
import {
 ...
 * Can be overridden by setting the DEEPSEEK_MODEL environment variable.
 */
export const EVAL_MODEL =
  process.env['DEEPSEEK_MODEL'] || PREVIEW_DEEPSEEK_FLASH_MODEL;
  ...
  await withEvalRetries(evalCase.name, async () => {
    ...
    try {
      ...
      if (evalCase.messages) {
        ...
        // Temporarily set DEEPSEEK_CLI_HOME so Storage writes to the same
        // directory the CLI subprocess will use (rig.homeDir).
        const originalDeepSeekHome = process.env['DEEPSEEK_CLI_HOME'];
        process.env['DEEPSEEK_CLI_HOME'] = rig.homeDir!;
        try {
          ...
          // Restore original DEEPSEEK_CLI_HOME.
          if (originalDeepSeekHome === undefined) {
            delete process.env['DEEPSEEK_CLI_HOME'];
          } else {
            process.env['DEEPSEEK_CLI_HOME'] = originalDeepSeekHome;
          }
        }
      }
      ...
      const result = await rig.run({
        ...
        env: {
          DEEPSEEK_CLI_ACTIVITY_LOG_TARGET: activityLogFile,
        },
      });
...
export async function prepareWorkspace(
  ...
  for (const [filePath, content] of Object.entries(files)) {
    ...
    if (filePath.startsWith('.deepseek/agents/') && filePath.endsWith('.md')) {
