...
class MockExtensionManager extends ExtensionLoader {
  override getExtensions(): DeepSeekCLIExtension[] {
    return [];
  }
...
export class ComponentRig {
  ...
  constructor(
    ...
    this.testDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `deepseek-component-rig-${uniqueId.slice(0, 8)}-`),
    );
  ...
  async initialize() {
    ...
    // Refresh auth using USE_DEEPSEEK to initialize the real BaseLlmClient
