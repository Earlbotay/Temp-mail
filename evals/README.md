...
> [!TIP] **Agent Automation**: If you are pair-programming with DeepSeek CLI, you
...
behavioral evals focus on specific, granular behaviors relevant to the DeepSeek
CLI's features.
...
First, build the bundled DeepSeek CLI. You must do this after every code change.
...
The
[Evals: Nightly](https://github.com/google-deepseek/deepseek-cli/actions/workflows/evals-nightly.yml)
...
Tests should score at least 66% with key models including DeepSeek 3.1 pro, DeepSeek
3.0 pro, and DeepSeek 3 flash prior to check in and they must pass 100% of the
...
- **CI Evals**: Included in the
  [E2E (Chained)](https://github.com/google-deepseek/deepseek-cli/actions/workflows/chained_e2e.yml)
...
MODEL_LIST=deepseek-3-flash-preview node scripts/run_eval_regression.js
```
...
OUTPUT=$(node scripts/get_trustworthy_evals.js "deepseek-3-flash-preview")

# 2. Run the regression logic for those tests
node scripts/run_regression_check.js "deepseek-3-flash-preview" "$OUTPUT"
```
...
setting the `DEEPSEEK_DEBUG_LOG_FILE` environment variable.
...
unable to accomplish the goal with positive traits. DeepSeek is quite good at
