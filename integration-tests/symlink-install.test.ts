...
describe.skipIf(skipFlaky)(
  ...
  () => {
    ...
    it('canonicalizes the trust path and prevents symlink spoofing', async () => {
      ...
      writeFileSync(join(realExtPath, 'deepseek-extension.json'), extension);
      ...
      writeFileSync(
        join(maliciousExtPath, 'deepseek-extension.json'),
        otherExtension,
      );
      ...
      const runPty = (args: string[]) => {
        const ptyProcess = pty.spawn(process.execPath, [BUNDLE_PATH, ...args], {
          ...
          env: {
            ...process.env,
            DEEPSEEK_CLI_HOME: rig.homeDir!,
            DEEPSEEK_CLI_INTEGRATION_TEST: 'true',
            DEEPSEEK_PTY_INFO: 'node-pty',
          },
        });
