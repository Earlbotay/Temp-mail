...
import { GitService, Storage } from '@google/deepseek-cli-core';
...
describe('Checkpointing Integration', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'deepseek-checkpoint-test-'),
    );
...
`;
    ...
    expect(logOutput).toBe('DeepSeek CLI <deepseek-cli@google.com>');
