...
import { ExitCodes } from '@google/deepseek-cli-core/src/index.js';
...
describe('JSON output', () => {
  ...
  it('should return a JSON error for sd auth mismatch before running', async () => {
    await rig.setup('json-output-auth-mismatch', {
      settings: {
        security: {
          auth: { enforcedType: 'deepseek-api-key', selectedType: '' },
        },
      },
    });
    ...
    expect(payload.error.message).toContain(
      "enforced authentication type is 'deepseek-api-key'",
    );
