...
import { TestRig, DEEPSEEK_DIR } from './test-helper.js';
...
describe('User Policy Regression Repro', () => {
  ...
  it('should respect policies in ~/.deepseek/policies/allowed-tools.toml', async () => {
    rig.setup('user-policy-test', {
      fakeResponsesPath: join(import.meta.dirname, 'user-policy.responses'),
    });

    // Create ~/.deepseek/policies/allowed-tools.toml
    const userPoliciesDir = join(rig.homeDir!, DEEPSEEK_DIR, 'policies');
...
priority = 100
    ...
    // Run deepseek with a prompt that triggers ls -F
    // approvalMode: 'default' in headless mode will DENY if it hits ASK_USER
    const result = await rig.run({
      args: ['-p', 'Run ls -F', '--model', 'deepseek-3.1-pro-preview'],
