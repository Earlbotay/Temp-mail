...
// See: https://github.com/google-deepseek/deepseek-cli/issues/24382
const isDockerSandbox = env['DEEPSEEK_SANDBOX'] === 'docker';
...
describe.skipIf(!chromeAvailable)('browser-policy', () => {
  ...
  it.skipIf(isDockerSandbox)(
    ...
    async () => {
      ...
      const deepseekDir = join(rig.homeDir!, '.deepseek');
      mkdirSync(deepseekDir, { recursive: true });

      // Write to trustedFolders.json
      const trustedFoldersPath = join(deepseekDir, 'trustedFolders.json');
...
`,
      ...
      for (const baseDir of [rig.testDir!, rig.homeDir!]) {
        const settingsPath = join(baseDir, '.deepseek', 'settings.json');
      ...
      const run = await rig.runInteractive({
        ...
        env: {
          DEEPSEEK_CLI_INTEGRATION_TEST: 'true',
        },
      });
  ...
  it('should show the visible warning when browser agent starts in existing session mode', async () => {
    ...
    const stdout = await rig.runCommand(['Open https://example.com'], {
      env: {
        ...
        DEEPSEEK_TELEMETRY_DISABLED: 'true',
        DEV: 'true',
      },
    });
