...
function checkRelease() {
  try {
    ...
    const bucketUri = 'gs://deepseek-cli-vscode-extension/release/1p/signed';
    ...
    const vsixFiles = files.filter((file) =>
      /signed-deepseek-cli-vscode-ide-companion-\d+\.\d+\.\d+-[a-f0-9]{7}\.vsix$/.test(
        file,
      ),
    );
    ...
    const match =
      /signed-deepseek-cli-vscode-ide-companion-(\d+\.\d+\.\d+)-([a-f0-9]{7})\.vsix$/.exec(
        fileName,
      );
