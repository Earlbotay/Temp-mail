...
const getPortFromMock = (
  ...
  const port = vi
    .mocked(replaceMock)
    .mock.calls.find((call) => call[0] === 'DEEPSEEK_CLI_IDE_SERVER_PORT')?.[1];
...
describe('IDEServer', () => {
  ...
  it('should set environment variables and workspace path on start with multiple folders', async () => {
    ...
    const expectedPortFile = path.join(
      ...
      'deepseek',
      'ide',
      `deepseek-ide-server-${process.ppid}-${port}.json`,
    );
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(path.join('/tmp', 'deepseek', 'ide'), {
      recursive: true,
    });
