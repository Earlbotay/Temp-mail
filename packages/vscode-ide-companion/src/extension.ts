...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@google/deepseek-cli-core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'Google.deepseek-cli-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'deepseekCliInfoMessageShown';
export const DIFF_SCHEME = 'deepseek-diff';
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('deepseek-cli.runDeepSeekCLI', async () => {
      ...
      if (selectedFolder) {
        const deepseekCmd = 'deepseek';
        const terminal = vscode.window.createTerminal({
          name: `DeepSeek CLI (${selectedFolder.name})`,
          cwd: selectedFolder.uri.fsPath,
        });
        terminal.show();
        terminal.sendText(deepseekCmd);
      }
    }),
    vscode.commands.registerCommand('deepseek-cli.showNotices', async () => {
