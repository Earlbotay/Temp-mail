...
> **Status:** Implemented. `DeepSeekCliAgent` supports `session()` and
> `resumeSession()`.

Equivalent to `deepseek -p "what does this project do?"`. Loads all workspace and
user settings.

```ts
import { DeepSeekCliAgent } from '@google/deepseek-cli-sdk';

const simpleAgent = new DeepSeekCliAgent({
...
- Static string instructions show up where Deepseek.md content normally would in
  model call
- Dynamic instructions show up and contain dynamic content.

## `Custom Tools`

> **Status:** Implemented. `tool()` helper and `DeepSeekCliAgent` support custom
> tool definitions and execution.

```ts
import { DeepSeekCliAgent, tool, z } from "@google/deepseek-cli-sdk";
...
const toolAgent = new DeepSeekCliAgent({
  tools: [addTool],
});
...
```ts
import { DeepSeekCliAgent, hook, z } from '@google/deepseek-cli-sdk';
...
const myHook = hook(
  ...
  (hook, ctx) => {
    ...
    // ctx.fs gives us a filesystem interface that obeys DeepSeek CLI permissions/sandbox
...
```ts
import { hook } from "@google/deepseek-cli-sdk";
...
> **Status:** Implemented. `skillDir` helper and `DeepSeekCliAgent` support
...
```typescript
import { DeepSeekCliAgent, skillDir } from '@google/deepseek-sdk';
...
```ts
import { DeepSeekCliAgent, subagent } from "@google/deepseek-cli";
...
Potentially the most important feature of the DeepSeek CLI SDK is support for
extensions, which modularly encapsulate all of the primitives listed above:

```ts
import { DeepSeekCliAgent, extension } from "@google/deepseek-cli-sdk";
...
```ts
import { DeepSeekCliAgent } from "@google/deepseek-cli-sdk";
import { DeepSeekCliAcpServer } from "@google/deepseek-cli-sdk/acp";

const server = new DeepSeekCliAcpServer(new DeepSeekCliAgent({...}));
...
export interface SessionContext {
  ...
  agent: DeepSeekCliAgent;
  session: DeepSeekCliSession;
}
...
export interface AgentShellProcess {
  ...
  // investigate how DeepSeek CLI already does this
}
```
...
- To validate the SDK, it would be useful to have a robust way to mock the
  ...
  deterministic.
- Need to work in both DeepSeek-CLI-triggered approvals and optional
...
a `hooks` option to `DeepSeekCliAgentOptions`.

**Tasks:**

1.  Define `Hook` interface and helper functions.
2.  Add `hooks` option to `DeepSeekCliAgentOptions`.
3.  Implement hook registration logic in `DeepSeekCliAgent`.
