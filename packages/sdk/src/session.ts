...
export class DeepSeekCliSession {
  ...
  private client: DeepSeekClient | undefined;
  private initialized = false;

  constructor(
    options: DeepSeekCliAgentOptions,
    private readonly sessionId: string,
    private readonly agent: DeepSeekCliAgent,
    ...
    const configParams: ConfigParameters = {
      ...
      model: options.model || PREVIEW_DEEPSEEK_MODEL_AUTO,
  ...
  async initialize(): Promise<void> {
    ...
    this.client = loopContext2.deepseekClient;

    if (this.resumedData) {
      const history: Content[] = this.resumedData.conversation.messages.map(
        (m) => {
          const role = m.type === 'deepseek' ? 'model' : 'user';
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          let parts: any[] = [];
  ...
  async *sendStream(
    ...
    let request: Parameters<DeepSeekClient['sendMessageStream']>[0] = [
      { text: prompt },
    ];
