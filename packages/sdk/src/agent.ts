...
export class DeepSeekCliAgent {
  private options: DeepSeekCliAgentOptions;

  constructor(options: DeepSeekCliAgentOptions) {
    this.options = options;
  }

  session(options?: { sessionId?: string }): DeepSeekCliSession {
    const sessionId = options?.sessionId || createSessionId();
    return new DeepSeekCliSession(this.options, sessionId, this);
  }

  async resumeSession(sessionId: string): Promise<DeepSeekCliSession> {
