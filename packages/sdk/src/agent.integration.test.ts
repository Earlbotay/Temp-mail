...
import { DeepSeekCliAgent } from './agent.js';
...
describe('DeepSeekCliAgent Integration', () => {
  beforeEach(() => {
  ...
  it('throws on invalid instructions', () => {
    ...
    expect(() => new DeepSeekCliAgent({} as any).session()).not.toThrow();

    expect(() =>
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      new DeepSeekCliAgent({ instructions: 123 as any }).session(),
