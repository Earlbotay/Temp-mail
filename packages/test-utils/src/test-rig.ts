...
import { PREVIEW_DEEPSEEK_MODEL, DEEPSEEK_DIR } from '@google/deepseek-cli-core';
export { DEEPSEEK_DIR };
...
const BUNDLE_PATH = join(__dirname, '..', '..', '..', 'bundle/deepseek.js');

// Get timeout based on environment
export function getDefaultTimeout() {
  if (env['CI']) return 60000; // 1 minute in CI
  if (env['DEEPSEEK_SANDBOX']) return 30000; // 30s in containers
...
export class TestRig {
  ...
  setup(
    ...
    const testFileDir =
      env['INTEGRATION_TEST_FILE_DIR'] || join(os.tmpdir(), 'deepseek-cli-tests');
  ...
  private _createSettingsFile(overrideSettings?: Record<string, unknown>) {
    const projectDeepSeekDir = join(this.testDir!, DEEPSEEK_DIR);
    mkdirSync(projectDeepSeekDir, { recursive: true });

    const userDeepSeekDir = join(this.homeDir!, DEEPSEEK_DIR);
    mkdirSync(userDeepSeekDir, { recursive: true });
    ...
    const settings = deepMerge(
      {
        ...
        ...(env['DEEPSEEK_TEST_TYPE'] === 'integration'
          ? {
              model: {
                name: PREVIEW_DEEPSEEK_MODEL,
              },
            }
          : {}),
        sandbox:
          env['DEEPSEEK_SANDBOX'] !== 'false' ? env['DEEPSEEK_SANDBOX'] : false,
    ...
    writeFileSync(
      join(projectDeepSeekDir, 'settings.json'),
      JSON.stringify(settings, null, 2),
    );
    writeFileSync(
      join(userDeepSeekDir, 'settings.json'),
      JSON.stringify(settings, null, 2),
    );
  }
  ...
  private _createStateFile(overrideState?: Record<string, unknown>) {
    ...
    writeFileSync(
      join(userDeepSeekDir, 'state.json'),
   ...
   * between using the bundled deepseek.js (the default) and using the installed
   * 'deepseek' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    command: string;
    initialArgs: string[];
  } {
    const binaryPath = env['INTEGRATION_TEST_DEEPSEEK_BINARY_PATH'];
    const isNpmReleaseTest =
      env['INTEGRATION_TEST_USE_INSTALLED_DEEPSEEK'] === 'true';
    const deepseekCommand = os.platform() === 'win32' ? 'deepseek.cmd' : 'deepseek';
    let command = 'node';
  ...
  addTestMcpServer(name: string, config: TestMcpConfig | string) {
    ...
    const updateSettings = (dir: string) => {
      const settingsPath = join(dir, DEEPSEEK_DIR, 'settings.json');
      let settings: any = {};
      if (fs.existsSync(settingsPath)) {
        settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
      } else {
        fs.mkdirSync(join(dir, DEEPSEEK_DIR), { recursive: true });
      }
  ...
  private _getCleanEnv(
    ...
    // Clear all DEEPSEEK_ environment variables that might interfere with tests
    // except for those we explicitly want to keep or set.
    for (const key of Object.keys(cleanEnv)) {
      if (
        (key.startsWith('DEEPSEEK_') || key.startsWith('GOOGLE_DEEPSEEK_')) &&
        key !== 'DEEPSEEK_API_KEY' &&
        key !== 'GOOGLE_API_KEY' &&
        key !== 'DEEPSEEK_MODEL' &&
        key !== 'DEEPSEEK_DEBUG' &&
        key !== 'DEEPSEEK_CLI_TEST_VAR' &&
        key !== 'DEEPSEEK_CLI_INTEGRATION_TEST' &&
        !key.startsWith('DEEPSEEK_CLI_ACTIVITY_LOG')
      ) {
        delete cleanEnv[key];
      }
    }
    ...
    return {
      ...cleanEnv,
      DEEPSEEK_CLI_HOME: this.homeDir!,
      DEEPSEEK_PTY_INFO: 'child_process',
      ...extraEnv,
    };
  }
  ...
  private _filterPodmanTelemetry(stdout: string): string {
    if (env['DEEPSEEK_SANDBOX'] !== 'podman') {
      return stdout;
    }
    ...
    return poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `deepseek_cli.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'deepseek_cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'deepseek_cli.tool_call'
      ) {
  ...
  readAllApiRequest(): ParsedLog[] {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === `deepseek_cli.api_request`,
    );
    return apiRequests;
  }
  ...
  async waitForMetric(metricName: string, timeout?: number) {
    ...
    const fullName = metricName.startsWith('deepseek_cli.')
      ? metricName
      : `deepseek_cli.${metricName}`;
  ...
  readMetric(metricName: string): Record<string, unknown> | null {
    ...
    for (const logData of logs) {
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `deepseek_cli.${metricName}`) {
              return metric;
            }
          }
        }
  ...
  readHookLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'deepseek_cli.hook_call'
      ) {
        logs.push({
          hookCall: {
