...
async function getMemoryNodeArgs(): Promise<string[]> {
  ...
  try {
    ...
    // Respect DEEPSEEK_CLI_HOME environment variable, falling back to os.homedir()
    const baseDir =
      process.env['DEEPSEEK_CLI_HOME'] || join(os.homedir(), '.deepseek');
...
async function run() {
  if (!process.env['DEEPSEEK_CLI_NO_RELAUNCH'] && !process.env['SANDBOX']) {
    ...
    const newEnv = { ...process.env, DEEPSEEK_CLI_NO_RELAUNCH: 'true' };
    ...
    const { main } = await import('./src/deepseek.js');
