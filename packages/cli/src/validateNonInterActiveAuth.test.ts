...
describe('validateNonInterActiveAuth', () => {
  let originalEnvDeepSeekApiKey: string | undefined;
  ...
  beforeEach(() => {
    originalEnvDeepSeekApiKey = process.env['DEEPSEEK_API_KEY'];
  ...
  afterEach(() => {
    if (originalEnvDeepSeekApiKey !== undefined) {
      process.env['DEEPSEEK_API_KEY'] = originalEnvDeepSeekApiKey;
    } else {
      delete process.env['DEEPSEEK_API_KEY'];
    }
  ...
  it('uses USE_DEEPSEEK if DEEPSEEK_API_KEY is set', async () => {
  ...
  it('uses USE_VERTEX_AI if both DEEPSEEK_API_KEY and GOOGLE_GENAI_USE_VERTEXAI are set', async () => {
    process.env['DEEPSEEK_API_KEY'] = 'fake-key';
    process.env['GOOGLE_GENAI_USE_VERTEXAI'] = 'true';
    process.env['GOOGLE_CLOUD_PROJECT'] = 'test-project';
    process.env['GOOGLE_CLOUD_LOCATION'] = 'us-central1';
  ...
  it('uses USE_DEEPSEEK if GOOGLE_GENAI_USE_VERTEXAI is false, DEEPSEEK_API_KEY is set, and project/location are available', async () => {
  ...
  it('succeeds if effectiveAuthType matches enforcedAuthType', async () => {
    mockSettings.merged.security.auth.enforcedType = AuthType.USE_DEEPSEEK;
    process.env['DEEPSEEK_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
  ...
  it('exits if configuredAuthType does not match enforcedAuthType', async () => {
    ...
    expect(debugLoggerErrorSpy).toHaveBeenCalledWith(
      "The enforced authentication type is 'oauth-personal', but the current type is 'deepseek-api-key'. Please re-authenticate with the correct type.",
    );
    expect(processExitSpy).toHaveBeenCalledWith(
      ExitCodes.FATAL_AUTHENTICATION_ERROR,
    );
  ...
  describe('JSON output mode', () => {
    ...
    it(`prints JSON error when enforced auth mismatches current auth and exits with code ${ExitCodes.FATAL_AUTHENTICATION_ERROR}`, async () => {
      ...
      );
      {
        ...
        expect(payload.error.code).toBe(ExitCodes.FATAL_AUTHENTICATION_ERROR);
        expect(payload.error.message).toContain(
          "The enforced authentication type is 'deepseek-api-key', but the current type is 'oauth-personal'. Please re-authenticate with the correct type.",
        );
      }
    });
