...
import { ExitCodes } from '@google/deepseek-cli-core';
...
vi.mock('@google/deepseek-cli-core', async () => {
  const actual = await vi.importActual('@google/deepseek-cli-core');
  return {
    ...actual,
...
describe('deferred', () => {
  ...
  describe('runDeferredCommand', () => {
    ...
    it('should execute the deferred command if enabled', async () => {
      ...
      setDeferredCommand({
        handler: mockHandler,
        argv: { _: [], $0: 'deepseek' } as ArgumentsCamelCase,
        commandName: 'mcp',
      });
    ...
    it('should exit with FATAL_CONFIG_ERROR if MCP is disabled', async () => {
      ...
      expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith(
        'error',
        'MCP is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-deepseek-cli',
      );
      expect(mockRunExitCleanup).toHaveBeenCalled();
    ...
    it('should exit with FATAL_CONFIG_ERROR if extensions are disabled', async () => {
      ...
      expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith(
        'error',
        'Extensions is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-deepseek-cli',
      );
      expect(mockRunExitCleanup).toHaveBeenCalled();
      expect(mockExit).toHaveBeenCalledWith(ExitCodes.FATAL_CONFIG_ERROR);
    });
    ...
    it('should exit with FATAL_CONFIG_ERROR if skills are disabled', async () => {
      ...
      expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith(
        'error',
        'Agent skills is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-deepseek-cli',
      );
      expect(mockRunExitCleanup).toHaveBeenCalled();
      expect(mockExit).toHaveBeenCalledWith(ExitCodes.FATAL_CONFIG_ERROR);
    });
  ...
  describe('defer', () => {
    it('should wrap a command module and defer execution', async () => {
      ...
      const argv = { _: [], $0: 'deepseek' } as ArgumentsCamelCase;
