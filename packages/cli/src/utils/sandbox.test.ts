...
import { FatalSandboxError, type SandboxConfig } from '@google/deepseek-cli-core';
  ...
  describe('start_sandbox', () => {
    ...
    it('should mount volumes correctly', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.arrayContaining([
          ...
          '--volume',
          expect.stringMatching(/[\\/]home[\\/]user[\\/]\.deepseek/),
        ]),
        expect.any(Object),
      );
    });
    ...
    it('should handle networkAccess: false in Docker', async () => {
      ...
      expect(execSync).toHaveBeenCalledWith(
        expect.stringContaining('network create --internal deepseek-cli-sandbox'),
        expect.any(Object),
      );
      expect(spawn).toHaveBeenCalledWith(
        'docker',
        expect.arrayContaining(['--network', 'deepseek-cli-sandbox']),
        expect.any(Object),
      );
    });
    ...
    it('should pass through GOOGLE_DEEPSEEK_BASE_URL and GOOGLE_VERTEX_BASE_URL', async () => {
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'docker',
        image: 'deepseek-cli-sandbox',
      });
      process.env['GOOGLE_DEEPSEEK_BASE_URL'] = 'http://deepseek.proxy';
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          '--env',
          'GOOGLE_DEEPSEEK_BASE_URL=http://deepseek.proxy',
          '--env',
    ...
    it('should handle user creation on Linux if needed', async () => {
      ...
      expect(entrypointCmd).toContain('su -p deepseek');
    });

    describe('LXC sandbox', () => {
      const LXC_RUNNING = JSON.stringify([
        { name: 'deepseek-sandbox', status: 'Running' },
      ]);
      const LXC_STOPPED = JSON.stringify([
        { name: 'deepseek-sandbox', status: 'Stopped' },
      ]);
      ...
      it('should run lxc exec with correct args for a running container', async () => {
        ...
        expect(spawn).toHaveBeenCalledWith(
          'lxc',
          expect.arrayContaining(['exec', 'deepseek-sandbox', '--cwd']),
          expect.objectContaining({ stdio: 'inherit' }),
        );
      });
  ...
  describe('gVisor (runsc)', () => {
    it('should use docker with --runtime=runsc on Linux', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        1,
        'docker',
        expect.arrayContaining(['images', '-q', 'deepseek-cli-sandbox']),
      );
