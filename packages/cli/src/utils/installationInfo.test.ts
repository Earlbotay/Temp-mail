...
import { isGitRepository, debugLogger } from '@google/deepseek-cli-core';
...
describe('getInstallationInfo', () => {
  ...
  it('should detect running via npx', () => {
    const npxPath = `/Users/test/.npm/_npx/12345/bin/deepseek`;
  ...
  it('should detect running via pnpx', () => {
    const pnpxPath = `/Users/test/.pnpm/_pnpx/12345/bin/deepseek`;
  ...
  it('should detect running via bunx', () => {
    const bunxPath = `/Users/test/.bun/install/cache/12345/bin/deepseek`;
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/opt/homebrew/Cellar/deepseek-cli/1.0.0/bin/deepseek';
    process.argv[1] = cliPath;

    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew --prefix deepseek-cli')) {
        return '/opt/homebrew/opt/deepseek-cli';
      }
    ...
    mockedRealPathSync.mockImplementation((p) => {
      ...
      if (p === '/opt/homebrew/opt/deepseek-cli') {
        return '/opt/homebrew/Cellar/deepseek-cli/1.0.0';
      }
      return String(p);
    });
    ...
    expect(mockedExecSync).toHaveBeenCalledWith(
      expect.stringContaining('brew --prefix deepseek-cli'),
      expect.anything(),
    );
    expect(info.packageManager).toBe(PackageManager.HOMEBREW);
    expect(info.isGlobal).toBe(true);
    expect(info.updateMessage).toBe(
      'Installed via Homebrew. Please update with "brew upgrade deepseek-cli".',
    );
  });
  ...
  it('should fall through if brew command fails', () => {
    ...
    const cliPath = '/usr/local/bin/deepseek';
    process.argv[1] = cliPath;
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@google/deepseek-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @google/deepseek-cli@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@google/deepseek-cli/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add @google/deepseek-cli@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/install/global/node_modules/@google/deepseek-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @google/deepseek-cli@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect local installation and identify yarn from lockfile', () => {
    const localPath = `${projectRoot}/node_modules/.bin/deepseek`;
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    const globalPath = `/usr/local/bin/deepseek`;
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g @google/deepseek-cli@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should NOT detect Homebrew if deepseek-cli is installed in brew but running from npm location', () => {
    Object.defineProperty(process, 'platform', {
      value: 'darwin',
    });
    // Path looks like standard global NPM
    const cliPath =
      '/usr/local/lib/node_modules/@google/deepseek-cli/dist/index.js';
    process.argv[1] = cliPath;

    // Setup mocks
    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew list')) {
        return Buffer.from('deepseek-cli\n');
      }
