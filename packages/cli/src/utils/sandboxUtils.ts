...
import { debugLogger, DEEPSEEK_DIR } from '@google/deepseek-cli-core';

export const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'deepseek-cli-sandbox';
export const SANDBOX_NETWORK_NAME = 'deepseek-cli-sandbox';
export const SANDBOX_PROXY_NAME = 'deepseek-cli-sandbox-proxy';
...
export function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const projectSandboxBashrc = `${DEEPSEEK_DIR}/sandbox.bashrc`;
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : isDebugMode
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which deepseek)`
        : 'deepseek';
