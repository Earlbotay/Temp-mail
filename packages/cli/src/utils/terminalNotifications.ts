...
import { debugLogger, writeToStdout } from '@google/deepseek-cli-core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'DeepSeek CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open DeepSeek CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'DeepSeek CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open DeepSeek CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'DeepSeek CLI session complete',
