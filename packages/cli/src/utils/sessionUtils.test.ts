...
describe('SessionSelector', () => {
  ...
  it('should list session with deepseek message even without user message', async () => {
    const sessionIdDeepSeekOnly = randomUUID();

    // Create test session files
    const chatsDir = path.join(tmpDir, 'chats');
    await fs.mkdir(chatsDir, { recursive: true });

    // Session with only deepseek message - should be listed
    const sessionDeepSeekOnly = {
      sessionId: sessionIdDeepSeekOnly,
      projectHash: 'test-hash',
      startTime: '2024-01-01T10:00:00.000Z',
      lastUpdated: '2024-01-01T10:30:00.000Z',
      messages: [
    ...
    await fs.writeFile(
      path.join(
        chatsDir,
        `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionIdDeepSeekOnly.slice(0, 8)}.json`,
      ),
      JSON.stringify(sessionDeepSeekOnly, null, 2),
    );

    const sessionSelector = new SessionSelector(storage);
    const sessions = await sessionSelector.listSessions();

    // Should list the session with deepseek message
    expect(sessions.length).toBe(1);
    expect(sessions[0].id).toBe(sessionIdDeepSeekOnly);
  });
...
describe('hasUserOrAssistantMessage', () => {
  ...
  it('should return true when session has deepseek message', () => {
    const messages = [
      {
        type: 'deepseek',
        content: 'Hello, how can I help?',
  ...
  it('should return true when session has both user and deepseek messages', () => {
    const messages = [
      {
        type: 'user',
        content: 'Hello',
