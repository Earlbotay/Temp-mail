...
import { debugLogger, isGitRepository } from '@google/deepseek-cli-core';
...
export function getInstallationInfo(
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        const brewPrefix = childProcess
          .execSync('brew --prefix deepseek-cli', {
            encoding: 'utf8',
        ...
        // Brew is not installed or deepseek-cli is not installed via brew.
    ...
    if (
      ...
      const updateCommand = 'pnpm add -g @google/deepseek-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @google/deepseek-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/install/global')) {
      const updateCommand = 'bun add -g @google/deepseek-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @google/deepseek-cli@latest';
    return {
