...
import { debugLogger, type Config } from '@google/deepseek-cli-core';
...
async function startOrJoinDevTools(
  ...
  const mod = await import('@google/deepseek-cli-devtools');
...
export async function setupInitialActivityLogger(config: Config) {
  const target = process.env['DEEPSEEK_CLI_ACTIVITY_LOG_TARGET'];
