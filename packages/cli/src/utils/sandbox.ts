...
import type { Config, SandboxConfig } from '@google/deepseek-cli-core';
import {
  coreEvents,
  debugLogger,
  FatalSandboxError,
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        profileFile = path.join(DEEPSEEK_DIR, `sandbox-macos-${profile}.sb`);
      }
      ...
      // start and set up proxy if DEEPSEEK_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['DEEPSEEK_SANDBOX_PROXY_COMMAND'];
    ...
    // determine full path for deepseek-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under deepseek-cli repo
    //
    // note this can only be done with binary linked from deepseek-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('deepseek-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed deepseek binary; ' +
            'run `npm link ./packages/cli` under deepseek-cli repo to switch to linked binary.',
        );
      } else {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              DEEPSEEK_SANDBOX: command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    if (!(await ensureSandboxImageIsPresent(command, image, cliConfig))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the deepseek-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify deepseek-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
    ...
    const userSettingsDirInSandbox = getContainerPath(
      `/home/node/${DEEPSEEK_DIR}`,
    );
    if (!fs.existsSync(userHomeDirOnHost)) {
      fs.mkdirSync(userHomeDirOnHost, { recursive: true });
    }
    const userSettingsDirOnHost = path.join(userHomeDirOnHost, DEEPSEEK_DIR);
    ...
    // DEEPSEEK_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['DEEPSEEK_SANDBOX_PROXY_COMMAND'];
    ...
    const isIntegrationTest =
      process.env['DEEPSEEK_CLI_INTEGRATION_TEST'] === 'true';
    let containerName;
    if (isIntegrationTest) {
      containerName = `deepseek-cli-integration-test-${randomBytes(4).toString(
        'hex',
      )}`;
    ...
    // copy DEEPSEEK_CLI_TEST_VAR for integration tests
    if (process.env['DEEPSEEK_CLI_TEST_VAR']) {
      args.push(
        '--env',
        `DEEPSEEK_CLI_TEST_VAR=${process.env['DEEPSEEK_CLI_TEST_VAR']}`,
      );
    }

    // copy DEEPSEEK_API_KEY(s)
    ...
    // copy GOOGLE_DEEPSEEK_BASE_URL and GOOGLE_VERTEX_BASE_URL
    if (process.env['GOOGLE_DEEPSEEK_BASE_URL']) {
      args.push(
        '--env',
        `GOOGLE_DEEPSEEK_BASE_URL=${process.env['GOOGLE_DEEPSEEK_BASE_URL']}`,
      );
    }
    ...
    if (process.env['DEEPSEEK_MODEL']) {
      args.push('--env', `DEEPSEEK_MODEL=${process.env['DEEPSEEK_MODEL']}`);
    }
    ...
    if (
      ...
      const sandboxVenvPath = path.resolve(DEEPSEEK_DIR, 'sandbox.venv');
    ...
    if (process.env['DEEPSEEK_CLI_INTEGRATION_TEST'] === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
      ...
      // finally switch to that user to run the deepseek process. This is
      // necessary on Linux to ensure the user exists within the
      // container's /etc/passwd file, which is required by os.userInfo().
      const username = 'deepseek';
...
// image. The user creates and manages their own LXC container; DeepSeek runs
// inside it via `lxc exec`. The container name is stored in config.image
// (default: "deepseek-sandbox"). The workspace is bind-mounted into the
...
async function start_lxc_sandbox(
  ...
  const containerName = config.image || 'deepseek-sandbox';
  const workdir = path.resolve(process.cwd());
  ...
  try {
    ...
    const workspaceDeviceName = `deepseek-workspace-${randomBytes(4).toString(
      'hex',
    )}`;
    ...
    if (config.allowedPaths) {
      for (const hostPath of config.allowedPaths) {
        if (hostPath && path.isAbsolute(hostPath) && fs.existsSync(hostPath)) {
          const allowedDeviceName = `deepseek-allowed-${randomBytes(4).toString(
            'hex',
          )}`;
    ...
    const envVarsToForward: Record<string, string | undefined> = {
      ...
      DEEPSEEK_CLI_IDE_SERVER_PORT: process.env['DEEPSEEK_CLI_IDE_SERVER_PORT'],
      DEEPSEEK_CLI_IDE_WORKSPACE_PATH:
        process.env['DEEPSEEK_CLI_IDE_WORKSPACE_PATH'],
