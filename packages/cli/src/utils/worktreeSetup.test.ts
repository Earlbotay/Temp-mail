...
import * as coreFunctions from '@google/deepseek-cli-core';
...
describe('setupWorktree', () => {
  ...
  beforeEach(() => {
    ...
    mockService.setup.mockResolvedValue({
      name: 'my-feature',
      path: '/mock/project/.deepseek/worktrees/my-feature',
      baseSha: 'base-sha',
    });
  });
  ...
  it('should create and switch to a new worktree', async () => {
    ...
    expect(process.env['DEEPSEEK_CLI_WORKTREE_HANDLED']).toBe('1');
  });

  it('should generate a name if worktreeName is undefined', async () => {
    mockService.setup.mockResolvedValue({
      name: 'generated-name',
      path: '/mock/project/.deepseek/worktrees/generated-name',
      baseSha: 'base-sha',
    });
  ...
  it('should skip worktree creation if DEEPSEEK_CLI_WORKTREE_HANDLED is set', async () => {
    process.env['DEEPSEEK_CLI_WORKTREE_HANDLED'] = '1';
