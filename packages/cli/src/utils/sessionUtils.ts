...
/**
 * Checks if a session has at least one user or assistant (deepseek) message.
...
export const hasUserOrAssistantMessage = (messages: MessageRecord[]): boolean =>
  messages.some((msg) => msg.type === 'user' || msg.type === 'deepseek');
...
export function convertSessionToHistoryFormats(
  ...
  for (const msg of messages) {
    ...
    if (msg.type === 'deepseek' && msg.thoughts && msg.thoughts.length > 0) {
