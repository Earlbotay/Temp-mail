...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-deepseek-cli/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-deepseek-cli release:`,
      error,
    );
    throw new Error(
      `Unable to determine the latest run-deepseek-cli release on GitHub.`,
    );
  }
};
