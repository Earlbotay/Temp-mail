...
vi.mock('@google/deepseek-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    deepseekPartsToContentParts: original.deepseekPartsToContentParts,
...
describe('runNonInteractive', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getDeepSeekClient: vi.fn().mockReturnValue(mockDeepSeekClient),
      getToolRegistry: vi.fn().mockReturnValue(mockToolRegistry),
      getMaxSessionTurns: vi.fn().mockReturnValue(10),
      getSessionId: vi.fn().mockReturnValue('test-session-id'),
      getProjectRoot: vi.fn().mockReturnValue('/test/project'),
      storage: {
        getProjectTempDir: vi.fn().mockReturnValue('/test/project/.deepseek/tmp'),
      },
  ...
  async function* createStreamFromEvents(
    events: ServerDeepSeekStreamEvent[],
  ): AsyncGenerator<ServerDeepSeekStreamEvent> {
  ...
  it('should process input and write text output', async () => {
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Hello' },
      { type: DeepSeekEventType.Content, value: ' World' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ];
    mockDeepSeekClient.sendMessageStream.mockReturnValue(
    ...
    expect(mockDeepSeekClient.sendMessageStream).toHaveBeenCalledWith(
      [{ text: 'Test input' }],
  ...
  it('should stream the specific stream started by send', async () => {
    const { LegacyAgentSession } = await import('@google/deepseek-cli-core');
    const streamSpy = vi.spyOn(LegacyAgentSession.prototype, 'stream');
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Hello again' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should register activity logger when DEEPSEEK_CLI_ACTIVITY_LOG_TARGET is set', async () => {
    vi.stubEnv('DEEPSEEK_CLI_ACTIVITY_LOG_TARGET', '/tmp/test.jsonl');
    const events: ServerDeepSeekStreamEvent[] = [
      {
        type: DeepSeekEventType.Finished,
  ...
  it('should not register activity logger when DEEPSEEK_CLI_ACTIVITY_LOG_TARGET is not set', async () => {
    vi.stubEnv('DEEPSEEK_CLI_ACTIVITY_LOG_TARGET', '');
    const events: ServerDeepSeekStreamEvent[] = [
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 0 } },
  ...
  it('should handle a single tool call and respond', async () => {
    const toolCallEvent: ServerDeepSeekStreamEvent = {
    ...
    const firstCallEvents: ServerDeepSeekStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Final answer' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ...
    mockDeepSeekClient.sendMessageStream
    ...
    expect(mockDeepSeekClient.sendMessageStream).toHaveBeenCalledTimes(2);
    expect(mockSchedulerSchedule).toHaveBeenCalledWith(
      [expect.objectContaining({ name: 'testTool' })],
      expect.any(AbortSignal),
    );
    expect(mockDeepSeekClient.sendMessageStream).toHaveBeenNthCalledWith(
      2,
      [{ text: 'Tool response' }],
      expect.any(AbortSignal),
      'prompt-id-2',
  ...
  it('should write a single newline between sequential text outputs from the model', async () => {
    ...
    const modelTurn1: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Use mock tool' },
      toolCallEvent,
    ];
    // Turn 2: Model outputs more text, then requests another tool call.
    const modelTurn2: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Use mock tool again' },
      toolCallEvent,
    ];
    // Turn 3: Model outputs a final answer.
    const modelTurn3: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Finished.' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle error during tool execution and should send error back to the model', async () => {
    ...
    const finalResponse: ServerDeepSeekStreamEvent[] = [
      {
        type: DeepSeekEventType.Content,
  ...
  it('should exit with error if sendMessageStream throws initially', async () => {
    ...
    mockDeepSeekClient.sendMessageStream.mockImplementation(() => {
      throw apiError;
    });
  ...
  it('should preprocess @include commands before sending to the model', async () => {
    ...
    // Mock a simple stream response from the DeepSeek client
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Summary complete.' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should process input and write JSON output with stats', async () => {
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Hello World' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should write JSON output with stats for tool-only commands (no text response)', async () => {
    ...
    const firstCallEvents: ServerDeepSeekStreamEvent[] = [
      toolCallEvent,
      {
        type: DeepSeekEventType.Finished,
    ...
    mockDeepSeekClient.sendMessageStream
      .mockReturnValueOnce(
        createStreamFromEvents([
          { type: DeepSeekEventType.Content, value: 'Let me check that...' },
          toolCallEvent,
          {
            type: DeepSeekEventType.Finished,
            value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
  ...
  it('should execute a slash command that returns a prompt', async () => {
    ...
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Response from command' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should handle slash commands', async () => {
    ...
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Response to slash command' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle cancellation (Ctrl+C)', async () => {
    ...
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Thinking...' },
    ];
  ...
  it('should treat an unknown slash command as a regular prompt', async () => {
    ...
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Response to unknown' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should pass arguments to the slash command action', async () => {
    ...
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Acknowledged' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 1 } },
      },
  ...
  it('should allow a normally-excluded tool when --allowed-tools is set', async () => {
    ...
    const firstCallEvents: ServerDeepSeekStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'file.txt' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should resume chat when resumedSessionData is provided', async () => {
    const events: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Resumed' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    expect(mockDeepSeekClient.resumeChat).toHaveBeenCalledWith(
      expect.any(Array),
      resumedSessionData,
    );
  ...
  it.each([
    {
      name: 'loop detected',
      events: [
        { type: DeepSeekEventType.LoopDetected },
      ] as ServerDeepSeekStreamEvent[],
      input: 'Loop test',
    ...
    },
    {
      ...
      events: [
        { type: DeepSeekEventType.MaxSessionTurns },
      ] as ServerDeepSeekStreamEvent[],
    ...
    async ({ events, input, promptId }) => {
      ...
      const streamEvents: ServerDeepSeekStreamEvent[] = [
        ...events,
        {
          type: DeepSeekEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 0 } },
  ...
  it('should log error when tool recording fails', async () => {
    ...
    const events: ServerDeepSeekStreamEvent[] = [
      toolCallEvent,
      { type: DeepSeekEventType.Content, value: 'Done' },
      {
        type: DeepSeekEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    mockDeepSeekClient.getChat = vi.fn().mockReturnValue(mockChat);
    mockDeepSeekClient.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('model-1');

    // Mock debugLogger.error
    const { debugLogger } = await import('@google/deepseek-cli-core');
  ...
  it('should stop agent execution immediately when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Executing tool...' },
      toolCallEvent,
    ];
    ...
    expect(mockDeepSeekClient.sendMessageStream).toHaveBeenCalledTimes(1);
  ...
  it('should write JSON output when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerDeepSeekStreamEvent[] = [
      { type: DeepSeekEventType.Content, value: 'Partial content' },
      toolCallEvent,
    ];
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event', async () => {
      const events: ServerDeepSeekStreamEvent[] = [
        {
          type: DeepSeekEventType.AgentExecutionStopped,
    ...
    it('should handle AgentExecutionBlocked event', async () => {
      const allEvents: ServerDeepSeekStreamEvent[] = [
        {
          type: DeepSeekEventType.AgentExecutionBlocked,
  ...
  describe('Output Sanitization', () => {
    ...
    it('should sanitize ANSI output by default', async () => {
      const events: ServerDeepSeekStreamEvent[] = [
        { type: DeepSeekEventType.Content, value: ANSI_SEQUENCE },
        { type: DeepSeekEventType.Content, value: ' ' },
        { type: DeepSeekEventType.Content, value: OSC_HYPERLINK },
        {
          type: DeepSeekEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
        },
    ...
    it('should emit warning event for loop_detected in streaming JSON mode', async () => {
      ...
      const streamEvents: ServerDeepSeekStreamEvent[] = [
        { type: DeepSeekEventType.LoopDetected } as ServerDeepSeekStreamEvent,
        { type: DeepSeekEventType.Content, value: 'Continuing after loop' },
        {
          type: DeepSeekEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
        },
