...
to the DeepSeek CLI Policy Engine.
...
- `deepseek-extension.json`: The manifest file.
- `policies/`: Contains the `.toml` policy files.

## How to use

1.  Link this extension to your local DeepSeek CLI installation:

    ```bash
    deepseek extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your DeepSeek CLI session.
...
For security, DeepSeek CLI ignores any `allow` decisions or `yolo` mode
