...
describe('migrate command', () => {
  ...
  beforeEach(() => {
    ...
    mockedLoadSettings.mockReturnValue({
      ...
      workspace: { path: '/test/project/.deepseek' },
    });
  });
  ...
  it('should migrate hooks from settings.json when it exists', async () => {
    ...
    expect(debugLoggerLogSpy).toHaveBeenCalledWith(
      '✓ Hooks successfully migrated to .deepseek/settings.json',
    );
  });
  ...
  it('should replace $CLAUDE_PROJECT_DIR with $DEEPSEEK_PROJECT_DIR', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
  ...
  it('should merge with existing DeepSeek hooks', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
