...
import { debugLogger, getErrorMessage } from '@google/deepseek-cli-core';
...
/**
 * Mapping from Claude Code event names to DeepSeek event names
 */
const EVENT_MAPPING: Record<string, string> = {
  ...
  Stop: 'AfterAgent',
  SubAgentStop: 'AfterAgent', // DeepSeek doesn't have sub-agents, map to AfterAgent
...
/**
 * Mapping from Claude Code tool names to DeepSeek tool names
 */
...
/**
 * Transform a matcher regex to update tool names from Claude to DeepSeek
 */
function transformMatcher(matcher: string | undefined): string | undefined {
  if (!matcher) return matcher;

  let transformed = matcher;
  for (const [claudeName, deepseekName] of Object.entries(TOOL_NAME_MAPPING)) {
...
/**
 * Migrate a Claude Code hook configuration to DeepSeek format
 */
function migrateClaudeHook(claudeHook: unknown): unknown {
  ...
  if ('command' in hook) {
    ...
    // Replace CLAUDE_PROJECT_DIR with DEEPSEEK_PROJECT_DIR in command
  ...
  // Map timeout field (Claude uses seconds, DeepSeek uses seconds)
  // eslint-disable-next-line no-restricted-syntax
...
/**
 * Migrate Claude Code hooks configuration to DeepSeek format
 */
function migrateClaudeHooks(claudeConfig: unknown): Record<string, unknown> {
  ...
  const deepseekHooks: Record<string, unknown> = {};
  ...
  for (const [eventName, eventConfig] of Object.entries(hooksSection)) {
    // Map event name
    const deepseekEventName = EVENT_MAPPING[eventName] || eventName;
    ...
    deepseekHooks[deepseekEventName] = migratedDefinitions;
  }
...
export async function handleMigrateFromClaude() {
  ...
  // Load current DeepSeek settings
  ...
  try {
    ...
    debugLogger.log('✓ Hooks successfully migrated to .deepseek/settings.json');
    debugLogger.log(
      '\nMigration complete! Please review the migrated hooks in .deepseek/settings.json',
    );
  } catch (error) {
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to DeepSeek CLI',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: deepseek hooks migrate --from-claude\n\nMigrate hooks from Claude Code to DeepSeek CLI format.',
      );
    }
    await exitCli();
  },
