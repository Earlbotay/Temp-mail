...
export async function runNonInteractive(
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    if (process.env['DEEPSEEK_CLI_ACTIVITY_LOG_TARGET']) {
    ...
    try {
      ...
      if (resumedSessionData) {
        await deepseekClient.resumeChat(
          convertSessionToClientHistory(
      ...
      while (true) {
        ...
        const responseStream = deepseekClient.sendMessageStream(
        ...
        for await (const event of responseStream) {
          ...
          if (event.type === DeepSeekEventType.Content) {
            const isRaw =
          ...
          } else if (event.type === DeepSeekEventType.ToolCallRequest) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === DeepSeekEventType.LoopDetected) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === DeepSeekEventType.MaxSessionTurns) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
                type: JsonStreamEventType.ERROR,
                timestamp: new Date().toISOString(),
          ...
          } else if (event.type === DeepSeekEventType.Error) {
            throw event.value.error;
          } else if (event.type === DeepSeekEventType.AgentExecutionStopped) {
            ...
            return;
          } else if (event.type === DeepSeekEventType.AgentExecutionBlocked) {
        ...
        if (toolCallRequests.length > 0) {
          ...
          // Record tool calls with full metadata before sending responses to DeepSeek
          try {
            const currentModel =
              deepseekClient.getCurrentSequenceModel() ?? config.getModel();
            deepseekClient
              .getChat()
