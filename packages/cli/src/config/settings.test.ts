...
// Use the (mocked) DEEPSEEK_DIR for consistency
...
vi.mock('@google/deepseek-cli-core', async (importOriginal) => {
  ...
  vi.spyOn(actual.Storage.prototype, 'isWorkspaceHomeDir').mockImplementation(
    function (this: Storage) {
      const target = testResolve(pathMod.dirname(this.getDeepSeekDir()));
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    describe('when DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH is set', () => {
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      delete process.env['DEEPSEEK_API_KEY']; // reset
      delete process.env['TESTTEST']; // reset
      const deepseekEnvPath = path.resolve(
        path.join(MOCK_WORKSPACE_DIR, DEEPSEEK_DIR, '.env'),
      );
      ...
      (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) => {
        ...
        return [path.resolve(USER_SETTINGS_PATH), deepseekEnvPath].includes(
          normalizedP,
        );
      });
      ...
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          ...
          if (normalizedP === path.resolve(USER_SETTINGS_PATH))
            return JSON.stringify(userSettingsContent);
          if (normalizedP === deepseekEnvPath)
    ...
    it('sets environment variables from .env files', () => {
      ...
      expect(process.env['DEEPSEEK_API_KEY']).toEqual('test-key');
    });
    ...
    it('loads whitelisted env files from untrusted spaces if sandboxing is enabled', () => {
      ...
      // DEEPSEEK_API_KEY is in the whitelist, so it should be loaded.
      expect(process.env['DEEPSEEK_API_KEY']).toEqual('test-key');
  ...
  describe('Security and Sandbox', () => {
    ...
    describe('sandbox detection', () => {
      it('should detect sandbox when -s is a real flag', () => {
        process.argv = ['node', 'deepseek', '-s', 'some prompt'];
        ...
        // If sandboxed and untrusted, FOO should NOT be loaded, but DEEPSEEK_API_KEY should be.
        expect(process.env['FOO']).toBeUndefined();
        expect(process.env['DEEPSEEK_API_KEY']).toBe('secret');
      });

      it('should detect sandbox when --sandbox is a real flag', () => {
        process.argv = ['node', 'deepseek', '--sandbox', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
        vi.mocked(fs.existsSync).mockReturnValue(true);
        vi.mocked(fs.readFileSync).mockReturnValue('DEEPSEEK_API_KEY=secret');
      ...
      it('should ignore sandbox flags if they appear after --', () => {
        ...
        expect(process.env['DEEPSEEK_API_KEY']).toEqual('secret');
      });

      it('should NOT be tricked by positional arguments that look like flags', () => {
        process.argv = ['node', 'deepseek', 'my -s prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
    ...
    describe('env var sanitization', () => {
      it('should strictly enforce whitelist in untrusted/sandboxed mode', () => {
        process.argv = ['node', 'deepseek', '-s', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
...
GOOGLE_API_KEY=another-secret
    `);
        ...
        expect(process.env['DEEPSEEK_API_KEY']).toBe('secret-key');
      ...
      it('should sanitize shell injection characters in whitelisted env vars in untrusted mode', () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(
          `DEEPSEEK_API_KEY=${maliciousPayload}`,
        );
        ...
        expect(process.env['DEEPSEEK_API_KEY']).toBe('key-whoami-id-');
      });
      ...
      it('should allow . and / in whitelisted env vars but sanitize other characters in untrusted mode', () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(
          `DEEPSEEK_API_KEY=${complexPayload}`,
        );
      ...
      it('should NOT sanitize variables from trusted sources', () => {
        process.argv = ['node', 'deepseek', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: true,
          source: 'file',
        });
      ...
      it('should load environment variables normally when workspace is TRUSTED even if "sandboxed"', () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(`
DEEPSEEK_API_KEY=un-sanitized;key!
MALICIOUS_VAR=allowed-because-trusted
    `);
        ...
        expect(process.env['DEEPSEEK_API_KEY']).toBe('un-sanitized;key!');
