...
import * as ServerConfig from '@google/deepseek-cli-core';
...
vi.mock('@google/deepseek-cli-core', async () => {
  const actualServer = await vi.importActual<typeof ServerConfig>(
    '@google/deepseek-cli-core',
  );
  return {
    ...actualServer,
    IdeClient: {
    ...
    getAdminErrorMessage: vi.fn(
      (_feature) =>
        `YOLO mode is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-deepseek-cli`,
    ),
    ...
    it.each([
      {
        ...
        argv: ['node', 'script.js', 'Hi DeepSeek'],
        expectedQuery: 'Hi DeepSeek',
      ...
      },
      {
        ...
        expectedQuery: '@path ./file.md',
        expectedModel: 'deepseek-2.5-pro',
        debug: false,
      },
      {
        description:
...
describe('loadCliConfig', () => {
  ...
  it('should add IDE workspace folders from DEEPSEEK_CLI_IDE_WORKSPACE_PATH to include directories', async () => {
    vi.stubEnv(
      'DEEPSEEK_CLI_IDE_WORKSPACE_PATH',
  ...
  it('should skip inaccessible workspace folders from DEEPSEEK_CLI_IDE_WORKSPACE_PATH', async () => {
  ...
  it('should use default fileFilter options when unconfigured', async () => {
    ...
    expect(config.getFileFilteringRespectDeepSeekIgnore()).toBe(
      DEFAULT_FILE_FILTERING_OPTIONS.respectDeepSeekIgnore,
    );
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('DEEPSEEK_CLI_IDE_WORKSPACE_PATH', '');
    ...
    vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([
      {
        ...
        id: 'ext1-id',
        version: '1.0.0',
        contextFiles: ['/path/to/ext1/Deepseek.md'],
        isActive: true,
      },
      {
        path: '/path/to/ext2',
...
describe('loadCliConfig model selection', () => {
  ...
  it('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('deepseek-2.5-pro');
  });

  it('uses the default deepseek model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe('auto-deepseek-3');
  });

  it('always prefers model from argv', async () => {
    process.argv = ['node', 'script.js', '--model', 'deepseek-2.5-flash-preview'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings({
        model: {
    ...
    expect(config.getModel()).toBe('deepseek-2.5-flash-preview');
  });
...
describe('loadCliConfig folderTrust', () => {
  ...
  beforeEach(() => {
    ...
    originalIntegrationTest = process.env['DEEPSEEK_CLI_INTEGRATION_TEST'];
    delete process.env['VITEST'];
    delete process.env['DEEPSEEK_CLI_INTEGRATION_TEST'];
  });
  ...
  afterEach(() => {
    ...
    if (originalIntegrationTest !== undefined) {
      process.env['DEEPSEEK_CLI_INTEGRATION_TEST'] = originalIntegrationTest;
    }
...
describe('loadCliConfig interactive', () => {
  ...
  it('should be interactive if positional prompt words are provided with other flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'deepseek-2.5-pro', 'Hello'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
      'test-session',
  ...
  it('should handle multiple positional words with flags', async () => {
    ...
    expect(argv.model).toBe('deepseek-2.5-pro');
  });
  ...
  it('should be interactive if no positional prompt words are provided with flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'deepseek-2.5-pro'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
      'test-session',
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectDeepSeekIgnore',
      getter: (c) => c.getFileFilteringRespectDeepSeekIgnore(),
      value: true,
    },
    {
      property: 'respectDeepSeekIgnore',
...
describe('Telemetry configuration via environment variables', () => {
  ...
  it('should prioritize DEEPSEEK_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize DEEPSEEK_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should throw when DEEPSEEK_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize DEEPSEEK_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv('DEEPSEEK_TELEMETRY_OTLP_ENDPOINT', 'http://deepseek.env.com');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOtlpEndpoint()).toBe('http://deepseek.env.com');
  });

  it('should prioritize DEEPSEEK_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize DEEPSEEK_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize DEEPSEEK_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_OUTFILE', '/deepseek/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOutfile()).toBe('/deepseek/env/telemetry.log');
  });

  it('should prioritize DEEPSEEK_TELEMETRY_USE_COLLECTOR over settings', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_USE_COLLECTOR', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should use settings value when DEEPSEEK_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
  ...
  it('should use settings value when DEEPSEEK_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
      telemetry: { target: ServerConfig.TelemetryTarget.LOCAL },
  ...
  it("should treat DEEPSEEK_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
  ...
  it("should treat DEEPSEEK_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
  ...
  it("should treat DEEPSEEK_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
  ...
  it("should treat DEEPSEEK_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('DEEPSEEK_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
  ...
  describe('extension plan settings', () => {
    beforeEach(() => {
      vi.spyOn(Storage.prototype, 'getProjectTempDir').mockReturnValue(
        '/mock/home/user/.deepseek/tmp/test-project',
      );
    });
    ...
    it('should use plan directory from active extension when user has not specified one', async () => {
      ...
      vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([
        ...
        } as unknown as DeepSeekCLIExtension,
      ]);
