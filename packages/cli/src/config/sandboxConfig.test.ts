...
import { getPackageJson } from '@google/deepseek-cli-core';
...
describe('loadSandboxConfig', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['DEEPSEEK_SANDBOX'];
  ...
  describe('with DEEPSEEK_SANDBOX environment variable', () => {
    it('should use docker if DEEPSEEK_SANDBOX=docker and it exists', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'docker';
    ...
    it('should throw if DEEPSEEK_SANDBOX is an invalid command', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'invalid-command';
    ...
    it('should throw if DEEPSEEK_SANDBOX command does not exist', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'docker' (from DEEPSEEK_SANDBOX)",
      );
    });

    it('should use lxc if DEEPSEEK_SANDBOX=lxc and it exists', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
        enabled: true,
    ...
    it('should throw if DEEPSEEK_SANDBOX=lxc but lxc command does not exist', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'lxc' (from DEEPSEEK_SANDBOX)",
      );
    });
  });
  ...
  describe('with sandbox: true', () => {
    ...
    it('should throw if sandbox: true but no command is found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(
        'DEEPSEEK_SANDBOX is true but failed to determine command for sandbox; ' +
          'install docker or podman or specify command in DEEPSEEK_SANDBOX',
      );
    });
  });
  ...
  describe("with sandbox: 'command'", () => {
    ...
    it('should throw if the specified command does not exist', async () => {
      ...
      await expect(
        ...
        "Missing sandbox command 'podman' (from DEEPSEEK_SANDBOX)",
      );
    });
  ...
  describe('image configuration', () => {
    it('should use image from DEEPSEEK_SANDBOX_IMAGE env var if set', async () => {
      process.env['DEEPSEEK_SANDBOX_IMAGE'] = 'env/image';
      process.env['DEEPSEEK_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
  ...
  describe('with sandbox: runsc (gVisor)', () => {
    ...
    it('should use runsc via DEEPSEEK_SANDBOX environment variable', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig({}, {});
    ...
    it('should prioritize DEEPSEEK_SANDBOX over CLI and settings', async () => {
      process.env['DEEPSEEK_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig(
    ...
    it('should throw if runsc binary not found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow(
        "Missing sandbox command 'runsc' (from DEEPSEEK_SANDBOX)",
      );
    });
