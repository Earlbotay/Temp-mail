...
export class ExtensionManager extends ExtensionLoader {
  ...
  private loadedExtensions: DeepSeekCLIExtension[] | undefined;
  private loadingPromise: Promise<DeepSeekCLIExtension[]> | null = null;
  ...
  async installOrUpdateExtension(
    ...
    let extension: DeepSeekCLIExtension | null;
    try {
...
Would you like to attempt to install via "git clone" instead?`,
      ...
      try {
        ...
        if (missingSettings.length > 0) {
          const message = `Extension "${newExtensionConfig.name}" has missing settings: ${missingSettings
            ...
            .join(
              ', ',
            )}. Please run "deepseek extensions config ${newExtensionConfig.name} [setting-name]" to configure them.`;
  ...
  protected override async startExtension(extension: DeepSeekCLIExtension) {
  ...
  protected override async stopExtension(extension: DeepSeekCLIExtension) {
   ...
   */
  async loadExtensions(): Promise<DeepSeekCLIExtension[]> {
    if (this.loadedExtensions) {
    ...
    this.loadingPromise = (async () => {
      try {
        ...
        const builtExtensions = builtExtensionsOrNull.filter(
          (ext): ext is DeepSeekCLIExtension => ext !== null,
        );
  ...
  async loadExtension(
    extensionDir: string,
  ): Promise<DeepSeekCLIExtension | null> {
    if (this.loadingPromise) {
  ...
  toOutputString(extension: DeepSeekCLIExtension): string {
