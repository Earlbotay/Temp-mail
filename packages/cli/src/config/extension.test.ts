...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = getRealPath(
      fs.mkdtempSync(path.join(os.tmpdir(), 'deepseek-cli-test-home-')),
    );
    tempWorkspaceDir = getRealPath(
      fs.mkdtempSync(path.join(tempHomeDir, 'deepseek-cli-test-workspace-')),
    );
  ...
  describe('loadExtensions', () => {
    ...
    it('should load context file path when Deepseek.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'Deepseek.md'),
      ]);
...
`;
    ...
    describe('id generation', () => {
      ...
      it('should warn about hooks during installation', async () => {
        ...
        fs.writeFileSync(
          path.join(sourceExtDir, 'deepseek-extension.json'),
          JSON.stringify({
  ...
  describe('installExtension', () => {
    ...
    it('should throw an error and cleanup if deepseek-extension.json is missing', async () => {
    ...
    it('should throw an error for invalid JSON in deepseek-extension.json', async () => {
    ...
    it('should throw an error for missing name in deepseek-extension.json', async () => {
      const sourceExtDir = getRealPath(
        createExtension({
          extensionsDir: tempHomeDir,
...
${INSTALL_WARNING_MESSAGE}`,
    ...
    describe('installing from github', () => {
      const gitUrl = 'https://github.com/google/deepseek-test-extension.git';
      const extensionName = 'deepseek-test-extension';
      ...
      it('should fallback to git clone without consent if no release data is found on first install', async () => {
        ...
        expect(mockRequestConsent).toHaveBeenCalledExactlyOnceWith(
          expect.stringContaining(
            'Installing extension "deepseek-test-extension"',
          ),
        );
        expect(mockGit.clone).toHaveBeenCalled();
        const metadataPath = path.join(
  ...
  describe('uninstallExtension', () => {
    ...
    it('should uninstall an extension by its source URL', async () => {
      const gitUrl = 'https://github.com/google/deepseek-sql-extension.git';
      const sourceExtDir = createExtension({
        extensionsDir: userExtensionsDir,
        name: 'deepseek-sql-extension',
        version: '1.0.0',
        installMetadata: {
          source: gitUrl,
          type: 'git',
      ...
      expect(mockLogExtensionUninstall).toHaveBeenCalled();
      expect(ExtensionUninstallEvent).toHaveBeenCalledWith(
        'deepseek-sql-extension',
        hashValue('deepseek-sql-extension'),
        hashValue('https://github.com/google/deepseek-sql-extension'),
        'success',
      );
    });
  ...
  describe('enableExtension', () => {
    ...
    const getActiveExtensions = (): DeepSeekCLIExtension[] => {
      const extensions = extensionManager.getExtensions();
