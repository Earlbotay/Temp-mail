...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/DeepSeekCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\deepseek-cli\\settings.json';
  } else {
    return '/etc/deepseek-cli/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['DEEPSEEK_CLI_SYSTEM_DEFAULTS_PATH']) {
    return process.env['DEEPSEEK_CLI_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeDeepSeekEnvPath = path.join(homedir(), DEEPSEEK_DIR, '.env');
...
export function loadEnvironment(
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(DEEPSEEK_DIR);
