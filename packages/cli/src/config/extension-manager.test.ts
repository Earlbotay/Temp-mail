...
describe('ExtensionManager', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'deepseek-cli-test-home-'),
    );
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, 'deepseek-cli-test-workspace-'),
    );
  ...
  describe('loadExtensions parallel loading', () => {
    ...
    it('should throw if there are duplicate extension names', async () => {
      ...
      fs.writeFileSync(path.join(ext1Dir, 'deepseek-extension.json'), config);
      fs.writeFileSync(
        path.join(ext1Dir, 'metadata.json'),
        JSON.stringify({ type: 'local', source: ext1Dir }),
      );

      fs.writeFileSync(path.join(ext2Dir, 'deepseek-extension.json'), config);
      fs.writeFileSync(
    ...
    it('should wait for loadExtensions to finish when loadExtension is called concurrently', async () => {
      ...
      fs.writeFileSync(
        path.join(externalDir, 'deepseek-extension.json'),
  ...
  describe('symlink handling', () => {
    ...
    beforeEach(() => {
      ...
      fs.writeFileSync(
        path.join(extensionDir, 'deepseek-extension.json'),
  ...
  describe('Extension Renaming', () => {
    it('should support renaming an extension during update', async () => {
      ...
      fs.writeFileSync(
        path.join(extDir, 'deepseek-extension.json'),
      ...
      fs.writeFileSync(
        path.join(newSourceDir, 'deepseek-extension.json'),
    ...
    it('should prevent renaming if the new name conflicts with an existing extension', async () => {
      ...
      fs.mkdirSync(ext1Dir, { recursive: true });
      fs.writeFileSync(
        path.join(ext1Dir, 'deepseek-extension.json'),
      ...
      fs.mkdirSync(ext2Dir, { recursive: true });
      fs.writeFileSync(
        path.join(ext2Dir, 'deepseek-extension.json'),
        JSON.stringify({ name: 'ext2', version: '1.0.0' }),
      );
      fs.writeFileSync(
        path.join(ext2Dir, 'metadata.json'),
