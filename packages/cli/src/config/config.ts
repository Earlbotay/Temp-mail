...
import {
  setDeepSeekMdFilename as setServerDeepSeekMdFilename,
  getCurrentDeepSeekMdFilename,
  ApprovalMode,
...
import type { ExtensionEvents } from '@google/deepseek-cli-core/src/utils/extensionLoader.js';
...
export async function parseArguments(
  ...
  yargsInstance
    .command('$0 [query..]', 'Launch DeepSeek CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('worktree', {
          ...
          skipValidation: true,
          description:
            'Start DeepSeek in a new git worktree. If no name is provided, one is generated automatically.',
        ...
        .option('allowed-tools', {
          ...
          nargs: 1,
          description:
            '[DEPRECATED: Use Policy Engine instead See https://deepseekcli.com/docs/core/policy-engine] Tools that are allowed to run without confirmation',
          coerce: coerceCommaSeparated,
        })
        .option('extensions', {
          alias: 'e',
        ...
        .option('resume', {
          ...
          coerce: (value: string): string => {
            // When --resume passed with a value (`deepseek --resume 123`): value = "123" (string)
            // When --resume passed without a value (`deepseek --resume`): value = "" (string)
...
export async function loadCliConfig(
  ...
  if (argv.sandbox) {
    process.env['DEEPSEEK_SANDBOX'] = 'true';
  }
  ...
  // directly to the Config constructor in core, and have core handle setDeepSeekMdFilename.
  // However, loadHierarchicalDeepSeekMemory is called *before* createServerConfig.
  if (settings.context?.fileName) {
    setServerDeepSeekMdFilename(settings.context.fileName);
  } else {
    // Reset to default if not provided in settings.
    setServerDeepSeekMdFilename(getCurrentDeepSeekMdFilename());
  }
  ...
  // so DeepSeek has context of all open folders, not just the cwd.
  const ideWorkspacePath = process.env['DEEPSEEK_CLI_IDE_WORKSPACE_PATH'];
  if (ideWorkspacePath) {
  ...
  let extensionRegistryURI =
    process.env['DEEPSEEK_CLI_EXTENSION_REGISTRY_URI'] ??
  ...
  if (!experimentalJitContext) {
    // Call the (now wrapper) loadHierarchicalDeepSeekMemory which calls the server's version
  ...
  const defaultModel = PREVIEW_DEEPSEEK_MODEL_AUTO;
  const specifiedModel =
    argv.model || process.env['DEEPSEEK_MODEL'] || settings.model?.name;

  const resolvedModel =
    specifiedModel === DEEPSEEK_MODEL_ALIAS_AUTO
      ? defaultModel
  ...
  return new Config({
    ...
    sessionId,
    clientVersion: await getVersion(),
    embeddingModel: DEFAULT_DEEPSEEK_EMBEDDING_MODEL,
    ...
    deepseekMdFileCount: fileCount,
    deepseekMdFilePaths: filePaths,
    approvalMode,
    disableYoloMode:
...
async function resolveWorktreeSettings(
  ...
  try {
    ...
    if (isDeepSeekWorktree(toplevel, projectRoot)) {
