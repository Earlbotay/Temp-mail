...
import { Storage } from '@google/deepseek-cli-core';
...
vi.mock('@google/deepseek-cli-core');
...
describe('ExtensionStorage', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(Storage).mockImplementation(
      () =>
        ({
          getExtensionsDir: () =>
            path.join(mockHomeDir, '.deepseek', 'extensions'),
  ...
  it('should return the correct user extensions directory', () => {
    const expectedDir = path.join(mockHomeDir, '.deepseek', 'extensions');
    expect(ExtensionStorage.getUserExtensionsDir()).toBe(expectedDir);
  });

  it('should create a temporary directory', async () => {
    const mockTmpDir = '/tmp/deepseek-extension-123';
    ...
    expect(fs.promises.mkdtemp).toHaveBeenCalledWith(
      path.join('/tmp', 'deepseek-extension'),
    );
