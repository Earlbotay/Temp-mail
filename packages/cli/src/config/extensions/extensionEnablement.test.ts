...
function createTestDir() {
  const dirPath = `/virtual-tmp/deepseek-test-${Math.random().toString(36).substring(2, 15)}`;
...
describe('ExtensionEnablementManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(ExtensionStorage.getUserExtensionsDir).mockReturnValue(
      path.join(testDir.path, DEEPSEEK_DIR),
    );
  ...
  it('should correctly prioritize more specific enable rules', () => {
    ...
    manager.enable('ext-test', true, '/Users/chrstn/deepseek-cli');

    expect(manager.isEnabled('ext-test', '/Users/chrstn/deepseek-cli')).toBe(
      true,
    );
  });
