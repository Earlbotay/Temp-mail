...
// Mock @google/deepseek-cli-core
vi.mock('@google/deepseek-cli-core', async (importOriginal) => {
  const core = await importOriginal<typeof import('@google/deepseek-cli-core')>();
  return {
    ...core,
    homedir: mockHomedir,
...
describe('ExtensionManager agents loading', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deepseek-test-agents-'));
    mockHomedir.mockReturnValue(tempDir);

    // Create the extensions directory that ExtensionManager expects
    extensionsDir = path.join(tempDir, '.deepseek', EXTENSIONS_DIRECTORY_NAME);
