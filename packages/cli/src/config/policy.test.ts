...
import { writeToStderr } from '@google/deepseek-cli-core';
...
describe('resolveWorkspacePolicyState', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deepseek-cli-test-'));
    // Redirect DEEPSEEK_CLI_HOME to the temp directory to isolate integrity storage
    vi.stubEnv('DEEPSEEK_CLI_HOME', tempDir);

    workspaceDir = path.join(tempDir, 'workspace');
    fs.mkdirSync(workspaceDir);
    policiesDir = path.join(workspaceDir, '.deepseek', 'policies');
  ...
  it('should not return workspace policies if cwd is the home directory', async () => {
    const policiesDir = path.join(tempDir, '.deepseek', 'policies');
    fs.mkdirSync(policiesDir, { recursive: true });
    fs.writeFileSync(path.join(policiesDir, 'policy.toml'), 'rules = []');
