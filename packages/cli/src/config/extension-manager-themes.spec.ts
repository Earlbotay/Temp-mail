...
describe('ExtensionManager theme loading', () => {
  ...
  beforeAll(async () => {
    tempHomeDir = await fs.promises.mkdtemp(
      path.join(tmpdir(), 'deepseek-cli-test-'),
    );
  });
  ...
  beforeEach(() => {
    process.env['DEEPSEEK_CLI_HOME'] = tempHomeDir;
    userExtensionsDir = path.join(tempHomeDir, DEEPSEEK_DIR, 'extensions');
