...
vi.mock('./hooks/useDeepSeekStream.js');
...
import { useDeepSeekStream } from './hooks/useDeepSeekStream.js';
...
describe('AppContainer State Management', () => {
  ...
  const mockedUseDeepSeekStream = useDeepSeekStream as Mock;
  ...
  const DEFAULT_DEEPSEEK_STREAM_MOCK = {
    streamingState: 'idle',
    submitQuery: vi.fn(),
    initError: null,
    pendingHistoryItems: [],
  ...
  beforeEach(() => {
    ...
    mockedUseDeepSeekStream.mockReturnValue(DEFAULT_DEEPSEEK_STREAM_MOCK);
  ...
  describe('State Initialization', () => {
    it('sends a macOS notification when confirmation is pending and terminal is unfocused', async () => {
      ...
      mockedUseDeepSeekStream.mockReturnValue({
        ...DEFAULT_DEEPSEEK_STREAM_MOCK,
        pendingHistoryItems: [
          {
            type: 'tool_group',
            tools: [
    ...
    it('sends a macOS notification when a response completes while unfocused', async () => {
      ...
      mockedUseDeepSeekStream.mockImplementation(() => ({
        ...DEFAULT_DEEPSEEK_STREAM_MOCK,
      ...
      expect(
        ...
        expect.objectContaining({
          ...
          detail: 'DeepSeek CLI finished responding.',
        }),
      );
      expect(terminalNotificationsMocks.notifyViaTerminal).toHaveBeenCalled();
  ...
  describe('Session Resumption', () => {
    ...
    it('initializes chat recording service when config has it', async () => {
      ...
      vi.spyOn(configWithRecording, 'getDeepSeekClient').mockReturnValue(
        mockDeepSeekClient as unknown as ReturnType<Config['getDeepSeekClient']>,
      );
  ...
  describe('Session Recording Integration', () => {
    it('provides chat recording service configuration', async () => {
      ...
      expect(configWithRecording.getDeepSeekClient).toBeDefined();
      expect(mockDeepSeekClient.getChatRecordingService).toBeDefined();
  ...
  describe('Session Resume Flow', () => {
    it('accepts resumed session data', async () => {
      ...
      vi.spyOn(configWithClient, 'getDeepSeekClient').mockReturnValue(
        mockDeepSeekClient as unknown as ReturnType<Config['getDeepSeekClient']>,
      );
      ...
      expect(mockDeepSeekClient.resumeChat).toBeDefined();
  ...
  describe('Terminal Title Update Feature', () => {
    ...
    it('should use legacy terminal title when dynamicWindowTitle is false', async () => {
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'DeepSeek CLI (workspace)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
    ...
    it('should use CLI_TITLE environment variable when set', async () => {
      ...
      vi.stubEnv('CLI_TITLE', 'Custom DeepSeek Title');
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'✦  Working… (Custom DeepSeek Title)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
  });
  ...
  describe('Keyboard Input Handling (CTRL+C / CTRL+D)', () => {
    ...
    describe('Background Shell Toggling (CTRL+B)', () => {
      ...
      it('should show and focus background shell on Ctrl+B if hidden', async () => {
        ...
        mockedUseDeepSeekStream.mockReturnValue(deepseekStreamMock);

        await setupKeypressTest();

        // Update the mock state when toggled to simulate real behavior
        mockToggleBackgroundTask.mockImplementation(() => {
          deepseekStreamMock.isBackgroundTaskVisible = true;
        });
  ...
  describe('Banner Text', () => {
    it('should render placeholder banner text for USE_DEEPSEEK auth type', async () => {
  ...
  describe('onCancelSubmit Behavior', () => {
    ...
    // Helper to extract arguments from the useDeepSeekStream hook call
    // This isolates the positional argument dependency to a single location
    const extractUseDeepSeekStreamArgs = (args: unknown[]) => ({
    ...
    it('preserves buffer when cancelling, even if empty (user is in control)', async () => {
      ...
      const { onCancelSubmit } = extractUseDeepSeekStreamArgs(
        mockedUseDeepSeekStream.mock.lastCall!,
      );
  ...
  describe('Permission Handling', () => {
    ...
    it.each([true, false])(
      ...
      async (allowed) => {
        ...
        const { submitQuery } = mockedUseDeepSeekStream();
