...
describe('DeepSeekPrivacyNotice', () => {
  const onExit = vi.fn();
  ...
  it('renders correctly', async () => {
    const { lastFrame, unmount } = await render(
      <DeepSeekPrivacyNotice onExit={onExit} />,
    );

    expect(lastFrame()).toContain('DeepSeek API Key Notice');
    expect(lastFrame()).toContain('By using the DeepSeek API');
