...
import { makeFakeConfig } from '@google/deepseek-cli-core';
...
describe('<HistoryItemDisplay />', () => {
  ...
  it('renders AboutBox for "about" type', async () => {
    ...
    expect(lastFrame()).toContain('About DeepSeek CLI');
    unmount();
  });
  ...
  describe.each([true, false])(
    'deepseek items (alternateBuffer=%s)',
    (useAlternateBuffer) => {
      const longCode =
        '# Example code block:\n' +
        '```python\n' +
      ...
      it('should render a truncated deepseek item', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'deepseek',
          text: longCode,
      ...
      it('should render a full deepseek item when using availableTerminalHeightDeepSeek', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'deepseek',
          text: longCode,
        ...
        const { lastFrame, unmount } = await renderWithProviders(
          <HistoryItemDisplay
            ...
            terminalWidth={80}
            availableTerminalHeight={10}
            availableTerminalHeightDeepSeek={Number.MAX_SAFE_INTEGER}
          />,
          {
            config: makeFakeConfig({ useAlternateBuffer }),
            settings: createMockSettings({ ui: { useAlternateBuffer } }),
      ...
      it('should render a truncated deepseek_content item', async () => {
        const item: HistoryItem = {
          id: 1,
      ...
      it('should render a full deepseek_content item when using availableTerminalHeightDeepSeek', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'deepseek_content',
          text: longCode,
