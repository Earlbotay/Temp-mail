...
describe('<Footer />', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('DEEPSEEK_CLI_HOME', path.join(root, 'Users', 'test'));
    vi.stubEnv('SANDBOX', '');
  ...
  describe('sandbox and trust info', () => {
    ...
    it('should display "current process" for custom sandbox when SANDBOX env is set', async () => {
      vi.stubEnv('SANDBOX', 'deepseek-cli-test-sandbox');
      const { lastFrame, unmount } = await renderWithProviders(<Footer />, {
        config: mockConfig,
        width: 120,
        uiState: {
  ...
  describe('fallback mode display', () => {
    it('should display Flash model when in fallback mode, not the configured Pro model', async () => {
      const { lastFrame, unmount } = await renderWithProviders(<Footer />, {
        ...
        uiState: {
          sessionStats: mockSessionStats,
          currentModel: 'deepseek-2.5-flash', // Fallback active, showing Flash
        },
      });

      // Footer should show the effective model (Flash), not the config model (Pro)
      expect(lastFrame()).toContain('deepseek-2.5-flash');
      expect(lastFrame()).not.toContain('deepseek-2.5-pro');
      unmount();
    });
    ...
    it('should display Pro model when NOT in fallback mode', async () => {
      const { lastFrame, unmount } = await renderWithProviders(<Footer />, {
        ...
        uiState: {
          sessionStats: mockSessionStats,
          currentModel: 'deepseek-2.5-pro', // Normal mode, showing Pro
        },
      });

      expect(lastFrame()).toContain('deepseek-2.5-pro');
      unmount();
    });
  });
});
