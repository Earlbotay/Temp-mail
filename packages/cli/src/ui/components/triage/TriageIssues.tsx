...
export const TriageIssues = ({
  ...
  const analyzeIssue = useCallback(
    async (issue: Issue): Promise<AnalysisResult> => {
      ...
      const prompt = `
I am triaging GitHub issues for the DeepSeek CLI project. I need to identify issues that should be closed because they are:
...
`;
      const response = await client.generateJson({
        modelConfigKey: { model: 'deepseek-3-flash-preview' },
    ...
    <Box flexDirection="column">
      ...
      >
        {state.status === 'analyzing' ? (
          <Box>
            <Spinner type="dots" />
            <Text> Analyzing issue with DeepSeek...</Text>
          </Box>
        ) : analysis ? (
          <>
