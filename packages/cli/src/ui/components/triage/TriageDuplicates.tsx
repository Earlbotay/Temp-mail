...
interface AnalysisResult {
  ...
  recommendation: DeepSeekRecommendation;
}
...
`;
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
      const rec = response as unknown as DeepSeekRecommendation;
    ...
    try {
      if (action === 'duplicate' && state.canonicalIssue) {
        ...
        await spawnAsync('gh', [
          ...
          '-X',
          'PATCH',
          `repos/google-deepseek/deepseek-cli/issues/${String(state.currentIssue.number).replace(/[^a-zA-Z0-9-]/g, '')}`, // Sanitize issue number
          '-f',
          'state=closed',
          '-f',
