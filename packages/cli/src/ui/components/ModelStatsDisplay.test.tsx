...
import { ToolCallDecision, LlmRole } from '@google/deepseek-cli-core';
...
const renderWithMockedStats = async (
  ...
  width?: number,
  currentModel: string = 'deepseek-2.5-pro',
) => {
...
describe('<ModelStatsDisplay />', () => {
  ...
  it('should display stats for multiple models correctly', async () => {
    ...
    const output = lastFrame();
    expect(output).toContain('deepseek-2.5-pro');
    expect(output).toContain('deepseek-2.5-flash');
    expect(output).toMatchSnapshot();
    unmount();
  });
  ...
  it('should display a single model correctly', async () => {
    ...
    const output = lastFrame();
    expect(output).toContain('deepseek-2.5-pro');
    expect(output).not.toContain('deepseek-2.5-flash');
    expect(output).toMatchSnapshot();
    unmount();
  });

  it('should handle models with long names (deepseek-3-*-preview) without layout breaking', async () => {
    const { lastFrame, unmount } = await renderWithMockedStats(
      {
        models: {
    ...
    const output = lastFrame();
    expect(output).toContain('deepseek-3-pro-');
    expect(output).toContain('deepseek-3-flash-');
    expect(output).toMatchSnapshot();
    unmount();
  });
