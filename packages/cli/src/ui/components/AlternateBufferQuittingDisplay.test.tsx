...
import { CoreToolCallStatus } from '@google/deepseek-cli-core';
...
vi.mock('../DeepSeekRespondingSpinner.js', () => ({
  DeepSeekRespondingSpinner: () => <Text>Spinner</Text>,
}));
...
describe('AlternateBufferQuittingDisplay', () => {
  ...
  it('renders with user and deepseek messages', async () => {
    persistentStateMock.setData({ tipsShown: 0 });
    const history: HistoryItem[] = [
      { id: 1, type: 'user', text: 'Hello DeepSeek' },
      { id: 2, type: 'deepseek', text: 'Hello User!' },
    ];
    const { lastFrame, unmount } = await renderWithProviders(
      <AlternateBufferQuittingDisplay />,
      {
    ...
    expect(lastFrame()).toMatchSnapshot('with_user_deepseek_messages');
    unmount();
  });
});
