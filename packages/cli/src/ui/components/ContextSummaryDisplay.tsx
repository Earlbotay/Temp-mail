...
import { type IdeContext, type MCPServerConfig } from '@google/deepseek-cli-core';
...
export const ContextSummaryDisplay: React.FC<ContextSummaryDisplayProps> = ({
  ...
  const deepseekMdText = (() => {
    if (deepseekMdFileCount === 0) {
      return '';
    }
    const allNamesTheSame = new Set(contextFileNames).size < 2;
    const name = allNamesTheSame ? contextFileNames[0] : 'context';
    return `${deepseekMdFileCount} ${name} file${
      deepseekMdFileCount > 1 ? 's' : ''
    }`;
  })();
