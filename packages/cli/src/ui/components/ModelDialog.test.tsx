...
import type { Config, ModelSlashCommandEvent } from '@google/deepseek-cli-core';
...
vi.mock('@google/deepseek-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    PREVIEW_DEEPSEEK_3_1_FLASH_LITE_MODEL: 'deepseek-3.1-flash-lite-preview',
  };
});
...
describe('<ModelDialog />', () => {
  ...
  const mockGetDeepSeek31LaunchedSync = vi.fn();
  const mockGetDeepSeek31FlashLiteLaunchedSync = vi.fn();
  ...
  const mockConfig: MockConfig = {
    ...
    getIdeMode: () => false,
    getDeepSeek31LaunchedSync: mockGetDeepSeek31LaunchedSync,
    getDeepSeek31FlashLiteLaunchedSync: mockGetDeepSeek31FlashLiteLaunchedSync,
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    mockGetModel.mockReturnValue(DEFAULT_DEEPSEEK_MODEL_AUTO);
    mockGetHasAccessToPreviewModel.mockReturnValue(false);
    mockGetDeepSeek31LaunchedSync.mockReturnValue(false);
    mockGetDeepSeek31FlashLiteLaunchedSync.mockReturnValue(false);
    mockGetProModelNoAccess.mockResolvedValue(false);
    mockGetProModelNoAccessSync.mockReturnValue(false);

    // Default implementation for getDisplayString
    mockGetDisplayString.mockImplementation((val: string) => {
      if (val === 'auto-deepseek-2.5') return 'Auto (DeepSeek 2.5)';
      if (val === 'auto-deepseek-3') return 'Auto (Preview)';
      return val;
    });
  });
  ...
  it('renders the "manual" view initially for users with no pro access and filters Pro models with correct order', async () => {
    ...
    mockGetDeepSeek31FlashLiteLaunchedSync.mockReturnValue(true);
    ...
    expect(output).not.toContain(DEFAULT_DEEPSEEK_MODEL);
    expect(output).not.toContain(PREVIEW_DEEPSEEK_MODEL);

    // Verify order: Flash Preview -> Flash Lite Preview -> Flash -> Flash Lite
    const flashPreviewIdx = output.indexOf(PREVIEW_DEEPSEEK_FLASH_MODEL);
    const flashLitePreviewIdx = output.indexOf(
      PREVIEW_DEEPSEEK_3_1_FLASH_LITE_MODEL,
    );
    const flashIdx = output.indexOf(DEFAULT_DEEPSEEK_FLASH_MODEL);
    const flashLiteIdx = output.indexOf(DEFAULT_DEEPSEEK_FLASH_LITE_MODEL);
  ...
  it('switches to "manual" view when "Manual" is selected and uses getDisplayString for models', async () => {
    mockGetDisplayString.mockImplementation((val: string) => {
      if (val === DEFAULT_DEEPSEEK_MODEL) return 'Formatted Pro Model';
      if (val === DEFAULT_DEEPSEEK_FLASH_MODEL) return 'Formatted Flash Model';
      if (val === DEFAULT_DEEPSEEK_FLASH_LITE_MODEL)
  ...
  it('sets model and closes when a model is selected in "manual" view', async () => {
    ...
    // Now in manual view. Default selection is first item (DEFAULT_DEEPSEEK_MODEL)
    await act(async () => {
      stdin.write('\r');
    });
    await waitUntilReady();
    ...
    await waitFor(() => {
      expect(mockSetModel).toHaveBeenCalledWith(DEFAULT_DEEPSEEK_MODEL, true);
      expect(mockOnClose).toHaveBeenCalled();
    });
    unmount();
  });
  ...
  it('goes back to "main" view on escape in "manual" view', async () => {
    ...
    await waitFor(() => {
      expect(lastFrame()).toContain(DEFAULT_DEEPSEEK_MODEL);
    });
  ...
  it('shows the preferred manual model in the main view option using getDisplayString', async () => {
    mockGetModel.mockReturnValue(DEFAULT_DEEPSEEK_MODEL);
    mockGetDisplayString.mockImplementation((val: string) => {
      if (val === DEFAULT_DEEPSEEK_MODEL) return 'My Custom Model Display';
      if (val === 'auto-deepseek-2.5') return 'Auto (DeepSeek 2.5)';
      return val;
    });
    const { lastFrame, unmount } = await renderComponent();
  ...
  describe('Preview Models', () => {
    ...
    it('shows DeepSeek 3 models in manual view when DeepSeek 3.1 is NOT launched', async () => {
      mockGetDeepSeek31LaunchedSync.mockReturnValue(false);
      const { lastFrame, stdin, waitUntilReady, unmount } =
        await renderComponent();
      ...
      const output = lastFrame();
      expect(output).toContain(PREVIEW_DEEPSEEK_MODEL);
      expect(output).toContain(PREVIEW_DEEPSEEK_FLASH_MODEL);
      unmount();
    });

    it('shows DeepSeek 3.1 models in manual view when DeepSeek 3.1 IS launched', async () => {
      mockGetDeepSeek31LaunchedSync.mockReturnValue(true);
      const { lastFrame, stdin, waitUntilReady, unmount } =
      ...
      const output = lastFrame();
      expect(output).toContain(PREVIEW_DEEPSEEK_3_1_MODEL);
      expect(output).toContain(PREVIEW_DEEPSEEK_FLASH_MODEL);
      unmount();
    });

    it('uses custom tools model when DeepSeek 3.1 IS launched and auth is DeepSeek API Key', async () => {
      mockGetDeepSeek31LaunchedSync.mockReturnValue(true);
      const { stdin, waitUntilReady, unmount } = await renderComponent(
      ...
      // Select DeepSeek 3.1 (first item in preview section)
      await act(async () => {
        stdin.write('\r');
      });
      await waitUntilReady();
    ...
    it('shows Flash Lite Preview model regardless of tier when flag is enabled', async () => {
      ...
      const output = lastFrame();
      expect(output).toContain(PREVIEW_DEEPSEEK_3_1_FLASH_LITE_MODEL);
      unmount();
    });
  });
});
