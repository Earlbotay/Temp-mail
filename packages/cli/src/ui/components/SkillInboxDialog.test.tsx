...
import type { Config, InboxSkill } from '@google/deepseek-cli-core';
import {
  dismissInboxSkill,
  listInboxSkills,
  moveInboxSkill,
...
describe('SkillInboxDialog', () => {
  beforeEach(() => {
    ...
    mockMoveInboxSkill.mockResolvedValue({
      success: true,
      message: 'Moved "inbox-skill" to ~/.deepseek/skills.',
    });
  ...
  it('shows inline feedback when reloading skills fails after a move', async () => {
    ...
    await waitFor(() => {
      expect(lastFrame()).toContain(
        'Moved "inbox-skill" to ~/.deepseek/skills. Failed to reload skills: reload hook failed',
      );
    });
