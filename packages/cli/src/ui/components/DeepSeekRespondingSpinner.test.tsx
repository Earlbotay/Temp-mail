...
import { DeepSeekRespondingSpinner } from './DeepSeekRespondingSpinner.js';
...
vi.mock('./DeepSeekSpinner.js', () => ({
  DeepSeekSpinner: ({ altText }: { altText?: string }) => (
    <Text>DeepSeekSpinner {altText}</Text>
  ),
}));

describe('DeepSeekRespondingSpinner', () => {
  ...
  it('renders spinner when responding', async () => {
    ...
    const { lastFrame, unmount } = await render(<DeepSeekRespondingSpinner />);
    expect(lastFrame()).toContain('DeepSeekSpinner');
    unmount();
  });
  ...
  it('renders non-responding display when provided', async () => {
    ...
    const { lastFrame, unmount } = await render(
      <DeepSeekRespondingSpinner nonRespondingDisplay="Waiting..." />,
    );
