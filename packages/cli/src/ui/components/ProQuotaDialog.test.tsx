  ...
  describe('for flash model failures', () => {
    it('should render "Keep trying" and "Stop" options', async () => {
      const { unmount } = await render(
        <ProQuotaDialog
          failedModel={DEFAULT_DEEPSEEK_FLASH_MODEL}
          fallbackModel={DEFAULT_DEEPSEEK_FLASH_MODEL}
          message="flash error"
  ...
  describe('for non-flash model failures', () => {
    describe('when it is a terminal quota error', () => {
      it('should render switch, upgrade, and stop options for LOGIN_WITH_GOOGLE', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            failedModel="deepseek-2.5-pro"
            fallbackModel="deepseek-2.5-flash"
        ...
        expect(RadioButtonSelect).toHaveBeenCalledWith(
          expect.objectContaining({
            items: [
              {
                label: 'Switch to deepseek-2.5-flash',
                value: 'retry_always',
                key: 'retry_always',
              },
              {
      ...
      it('should NOT render upgrade option for USE_DEEPSEEK', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            failedModel="deepseek-2.5-pro"
            fallbackModel="deepseek-2.5-flash"
      ...
      it('should render "Keep trying" and "Stop" options when failed model and fallback model are the same', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            failedModel={PREVIEW_DEEPSEEK_MODEL}
            fallbackModel={PREVIEW_DEEPSEEK_MODEL}
            message="flash error"
            isTerminalQuotaError={true}
            onChoice={mockOnChoice}
          />,
      ...
      it('should NOT render upgrade option for LOGIN_WITH_GOOGLE if tier is Ultra', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            ...
            isModelNotFoundError={false}
            authType={AuthType.LOGIN_WITH_GOOGLE}
            tierName="DeepSeek Advanced Ultra"
            onChoice={mockOnChoice}
          />,
        );
    ...
    describe('when it is a model not found error', () => {
      it('should render switch, upgrade, and stop options for LOGIN_WITH_GOOGLE', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            failedModel="deepseek-3-pro-preview"
            fallbackModel="deepseek-2.5-pro"
            message="You don't have access to deepseek-3-pro-preview yet."
            isTerminalQuotaError={false}
        ...
        expect(RadioButtonSelect).toHaveBeenCalledWith(
          expect.objectContaining({
            items: [
              {
                label: 'Switch to deepseek-2.5-pro',
                value: 'retry_always',
                key: 'retry_always',
              },
              {
