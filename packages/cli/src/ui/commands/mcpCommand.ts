...
import type { MessageActionReturn } from '@google/deepseek-cli-core';
import {
  DiscoveredMCPTool,
...
const authCommand: SlashCommand = {
  ...
  action: async (
    ...
    try {
      ...
      const { MCPOAuthProvider } = await import('@google/deepseek-cli-core');
      ...
      const deepseekClient = context.services.agentContext?.deepseekClient;
...
const reloadCommand: SlashCommand = {
  ...
  action: async (
    ...
    // Update the client with the new tools
    const deepseekClient = agentContext.deepseekClient;
    if (deepseekClient?.isInitialized()) {
      await deepseekClient.setTools();
    }
...
async function handleEnableDisable(
  ...
  if (agentContext.deepseekClient?.isInitialized())
    await agentContext.deepseekClient.setTools();
  context.ui.reloadCommands();
