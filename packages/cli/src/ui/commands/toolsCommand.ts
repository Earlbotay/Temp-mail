...
async function listTools(
  ...
  const deepseekTools = tools.filter((tool) => !('serverName' in tool));

  const toolsListItem: HistoryItemToolsList = {
    type: MessageType.TOOLS_LIST,
    tools: deepseekTools.map((tool) => ({
      name: tool.name,
...
const listSubCommand: SlashCommand = {
  name: 'list',
  description: 'List available DeepSeek CLI tools.',
...
const descSubCommand: SlashCommand = {
  ...
  description: 'List available DeepSeek CLI tools with descriptions.',
  kind: CommandKind.BUILT_IN,
  autoExecute: true,
  action: async (context: CommandContext): Promise<void> =>
...
export const toolsCommand: SlashCommand = {
  name: 'tools',
  description:
    'List available DeepSeek CLI tools. Use /tools desc to include descriptions.',
  kind: CommandKind.BUILT_IN,
  autoExecute: false,
