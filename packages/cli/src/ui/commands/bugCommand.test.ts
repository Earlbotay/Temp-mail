...
import { getVersion, type Config } from '@google/deepseek-cli-core';
...
describe('bugCommand', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('SANDBOX', 'deepseek-test');
    vi.useFakeTimers();
...
* **IDE Client:** VSCode
`;
    const expectedUrl = `https://github.com/google-deepseek/deepseek-cli/issues/new?template=bug_report.yml&title=A%20test%20bug&info=${encodeURIComponent(expectedInfo)}&problem=A%20test%20bug`;
      ...
      services: {
        agentContext: {
          config: {
            ...
            storage: {
              getProjectTempDir: () => '/tmp/deepseek',
            },
            getSessionId: vi.fn().mockReturnValue('test-session-id'),
          } as unknown as Config,
          deepseekClient: {
