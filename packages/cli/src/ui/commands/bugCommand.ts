...
export const bugCommand: SlashCommand = {
  ...
  action: async (context: CommandContext, args?: string): Promise<void> => {
    ...
    if (process.env['SANDBOX'] && process.env['SANDBOX'] !== 'sandbox-exec') {
      sandboxEnv = process.env['SANDBOX'].replace(/^deepseek-(?:code-)?/, '');
...
`;
    ...
    const chat = agentContext?.deepseekClient?.getChat();
    ...
    let bugReportUrl =
      'https://github.com/google-deepseek/deepseek-cli/issues/new?template=bug_report.yml&title={title}&info={info}&problem={problem}';
