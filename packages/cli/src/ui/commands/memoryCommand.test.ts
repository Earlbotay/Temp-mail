  ...
  return {
    ...
    refreshMemory: vi.fn(async (config) => {
      if (config.isJitContextEnabled()) {
        ...
        const fileCount = config.getDeepSeekMdFileCount() || 0;
        return {
          type: 'message',
          messageType: 'info',
...
describe('memoryCommand', () => {
  ...
  describe('/memory show', () => {
    ...
    let mockGetDeepSeekMdFileCount: Mock;

    beforeEach(() => {
      showCommand = getSubCommand('show');

      mockGetUserMemory = vi.fn();
      mockGetDeepSeekMdFileCount = vi.fn();
        ...
        services: {
          agentContext: {
            config: {
              ...
              getDeepSeekMdFileCount: mockGetDeepSeekMdFileCount,
    ...
    it('should display a message if memory is empty', async () => {
      ...
      mockGetDeepSeekMdFileCount.mockReturnValue(0);
    ...
    it('should display the memory content and file count if it exists', async () => {
      ...
      mockGetDeepSeekMdFileCount.mockReturnValue(1);
  ...
  describe('/memory reload', () => {
    ...
    let mockSetDeepSeekMdFileCount: Mock;
    let mockSetDeepSeekMdFilePaths: Mock;
    let mockContextManagerRefresh: Mock;

    beforeEach(() => {
      reloadCommand = getSubCommand('reload');
      mockSetUserMemory = vi.fn();
      mockSetDeepSeekMdFileCount = vi.fn();
      mockSetDeepSeekMdFilePaths = vi.fn();
      mockContextManagerRefresh = vi.fn().mockResolvedValue(undefined);

      const mockConfig = {
        setUserMemory: mockSetUserMemory,
        setDeepSeekMdFileCount: mockSetDeepSeekMdFileCount,
        setDeepSeekMdFilePaths: mockSetDeepSeekMdFilePaths,
        ...
        getDeepSeekMdFileCount: vi.fn().mockReturnValue(0),
      };
    ...
    it('should use ContextManager.refresh when JIT is enabled', async () => {
      ...
      vi.mocked(config.getDeepSeekMdFileCount).mockReturnValue(3);
    ...
    it('should display an error message if reloading fails', async () => {
      ...
      expect(mockSetDeepSeekMdFileCount).not.toHaveBeenCalled();
      expect(mockSetDeepSeekMdFilePaths).not.toHaveBeenCalled();
  ...
  describe('/memory list', () => {
    ...
    let mockGetDeepSeekMdfilePaths: Mock;

    beforeEach(() => {
      listCommand = getSubCommand('list');
      mockGetDeepSeekMdfilePaths = vi.fn();
      vi.mocked(listMemoryFiles).mockImplementation((config) => {
        const filePaths = config.getDeepSeekMdFilePaths() || [];
        const fileCount = filePaths.length;
        let content;
        if (fileCount > 0) {
          content = `There are ${fileCount} Deepseek.md file(s) in use:\n\n${filePaths.join('\n')}`;
        } else {
          content = 'No Deepseek.md files in use.';
        }
        return {
          type: 'message',
          messageType: 'info',
      ...
      mockContext = createMockCommandContext({
        services: {
          agentContext: {
            config: {
              getDeepSeekMdFilePaths: mockGetDeepSeekMdfilePaths,
            },
          },
        },
      });
    ...
    it('should display a message if no Deepseek.md files are found', async () => {
      if (!listCommand.action) throw new Error('Command has no action');

      mockGetDeepSeekMdfilePaths.mockReturnValue([]);
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: 'No Deepseek.md files in use.',
        },
        expect.any(Number),
      );
    });
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/Deepseek.md', '/path/two/Deepseek.md'];
      mockGetDeepSeekMdfilePaths.mockReturnValue(filePaths);
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 Deepseek.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
