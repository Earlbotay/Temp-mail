...
async function finishAddingDirectories(
  ...
  if (added.length > 0) {
    try {
      ...
      addItem({
        ...
        text: `Successfully added Deepseek.md files from the following directories if there are:\n- ${added.join('\n- ')}`,
      });
    } catch (error) {
  ...
  if (added.length > 0) {
    const deepseek = config.deepseekClient;
    if (deepseek) {
      await deepseek.addDirectoryContext();

      // Persist directories to session file for resume support
      const chatRecordingService = deepseek.getChatRecordingService();
