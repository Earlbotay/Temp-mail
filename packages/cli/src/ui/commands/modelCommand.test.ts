...
describe('modelCommand', () => {
  ...
  describe('set subcommand', () => {
    it('should set the model and log the command', async () => {
      ...
      await setCommand!.action!(mockContext, 'deepseek-pro');

      expect(mockSetModel).toHaveBeenCalledWith('deepseek-pro', true);
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        expect.objectContaining({
          type: MessageType.INFO,
          text: expect.stringContaining('Model set to deepseek-pro'),
        }),
      );
    });
    ...
    it('should set the model with persistence when --persist is used', async () => {
      ...
      await setCommand!.action!(mockContext, 'deepseek-pro --persist');

      expect(mockSetModel).toHaveBeenCalledWith('deepseek-pro', false);
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        expect.objectContaining({
          type: MessageType.INFO,
          text: expect.stringContaining('Model set to deepseek-pro (persisted)'),
        }),
      );
    });
