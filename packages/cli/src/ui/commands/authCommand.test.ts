...
import type { DeepSeekClient } from '@google/deepseek-cli-core';
  ...
  describe('auth signout subcommand', () => {
    ...
    it('should strip thoughts from history', async () => {
      ...
      if (mockContext.services.agentContext?.config) {
        mockContext.services.agentContext.config.getDeepSeekClient = vi.fn(
          () => mockClient,
        );
      }
      ...
      expect(
        mockContext.services.agentContext?.deepseekClient
