...
import type { SubmitPromptActionReturn } from '@google/deepseek-cli-core';
...
describe('initCommand', () => {
  ...
  it('should inform the user if Deepseek.md already exists', async () => {
  ...
  it('should create Deepseek.md and submit a prompt if it does not exist', async () => {
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(deepseekMdPath, '', 'utf8');

    // Assert: Check that an informational message was added to the UI
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty Deepseek.md created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
    ...
    expect(result.content).toContain(
      'You are an AI agent that brings the power of DeepSeek',
    );
  });
