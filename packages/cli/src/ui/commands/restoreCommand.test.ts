...
describe('restoreCommand', () => {
  ...
  beforeEach(async () => {
    ...
    deepseekTempDir = path.join(testRootDir, DEEPSEEK_DIR);
    checkpointsDir = path.join(deepseekTempDir, 'checkpoints');
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempDir: vi.fn().mockReturnValue(deepseekTempDir),
      },
      deepseekClient: {
  ...
  describe('action', () => {
    it('should return an error if temp dir is not found', async () => {
      ...
      expect(
        ...
        type: 'message',
        messageType: 'error',
        content: 'Could not determine the .deepseek directory path.',
      });
    });
