...
import { convertToRestPayload } from '@google/deepseek-cli-core';
...
const getSavedChatTags = async (
  ...
  const deepseekDir = cfg?.storage?.getProjectTempDir();
  if (!deepseekDir) {
    return [];
  }
  try {
    ...
    const files = await fsPromises.readdir(deepseekDir);
    const chatDetails: ChatDetail[] = [];

    for (const file of files) {
      if (file.startsWith(file_head) && file.endsWith(file_tail)) {
        const filePath = path.join(deepseekDir, file);
...
const saveCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<SlashCommandActionReturn | void> => {
    ...
    const chat = context.services.agentContext?.deepseekClient?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
...
const resumeCheckpointCommand: SlashCommand = {
  ...
  action: async (context, args) => {
    ...
    for (const item of conversation.slice(INITIAL_HISTORY_LENGTH)) {
      ...
      uiHistory.push({
        type: (item.role && rolemap[item.role]) || MessageType.DEEPSEEK,
        text,
...
const shareCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<MessageActionReturn> => {
    ...
    if (!filePathArg) {
      filePathArg = `deepseek-conversation-${Date.now()}.json`;
    }
