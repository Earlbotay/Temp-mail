...
export const AppContainer = (props: AppContainerProps) => {
  ...
  const historyManager = useHistory({
    chatRecordingService: config.getDeepSeekClient()?.getChatRecordingService(),
  });
  ...
  const [paidTier, setPaidTier] = useState<DeepSeekUserTier | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (result) {
        ...
        const deepseekClient = config.getDeepSeekClient();
        if (additionalContext && deepseekClient) {
  ...
  const isDeepSeekClientInitialized = config.getDeepSeekClient()?.isInitialized();

  const { loadHistoryForResume, isResuming } = useSessionResume({
    config,
    historyManager,
    refreshStatic,
    isDeepSeekClientInitialized,
    setQuittingMessages,
    resumedSessionData,
    isAuthenticating,
  });
    ...
    async (authType: AuthType | undefined, scope: LoadableSettingScope) => {
      if (authType) {
        ...
        if (
          ...
          writeToStdout(`
----------------------------------------------------------------
Logging in with Google... Restarting DeepSeek CLI to continue.
----------------------------------------------------------------
          `);
          await relaunchApp();
        }
  ...
  useEffect(() => {
    if (
      ...
      // We skip validation for DeepSeek API key here because it might be stored
      // in the keychain, which we can't check synchronously.
      // The useAuth hook handles validation for this case.
      if (settings.merged.security.auth.selectedType === AuthType.USE_DEEPSEEK) {
        return;
      }
  ...
  const performMemoryRefresh = useCallback(async () => {
    historyManager.addItem(
      {
        type: MessageType.INFO,
        text: 'Refreshing hierarchical memory (Deepseek.md or other context files)...',
      },
      Date.now(),
    );
    try {
      ...
      if (config.isJitContextEnabled()) {
        ...
        fileCount = config.getDeepSeekMdFileCount();
      } else {
  ...
  const {
    ...
    submitQuery,
    initError,
    pendingHistoryItems: pendingDeepSeekHistoryItems,
    thought,
  ...
  const pendingHistoryItems = useMemo(
    () => [...pendingSlashCommandHistoryItems, ...pendingDeepSeekHistoryItems],
    [pendingSlashCommandHistoryItems, pendingDeepSeekHistoryItems],
  );
  ...
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because DeepSeek CLI is already doing that elsewhere
  }, [
    streamingState,
    thought,
  ...
  const [deepseekMdFileCount, setDeepSeekMdFileCount] = useState<number>(
    config.getDeepSeekMdFileCount(),
  );
  useEffect(() => {
    const handleMemoryChanged = (result: MemoryChangedPayload) => {
      setDeepSeekMdFileCount(result.fileCount);
    };
