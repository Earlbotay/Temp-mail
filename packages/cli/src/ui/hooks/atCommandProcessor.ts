...
import type { AnyToolInvocation, Config } from '@google/deepseek-cli-core';
import {
  debugLogger,
  getErrorMessage,
  isNodeError,
...
async function resolveFilePaths(
  ...
  for (const part of fileParts) {
    ...
    const deepseekIgnored =
      respectFileIgnore.respectDeepSeekIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectDeepSeekIgnore: true,
      });

    if (gitIgnored || deepseekIgnored) {
      const reason =
        gitIgnored && deepseekIgnored ? 'both' : gitIgnored ? 'git' : 'deepseek';
      ignoredFiles.push({ path: pathName, reason });
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and deepseek'
          : reason === 'git'
            ? 'git-ignored'
            : 'deepseek-ignored';
...
async function readLocalFiles(
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_deepseek_ignore: respectFileIgnore.respectDeepSeekIgnore,
    },
  };
...
function reportIgnoredFiles(
  ...
  if (ignoredByReason['deepseek'].length) {
    messages.push(`DeepSeek-ignored: ${ignoredByReason['deepseek'].join(', ')}`);
  }
