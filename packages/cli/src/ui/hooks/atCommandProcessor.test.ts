...
import * as core from '@google/deepseek-cli-core';
...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      storage: {
        getProjectTempDir: () => path.join(os.tmpdir(), 'deepseek-cli-temp'),
      },
  ...
  describe('deepseek-ignore filtering', () => {
    it('should skip deepseek-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, DEEPSEEK_IGNORE_FILE_NAME),
        'build/output.js',
      );
      const deepseekIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${deepseekIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${deepseekIgnoredFile} is deepseek-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nDeepSeek-ignored: ${deepseekIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .deepseekignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, DEEPSEEK_IGNORE_FILE_NAME),
      'build/output.js',
    );
  ...
  it('should handle mixed deepseek-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, DEEPSEEK_IGNORE_FILE_NAME),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${deepseekIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${getRelativePath(validFile)} @${deepseekIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${getRelativePath(validFile)}:\n` },
