...
import { createMockMessageBus } from '@google/deepseek-cli-core/src/test-utils/mock-message-bus.js';
...
describe('useToolScheduler', () => {
  ...
  it('preserves responseSubmittedToDeepSeek flag across updates', async () => {
    const { result } = await renderHook(() =>
      useToolScheduler(
        vi.fn().mockResolvedValue(undefined),
        mockConfig,
    ...
    expect(result.current[0][0].responseSubmittedToDeepSeek).toBe(true);
  ...
  it('resolves the schedule promise when scheduler resolves', async () => {
    ...
    const { Scheduler } = await import('@google/deepseek-cli-core');
  ...
  it('setToolCallsForDisplay re-groups tools by schedulerId (Multi-Scheduler support)', async () => {
    ...
    act(() => {
      ...
      setToolCalls((prev) =>
        prev.map((t) => ({ ...t, responseSubmittedToDeepSeek: true })),
      );
    });

    const [toolCalls2] = result.current;
    expect(toolCalls2).toHaveLength(2);
    expect(toolCalls2.every((t) => t.responseSubmittedToDeepSeek)).toBe(true);
  });
