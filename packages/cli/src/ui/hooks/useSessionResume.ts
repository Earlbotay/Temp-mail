...
interface UseSessionResumeParams {
  ...
  refreshStatic: () => void;
  isDeepSeekClientInitialized: boolean;
...
export function useSessionResume({
  ...
  const loadHistoryForResume = useCallback(
    async (
      ...
      // Wait for the client.
      if (!isDeepSeekClientInitialized) {
        return;
      }
      ...
      try {
        ...
        // Give the history to the DeepSeek client.
        await config.getDeepSeekClient()?.resumeChat(clientHistory, resumedData);
      } catch (error) {
    ...
    [config, isDeepSeekClientInitialized, setQuittingMessages],
  );
