...
vi.mock('@google/deepseek-cli-core', async () => {
  const actualServerModule = await vi.importActual('@google/deepseek-cli-core');
  return {
    ...actualServerModule,
    Config: vi.fn(),
...
interface MockConfigInstanceShape {
  ...
  getDeepSeekMdFileCount: Mock<() => number>;
  ...
  beforeEach(() => {
    ...
    (
      ...
      const instance: MockConfigInstanceShape = {
        ...
        getDeepSeekMdFileCount: vi.fn().mockReturnValue(0) as Mock<() => number>,
        getToolRegistry: vi
          .fn()
