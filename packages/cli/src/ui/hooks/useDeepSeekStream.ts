...
import {
  DeepSeekEventType as ServerDeepSeekEventType,
  getErrorMessage,
  isNodeError,
  MessageSenderType,
  logUserPrompt,
...
import type {
  ...
  EditorType,
  DeepSeekClient,
  ServerDeepSeekChatCompressedEvent,
  ServerDeepSeekContentEvent as ContentEvent,
  ServerDeepSeekFinishedEvent,
  ServerDeepSeekStreamEvent as DeepSeekEvent,
  ThoughtSummary,
  ToolCallRequestInfo,
  ToolCallResponseInfo,
  DeepSeekErrorEventValue,
  RetryAttemptPayload,
} from '@google/deepseek-cli-core';
...
function calculateStreamingState(
  ...
  const isAnyToolActive = toolCalls.some((tc) => {
    ...
    // if the result hasn't been submitted back to DeepSeek yet.
    if (
...
/**
 * Manages the DeepSeek stream, including user input, command processing,
...
export const useDeepSeekStream = (
  ...
  const [lastDeepSeekActivityTime, setLastDeepSeekActivityTime] =
    useState<number>(0);
  ...
  const [
    ...
    async (completedToolCallsFromScheduler) => {
      ...
      if (completedToolCallsFromScheduler.length > 0) {
        ...
        try {
          const currentModel =
            config.getDeepSeekClient().getCurrentSequenceModel() ??
            config.getModel();
          config
            .getDeepSeekClient()
            .getChat()
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < toolCalls.length; i++) {
      ...
      if (
        ...
        // Architecturally, `useDeepSeekStream` should not be responsible for stitching
  ...
  const prepareQueryForDeepSeek = useCallback(
    async (
      query: PartListUnion,
      userMessageTimestamp: number,
      ...
      let localQueryToSendToDeepSeek: PartListUnion | null = null;
      ...
      if (typeof query === 'string') {
          ...
          if (slashCommandResult) {
            switch (slashCommandResult.type) {
              case 'schedule_tool': {
                ...
                if (postSubmitPrompt) {
                  localQueryToSendToDeepSeek = postSubmitPrompt;
                  return {
                    queryToSend: localQueryToSendToDeepSeek,
                    shouldProceed: true,
                  };
                }
              ...
              case 'submit_prompt': {
                localQueryToSendToDeepSeek = slashCommandResult.content;
        ...
        if (isAtCommand(trimmedQuery)) {
          ...
          localQueryToSendToDeepSeek = atCommandResult.processedQuery;
        } else {
          // Normal query for DeepSeek
          addItem(
            { type: MessageType.USER, text: trimmedQuery },
            userMessageTimestamp,
          );
          localQueryToSendToDeepSeek = trimmedQuery;
        }
      } else {
        // It's a function response (PartListUnion that isn't a string)
        localQueryToSendToDeepSeek = query;
      }

      if (localQueryToSendToDeepSeek === null) {
        onDebugMessage(
          'Query processing resulted in null, not sending to DeepSeek.',
        );
        return { queryToSend: null, shouldProceed: false };
      }
      return { queryToSend: localQueryToSendToDeepSeek, shouldProceed: true };
    },
    [
      config,
      addItem,
  ...
  const handleContentEvent = useCallback(
    (
      ...
      currentDeepSeekMessageBuffer: string,
      userMessageTimestamp: number,
    ): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
      ...
      let newDeepSeekMessageBuffer = currentDeepSeekMessageBuffer + eventValue;
      if (
        pendingHistoryItemRef.current?.type !== 'deepseek' &&
        pendingHistoryItemRef.current?.type !== 'deepseek_content'
      ) {
        // Flush any pending item before starting deepseek content
        if (pendingHistoryItemRef.current) {
          addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        }
        setPendingHistoryItem({ type: 'deepseek', text: '' });
        newDeepSeekMessageBuffer = eventValue;
      }
      // Split large messages for better rendering performance. Ideally,
      // we should maximize the amount of output sent to <Static />.
      const splitPoint = findLastSafeSplitPoint(newDeepSeekMessageBuffer);
      if (splitPoint === newDeepSeekMessageBuffer.length) {
        // Update the existing message with accumulated content
        setPendingHistoryItem((item) => ({
          // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
          type: item?.type as 'deepseek' | 'deepseek_content',
          text: newDeepSeekMessageBuffer,
        }));
      } else {
        // This indicates that we need to split up this DeepSeek Message.
        ...
        const beforeText = newDeepSeekMessageBuffer.substring(0, splitPoint);
        const afterText = newDeepSeekMessageBuffer.substring(splitPoint);
        ...
        setPendingHistoryItem({ type: 'deepseek_content', text: afterText });
        newDeepSeekMessageBuffer = afterText;
      }
      return newDeepSeekMessageBuffer;
    },
  ...
  const handleErrorEvent = useCallback(
    (eventValue: DeepSeekErrorEventValue, userMessageTimestamp: number) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const handleFinishedEvent = useCallback(
    (event: ServerDeepSeekFinishedEvent, userMessageTimestamp: number) => {
  ...
  const handleChatCompressionEvent = useCallback(
    (
      eventValue: ServerDeepSeekChatCompressedEvent['value'],
      userMessageTimestamp: number,
    ) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
  ...
  const processDeepSeekStreamEvents = useCallback(
    async (
      stream: AsyncIterable<DeepSeekEvent>,
      userMessageTimestamp: number,
      signal: AbortSignal,
    ): Promise<StreamProcessingStatus> => {
      let deepseekMessageBuffer = '';
      const toolCallRequests: ToolCallRequestInfo[] = [];
      for await (const event of stream) {
        if (
          event.type !== ServerDeepSeekEventType.Thought &&
        ...
        switch (event.type) {
          case ServerDeepSeekEventType.Thought:
            setLastDeepSeekActivityTime(Date.now());
            handleThoughtEvent(event.value, userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.Content:
            setLastDeepSeekActivityTime(Date.now());
            deepseekMessageBuffer = handleContentEvent(
              event.value,
              deepseekMessageBuffer,
              userMessageTimestamp,
            );
            break;
          case ServerDeepSeekEventType.ToolCallRequest:
            toolCallRequests.push(event.value);
            break;
          case ServerDeepSeekEventType.UserCancelled:
            handleUserCancelledEvent(userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.Error:
            handleErrorEvent(event.value, userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.AgentExecutionStopped:
            ...
            break;
          case ServerDeepSeekEventType.AgentExecutionBlocked:
            ...
            break;
          case ServerDeepSeekEventType.ChatCompressed:
            handleChatCompressionEvent(event.value, userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.ToolCallConfirmation:
          case ServerDeepSeekEventType.ToolCallResponse:
            // do nothing
            break;
          case ServerDeepSeekEventType.MaxSessionTurns:
            handleMaxSessionTurnsEvent();
            break;
          case ServerDeepSeekEventType.ContextWindowWillOverflow:
            handleContextWindowWillOverflowEvent(
              event.value.estimatedRequestTokenCount,
              event.value.remainingTokenCount,
            );
            break;
          case ServerDeepSeekEventType.Finished:
            handleFinishedEvent(event, userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.Citation:
            handleCitationEvent(event.value, userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.ModelInfo:
            handleChatModelEvent(event.value, userMessageTimestamp);
            break;
          case ServerDeepSeekEventType.LoopDetected:
            // handle later because we want to move pending history to history
            // before we add loop detected message to history
            loopDetectedRef.current = true;
            break;
          case ServerDeepSeekEventType.Retry:
          case ServerDeepSeekEventType.InvalidStream:
  ...
  const submitQuery = useCallback(
      ...
      runInDevTraceSpan(
        {
          operation: options?.isContinuation
            ? DeepSeekCliOperation.SystemPrompt
            : DeepSeekCliOperation.UserPrompt,
        ...
        async ({ metadata: spanMetadata }) => {
          ...
          return promptIdContext.run(prompt_id, async () => {
            const { queryToSend, shouldProceed } = await prepareQueryForDeepSeek(
              query,
              userMessageTimestamp,
              abortSignal,
              prompt_id!,
            ...
            try {
              const stream = deepseekClient.sendMessageStream(
                queryToSend,
                abortSignal,
                prompt_id!,
                undefined,
              ...
              const processingStatus = await processDeepSeekStreamEvents(
                stream,
                userMessageTimestamp,
                abortSignal,
              );
    ...
    [
      ...
      prepareQueryForDeepSeek,
      processDeepSeekStreamEvents,
      pendingHistoryItemRef,
      addItem,
      setPendingHistoryItem,
      setInitError,
  ...
  const handleApprovalModeChange = useCallback(
    async (newApprovalMode: ApprovalMode) => {
      if (
        ...
        if (deepseekClient) {
          try {
            await deepseekClient.addHistory({
              role: 'user',
              parts: [
                {
    ...
    [config, toolCalls, deepseekClient, streamingState, addItem, onDebugMessage],
  );
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      const deepseekTools = completedAndReadyToSubmitTools.filter(
        (t) => !t.request.isClientInitiated,
      );

      if (isLowErrorVerbosity) {
        // Low-mode suppression applies only to model-initiated tool failures.
        suppressedToolErrorCountRef.current += deepseekTools.filter(
      ...
      const stopExecutionTool = deepseekTools.find(
      ...
      if (stopExecutionTool && stopExecutionTool.response.error) {
        ...
        const callIdsToMarkAsSubmitted = deepseekTools.map(
      ...
      // If all the tools were cancelled, don't submit a response to DeepSeek.
      // Note: we ignore the topic tool because the user doesn't have a chance to decline it.
      const declinableTools = deepseekTools.filter(
      ...
      const allToolsCancelled =
        ...
        deepseekTools.every((tc) => tc.status === CoreToolCallStatus.Cancelled);
      ...
      if (allDeclinableToolsCancelled || allToolsCancelled) {
        ...
        if (deepseekClient) {
          ...
          const combinedParts = deepseekTools.flatMap(
            (toolCall) => toolCall.response.responseParts,
          );
          // eslint-disable-next-line @typescript-eslint/no-floating-promises
          deepseekClient.addHistory({
            role: 'user',
            parts: combinedParts,
          });
        }
      ...
      const responsesToSend: Part[] = deepseekTools.flatMap(
        (toolCall) => toolCall.response.responseParts,
      );
      ...
      const prompt_ids = deepseekTools.map(
  ...
  }, [toolCalls, config, onDebugMessage, gitService, history, deepseekClient]);

  const lastOutputTime = Math.max(
    lastToolOutputTime,
    lastShellOutputTime,
    lastDeepSeekActivityTime,
  );
