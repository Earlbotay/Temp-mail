...
import { useDeepSeekStream } from './useDeepSeekStream.js';
...
const MockedDeepSeekClientClass = vi.hoisted(() =>
  vi.fn().mockImplementation(function (this: any, _config: any) {
    ...
    this.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('deepseek-2.0-flash-exp');
  }),
);
...
// --- Tests for useDeepSeekStream Hook ---
describe('useDeepSeekStream', () => {
  ...
  const mockGetDeepSeekClient = vi.fn().mockImplementation(() => {
  ...
  const mockConfig: Config = {
    ...
    getDeepSeekClient: mockGetDeepSeekClient,
    ...
    getModel: vi.fn(() => 'deepseek-2.5-pro'),
  ...
  beforeEach(() => {
    ...
    } as unknown as any); // DeepSeekChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.deepseek/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = async (
    ...
    const client = deepseekClient || mockConfig.getDeepSeekClient();
          ...
          lastToolCalls = lastToolCalls.map((tc) => {
            if (
              ...
              return {
                ...
                responseSubmittedToDeepSeek: true,
  ...
  const createMockToolCall = (
    ...
    responseSubmittedToDeepSeek: false,
    confirmationDetails:
  ...
  it('should inject steering hint prompt for continuation', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerDeepSeekEventType.Content,
    ...
    expect(mockRunInDevTraceSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: DeepSeekCliOperation.SystemPrompt,
      }),
      expect.any(Function),
    );
  ...
  describe('Retry Handling', () => {
    ...
    it('should reset retryStatus when isResponding becomes false', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ServerDeepSeekEventType.Content, value: 'Part 1' };
        await new Promise(() => {}); // Keep stream open
      })();
      mockSendMessageStream.mockReturnValue(mockStream);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call DeepSeek when a command is handled without a tool call', async () => {
    ...
    it('should call DeepSeek with prompt content when slash command returns a `submit_prompt` action', async () => {
    ...
    it('should record client-initiated tool calls in DeepSeekChat history', async () => {
      const { result, client: mockDeepSeekClient } = await renderTestHook();
      ...
      // Verify that the tool call and response were added to DeepSeekChat history
      expect(mockDeepSeekClient.addHistory).toHaveBeenCalledWith({
        role: 'model',
        parts: [
          {
            functionCall: {
    ...
    it('should NOT record other client-initiated tool calls (like save_memory) in history', async () => {
      ...
      expect(mockDeepSeekClient.addHistory).not.toHaveBeenCalled();
    });
  });
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerDeepSeekEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerDeepSeekEventType.ContextWindowWillOverflow,
                value: {
    ...
    it('should add informational messages when ChatCompressed event is received', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerDeepSeekEventType.ChatCompressed,
            value: {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    const mockStream = (async function* () {
      ...
      yield {
        type: ServerDeepSeekEventType.ToolCallRequest,
    ...
    // Expectation: addItem:deepseek (rationale) MUST happen before scheduleToolCalls_START
    const rationaleIndex = addItemOrder.indexOf('addItem:deepseek');
  ...
  it('should display user query, then tool execution, then model response', async () => {
    ...
    // Mock the DeepSeek stream to return a model response after the tool
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerDeepSeekEventType.Content,
  ...
  describe('Thought Reset', () => {
    it('should keep full thinking entries in history when mode is full', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerDeepSeekEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerDeepSeekEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerDeepSeekEventType.Error,
    ...
    it('should update lastOutputTime on DeepSeek thought and content events', async () => {
      vi.useFakeTimers();
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getDeepSeekClient = vi.fn().mockReturnValue({
        ...new MockedDeepSeekClientClass(mockConfig),
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerDeepSeekEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getDeepSeekClient = vi.fn().mockReturnValue(mockClient);
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerDeepSeekEventType.AgentExecutionStopped,
            value: {
              reason: 'hook-reason',
    ...
    it('should handle AgentExecutionBlocked event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerDeepSeekEventType.AgentExecutionBlocked,
            value: {
              reason: 'hook-reason',
  ...
  describe('Stream Splitting', () => {
    it('should not add empty history item when splitting message results in empty or whitespace-only beforeText', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerDeepSeekEventType.Content, value: 'test content' };
        })(),
      );
    ...
    it('should add whitespace-only history item when splitting message', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerDeepSeekEventType.Content, value: '   content' };
        })(),
      );
  ...
  it('should trace UserPrompt telemetry on submitQuery', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield { type: ServerDeepSeekEventType.Content, value: 'Response' };
      })(),
    );
    ...
    const userPromptCall = mockRunInDevTraceSpan.mock.calls.find(
      (call) =>
        call[0].operation === DeepSeekCliOperation.UserPrompt ||
