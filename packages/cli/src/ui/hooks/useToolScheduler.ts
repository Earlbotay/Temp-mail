...
/**
 * The shape expected by useDeepSeekStream.
 * It matches the Core ToolCall structure + the UI metadata flag.
 */
export type TrackedToolCall = ToolCall & {
  responseSubmittedToDeepSeek?: boolean;
  subagentHistory?: SubagentActivityItem[];
};

// Narrowed types for specific statuses (used by useDeepSeekStream)
...
export function useToolScheduler(
  ...
  const schedule: ScheduleFn = useCallback(
    async (request, signal) => {
      ...
      // 2. Trigger legacy reinjection logic (useDeepSeekStream loop)
        ...
        for (const [sid, calls] of Object.entries(nextMap)) {
          nextMap[sid] = calls.map((tc) =>
            callIdsToMark.includes(tc.request.callId)
              ? { ...tc, responseSubmittedToDeepSeek: true }
              : tc,
          );
        }
        return nextMap;
...
function adaptToolCalls(
  ...
  return coreCalls.map((coreCall): TrackedToolCall => {
    ...
    const responseSubmittedToDeepSeek = prev?.responseSubmittedToDeepSeek ?? false;
