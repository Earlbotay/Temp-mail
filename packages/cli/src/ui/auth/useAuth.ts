...
import { getErrorMessage } from '@google/deepseek-cli-core';
...
export function validateAuthMethodWithSettings(
  ...
  // If using DeepSeek API key, we don't validate it here as we might need to prompt for it.
...
export const useAuthCommand = (
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const defaultAuthType = process.env['DEEPSEEK_DEFAULT_AUTH_TYPE'];
      if (
        defaultAuthType &&
        ...
        onAuthError(
          `Invalid value for DEEPSEEK_DEFAULT_AUTH_TYPE: "${defaultAuthType}". ` +
