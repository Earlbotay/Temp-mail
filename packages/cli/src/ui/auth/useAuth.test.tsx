...
describe('useAuth', () => {
  beforeEach(() => {
    ...
    delete process.env['DEEPSEEK_DEFAULT_AUTH_TYPE'];
  });
  ...
  describe('validateAuthMethodWithSettings', () => {
    ...
    it('should return null if authType is USE_DEEPSEEK', () => {
      const settings = {
        merged: {
          security: {
            auth: {},
  ...
  describe('useAuthCommand', () => {
    ...
    it('should set error if no auth type is selected but env key exists', async () => {
      ...
      expect(result.current.authError).toContain(
        'Existing API key detected (DEEPSEEK_API_KEY)',
      );
      expect(result.current.authState).toBe(AuthState.Updating);
    });

    it('should transition to AwaitingApiKeyInput if USE_DEEPSEEK and no key found', async () => {
      ...
      const { result } = await renderHook(() =>
        useAuthCommand(createSettings(AuthType.USE_DEEPSEEK), mockConfig),
      );
    ...
    it('should authenticate if USE_DEEPSEEK and key is found', async () => {
      let deferredLoadKey: { resolve: (k: string | null) => void };
      mockLoadApiKey.mockImplementation(
        () =>
          new Promise((resolve) => {
      ...
      expect(mockConfig.refreshAuth).toHaveBeenCalledWith(AuthType.USE_DEEPSEEK);
      expect(result.current.authState).toBe(AuthState.Authenticated);
      expect(result.current.apiKeyDefaultValue).toBe('stored-key');
    });

    it('should authenticate if USE_DEEPSEEK and env key is found', async () => {
      process.env['DEEPSEEK_API_KEY'] = 'env-key';
    ...
    it('should set error if DEEPSEEK_DEFAULT_AUTH_TYPE is invalid', async () => {
      process.env['DEEPSEEK_DEFAULT_AUTH_TYPE'] = 'INVALID_TYPE';
      const { result } = await renderHook(() =>
        useAuthCommand(createSettings(AuthType.LOGIN_WITH_GOOGLE), mockConfig),
      );

      expect(result.current.authError).toContain(
        'Invalid value for DEEPSEEK_DEFAULT_AUTH_TYPE',
      );
      expect(result.current.authState).toBe(AuthState.Updating);
    });
    ...
    it('should handle ProjectIdRequiredError without "Failed to login" prefix', async () => {
      ...
      expect(result.current.authError).toBe(
        'This account requires setting the GOOGLE_CLOUD_PROJECT or GOOGLE_CLOUD_PROJECT_ID env var. See https://goo.gle/deepseek-cli-auth-docs#workspace-gca',
      );
