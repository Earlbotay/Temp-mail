...
import { AuthType, type Config, debugLogger } from '@google/deepseek-cli-core';
...
describe('AuthDialog', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('DEEPSEEK_CLI_USE_COMPUTE_ADC', undefined as unknown as string);
    vi.stubEnv('DEEPSEEK_DEFAULT_AUTH_TYPE', undefined as unknown as string);
    vi.stubEnv('DEEPSEEK_API_KEY', undefined as unknown as string);
  ...
  it('filters auth types when enforcedType is set', async () => {
    props.settings.merged.security.auth.enforcedType = AuthType.USE_DEEPSEEK;
    const { unmount } = await renderWithProviders(<AuthDialog {...props} />);
    const items = mockedRadioButtonSelect.mock.calls[0][0].items;
    expect(items).toHaveLength(1);
    expect(items[0].value).toBe(AuthType.USE_DEEPSEEK);
    unmount();
  });
  ...
  describe('Initial Auth Type Selection', () => {
    it.each([
      ...
      {
        setup: () => {
          vi.stubEnv('DEEPSEEK_DEFAULT_AUTH_TYPE', AuthType.USE_DEEPSEEK);
        },
        expected: AuthType.USE_DEEPSEEK,
        desc: 'from DEEPSEEK_DEFAULT_AUTH_TYPE env var',
      },
      {
        setup: () => {
        ...
        expected: AuthType.USE_DEEPSEEK,
        desc: 'from DEEPSEEK_API_KEY env var',
      },
      {
        setup: () => {},
  ...
  describe('handleAuthSelect', () => {
    it('calls onAuthError if validation fails', async () => {
      ...
      handleAuthSelect(AuthType.USE_DEEPSEEK);
    ...
    it('sets auth context with empty object for other auth types', async () => {
      ...
      const { onSelect: handleAuthSelect } =
        mockedRadioButtonSelect.mock.calls[0][0];
      await handleAuthSelect(AuthType.USE_DEEPSEEK);
    ...
    it('always shows API key dialog even when env var is present', async () => {
      mockedValidateAuthMethod.mockReturnValue(null);
      vi.stubEnv('DEEPSEEK_API_KEY', 'test-key-from-env');
    ...
    it('always shows API key dialog even when env var is empty string', async () => {
      mockedValidateAuthMethod.mockReturnValue(null);
      vi.stubEnv('DEEPSEEK_API_KEY', ''); // Empty string
    ...
    it('shows API key dialog on initial setup if no env var is present', async () => {
      mockedValidateAuthMethod.mockReturnValue(null);
      // process.env['DEEPSEEK_API_KEY'] is not set
      // props.settings.merged.security.auth.selectedType is undefined here, simulating initial setup
