...
import type { FileSystemService } from '@google/deepseek-cli-core';
...
describe('AcpFileSystemService', () => {
  ...
  describe('readTextFile', () => {
    it.each([
      ...
      {
        capability: true,
        path: '/home/user/.deepseek/tmp/file.md',
        root: '/home/user',
        desc: 'fallback if file is inside global deepseek dir, even if root overlaps',
        setup: () => {
          mockFallback.readTextFile.mockResolvedValue('content');
        },
        verify: () => {
          expect(mockFallback.readTextFile).toHaveBeenCalledWith(
            '/home/user/.deepseek/tmp/file.md',
          );
          expect(mockConnection.readTextFile).not.toHaveBeenCalled();
        },
      },
