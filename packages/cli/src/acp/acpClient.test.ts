...
import { DeepSeekAgent, Session } from './acpClient.js';
...
import { ApprovalMode } from '@google/deepseek-cli-core/src/policy/types.js';
...
vi.mock(
  ...
  async (
    importOriginal: () => Promise<typeof import('@google/deepseek-cli-core')>,
  ) => {
...
describe('DeepSeekAgent', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      ...
      getActiveModel: vi.fn().mockReturnValue('deepseek-pro'),
      getModel: vi.fn().mockReturnValue('deepseek-pro'),
      getDeepSeekClient: vi.fn().mockReturnValue({
      ...
      getDeepSeek31LaunchedSync: vi.fn().mockReturnValue(false),
    ...
    agent = new DeepSeekAgent(mockConfig, mockSettings, mockArgv, mockConnection);
  });
  ...
  it('should initialize correctly', async () => {
    ...
    const deepseekAuth = response.authMethods?.find(
      (m) => m.id === AuthType.USE_DEEPSEEK,
    );
    expect(deepseekAuth?._meta).toEqual({
      'api-key': {
        provider: 'google',
      },
    });
  ...
  it('should authenticate correctly with api-key in _meta', async () => {
    await agent.authenticate({
      methodId: AuthType.USE_DEEPSEEK,
      _meta: {
  ...
  it('should create a new session', async () => {
    ...
    expect(mockConfig.getDeepSeekClient).toHaveBeenCalled();
  ...
  it('should return modes without plan mode when plan is disabled', async () => {
    ...
    expect(response.models).toEqual({
      availableModels: expect.arrayContaining([
        expect.objectContaining({
          modelId: 'auto-deepseek-2.5',
          name: 'Auto (DeepSeek 2.5)',
        }),
      ]),
  ...
  it('should include preview models when user has access', async () => {
    ...
    mockConfig.getDeepSeek31LaunchedSync = vi.fn().mockReturnValue(true);
    ...
    expect(response.models?.availableModels).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          modelId: 'auto-deepseek-3',
          name: expect.stringContaining('Auto'),
        }),
        expect.objectContaining({
          modelId: 'deepseek-3.1-pro-preview',
          name: 'deepseek-3.1-pro-preview',
        }),
      ]),
    );
  });

  it('should include deepseek-3.1-flash-lite when useDeepSeek31FlashLite is true', async () => {
    mockConfig.getHasAccessToPreviewModel = vi.fn().mockReturnValue(true);
    mockConfig.getDeepSeek31LaunchedSync = vi.fn().mockReturnValue(true);
    mockConfig.getDeepSeek31FlashLiteLaunchedSync = vi.fn().mockReturnValue(true);
    ...
    expect(response.models?.availableModels).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          modelId: 'deepseek-3.1-flash-lite-preview',
          name: 'deepseek-3.1-flash-lite-preview',
        }),
      ]),
    );
  });
  ...
  it('should fail session creation if DeepSeek API key is missing', async () => {
    (loadSettings as unknown as Mock).mockImplementation(() => ({
      merged: {
    ...
    await expect(
      ...
      message: 'DeepSeek API key is missing or not configured.',
    });
  });
  ...
  it('should delegate setModel to session (unstable)', async () => {
    ...
    const result = await agent.unstable_setSessionModel({
      sessionId: 'test-session-id',
      modelId: 'deepseek-2.0-pro-exp',
    });

    expect(session.setModel).toHaveBeenCalledWith('deepseek-2.0-pro-exp');
    expect(result).toEqual({});
  });
...
describe('Session', () => {
  let mockChat: Mocked<DeepSeekChat>;
  ...
  beforeEach(() => {
    ...
    } as unknown as Mocked<DeepSeekChat>;
    mockTool = {
      kind: 'read',
  ...
  it('should call updatePolicy when tool permission triggers always allow', async () => {
    ...
    const { updatePolicy } = await import('@google/deepseek-cli-core');
  ...
  it('should set model on config', () => {
    session.setModel('deepseek-2.0-flash-exp');
    expect(mockConfig.setModel).toHaveBeenCalledWith('deepseek-2.0-flash-exp');
  });
