...
export async function runAcpClient(
  ...
  const connection = new acp.AgentSideConnection(
    (connection) => new DeepSeekAgent(config, settings, argv, connection),
    stream,
  );
...
export class DeepSeekAgent {
  ...
  static generateCallId(name: string): string {
    return `${name}-${Date.now()}-${++DeepSeekAgent.callIdCounter}`;
  }
  ...
  async initialize(
    ...
    const authMethods = [
      ...
      },
      {
        ...
        description: 'Use an API key with DeepSeek Developer API',
        _meta: {
          'api-key': {
            provider: 'google',
          },
  ...
  async newSession({
    ...
    const authType =
      loadedSettings.merged.security.auth.selectedType || AuthType.USE_DEEPSEEK;
    ...
    try {
      ...
      // Extra validation for DeepSeek API key
      ...
      if (
        ...
        authErrorMessage = 'DeepSeek API key is missing or not configured.';
      }
    } catch (e) {
      isAuthenticated = false;
    ...
    const chat = await deepseekClient.startChat();
  ...
  async loadSession({
    ...
    await deepseekClient.resumeChat(clientHistory, {
...
export class Session {
  ...
  private async runTool(
    ...
    const callId = fc.id ?? DeepSeekAgent.generateCallId(fc.name || 'unknown');
    const args = fc.args ?? {};
  ...
  async #resolvePrompt(
    ...
    if (pathSpecsToRead.length > 0) {
      ...
      const callId = DeepSeekAgent.generateCallId(readManyFilesTool.name);
