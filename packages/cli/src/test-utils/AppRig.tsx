...
// Mock DeepSeekRespondingSpinner to disable animations (avoiding 'act()' warnings) without triggering screen reader mode.
vi.mock('../ui/components/DeepSeekRespondingSpinner.js', async () => {
  const React = await import('react');
  const { Text } = await import('ink');
  return {
    DeepSeekSpinner: () => React.createElement(Text, null, '...'),
    DeepSeekRespondingSpinner: ({
...
export class AppRig {
  ...
  constructor(private options: AppRigOptions = {}) {
    ...
    this.testDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `deepseek-app-rig-${uniqueId.slice(0, 8)}-`),
    );
    ...
    const deepseekDir = path.join(this.testDir, '.deepseek');
    if (!fs.existsSync(deepseekDir)) {
      fs.mkdirSync(deepseekDir, { recursive: true });
    }
    fs.writeFileSync(
      path.join(deepseekDir, 'state.json'),
  ...
  async initialize() {
    ...
    await act(async () => {
      ...
      await this.config!.refreshAuth(AuthType.USE_DEEPSEEK);
    });
  }

  private setupEnvironment() {
    // Stub environment variables to avoid interference from developer's machine
    vi.stubEnv('DEEPSEEK_CLI_HOME', this.testDir);
    ...
    if (this.options.fakeResponsesPath) {
      ...
      if (!process.env['DEEPSEEK_API_KEY']) {
        throw new Error(
          'DEEPSEEK_API_KEY must be set in the environment for live model tests.',
        );
      }
  ...
  private createRigSettings(): LoadedSettings {
    return createMockSettings({
      user: {
        path: path.join(this.testDir, '.deepseek', 'user_settings.json'),
        settings: {
          security: {
            auth: {
  ...
  private stubRefreshAuth() {
    ...
    gcConfig.refreshAuth = async (authMethod: AuthType) => {
      ...
      const newContentGeneratorConfig = {
        ...
        apiKey: process.env['DEEPSEEK_API_KEY'] || 'test-api-key',
      };
      ...
      const { BaseLlmClient } = await import('@google/deepseek-cli-core');
