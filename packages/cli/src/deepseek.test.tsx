...
describe('deepseek.tsx main function', () => {
...
describe('initializeOutputListenersAndFlush', () => {
  ...
  it('should flush backlogs and setup listeners if no listeners exist', async () => {
    const { coreEvents } = await import('@google/deepseek-cli-core');
    const { initializeOutputListenersAndFlush } = await import('./deepseek.js');
...
describe('getNodeMemoryArgs', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['DEEPSEEK_CLI_NO_RELAUNCH'];
  ...
  it('should return empty array if DEEPSEEK_CLI_NO_RELAUNCH is set', () => {
    process.env['DEEPSEEK_CLI_NO_RELAUNCH'] = 'true';
...
describe('deepseek.tsx main function kitty protocol', () => {
  ...
  beforeEach(() => {
    ...
    originalEnvNoRelaunch = process.env['DEEPSEEK_CLI_NO_RELAUNCH'];
    process.env['DEEPSEEK_CLI_NO_RELAUNCH'] = 'true';
  ...
  afterEach(() => {
    ...
    if (originalEnvNoRelaunch !== undefined) {
      process.env['DEEPSEEK_CLI_NO_RELAUNCH'] = originalEnvNoRelaunch;
    } else {
      delete process.env['DEEPSEEK_CLI_NO_RELAUNCH'];
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('deepseek.tsx main function exit codes', () => {
  let originalEnvNoRelaunch: string | undefined;
  let originalIsTTY: boolean | undefined;
  ...
  it('should validate and refresh auth in non-interactive mode when no auth type is selected but env var is present', async () => {
    ...
    expect(refreshAuthSpy).toHaveBeenCalledWith(AuthType.USE_DEEPSEEK);
  });
});
...
describe('startInteractiveUI', () => {
  ...
  it('should enable mouse events when alternate buffer is enabled', async () => {
    const { enableMouseEvents } = await import('@google/deepseek-cli-core');
    await startTestInteractiveUI(
      mockConfig,
      mockSettings,
      mockStartupWarnings,
  ...
  it('should perform all startup tasks in correct order', async () => {
    const { getVersion } = await import('@google/deepseek-cli-core');
  ...
  it('should not recordSlowRender when less than threshold', async () => {
    const { recordSlowRender } = await import('@google/deepseek-cli-core');
