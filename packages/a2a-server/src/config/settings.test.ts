...
vi.mock('node:os', async (importOriginal) => {
  ...
  return {
    ...actual,
    homedir: () => path.join(actual.tmpdir(), `deepseek-home-${mocks.suffix}`),
  };
});
...
vi.mock('@google/deepseek-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    homedir: () => path.join(os.tmpdir(), `deepseek-home-${mocks.suffix}`),
  };
});

describe('loadSettings', () => {
  const mockHomeDir = path.join(os.tmpdir(), `deepseek-home-${mocks.suffix}`);
  const mockWorkspaceDir = path.join(
    os.tmpdir(),
    `deepseek-workspace-${mocks.suffix}`,
  );
  const mockDeepSeekHomeDir = path.join(mockHomeDir, '.deepseek');
  const mockDeepSeekWorkspaceDir = path.join(mockWorkspaceDir, '.deepseek');

  beforeEach(() => {
    vi.clearAllMocks();
    // Create the directories using the real fs
    if (!fs.existsSync(mockDeepSeekHomeDir)) {
      fs.mkdirSync(mockDeepSeekHomeDir, { recursive: true });
    }
    if (!fs.existsSync(mockDeepSeekWorkspaceDir)) {
      fs.mkdirSync(mockDeepSeekWorkspaceDir, { recursive: true });
    }
