...
export async function loadConfig(
  ...
  const folderTrust =
    ...
    process.env['DEEPSEEK_FOLDER_TRUST'] === 'true';
  ...
  const approvalMode =
    process.env['DEEPSEEK_YOLO_MODE'] === 'true'
      ? ApprovalMode.YOLO
  ...
  const configParams: ConfigParameters = {
    ...
    fileFiltering: {
      ...
      respectDeepSeekIgnore: settings.fileFiltering?.respectDeepSeekIgnore,
  ...
  const fileService = new FileDiscoveryService(workspaceDir, {
    ...
    respectDeepSeekIgnore: configParams?.fileFiltering?.respectDeepSeekIgnore,
  ...
  configParams.deepseekMdFileCount = fileCount;
  configParams.deepseekMdFilePaths = filePaths;
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // prefer deepseek-specific .env under DEEPSEEK_DIR
    const deepseekEnvPath = path.join(currentDir, DEEPSEEK_DIR, '.env');
    if (fs.existsSync(deepseekEnvPath)) {
      return deepseekEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      // check .env under home as fallback, again preferring deepseek-specific .env
      const homeDeepSeekEnvPath = path.join(process.cwd(), DEEPSEEK_DIR, '.env');
      if (fs.existsSync(homeDeepSeekEnvPath)) {
        return homeDeepSeekEnvPath;
      }
...
async function refreshAuthentication(
  ...
  if (process.env['USE_CCPA']) {
    ...
    try {
      ...
      if (isHeadless || useComputeAdc) {
        const reason = isHeadless
          ? 'headless mode'
          : 'DEEPSEEK_CLI_USE_COMPUTE_ADC=true';
    ...
    logger.info(`[${logPrefix}] Using DeepSeek API Key`);
    await config.refreshAuth(AuthType.USE_DEEPSEEK);
  } else {
    const errorMessage = `[${logPrefix}] Unable to set GeneratorConfig. Please provide a DEEPSEEK_API_KEY or set USE_CCPA.`;
