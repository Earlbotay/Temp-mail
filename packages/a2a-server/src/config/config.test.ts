...
describe('loadConfig', () => {
  ...
  it('should initialize FileDiscoveryService with correct options', async () => {
    ...
    expect(FileDiscoveryService).toHaveBeenCalledWith(expect.any(String), {
      respectGitIgnore: false,
      respectDeepSeekIgnore: undefined,
      customIgnoreFilePaths: [testPath],
    });
  });
  ...
  describe('tool configuration', () => {
    ...
    describe('YOLO mode', () => {
      it('should enable YOLO mode and add policy rule when DEEPSEEK_YOLO_MODE is true', async () => {
        vi.stubEnv('DEEPSEEK_YOLO_MODE', 'true');
        await loadConfig(mockSettings, mockExtensionLoader, taskId);
        expect(Config).toHaveBeenCalledWith(
          expect.objectContaining({
            approvalMode: 'yolo',
      ...
      it('should use default approval mode and empty rules when DEEPSEEK_YOLO_MODE is not true', async () => {
        vi.stubEnv('DEEPSEEK_YOLO_MODE', 'false');
        await loadConfig(mockSettings, mockExtensionLoader, taskId);
        expect(Config).toHaveBeenCalledWith(
          expect.objectContaining({
