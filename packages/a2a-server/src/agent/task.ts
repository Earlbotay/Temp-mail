...
export class Task {
  ...
  private constructor(
    ...
    this.deepseekClient = loopContext.deepseekClient;
  ...
  // state managed within the @deepseek-cli/core module.
  ...
  async acceptAgentMessage(event: ServerDeepSeekStreamEvent): Promise<void> {
    const stateChange: StateChange = {
      kind: CoderAgentEvent.StateChangeEvent,
    };
    const traceId =
    ...
    switch (event.type) {
      ...
      case DeepSeekEventType.ToolCallResponse:
        // This event type from ServerDeepSeekStreamEvent might be for when LLM *generates* a tool response part.
      ...
      default: {
        ...
        let errorEvent: ServerDeepSeekErrorEvent | undefined;
        if (
          event.type === DeepSeekEventType.Error &&
          event.value &&
  ...
  addToolResponsesToHistory(completedTools: CompletedToolCall[]): void {
    ...
    for (const response of responsesToAdd) {
      ...
      this.deepseekClient.addHistory({
        role: 'user',
        parts,
      });
    }
  ...
  async *sendCompletedToolsToLlm(
    ...
    yield* this.deepseekClient.sendMessageStream(
      llmParts,
      aborted,
