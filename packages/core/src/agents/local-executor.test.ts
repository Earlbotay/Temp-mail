...
vi.mock('../core/deepseekChat.js', () => ({
  StreamEventType: {
    CHUNK: 'chunk',
  },
  DeepSeekChat: vi.fn().mockImplementation(() => ({
...
const MockedDeepSeekChat = vi.mocked(DeepSeekChat);
...
describe('LocalAgentExecutor', () => {
  ...
  beforeEach(async () => {
    ...
    MockedDeepSeekChat.mockImplementation(
      () =>
        ({
          sendMessageStream: mockSendMessageStream,
          setSystemInstruction: mockSetSystemInstruction,
  ...
  describe('create (Initialization and Validation)', () => {
    it('should explicitly map execution context properties to prevent unintended propagation', async () => {
      ...
      const mockDeepSeekClient = {} as unknown as DeepSeekClient;
      ...
      const chatConstructorArgs = MockedDeepSeekChat.mock.calls[0];
      ...
      expect(executionContext.deepseekClient).toBe(extendedContext.deepseekClient);
    ...
    it('should propagate parentSessionId from context when creating executionContext', async () => {
      ...
      const chatConstructorArgs =
        MockedDeepSeekChat.mock.calls[MockedDeepSeekChat.mock.calls.length - 1];
      const executionContext = chatConstructorArgs[0];
  ...
  describe('run (Execution Loop and Logic)', () => {
    ...
    it('should execute successfully when model calls complete_task with output (Happy Path with Output)', async () => {
      ...
      const systemInstruction = MockedDeepSeekChat.mock.calls[0][1];
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should throw and log if DeepSeekChat creation fails', async () => {
      const definition = createTestDefinition();
      const initError = new Error('Chat creation failed');
      MockedDeepSeekChat.mockImplementationOnce(() => {
        throw initError;
      });
  ...
  describe('DeclarativeTool instance tools (browser agent pattern)', () => {
    ...
    /**
     * Helper to extract the functionDeclarations sent to DeepSeekChat.
     */
    const getSentFunctionDeclarations = () => {
      const chatCtorArgs = MockedDeepSeekChat.mock.calls[0];
