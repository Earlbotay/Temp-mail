...
import { DeepSeekChat, StreamEventType } from '../core/deepseekChat.js';
import {
  type Content,
  type Part,
  type FunctionCall,
...
import { DEFAULT_DEEPSEEK_MODEL, isAutoModel } from '../config/models.js';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private get executionContext(): AgentLoopContext {
    return {
      ...
      deepseekClient: this.context.deepseekClient,
  ...
  private async runInternal(
    ...
    let chat: DeepSeekChat | undefined;
  ...
  private async tryCompressChat(
    ...
    const model = this.definition.modelConfig.model ?? DEFAULT_DEEPSEEK_MODEL;
  ...
  private async callModel(
    ...
    if (isAutoModel(requestedModel)) {
      ...
      try {
        ...
        modelToUse = DEFAULT_DEEPSEEK_MODEL;
      }
    } else {
  ...
  /** Initializes a `DeepSeekChat` instance for the agent run. */
  private async createChatObject(
    ...
    try {
      ...
      await reportError(
        e,
        `Error initializing DeepSeek chat for agent ${this.definition.name}.`,
        startHistory,
        'startChat',
      );
