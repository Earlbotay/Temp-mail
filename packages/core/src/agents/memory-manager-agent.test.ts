...
describe('MemoryManagerAgent', () => {
  ...
  it('should have a system prompt with memory management instructions', () => {
    ...
    expect(prompt).toContain(`Global (${globalDeepSeekDir}`);
  ...
  it('should inject hierarchical memory into initial context', () => {
    const config = createMockConfig({
      global:
        '--- Context from: ../../.deepseek/Deepseek.md ---\nglobal context\n--- End of Context from: ../../.deepseek/Deepseek.md ---',
      project:
        '--- Context from: .deepseek/Deepseek.md ---\nproject context\n--- End of Context from: .deepseek/Deepseek.md ---',
    });
  ...
  it('should declare workspaceDirectories containing the global .deepseek directory', () => {
    const agent = MemoryManagerAgent(createMockConfig());
    const globalDeepSeekDir = Storage.getGlobalDeepSeekDir();
    expect(agent.workspaceDirectories).toBeDefined();
    expect(agent.workspaceDirectories).toContain(globalDeepSeekDir);
  });
});
