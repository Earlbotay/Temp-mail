...
describe('AgentRegistry Acknowledgement', () => {
  ...
  let config: Config;
  let tempDir: string;
  let originalDeepSeekCliHome: string | undefined;
  let ackService: AcknowledgedAgentsService;

  beforeEach(async () => {
    // Create a unique temp directory for each test
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'deepseek-cli-test-'));

    // Override DEEPSEEK_CLI_HOME to point to the temp directory
    originalDeepSeekCliHome = process.env['DEEPSEEK_CLI_HOME'];
    process.env['DEEPSEEK_CLI_HOME'] = tempDir;
    ...
    vi.mocked(tomlLoader.loadAgentsFromDirectory).mockImplementation(
      async (dir) => {
        if (dir === '/project/.deepseek/agents') {
          return {
  ...
  afterEach(async () => {
    ...
    if (originalDeepSeekCliHome) {
      process.env['DEEPSEEK_CLI_HOME'] = originalDeepSeekCliHome;
    } else {
