...
// Default browser profile directory name within ~/.deepseek/
...
export class BrowserManager {
  ...
  private async connectMcp(): Promise<void> {
    ...
    this.rawMcpClient = new Client(
      {
        name: 'deepseek-cli-browser-agent',
        version: '1.0.0',
      },
      {
        capabilities: {},
    ...
    // - "persistent": Persistent profile at ~/.deepseek/cli-browser-profile/ (default)
    ...
    if (browserConfig.customConfig.profilePath) {
      ...
      // Default persistent profile lives under ~/.deepseek/cli-browser-profile
