...
import { DEEPSEEK_MODEL_ALIAS_FLASH } from '../config/models.js';
 ...
 * and organizing memories in the global Deepseek.md file.
 *
 * Users can override this agent by placing a custom save_memory.md
 * in ~/.deepseek/agents/ or .deepseek/agents/.
 */
export const MemoryManagerAgent = (
  config: Config,
): LocalAgentDefinition<typeof MemoryManagerSchema> => {
  const globalDeepSeekDir = Storage.getGlobalDeepSeekDir();
  ...
  const buildSystemPrompt = (): string =>
    `
You are a memory management agent maintaining user memories in Deepseek.md files.

# Memory Hierarchy

## Global (${globalDeepSeekDir})
- \`${globalDeepSeekDir}/Deepseek.md\` — Cross-project user preferences, key personal info,
  and habits that apply everywhere.

## Project (./)
- \`./Deepseek.md\` — **Table of Contents** for project-specific context:
  architecture decisions, conventions, key contacts, and references to
  subdirectory Deepseek.md files for detailed context.
- Subdirectory Deepseek.md files (e.g. \`src/Deepseek.md\`, \`docs/Deepseek.md\`) —
  detailed, domain-specific context for that part of the project. Reference
  these from the root \`./Deepseek.md\`.
...
- **Subdirectory**: Detailed context about a specific module or directory → **subdirectory
  Deepseek.md**, with a reference added to the project root
...
# Restrictions
- Keep Deepseek.md files lean — they are loaded into context every session.
- Keep entries concise.
- Edit surgically — preserve existing structure and user-authored content.
- NEVER write or read any files other than Deepseek.md files.
...
- **Do not perform any exploration of the codebase.** Try to use the provided file context and only search additional Deepseek.md files as needed to accomplish your task.
...
- **Minimize file system operations.** You should typically only modify the Deepseek.md files that are already provided in your context. Only read or write to other files if explicitly directed or if you are following a specific reference from an existing memory file.
...
`.trim();
  ...
  return {
    ...
    modelConfig: {
      model: DEEPSEEK_MODEL_ALIAS_FLASH,
    },
    workspaceDirectories: [globalDeepSeekDir],
    toolConfig: {
      tools: [
        READ_FILE_TOOL_NAME,
        EDIT_TOOL_NAME,
