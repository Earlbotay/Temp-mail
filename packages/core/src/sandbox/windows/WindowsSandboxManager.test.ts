...
describe('WindowsSandboxManager', () => {
  ...
  function createTempDir(name: string, parent = os.tmpdir()): string {
    const rawPath = fs.mkdtempSync(path.join(parent, `deepseek-test-${name}-`));
  ...
  it('should prepare a DeepSeekSandbox.exe command', async () => {
    ...
    expect(result.program).toContain('DeepSeekSandbox.exe');
  ...
  it('should ensure governance files exist', async () => {
    ...
    expect(fs.existsSync(path.join(testCwd, '.deepseekignore'))).toBe(true);
