...
export class WindowsSandboxManager implements SandboxManager {
  static readonly HELPER_EXE = 'DeepSeekSandbox.exe';
  ...
  async prepareCommand(req: SandboxRequest): Promise<SandboxedCommand> {
    ...
    // Native commands __read and __write are passed directly to DeepSeekSandbox.exe
    ...
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'deepseek-cli-forbidden-'),
    );
    ...
    // DeepSeekSandbox.exe <network:0|1> <cwd> --forbidden-manifest <path> <command> [args...]
