...
export function validateWorkspacePath(
  ...
  if (!isWithinWorkspace) {
    return {
      isValid: false,
      error: `Directory mismatch. DeepSeek CLI is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
        ', ',
      )}`,
    };
  }
...
export function getPortFromEnv(): string | undefined {
  const port = process.env['DEEPSEEK_CLI_IDE_SERVER_PORT'];
  if (!port) {
    return undefined;
  }
  return port;
...
export function getStdioConfigFromEnv(): StdioConfig | undefined {
  const command = process.env['DEEPSEEK_CLI_IDE_SERVER_STDIO_COMMAND'];
  if (!command) {
    return undefined;
  }

  const argsStr = process.env['DEEPSEEK_CLI_IDE_SERVER_STDIO_ARGS'];
  let args: string[] = [];
  if (argsStr) {
    try {
      ...
      if (Array.isArray(parsedArgs)) {
        ...
        logger.error(
          'DEEPSEEK_CLI_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
        );
      }
    } catch (e) {
      logger.error('Failed to parse DEEPSEEK_CLI_IDE_SERVER_STDIO_ARGS:', e);
    }
  }
...
const IDE_SERVER_FILE_REGEX = /^deepseek-ide-server-(\d+)-\d+\.json$/;
...
> {
  ...
  try {
    ...
    // /^deepseek-ide-server-${pid}-\d+\.json$/. If multiple IDE
    // windows are open, multiple files matching the pattern are expected to
    // exist.
  }

  const portFileDir = path.join(os.tmpdir(), 'deepseek', 'ide');
  let portFiles;
  try {
