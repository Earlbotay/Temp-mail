 ...
 * process).
 *
 * This function can be overridden by setting the `DEEPSEEK_CLI_IDE_PID`
 * environment variable. This is useful for launching DeepSeek CLI in a
 * standalone terminal while still connecting to an IDE instance.
 *
 * If `DEEPSEEK_CLI_IDE_PID` is set, the function uses that PID and fetches
 * the command for it.
 *
...
export async function getIdeProcessInfo(): Promise<{
  ...
  if (process.env['DEEPSEEK_CLI_IDE_PID']) {
    const idePid = parseInt(process.env['DEEPSEEK_CLI_IDE_PID'], 10);
