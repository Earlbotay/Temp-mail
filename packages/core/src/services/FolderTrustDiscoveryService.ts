...
export class FolderTrustDiscoveryService {
  ...
  static async discover(workspaceDir: string): Promise<FolderDiscoveryResults> {
    ...
    const deepseekDir = path.join(workspaceDir, DEEPSEEK_DIR);
    if (!(await this.exists(deepseekDir))) {
      return results;
    }

    await Promise.all([
      this.discoverCommands(deepseekDir, results),
      this.discoverSkills(deepseekDir, results),
      this.discoverAgents(deepseekDir, results),
      this.discoverSettings(deepseekDir, results),
    ]);
