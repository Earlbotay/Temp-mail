 ...
 *
 * Sessions are stored as JSON files in ~/.deepseek/tmp/<project_hash>/chats/
 */
export class ChatRecordingService {
    ...
    try {
      this.updateConversation((conversation) => {
        ...
        if (msg.type === 'deepseek') {
          // If it's a new DeepSeek message then incorporate any queued thoughts.
  ...
  /**
   * Updates the tokens for the last message in the conversation (which should be by DeepSeek).
   */
  recordMessageTokens(
    ...
    try {
      ...
      if (lastMsg && lastMsg.type === 'deepseek' && !lastMsg.tokens) {
  ...
  /**
   * Adds tool calls to the last message in the conversation (which should be by DeepSeek).
  ...
  recordToolCalls(model: string, toolCalls: ToolCallRecord[]): void {
    ...
    try {
      this.updateConversation((conversation) => {
        const lastMsg = this.getLastMessage(conversation);
        // If a tool call was made, but the last message isn't from DeepSeek, it's because DeepSeek is
        // calling tools without starting the message with text.  So the user submits a prompt, and
        // DeepSeek immediately calls a tool (maybe with some thinking first).  In that case, create
        // a new empty DeepSeek message.
        // Also if there are any queued thoughts, it means this tool call(s) is from a new DeepSeek
        // message--because it's thought some more since we last, if ever, created a new DeepSeek
        // message from tool calls, when we dequeued the thoughts.
        if (
          !lastMsg ||
          lastMsg.type !== 'deepseek' ||
          this.queuedThoughts.length > 0
        ) {
          const newMsg: MessageRecord = {
            ...this.newMessage('deepseek' as const, ''),
          ...
          // The last message is an existing DeepSeek message that we need to update.
    ...
    try {
      this.updateConversation((conversation) => {
        ...
        for (const message of conversation.messages) {
          if (message.type === 'deepseek' && message.toolCalls) {
