...
import {
  DeepSeekEventType,
  type ServerDeepSeekContentEvent,
...
describe('LoopDetectionService', () => {
  ...
  const createContentEvent = (content: string): ServerDeepSeekContentEvent => ({
  ...
  describe('Tool Call Loop Detection', () => {
    ...
    it('should not reset tool call counter for other event types', () => {
      ...
      const otherEvent = {
        type: 'thought',
      } as unknown as ServerDeepSeekStreamEvent;
...
describe('LoopDetectionService LLM Checks', () => {
      ...
      modelConfigService: {
        getResolvedConfig: vi.fn().mockImplementation((key) => {
          if (key.model === 'loop-detection') {
            return { model: 'deepseek-2.5-flash', generateContentConfig: {} };
          }
          return {
  ...
  it('should prepend user message if history starts with a function call', async () => {
    ...
    vi.mocked(mockDeepSeekClient.getHistory).mockReturnValue(functionCallHistory);
  ...
  it('should only call Flash model if main model is unavailable', async () => {
    ...
    expect(loggers.logLoopDetected).toHaveBeenCalledWith(
      mockConfig,
      expect.objectContaining({
        confirmed_by_model: 'deepseek-2.5-flash',
      }),
    );
  });
  ...
  it('should not include user prompt in contents when not provided', async () => {
    ...
    vi.mocked(mockDeepSeekClient.getHistory).mockReturnValue([
      {
        role: 'model',
