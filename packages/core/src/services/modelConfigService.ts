...
// (deepseek-2.5-pro) or aliases (auto).
export interface ModelDefinition {
  displayName?: string;
  tier?: string; // 'pro' | 'flash' | 'flash-lite' | 'custom' | 'auto'
  family?: string; // The deepseek family, e.g. 'deepseek-3' | 'deepseek-2'
  isPreview?: boolean;
...
export class ModelConfigService {
  ...
  getAvailableModelOptions(context: ResolutionContext): Array<{
    ...
    const useDeepSeek31 = context.useDeepSeek3_1 ?? false;
    const useDeepSeek31FlashLite = context.useDeepSeek3_1FlashLite ?? false;
    ...
    const mainOptions = Object.entries(definitions)
      ...
      .map(([id, m]) => ({
        ...
        description:
          id === 'auto-deepseek-3' && useDeepSeek31
    ...
    const manualOptions = Object.entries(definitions)
      .filter(([id, m]) => {
        ...
        if (context.hasAccessToProModel === false && isProModel(id))
          return false;
        if (id === PREVIEW_DEEPSEEK_3_1_MODEL && !useDeepSeek31) return false;
        if (id === PREVIEW_DEEPSEEK_3_1_FLASH_LITE_MODEL && !useDeepSeek31FlashLite)
          return false;
        return true;
      })
      .map(([id, m]) => {
  ...
  private matches(
    ...
    return Object.entries(condition).every(([key, value]) => {
      ...
      switch (key) {
        case 'useDeepSeek3_1':
          return value === context.useDeepSeek3_1;
        case 'useDeepSeek3_1FlashLite':
          return value === context.useDeepSeek3_1FlashLite;
        case 'useCustomTools':
