...
export class FileDiscoveryService {
  ...
  private deepseekIgnoreFilter: IgnoreFileFilter | null = null;
  ...
  constructor(projectRoot: string, options?: FilterFilesOptions) {
    ...
    this.deepseekIgnoreFilter = new IgnoreFileParser(
      this.projectRoot,
      DEEPSEEK_IGNORE_FILE_NAME,
    );
    ...
    if (this.gitIgnoreFilter) {
      const deepseekPatterns = this.deepseekIgnoreFilter.getPatterns();
      const customPatterns = this.customIgnoreFilter
        ? this.customIgnoreFilter.getPatterns()
        : [];
      // Create combined parser: .gitignore + .deepseekignore + custom ignore
      this.combinedIgnoreFilter = new GitIgnoreParser(
        this.projectRoot,
        // customPatterns should go the last to ensure overwriting of deepseekPatterns
        [...deepseekPatterns, ...customPatterns],
      );
    } else {
  ...
  private applyFilterFilesOptions(options?: FilterFilesOptions): void {
    ...
    if (options.respectDeepSeekIgnore !== undefined) {
      this.defaultFilterFileOptions.respectDeepSeekIgnore =
        options.respectDeepSeekIgnore;
    }
  ...
  private _shouldIgnore(
    ...
    const {
      ...
      respectDeepSeekIgnore = this.defaultFilterFileOptions.respectDeepSeekIgnore,
    } = options;

    if (respectGitIgnore && respectDeepSeekIgnore && this.combinedIgnoreFilter) {
    ...
    if (
      ...
      this.deepseekIgnoreFilter?.isIgnored(filePath, isDirectory)
    ) {
      return true;
    }
  ...
  /**
   * Returns the list of ignore files being used (e.g. .deepseekignore) excluding .gitignore.
   */
  getIgnoreFilePaths(): string[] {
    const paths: string[] = [];
    if (
      this.deepseekIgnoreFilter &&
      this.defaultFilterFileOptions.respectDeepSeekIgnore
    ) {
      paths.push(...this.deepseekIgnoreFilter.getIgnoreFilePaths());
    }
