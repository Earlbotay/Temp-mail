...
describe('worktree utilities', () => {
  ...
  describe('getProjectRootForWorktree', () => {
    ...
    it('should resolve absolute git common dir paths (as seen in worktrees)', async () => {
      ...
      const result = await getProjectRootForWorktree(
        '/mock/project/.deepseek/worktrees/my-feature',
      );
  ...
  describe('isDeepSeekWorktree', () => {
    it('should return true for a valid deepseek worktree path', () => {
      expect(isDeepSeekWorktree(expectedPath, projectRoot)).toBe(true);
      expect(
        isDeepSeekWorktree(path.join(expectedPath, 'src'), projectRoot),
      ).toBe(true);
    });

    it('should return false for a path outside deepseek worktrees', () => {
      expect(isDeepSeekWorktree(path.join(projectRoot, 'src'), projectRoot)).toBe(
        false,
      );
      expect(
        isDeepSeekWorktree(path.resolve('/some/other/path'), projectRoot),
      ).toBe(false);
    });
  });
...
describe('WorktreeService', () => {
  ...
  describe('maybeCleanup', () => {
    const info = {
      name: 'feature-x',
      path: path.join(projectRoot, '.deepseek', 'worktrees', 'feature-x'),
      baseSha: 'base-sha',
    };
