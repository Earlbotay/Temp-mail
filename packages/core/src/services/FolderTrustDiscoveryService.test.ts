...
describe('FolderTrustDiscoveryService', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'deepseek-discovery-test-'),
    );
  });
  ...
  it('should discover commands, skills, mcps, and hooks', async () => {
    const deepseekDir = path.join(tempDir, DEEPSEEK_DIR);
    await fs.mkdir(deepseekDir, { recursive: true });

    // Mock commands
    const commandsDir = path.join(deepseekDir, 'commands');
    ...
    // Mock skills
    const skillsDir = path.join(deepseekDir, 'skills');
    ...
    // Mock agents
    const agentsDir = path.join(deepseekDir, 'agents');
    ...
    await fs.writeFile(
      path.join(deepseekDir, 'settings.json'),
  ...
  it('should handle missing .deepseek directory', async () => {
    const results = await FolderTrustDiscoveryService.discover(tempDir);
  ...
  it('should handle malformed settings.json', async () => {
    const deepseekDir = path.join(tempDir, DEEPSEEK_DIR);
    await fs.mkdir(deepseekDir, { recursive: true });
    await fs.writeFile(path.join(deepseekDir, 'settings.json'), 'invalid json');
  ...
  it('should handle null settings.json', async () => {
    const deepseekDir = path.join(tempDir, DEEPSEEK_DIR);
    await fs.mkdir(deepseekDir, { recursive: true });
    await fs.writeFile(path.join(deepseekDir, 'settings.json'), 'null');
  ...
  it('should handle array settings.json', async () => {
    const deepseekDir = path.join(tempDir, DEEPSEEK_DIR);
    await fs.mkdir(deepseekDir, { recursive: true });
    await fs.writeFile(path.join(deepseekDir, 'settings.json'), '[]');
  ...
  it('should handle string settings.json', async () => {
    const deepseekDir = path.join(tempDir, DEEPSEEK_DIR);
    await fs.mkdir(deepseekDir, { recursive: true });
    await fs.writeFile(path.join(deepseekDir, 'settings.json'), '"string"');
