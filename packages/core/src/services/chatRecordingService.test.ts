...
describe('ChatRecordingService', () => {
  ...
  describe('recordMessageTokens', () => {
    ...
    it('should update the last message with token info', () => {
      ...
      const deepseekMsg = conversation.messages[0] as MessageRecord & {
        type: 'deepseek';
      };
      expect(deepseekMsg.tokens).toEqual({
        input: 1,
        output: 2,
        total: 3,
        cached: 0,
    ...
    it('should not write to disk when queuing tokens (no last deepseek message)', () => {
      const writeFileSyncSpy = vi.spyOn(fs, 'writeFileSync');

      // Clear spy call count after initialize writes the initial file
      writeFileSyncSpy.mockClear();

      // No deepseek message recorded yet, so tokens should only be queued
      chatRecordingService.recordMessageTokens({
        promptTokenCount: 5,
  ...
  describe('recordToolCalls', () => {
    ...
    it('should add new tool calls to the last message', () => {
      ...
      chatRecordingService.recordToolCalls('deepseek-pro', [toolCall]);
      ...
      expect(deepseekMsg.toolCalls).toHaveLength(1);
      expect(deepseekMsg.toolCalls![0].name).toBe('testTool');
    });
    ...
    it('should preserve dynamic description and NOT overwrite with generic one', () => {
      ...
      expect(deepseekMsg.toolCalls![0].description).toBe(dynamicDescription);
    });

    it('should create a new message if the last message is not from deepseek', () => {
      chatRecordingService.recordMessage({
        type: 'user',
        content: 'call a tool',
        model: 'deepseek-pro',
      ...
      expect(conversation.messages[1].type).toBe('deepseek');
      expect(
        (conversation.messages[1] as MessageRecord & { type: 'deepseek' })
          .toolCalls,
      ).toHaveLength(1);
    });
  });
  ...
  describe('updateMessagesFromHistory', () => {
    ...
    it('should update tool results from API history (masking sync)', () => {
      ...
      chatRecordingService.recordToolCalls('deepseek-pro', [
        {
          id: callId,
          name: 'list_files',
          args: { path: '.' },
      ...
      const deepseekMsg = conversation.messages[0];
      if (deepseekMsg.type !== 'deepseek')
        throw new Error('Expected deepseek message');
      expect(deepseekMsg.toolCalls).toBeDefined();
      expect(deepseekMsg.toolCalls![0].id).toBe(callId);
      // The implementation stringifies the response object
      const result = deepseekMsg.toolCalls![0].result;
