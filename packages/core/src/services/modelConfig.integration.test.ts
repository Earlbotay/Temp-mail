...
describe('ModelConfigService Integration', () => {
  const complexConfig: ModelConfigServiceConfig = {
    ...
    overrides: [
      ...
      {
        match: { model: 'deepseek-1.5-flash-latest' },
        modelConfig: {
          generateContentConfig: {
  ...
  it('should correctly apply a simple inherited alias and merge with global defaults', () => {
    ...
    expect(resolved.model).toBe('deepseek-1.5-pro-latest'); // from alias
    expect(resolved.generateContentConfig).toEqual({
      temperature: 0.5, // from agent override
      topP: 0.95, // from base
      topK: 40, // from alias
  ...
  it('should resolve a multi-level inherited alias', () => {
    ...
    expect(resolved.model).toBe('deepseek-1.5-pro-latest'); // from default-text-model
    expect(resolved.generateContentConfig).toEqual({
      temperature: 0.5, // from agent override
      topP: 0.95, // from base
      topK: 50, // from alias
  ...
  it('should apply an inherited alias and a broad model-based override', () => {
    ...
    expect(resolved.model).toBe('deepseek-1.5-pro-latest'); // now overridden by 'base'
    expect(resolved.generateContentConfig).toEqual({
      topP: 0.95, // from base
      topK: 64, // from base
  ...
  it('should resolve an abstract alias if a specific override provides the model', () => {
    ...
    expect(resolved.model).toBe('deepseek-1.5-pro-latest'); // from override
    expect(resolved.generateContentConfig).toEqual({
      temperature: 0.5, // from agent override
