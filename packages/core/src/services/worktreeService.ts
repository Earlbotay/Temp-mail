...
export function getWorktreePath(projectRoot: string, name: string): string {
  return path.join(projectRoot, '.deepseek', 'worktrees', name);
}
...
export function isDeepSeekWorktree(
  dirPath: string,
  projectRoot: string,
): boolean {
  try {
    ...
    const worktreesBaseDir = path.join(realProjectRoot, '.deepseek', 'worktrees');
