...
import { DEEPSEEK_DIR, homedir } from '../utils/paths.js';
...
export class FileKeychain implements Keychain {
  ...
  constructor() {
    const configDir = path.join(homedir(), DEEPSEEK_DIR);
    this.tokenFilePath = path.join(configDir, 'deepseek-credentials.json');
    this.encryptionKey = this.deriveEncryptionKey();
  }

  private deriveEncryptionKey(): Buffer {
    const salt = `${os.hostname()}-${os.userInfo().username}-deepseek-cli`;
    return crypto.scryptSync('deepseek-cli-oauth', salt, 32);
  }
