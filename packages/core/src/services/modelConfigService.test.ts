...
describe('ModelConfigService', () => {
  it('should resolve a basic alias to its model and settings', () => {
    const config: ModelConfigServiceConfig = {
      aliases: {
        classifier: {
          modelConfig: {
            model: 'deepseek-1.5-flash-latest',
    ...
    expect(resolved.model).toBe('deepseek-1.5-flash-latest');
  ...
  it('should apply the most specific override rule', () => {
    const config: ModelConfigServiceConfig = {
      ...
      overrides: [
        {
          match: { model: 'deepseek-pro' },
          modelConfig: { generateContentConfig: { temperature: 0.5 } },
        },
        {
          match: { model: 'deepseek-pro', overrideScope: 'my-agent' },
    ...
    expect(resolved.model).toBe('deepseek-pro');
  ...
  it('should use the last override in case of a tie in specificity', () => {
    ...
    const service = new ModelConfigService(config);
    const resolved = service.getResolvedConfig({ model: 'deepseek-pro' });
  ...
  it('should merge settings from global, alias, and multiple matching overrides', () => {
    const config: ModelConfigServiceConfig = {
      ...
      overrides: [
        {
          match: { model: 'deepseek-test-model' },
          modelConfig: {
            generateContentConfig: {
              topK: 40,
        ...
        {
          match: { model: 'deepseek-test-model', overrideScope: 'test-agent' },
          modelConfig: {
            generateContentConfig: {
              temperature: 0.2,
            },
    ...
    expect(resolved.model).toBe('deepseek-test-model');
    expect(resolved.generateContentConfig).toEqual({
  ...
  describe('alias inheritance', () => {
    ...
    describe('abstract aliases', () => {
      it('should allow an alias to extend an abstract alias without a model', () => {
        ...
        expect(resolved.model).toBe('deepseek-1.5-pro-latest');
        expect(resolved.generateContentConfig).toEqual({
          temperature: 0.1,
          topP: 0.9,
        });
    ...
    it('should throw an error if the alias chain is too deep', () => {
      ...
      for (let i = 0; i < 101; i++) {
        aliases[`alias-${i}`] = {
          ...
          modelConfig: i === 100 ? { model: 'deepseek-pro' } : {},
        };
      }
  ...
  describe('runtime aliases', () => {
    it('should resolve a simple runtime-registered alias', () => {
      ...
      expect(resolved.model).toBe('deepseek-runtime-model');
      expect(resolved.generateContentConfig).toEqual({
        temperature: 0.123,
      });
    });
  ...
  describe('custom aliases', () => {
    it('should resolve a custom alias', () => {
      ...
      expect(resolved.model).toBe('deepseek-custom');
      expect(resolved.generateContentConfig).toEqual({
        temperature: 0.9,
      });
    });
    ...
    it('should allow custom aliases to override built-in aliases', () => {
      const config: ModelConfigServiceConfig = {
        ...
        customAliases: {
          'standard-alias': {
            modelConfig: {
              model: 'deepseek-custom-override',
              generateContentConfig: {
                temperature: 0.1,
              },
            },
      ...
      expect(resolved.model).toBe('deepseek-custom-override');
      expect(resolved.generateContentConfig).toEqual({
        temperature: 0.1,
      });
    });
  ...
  describe('getAvailableModelOptions', () => {
    it('should filter out Pro models when hasAccessToProModel is false', () => {
      const config: ModelConfigServiceConfig = {
        modelDefinitions: {
          'deepseek-3-pro': { isVisible: true, tier: 'pro' },
          'deepseek-3-flash': { isVisible: true, tier: 'flash' },
        },
      };
      const service = new ModelConfigService(config);
      ...
      expect(options.map((o) => o.modelId)).not.toContain('deepseek-3-pro');
      expect(options.map((o) => o.modelId)).toContain('deepseek-3-flash');
    });
    ...
    it('should include Pro models when hasAccessToProModel is true or undefined', () => {
      ...
      expect(optionsWithTrue.map((o) => o.modelId)).toContain('deepseek-3-pro');
