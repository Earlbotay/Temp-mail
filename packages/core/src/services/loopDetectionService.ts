...
import { DeepSeekEventType, type ServerDeepSeekStreamEvent } from '../core/turn.js';
import {
  logLoopDetected,
...
export class LoopDetectionService {
   ...
   */
  addAndCheck(event: ServerDeepSeekStreamEvent): LoopDetectionResult {
    if (
    ...
    if (isLoop) {
      ...
      this.lastLoopType =
        event.type === DeepSeekEventType.ToolCallRequest
  ...
  private async checkForLoopWithLLM(signal: AbortSignal): Promise<{
    ...
    const recentHistory = this.context.deepseekClient
      .getHistory()
