...
export class PromptProvider {
  ...
  getCoreSystemPrompt(
    ...
    const systemMdResolution = resolvePathFromEnv(
      process.env['DEEPSEEK_SYSTEM_MD'],
    );
    ...
    const desiredModel = resolveModel(
      ...
      context.config.getDeepSeek31LaunchedSync?.() ?? false,
      context.config.getDeepSeek31FlashLiteLaunchedSync?.() ?? false,
      false,
    ...
    const contextFilenames = getAllDeepSeekMdFilenames();
    ...
    if (systemMdResolution.value && !systemMdResolution.isDisabled) {
      let systemMdPath = path.resolve(path.join(DEEPSEEK_DIR, 'system.md'));
    ...
    this.maybeWriteSystemMd(
      sanitizedPrompt,
      systemMdResolution,
      path.resolve(path.join(DEEPSEEK_DIR, 'system.md')),
    );
  ...
  private maybeWriteSystemMd(
    ...
    const writeSystemMdResolution = resolvePathFromEnv(
      process.env['DEEPSEEK_WRITE_SYSTEM_MD'],
    );
