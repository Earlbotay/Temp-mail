...
vi.mock('../tools/memoryTool.js', async (importOriginal) => {
  ...
  return {
    ...(actual as object),
    getAllDeepSeekMdFilenames: vi.fn(),
  };
});
...
describe('PromptProvider', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('DEEPSEEK_SYSTEM_MD', '');
    vi.stubEnv('DEEPSEEK_WRITE_SYSTEM_MD', '');
    ...
    mockConfig = {
      ...
      getDeepSeek31LaunchedSync: vi.fn().mockReturnValue(true),
  ...
  it('should handle multiple context filenames in the system prompt', () => {
    vi.mocked(getAllDeepSeekMdFilenames).mockReturnValue([
      DEFAULT_CONTEXT_FILENAME,
      'CUSTOM.md',
      'ANOTHER.md',
    ]);
  ...
  describe('Topic & Update Narration', () => {
    beforeEach(() => {
      ...
      vi.mocked(mockConfig.getDeepSeek31LaunchedSync).mockReturnValue(true);
    });
