...
const createMockConfig = (overrides: Partial<Config> = {}): Config => {
  const config = {
    ...
    getModel: () => 'deepseek-2.5-pro',
    getDeepSeek31LaunchedSync: () => false,
    getDeepSeek31FlashLiteLaunchedSync: () => false,
    getUseCustomToolModelSync: () => {
      const useDeepSeek31 = config.getDeepSeek31LaunchedSync();
      const authType = config.getContentGeneratorConfig().authType;
      return useDeepSeek31 && authType === AuthType.USE_DEEPSEEK;
    },
...
describe('policyHelpers', () => {
  describe('resolvePolicyChain', () => {
    ...
    it('leaves catalog order untouched when active model already present', () => {
      ...
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe('deepseek-2.5-pro');
    });

    it('returns the default chain when active model is "auto"', () => {
      const config = createMockConfig({
        getModel: () => DEFAULT_DEEPSEEK_MODEL_AUTO,
      });
      const chain = resolvePolicyChain(config);
      ...
      expect(chain[0]?.model).toBe('deepseek-2.5-pro');
      expect(chain[1]?.model).toBe('deepseek-2.5-flash');
    });
    ...
    it('uses auto chain when preferred model is auto', () => {
      ...
      const chain = resolvePolicyChain(config, DEFAULT_DEEPSEEK_MODEL_AUTO);
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('deepseek-2.5-pro');
      expect(chain[1]?.model).toBe('deepseek-2.5-flash');
    });
    ...
    it('uses auto chain when configured model is auto even if preferred is concrete', () => {
      ...
      const chain = resolvePolicyChain(config, 'deepseek-2.5-pro');
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('deepseek-2.5-pro');
      expect(chain[1]?.model).toBe('deepseek-2.5-flash');
    });
    ...
    it('starts chain from preferredModel when model is "auto"', () => {
      ...
      const chain = resolvePolicyChain(config, 'deepseek-2.5-flash');
      expect(chain).toHaveLength(1);
      expect(chain[0]?.model).toBe('deepseek-2.5-flash');
    });
    ...
    it('returns flash-lite chain when preferred model is flash-lite', () => {
      ...
      const chain = resolvePolicyChain(config, DEFAULT_DEEPSEEK_FLASH_LITE_MODEL);
      expect(chain).toHaveLength(3);
      expect(chain[0]?.model).toBe('deepseek-2.5-flash-lite');
      expect(chain[1]?.model).toBe('deepseek-2.5-flash');
      expect(chain[2]?.model).toBe('deepseek-2.5-pro');
    });

    it('returns flash-lite chain when configured model is flash-lite', () => {
      const config = createMockConfig({
        getModel: () => DEFAULT_DEEPSEEK_FLASH_LITE_MODEL,
      });
      const chain = resolvePolicyChain(config);
      expect(chain).toHaveLength(3);
      expect(chain[0]?.model).toBe('deepseek-2.5-flash-lite');
    ...
    it('wraps around the chain when wrapsAround is true', () => {
      ...
      const chain = resolvePolicyChain(config, 'deepseek-2.5-flash', true);
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('deepseek-2.5-flash');
      expect(chain[1]?.model).toBe('deepseek-2.5-pro');
    });

    it('proactively returns DeepSeek 2.5 chain if DeepSeek 3 requested but user lacks access', () => {
      const config = createMockConfig({
        getModel: () => 'auto-deepseek-3',
    ...
    it('returns DeepSeek 3.1 Pro chain when launched and auto-deepseek-3 requested', () => {
      const config = createMockConfig({
        getModel: () => 'auto-deepseek-3',
        getDeepSeek31LaunchedSync: () => true,
      });
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe(PREVIEW_DEEPSEEK_3_1_MODEL);
      expect(chain[1]?.model).toBe('deepseek-3-flash-preview');
    });

    it('returns DeepSeek 3.1 Pro Custom Tools chain when launched, auth is DeepSeek, and auto-deepseek-3 requested', () => {
      const config = createMockConfig({
        getModel: () => 'auto-deepseek-3',
        getDeepSeek31LaunchedSync: () => true,
        getContentGeneratorConfig: () => ({ authType: AuthType.USE_DEEPSEEK }),
      });
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe(PREVIEW_DEEPSEEK_3_1_CUSTOM_TOOLS_MODEL);
      expect(chain[1]?.model).toBe('deepseek-3-flash-preview');
    });
  });
  ...
  describe('resolvePolicyChain behavior is identical between dynamic and legacy implementations', () => {
    const testCases = [
      { name: 'Default Auto', model: DEFAULT_DEEPSEEK_MODEL_AUTO },
      { name: 'DeepSeek 3 Auto', model: 'auto-deepseek-3' },
      { name: 'Flash Lite', model: DEFAULT_DEEPSEEK_FLASH_LITE_MODEL },
      {
        name: 'DeepSeek 3 Auto (3.1 Enabled)',
        model: 'auto-deepseek-3',
        useDeepSeek31: true,
      },
      {
        name: 'DeepSeek 3 Auto (3.1 + Custom Tools)',
        model: 'auto-deepseek-3',
        useDeepSeek31: true,
        authType: AuthType.USE_DEEPSEEK,
      },
      {
        name: 'DeepSeek 3 Auto (No Access)',
        model: 'auto-deepseek-3',
        hasAccess: false,
      },
      { name: 'Concrete Model (2.5 Pro)', model: 'deepseek-2.5-pro' },
  ...
  describe('applyModelSelection', () => {
    ...
    it('returns requested model if it is available', () => {
      ...
      expect(config.setActiveModel).toHaveBeenCalledWith('deepseek-pro');
    });
    ...
    it('switches to backup model and updates config if requested is unavailable', () => {
      ...
      expect(config.setActiveModel).toHaveBeenCalledWith('deepseek-flash');
    });
    ...
    it('does not consume sticky attempt if consumeAttempt is false', () => {
      ...
      const result = applyModelSelection(
        config,
        { model: 'deepseek-pro', isChatModel: true },
        {
