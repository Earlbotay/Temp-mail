...
export function resolvePolicyChain(
  ...
  if (config.getExperimentalDynamicModelConfiguration?.() === true) {
    ...
    if (resolvedModel === DEFAULT_DEEPSEEK_FLASH_LITE_MODEL) {
      chain = config.modelConfigService.resolveChain('lite', context);
    } else if (
      isDeepSeek3Model(resolvedModel, config) ||
      isAutoModel(preferredModel ?? '', config) ||
      ...
      if (!chain) {
        const previewEnabled =
          hasAccessToPreview &&
          (isDeepSeek3Model(resolvedModel, config) ||
            preferredModel === PREVIEW_DEEPSEEK_MODEL_AUTO ||
            configuredModel === PREVIEW_DEEPSEEK_MODEL_AUTO);
  ...
  if (resolvedModel === DEFAULT_DEEPSEEK_FLASH_LITE_MODEL) {
    ...
    if (hasAccessToPreview) {
      ...
      // User requested DeepSeek 3 but has no access. Proactively downgrade
      // to the stable DeepSeek 2.5 chain.
      chain = getModelPolicyChain({
...
export function selectModelForAvailability(
  ...
  const backupModel =
    chain.find((p) => p.isLastResort)?.model ?? DEFAULT_DEEPSEEK_MODEL;
