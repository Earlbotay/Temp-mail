...
export interface ModelPolicyOptions {
  ...
  userTier?: UserTierId;
  useDeepSeek31?: boolean;
  useDeepSeek31FlashLite?: boolean;
...
const DEFAULT_CHAIN: ModelPolicyChain = [
  definePolicy({ model: DEFAULT_DEEPSEEK_MODEL }),
  definePolicy({ model: DEFAULT_DEEPSEEK_FLASH_MODEL, isLastResort: true }),
];

const FLASH_LITE_CHAIN: ModelPolicyChain = [
  definePolicy({
    model: DEFAULT_DEEPSEEK_FLASH_LITE_MODEL,
...
export function getModelPolicyChain(
  ...
  if (options.previewEnabled) {
    const previewModel = resolveModel(
      PREVIEW_DEEPSEEK_MODEL,
      options.useDeepSeek31,
      options.useDeepSeek31FlashLite,
      options.useCustomToolModel,
    );
    return [
      definePolicy({ model: previewModel }),
      definePolicy({ model: PREVIEW_DEEPSEEK_FLASH_MODEL, isLastResort: true }),
    ];
  }
