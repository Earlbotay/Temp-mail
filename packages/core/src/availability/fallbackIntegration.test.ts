...
describe('Fallback Integration', () => {
  ...
  beforeEach(() => {
    ...
    config = {
      getModel: () => PREVIEW_DEEPSEEK_MODEL_AUTO,
      getActiveModel: () => PREVIEW_DEEPSEEK_MODEL_AUTO,
  ...
  it('should select fallback model when primary model is terminal and config is in AUTO mode', () => {
    ...
    // The policy chain for PREVIEW_DEEPSEEK_MODEL_AUTO is [PREVIEW_DEEPSEEK_MODEL, PREVIEW_DEEPSEEK_FLASH_MODEL]
    availabilityService.markTerminal(PREVIEW_DEEPSEEK_MODEL, 'quota');

    // 2. Request "Pro" explicitly (as Agent would)
    const requestedModel = PREVIEW_DEEPSEEK_MODEL;
    ...
    expect(result.model).toBe(PREVIEW_DEEPSEEK_FLASH_MODEL);
  ...
  it('should fallback for DeepSeek 3 models even if config is NOT in AUTO mode', () => {
    // 1. Config is explicitly set to Pro, not Auto
    vi.spyOn(config, 'getModel').mockReturnValue(PREVIEW_DEEPSEEK_MODEL);
    ...
    // 5. Expect it to fallback to Flash (because DeepSeek 3 uses PREVIEW_CHAIN)
    expect(result.model).toBe(PREVIEW_DEEPSEEK_FLASH_MODEL);
  });
});
