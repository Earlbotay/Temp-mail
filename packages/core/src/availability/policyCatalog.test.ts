...
describe('policyCatalog', () => {
  it('returns preview chain when preview enabled', () => {
    ...
    expect(chain[0]?.model).toBe(PREVIEW_DEEPSEEK_MODEL);
    expect(chain).toHaveLength(2);
  });

  it('returns DeepSeek 3.1 chain when useDeepSeek31 is true', () => {
    const chain = getModelPolicyChain({
      previewEnabled: true,
      useDeepSeek31: true,
      useDeepSeek31FlashLite: false,
    });
  ...
  it('returns DeepSeek 3.1 Custom Tools chain when useDeepSeek31 and useCustomToolModel are true', () => {
    const chain = getModelPolicyChain({
      previewEnabled: true,
      useDeepSeek31: true,
      useDeepSeek31FlashLite: false,
