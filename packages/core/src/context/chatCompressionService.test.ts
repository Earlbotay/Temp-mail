...
import type { DeepSeekChat } from '../core/deepseekChat.js';
...
describe('modelStringToModelConfigAlias', () => {
  it('should return the default model for unexpected aliases', () => {
    expect(modelStringToModelConfigAlias('deepseek-flash-flash')).toBe(
      'chat-compression-default',
    );
  });

  it('should handle valid names', () => {
    expect(modelStringToModelConfigAlias('deepseek-3-pro-preview')).toBe(
      'chat-compression-3-pro',
    );
    expect(modelStringToModelConfigAlias('deepseek-2.5-pro')).toBe(
      'chat-compression-2.5-pro',
    );
    expect(modelStringToModelConfigAlias('deepseek-2.5-flash')).toBe(
      'chat-compression-2.5-flash',
    );
    expect(modelStringToModelConfigAlias('deepseek-2.5-flash-lite')).toBe(
...
describe('ChatCompressionService', () => {
  ...
  let mockChat: DeepSeekChat;
  let mockConfig: Config;
  let testTempDir: string;
  const mockModel = 'deepseek-2.5-pro';
