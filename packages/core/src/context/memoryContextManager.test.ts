...
describe('MemoryContextManager', () => {
    ...
    mockConfig = {
      ...
      storage: {
        getProjectMemoryDir: vi
          .fn()
          .mockReturnValue('/home/user/.deepseek/memory/test-project'),
      },
  ...
  describe('refresh', () => {
    it('should load and format global and environment memory', async () => {
      const globalPaths = ['/home/user/.deepseek/Deepseek.md'];
      const envPaths = ['/app/Deepseek.md'];
      ...
      vi.mocked(memoryDiscovery.readDeepSeekMdFiles).mockResolvedValue([
      ...
      expect(memoryDiscovery.readDeepSeekMdFiles).toHaveBeenCalledWith(
    ...
    it('should emit MemoryChanged event when memory is refreshed', async () => {
      ...
      vi.mocked(memoryDiscovery.readDeepSeekMdFiles).mockResolvedValue([
        { filePath: '/app/Deepseek.md', content: 'content' },
        { filePath: '/app/src/Deepseek.md', content: 'env content' },
      ]);
    ...
    it('should not load environment memory if folder is not trusted', async () => {
      ...
      vi.mocked(memoryDiscovery.getGlobalMemoryPaths).mockResolvedValue([
        '/home/user/.deepseek/Deepseek.md',
      ]);
      vi.mocked(memoryDiscovery.readDeepSeekMdFiles).mockResolvedValue([
        { filePath: '/home/user/.deepseek/Deepseek.md', content: 'Global Content' },
      ]);
    ...
    it('should deduplicate files by file identity in case-insensitive filesystems', async () => {
      const globalPaths = ['/home/user/.deepseek/Deepseek.md'];
      const envPaths = ['/app/deepseek.md', '/app/Deepseek.md'];
      ...
      vi.mocked(
        ...
        paths: ['/home/user/.deepseek/Deepseek.md', '/app/deepseek.md'],
        identityMap: new Map<string, string>(),
      });

      vi.mocked(memoryDiscovery.readDeepSeekMdFiles).mockResolvedValue([
        { filePath: '/home/user/.deepseek/Deepseek.md', content: 'Global Content' },
        { filePath: '/app/deepseek.md', content: 'Project Content' },
      ]);
      ...
      expect(memoryDiscovery.readDeepSeekMdFiles).toHaveBeenCalledWith(
        ['/home/user/.deepseek/Deepseek.md', '/app/deepseek.md'],
        'tree',
        ['.git'],
      );
      expect(memoryContextManager.getEnvironmentMemory()).toContain(
  ...
  describe('discoverContext', () => {
    it('should discover and load new context', async () => {
      const mockResult: memoryDiscovery.MemoryLoadResult = {
        files: [{ path: '/app/src/Deepseek.md', content: 'Src Content' }],
      };
      ...
      expect(result).toMatch(/--- Context from: \/app\/src\/DEEPSEEK\.md ---/);
