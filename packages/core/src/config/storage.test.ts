...
import { DEEPSEEK_DIR, homedir, resolveToRealPath } from '../utils/paths.js';
...
describe('Storage – initialize', () => {
  ...
  it('sets up the registry and performs migration if `getProjectTempDir` is called', async () => {
    ...
    expect(storage.getProjectTempDir()).toBe(
      path.join(os.homedir(), DEEPSEEK_DIR, 'tmp', PROJECT_SLUG),
    );
...
describe('Storage – getGlobalSettingsPath', () => {
  it('returns path to ~/.deepseek/settings.json', () => {
    const expected = path.join(os.homedir(), DEEPSEEK_DIR, 'settings.json');
    expect(Storage.getGlobalSettingsPath()).toBe(expected);
  });
});

describe('Storage - Security', () => {
  it('falls back to tmp for deepseek but returns empty for agents if the home directory cannot be determined', () => {
    vi.mocked(homedir).mockReturnValue('');

    // .deepseek falls back for backward compatibility
    expect(Storage.getGlobalDeepSeekDir()).toBe(
      path.join(os.tmpdir(), DEEPSEEK_DIR),
    );
...
describe('Storage – additional helpers', () => {
  ...
  it('getWorkspaceSettingsPath returns project/.deepseek/settings.json', () => {
    const expected = path.join(projectRoot, DEEPSEEK_DIR, 'settings.json');
    expect(storage.getWorkspaceSettingsPath()).toBe(expected);
  });

  it('getUserCommandsDir returns ~/.deepseek/commands', () => {
    const expected = path.join(os.homedir(), DEEPSEEK_DIR, 'commands');
    expect(Storage.getUserCommandsDir()).toBe(expected);
  });

  it('getProjectCommandsDir returns project/.deepseek/commands', () => {
    const expected = path.join(projectRoot, DEEPSEEK_DIR, 'commands');
    expect(storage.getProjectCommandsDir()).toBe(expected);
  });

  it('getUserSkillsDir returns ~/.deepseek/skills', () => {
    const expected = path.join(os.homedir(), DEEPSEEK_DIR, 'skills');
    expect(Storage.getUserSkillsDir()).toBe(expected);
  });

  it('getProjectSkillsDir returns project/.deepseek/skills', () => {
    const expected = path.join(projectRoot, DEEPSEEK_DIR, 'skills');
    expect(storage.getProjectSkillsDir()).toBe(expected);
  });

  it('getUserAgentsDir returns ~/.deepseek/agents', () => {
    const expected = path.join(os.homedir(), DEEPSEEK_DIR, 'agents');
    expect(Storage.getUserAgentsDir()).toBe(expected);
  });

  it('getProjectAgentsDir returns project/.deepseek/agents', () => {
    const expected = path.join(projectRoot, DEEPSEEK_DIR, 'agents');
    expect(storage.getProjectAgentsDir()).toBe(expected);
  });

  it('getProjectMemoryDir returns ~/.deepseek/tmp/<identifier>/memory', async () => {
    await storage.initialize();
  ...
  it('getMcpOAuthTokensPath returns ~/.deepseek/mcp-oauth-tokens.json', () => {
    const expected = path.join(
      os.homedir(),
      DEEPSEEK_DIR,
  ...
  it('getGlobalBinDir returns ~/.deepseek/tmp/bin', () => {
    const expected = path.join(os.homedir(), DEEPSEEK_DIR, 'tmp', 'bin');
    expect(Storage.getGlobalBinDir()).toBe(expected);
  });

  it('getProjectTempPlansDir returns ~/.deepseek/tmp/<identifier>/plans when no sessionId is provided', async () => {
    await storage.initialize();
  ...
  it('getProjectTempPlansDir returns ~/.deepseek/tmp/<identifier>/<sessionId>/plans when sessionId is provided', async () => {
  ...
  it('getProjectTempTrackerDir returns ~/.deepseek/tmp/<identifier>/tracker when no sessionId is provided', async () => {
    await storage.initialize();
    const tempDir = storage.getProjectTempDir();
  ...
  it('getProjectTempTrackerDir returns ~/.deepseek/tmp/<identifier>/<sessionId>/tracker when sessionId is provided', async () => {
    const sessionId = 'test-session-id';
    const storageWithSession = new Storage(projectRoot, sessionId);
    ProjectRegistry.prototype.getShortId = vi
      .fn()
...
describe('Storage - System Paths', () => {
  const originalEnv = process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH'];

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH'] = originalEnv;
    } else {
      delete process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH'];
    }
  });
  ...
  it('getSystemSettingsPath returns correct path based on platform (default)', () => {
    ...
    if (platform === 'darwin') {
      expect(result).toBe(
        '/Library/Application Support/DeepSeekCli/settings.json',
      );
    } else if (platform === 'win32') {
      expect(result).toBe('C:\\ProgramData\\deepseek-cli\\settings.json');
    } else {
      expect(result).toBe('/etc/deepseek-cli/settings.json');
    }
  });

  it('getSystemSettingsPath follows DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH if set', () => {
    const customPath = '/custom/path/settings.json';
    process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH'] = customPath;
  ...
  it('getSystemPoliciesDir returns correct path based on platform and ignores env var', () => {
    process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH'] =
    ...
    if (platform === 'darwin') {
      expect(result).toBe('/Library/Application Support/DeepSeekCli/policies');
    } else if (platform === 'win32') {
      expect(result).toBe('C:\\ProgramData\\deepseek-cli\\policies');
    } else {
      expect(result).toBe('/etc/deepseek-cli/policies');
    }
  });
});
