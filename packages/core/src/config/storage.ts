...
export class Storage {
  ...
  static getGlobalDeepSeekDir(): string {
    const homeDir = homedir();
    if (!homeDir) {
      return path.join(os.tmpdir(), DEEPSEEK_DIR);
    }
    return path.join(homeDir, DEEPSEEK_DIR);
  }
  ...
  static getMcpOAuthTokensPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'mcp-oauth-tokens.json');
  }

  static getA2AOAuthTokensPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'a2a-oauth-tokens.json');
  }

  static getGlobalSettingsPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'settings.json');
  }

  static getInstallationIdPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'installation_id');
  }

  static getGoogleAccountsPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), GOOGLE_ACCOUNTS_FILENAME);
  }

  static getUserCommandsDir(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'commands');
  }

  static getUserSkillsDir(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'skills');
  }
  ...
  static getGlobalMemoryFilePath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'memory.md');
  }

  static getUserPoliciesDir(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'policies');
  }

  static getUserKeybindingsPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'keybindings.json');
  }

  static getUserAgentsDir(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'agents');
  }

  static getAcknowledgedAgentsPath(): string {
    return path.join(
      Storage.getGlobalDeepSeekDir(),
      'acknowledgments',
      'agents.json',
    );
  }
  ...
  static getPolicyIntegrityStoragePath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), 'policy_integrity.json');
  }

  private static getSystemConfigDir(): string {
    if (os.platform() === 'darwin') {
      return '/Library/Application Support/DeepSeekCli';
    } else if (os.platform() === 'win32') {
      return 'C:\\ProgramData\\deepseek-cli';
    } else {
      return '/etc/deepseek-cli';
    }
  }

  static getSystemSettingsPath(): string {
    if (process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH']) {
      return process.env['DEEPSEEK_CLI_SYSTEM_SETTINGS_PATH'];
    }
  ...
  static getGlobalTempDir(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), TMP_DIR_NAME);
  }
  ...
  getDeepSeekDir(): string {
    return path.join(this.targetDir, DEEPSEEK_DIR);
  }
  ...
  getWorkspacePoliciesDir(): string {
    return path.join(this.getDeepSeekDir(), 'policies');
  }
  ...
  static getOAuthCredsPath(): string {
    return path.join(Storage.getGlobalDeepSeekDir(), OAUTH_FILE);
  }
  ...
  async initialize(): Promise<void> {
    ...
    this.initPromise = (async () => {
      ...
      const registry = new ProjectRegistry(registryPath, [
        ...
        path.join(Storage.getGlobalDeepSeekDir(), 'history'),
      ]);
  ...
  private async performMigration(): Promise<void> {
    ...
    // Migrate History Dir
    const historyDir = path.join(Storage.getGlobalDeepSeekDir(), 'history');
  ...
  getWorkspaceSettingsPath(): string {
    return path.join(this.getDeepSeekDir(), 'settings.json');
  }

  getProjectCommandsDir(): string {
    return path.join(this.getDeepSeekDir(), 'commands');
  }

  getProjectSkillsDir(): string {
    return path.join(this.getDeepSeekDir(), 'skills');
  }
  ...
  getProjectAgentsDir(): string {
    return path.join(this.getDeepSeekDir(), 'agents');
  }
  ...
  getExtensionsDir(): string {
    return path.join(this.getDeepSeekDir(), 'extensions');
  }

  getExtensionsConfigPath(): string {
    return path.join(this.getExtensionsDir(), 'deepseek-extension.json');
  }
