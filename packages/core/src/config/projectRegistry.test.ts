...
describe('ProjectRegistry', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deepseek-registry-test-'));
  ...
  it('handles collisions with unique suffixes', async () => {
    ...
    const id1 = await registry.getShortId(path.join(tempDir, 'one', 'deepseek'));
    const id2 = await registry.getShortId(path.join(tempDir, 'two', 'deepseek'));
    const id3 = await registry.getShortId(
      path.join(tempDir, 'three', 'deepseek'),
    );

    expect(id1).toBe('deepseek');
    expect(id2).toBe('deepseek-1');
    expect(id3).toBe('deepseek-2');
  });
  ...
  it('handles collisions if a slug is taken on disk by another project', async () => {
    // 1. project-y takes 'deepseek' on disk
    ...
    // 2. project-z tries to get shortId for 'deepseek'
    const registry = new ProjectRegistry(registryPath, [baseDir1]);
    await registry.initialize();
    const projectZ = normalizePath(path.join(tempDir, 'deepseek'));
    const shortId = await registry.getShortId(projectZ);

    // 3. It should avoid 'deepseek' and pick 'deepseek-1' (or similar)
    expect(shortId).not.toBe('deepseek');
    expect(shortId).toBe('deepseek-1');
  });
