...
import { createMockSandboxConfig } from '@google/deepseek-cli-test-utils';
...
import { setDeepSeekMdFilename as mockSetDeepSeekMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../tools/memoryTool', () => ({
  MemoryTool: vi.fn(),
  setDeepSeekMdFilename: vi.fn(),
  getCurrentDeepSeekMdFilename: vi.fn(() => 'Deepseek.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'Deepseek.md',
  DEEPSEEK_DIR: '.deepseek',
}));
...
describe('Server Config (config.ts)', () => {
  ...
  describe('initialize', () => {
    ...
    describe('getDeepSeek31LaunchedSync', () => {
      it.each([AuthType.USE_DEEPSEEK, AuthType.USE_VERTEX_AI, AuthType.GATEWAY])(
        ...
        async (authType) => {
          ...
          expect(config.getDeepSeek31LaunchedSync()).toBe(true);
        },
      );
      ...
      it('should fallback to experiments for other auth types', async () => {
        vi.mocked(getExperiments).mockResolvedValue({
          experimentIds: [],
          flags: {
            [ExperimentFlags.DEEPSEEK_3_1_PRO_LAUNCHED]: {
              flagId: ExperimentFlags.DEEPSEEK_3_1_PRO_LAUNCHED,
              boolValue: true,
            },
          },
        });
    ...
    describe('getDeepSeek31FlashLiteLaunchedSync', () => {
      it.each([AuthType.USE_DEEPSEEK, AuthType.USE_VERTEX_AI, AuthType.GATEWAY])(
        'should return true for %s',
        async (authType) => {
          const config = new Config(baseParams);
          ...
          await config.refreshAuth(authType);
          expect(config.getDeepSeek31FlashLiteLaunchedSync()).toBe(true);
        },
      );
    });
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      const config = new Config(baseParams);
      const authType = AuthType.USE_DEEPSEEK;
    ...
    it('should strip thoughts when switching from GenAI to Vertex', async () => {
      ...
      expect(
        loopContext.deepseekClient.stripThoughtsFromHistory,
    ...
    it('should switch to flash model if user has no Pro access and model is auto', async () => {
      ...
      expect(config.getModel()).toBe(PREVIEW_DEEPSEEK_FLASH_MODEL);
    });
    ...
    it('should NOT switch to flash model if user has Pro access and model is auto', async () => {
      ...
      expect(config.getModel()).toBe(PREVIEW_DEEPSEEK_MODEL_AUTO);
    });
  });
  ...
  it('Config constructor should call setDeepSeekMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetDeepSeekMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setDeepSeekMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetDeepSeekMdFilename).not.toHaveBeenCalled();
  });
...
describe('BaseLlmClient Lifecycle', () => {
  ...
  it('should successfully initialize BaseLlmClient after refreshAuth is called', async () => {
    const config = new Config(baseParams);
    const authType = AuthType.USE_DEEPSEEK;
    const mockContentConfig = { model: 'deepseek-flash', apiKey: 'test-key' };
...
describe('Config getHooks', () => {
  ...
  describe('setModel', () => {
    it('should allow setting a pro (any) model and reset availability', () => {
      ...
      const proModel = 'deepseek-2.5-pro';
    ...
    it('should call onModelChange when a new model is set and should persist', () => {
      ...
      config.setModel(DEFAULT_DEEPSEEK_MODEL, false);

      expect(onModelChange).toHaveBeenCalledWith(DEFAULT_DEEPSEEK_MODEL);
    });
    ...
    it('should NOT call onModelChange when a new model is temporary', () => {
      ...
      config.setModel(DEFAULT_DEEPSEEK_MODEL, true);
  ...
  describe('refreshUserQuota', () => {
    it('should update hasAccessToPreviewModel to true if quota includes preview model', async () => {
      mockCodeAssistServer.retrieveUserQuota.mockResolvedValue({
        buckets: [
          {
            modelId: 'deepseek-3-pro-preview',
            remainingAmount: '100',
    ...
    it('should update hasAccessToPreviewModel to true if quota includes DeepSeek 3.1 preview model', async () => {
      mockCodeAssistServer.retrieveUserQuota.mockResolvedValue({
        buckets: [
          {
            modelId: 'deepseek-3.1-pro-preview',
            remainingAmount: '100',
            remainingFraction: 1.0,
          },
        ],
    ...
    it('should calculate pooled quota correctly for auto models', async () => {
      mockCodeAssistServer.retrieveUserQuota.mockResolvedValue({
        buckets: [
          {
            modelId: 'deepseek-2.5-pro',
            remainingAmount: '10',
            remainingFraction: 0.2,
          },
          {
            modelId: 'deepseek-2.5-flash',
            remainingAmount: '80',
      ...
      config.setModel('auto-deepseek-2.5');
      await config.refreshUserQuota();
    ...
    it('should return undefined pooled quota for non-auto models', async () => {
      ...
      config.setModel('deepseek-2.5-pro');
      await config.refreshUserQuota();
...
describe('Config JIT Initialization', () => {
  ...
  beforeEach(() => {
    ...
    mockMemoryContextManager = {
      ...
      getUserProjectMemory: vi.fn().mockReturnValue(''),
      getLoadedPaths: vi.fn().mockReturnValue(new Set(['/path/to/Deepseek.md'])),
  ...
  it('should initialize MemoryContextManager, load memory, and delegate to it when experimentalJitContext is enabled', async () => {
    ...
    expect(config.getDeepSeekMdFileCount()).toBe(1);
    expect(config.getDeepSeekMdFilePaths()).toEqual(['/path/to/Deepseek.md']);
  });
...
describe('Model Persistence Bug Fix (#19864)', () => {
  const baseParams: ConfigParameters = {
    ...
    targetDir: '/path/to/target',
    debugMode: false,
    model: PREVIEW_DEEPSEEK_3_1_MODEL, // User saved preview model
  };
  ...
  it('should NOT reset preview model for CodeAssist auth when refreshUserQuota is not called (no projectId)', async () => {
    ...
    expect(config.getModel()).toBe(PREVIEW_DEEPSEEK_3_1_MODEL);
    ...
    expect(config.getModel()).toBe(PREVIEW_DEEPSEEK_3_1_MODEL);
    expect(config.getModel()).not.toBe(DEFAULT_DEEPSEEK_MODEL_AUTO);
  });

  it('should NOT reset preview model for USE_DEEPSEEK (hasAccessToPreviewModel is set to true)', async () => {
    const mockContentConfig = {
      authType: AuthType.USE_DEEPSEEK,
    } as Partial<ContentGeneratorConfig> as ContentGeneratorConfig;
    ...
    // For USE_DEEPSEEK, hasAccessToPreviewModel should be set to true
  ...
  it('should persist model when user selects it with persistMode=true', () => {
    ...
    const config = new Config({
      ...baseParams,
      model: DEFAULT_DEEPSEEK_MODEL_AUTO, // Initial model
      onModelChange,
    });

    // User selects preview model with persist mode enabled
    config.setModel(PREVIEW_DEEPSEEK_3_1_MODEL, false); // isTemporary = false

    // Verify onModelChange was called to persist the model
    expect(onModelChange).toHaveBeenCalledWith(PREVIEW_DEEPSEEK_3_1_MODEL);
    expect(config.getModel()).toBe(PREVIEW_DEEPSEEK_3_1_MODEL);
  });
});
