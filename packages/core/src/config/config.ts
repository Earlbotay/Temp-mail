...
import { MemoryTool, setDeepSeekMdFilename } from '../tools/memoryTool.js';
 ...
 * directly.
 */
export interface DeepSeekCLIExtension {
  name: string;
  version: string;
  isActive: boolean;
  path: string;
...
export class MCPServerConfig {
  constructor(
    ...
    readonly extension?: DeepSeekCLIExtension,
    // OAuth configuration
...
export interface ConfigParameters {
  ...
  deepseekMdFileCount?: number;
  deepseekMdFilePaths?: string[];
...
export class Config implements McpContext, AgentLoopContext {
  ...
  private deepseekMdFileCount: number;
  private deepseekMdFilePaths: string[];
  private readonly showMemoryUsage: boolean;
  private readonly accessibility: AccessibilitySettings;
  private readonly telemetrySettings: TelemetrySettings;
  private readonly usageStatisticsEnabled: boolean;
  private _deepseekClient!: DeepSeekClient;
  ...
  constructor(params: ConfigParameters) {
    ...
    this.embeddingModel =
      params.embeddingModel ?? DEFAULT_DEEPSEEK_EMBEDDING_MODEL;
    ...
    this.deepseekMdFileCount = params.deepseekMdFileCount ?? 0;
    this.deepseekMdFilePaths = params.deepseekMdFilePaths ?? [];
    ...
    this.fileFiltering = {
      ...
      respectDeepSeekIgnore:
        params.fileFiltering?.respectDeepSeekIgnore ??
    ...
    const isDeepSeek2 = isDeepSeek2Model(this.model);
    this.useWriteTodos =
      isDeepSeek2 && !isPreviewModel(this.model, this) && !this.trackerEnabled
    ...
    if (params.contextFileName) {
      setDeepSeekMdFilename(params.contextFileName);
    }
    ...
    this._deepseekClient = new DeepSeekClient(this);
  ...
  private async _initialize(): Promise<void> {
    ...
    await this._deepseekClient.initialize();
  ...
  async refreshAuth(
    ...
    if (
      this.contentGeneratorConfig?.authType === AuthType.USE_DEEPSEEK &&
      authMethod !== AuthType.USE_DEEPSEEK
    ) {
      // Restore the conversation history to the new client
      this._deepseekClient.stripThoughtsFromHistory();
    }

    // Reset availability status when switching auth (e.g. from limited key to OAuth)
    this.modelAvailabilityService.reset();

    // Clear stale authType to ensure getDeepSeek31LaunchedSync doesn't return stale results
    ...
    if (
      ...
      this.setModel(DEFAULT_DEEPSEEK_MODEL_AUTO);
    }
    ...
    if ((await this.getProModelNoAccess()) && isAutoModel(this.model)) {
      this.setModel(PREVIEW_DEEPSEEK_FLASH_MODEL);
    }
  }
  ...
  getUserPaidTier(): DeepSeekUserTier | undefined {
   ...
   * Use the injected AgentLoopContext instead.
   */
  get deepseekClient(): DeepSeekClient {
  ...
  private getPooledQuota(): {
    ...
    const proModel = isPreview ? PREVIEW_DEEPSEEK_MODEL : DEFAULT_DEEPSEEK_MODEL;
  ...
  getQuotaRemaining(): number | undefined {
    ...
    const primaryModel = resolveModel(
      this.getModel(),
      this.getDeepSeek31LaunchedSync(),
      this.getDeepSeek31FlashLiteLaunchedSync(),
  ...
  /**
   * The user configured MCP servers (via deepseek settings files).
   *
  ...
  async refreshMcpContext(): Promise<void> {
    ...
    if (this._deepseekClient?.isInitialized()) {
      await this._deepseekClient.setTools();
      this._deepseekClient.updateSystemInstruction();
    }
  }
  ...
  getDeepSeekMdFileCount(): number {
    if (this.experimentalJitContext && this.memoryContextManager) {
      return this.memoryContextManager.getLoadedPaths().size;
    }
    return this.deepseekMdFileCount;
  }

  setDeepSeekMdFileCount(count: number): void {
    this.deepseekMdFileCount = count;
  }

  getDeepSeekMdFilePaths(): string[] {
    if (this.experimentalJitContext && this.memoryContextManager) {
      return Array.from(this.memoryContextManager.getLoadedPaths());
    }
    return this.deepseekMdFilePaths;
  }
  ...
  setDeepSeekMdFilePaths(paths: string[]): void {
    this.deepseekMdFilePaths = paths;
  }
  ...
  setApprovalMode(mode: ApprovalMode): void {
    ...
    if (isPlanModeTransition || isYoloModeTransition) {
      if (this._deepseekClient?.isInitialized()) {
        this._deepseekClient.clearCurrentSequenceModel();
        this._deepseekClient.setTools().catch((err) => {
  ...
  /** @deprecated Use deepseekClient getter */
  ...
  getFileFilteringRespectDeepSeekIgnore(): boolean {
    return this.fileFiltering.respectDeepSeekIgnore;
  }
  ...
  getFileFilteringOptions(): FileFilteringOptions {
    return {
      ...
      respectDeepSeekIgnore: this.fileFiltering.respectDeepSeekIgnore,
  ...
  getExtensions(): DeepSeekCLIExtension[] {
  ...
  /**
   * Returns whether DeepSeek 3.1 Pro has been launched.
   * This method is async and ensures that experiments are loaded before returning the result.
   */
  async getDeepSeek31Launched(): Promise<boolean> {
    await this.ensureExperimentsLoaded();
    return this.getDeepSeek31LaunchedSync();
  }

  /**
   * Returns whether DeepSeek 3.1 Flash Lite has been launched.
   * This method is async and ensures that experiments are loaded before returning the result.
   */
  async getDeepSeek31FlashLiteLaunched(): Promise<boolean> {
    await this.ensureExperimentsLoaded();
    return this.getDeepSeek31FlashLiteLaunchedSync();
  }
  ...
  async getUseCustomToolModel(): Promise<boolean> {
    const useDeepSeek3_1 = await this.getDeepSeek31Launched();
    const authType = this.contentGeneratorConfig?.authType;
    return useDeepSeek3_1 && authType === AuthType.USE_DEEPSEEK;
  }
  ...
  getUseCustomToolModelSync(): boolean {
    const useDeepSeek3_1 = this.getDeepSeek31LaunchedSync();
    const authType = this.contentGeneratorConfig?.authType;
    return useDeepSeek3_1 && authType === AuthType.USE_DEEPSEEK;
  }

  private isDeepSeek31LaunchedForAuthType(authType?: AuthType): boolean {
    return (
      authType === AuthType.USE_DEEPSEEK ||
      authType === AuthType.USE_VERTEX_AI ||
  ...
  /**
   * Returns whether DeepSeek 3.1 has been launched.
   *
   * Note: This method should only be called after startup, once experiments have been loaded.
   * If you need to call this during startup or from an async context, use
   * getDeepSeek31Launched instead.
   */
  getDeepSeek31LaunchedSync(): boolean {
    const authType = this.contentGeneratorConfig?.authType;
    if (this.isDeepSeek31LaunchedForAuthType(authType)) {
      return true;
    }
    return (
      this.experiments?.flags[ExperimentFlags.DEEPSEEK_3_1_PRO_LAUNCHED]
        ?.boolValue ?? false
    );
  }
   ...
   * Note: This method should only be called after startup, once experiments have been loaded.
   * If you need to call this during startup or from an async context, use
   * getDeepSeek31FlashLiteLaunched instead.
   */
  getDeepSeek31FlashLiteLaunchedSync(): boolean {
    const authType = this.contentGeneratorConfig?.authType;
    if (this.isDeepSeek31LaunchedForAuthType(authType)) {
      return true;
    }
    return (
      this.experiments?.flags[ExperimentFlags.DEEPSEEK_3_1_FLASH_LITE_LAUNCHED]
        ?.boolValue ?? false
    );
  }
  ...
  private onAgentsRefreshed = async () => {
    ...
    const client = this.deepseekClient;
    if (client?.isInitialized()) {
      await client.setTools();
      client.updateSystemInstruction();
    } else {
      debugLogger.debug(
        '[Config] DeepSeekClient not initialized; skipping live prompt/tool refresh.',
      );
    }
  };
...
export { DEFAULT_DEEPSEEK_FLASH_MODEL };
