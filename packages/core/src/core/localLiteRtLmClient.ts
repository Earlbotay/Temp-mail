...
/**
 * A client for making single, non-streaming calls to a local DeepSeek-compatible API
...
export class LocalLiteRtLmClient {
  ...
  /**
   * Sends a prompt to the local DeepSeek model and expects a JSON object in response.
  ...
  async generateJson(
    ...
    const deepseekContents = contents.map((c) => ({
      role: c.role,
      parts: c.parts ? c.parts.map((p) => ({ text: p.text })) : [],
    }));

    if (reminder) {
      const lastContent = deepseekContents.at(-1);
    ...
    try {
      ...
      if (!text) {
        throw new Error(
          'Invalid response from Local DeepSeek API: No text found',
        );
      }
