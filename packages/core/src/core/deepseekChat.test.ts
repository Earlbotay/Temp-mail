...
describe('DeepSeekChat', () => {
  ...
  beforeEach(() => {
    ...
    let currentModel = 'deepseek-pro';
    let currentActiveModel = 'deepseek-pro';
    ...
    mockConfig = {
      ...
      modelConfigService: {
        getResolvedConfig: vi.fn().mockImplementation((modelConfigKey) => {
          ...
          const thinkingConfig = model.startsWith('deepseek-3')
            ? {
    ...
    chat = new DeepSeekChat(mockConfig);
  ...
  describe('constructor', () => {
    it('should initialize lastPromptTokenCount based on history size', () => {
      ...
      const chatWithHistory = new DeepSeekChat(mockConfig, '', [], history);
    ...
    it('should initialize lastPromptTokenCount for empty history', () => {
      const chatEmpty = new DeepSeekChat(mockConfig);
  ...
  describe('setHistory', () => {
    it('should recalculate lastPromptTokenCount when history is updated', () => {
      ...
      const chatWithHistory = new DeepSeekChat(
        mockConfig,
        '',
        [],
        initialHistory,
  ...
  describe('sendMessageStream', () => {
    ...
    it('should use thinkingLevel and remove thinkingBudget for deepseek-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
      ...
      const stream = await chat.sendMessageStream(
        { model: 'deepseek-3-test-only-model-string-for-testing' },
        'hello',
    ...
    it('should use thinkingBudget and remove thinkingLevel for non-deepseek-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
    ...
    it('should flush transcript before tool dispatch for pure tool call with no text or thoughts', async () => {
      ...
      const deepseekMessages = lastWriteData.messages.filter(
        (m) => m.type === 'deepseek',
      );
      expect(deepseekMessages.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('ensureActiveLoopHasThoughtSignatures', () => {
    it('should add thoughtSignature to the first functionCall in each model turn of the active loop', () => {
      const chat = new DeepSeekChat(mockConfig, '', [], []);
      const history: Content[] = [
  ...
  describe('Availability Service Integration', () => {
    ...
    it('should pass attempted model to onPersistent429 callback which calls handleFallback', async () => {
      ...
      // handleFallback is called with the ATTEMPTED model (model-a), not the requested one (deepseek-pro)
      expect(mockHandleFallback).toHaveBeenCalledWith(
        expect.anything(),
        'model-a',
        expect.anything(),
