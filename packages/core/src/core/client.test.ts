...
import {
  ...
  type ServerDeepSeekStreamEvent,
} from './turn.js';
import { getCoreSystemPrompt } from './prompts.js';
import { DEFAULT_DEEPSEEK_MODEL_AUTO } from '../config/models.js';
...
describe('DeepSeek Client (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the DeepSeekClient constructor kicks off an async process (startChat)
    ...
    client = new DeepSeekClient(mockConfig as unknown as AgentLoopContext);
    await client.initialize();
    vi.mocked(mockConfig.getDeepSeekClient).mockReturnValue(client);
    (mockConfig as unknown as { deepseekClient: DeepSeekClient }).deepseekClient =
      client;
  ...
  describe('startChat', () => {
    it('should include environment context when resuming a session', async () => {
      ...
      expect(history[0].parts?.[0]?.text).toContain('This is the DeepSeek CLI');
  ...
  describe('tryCompressChat', () => {
    ...
    function setup({
      ...
      const mockOriginalChat: Partial<DeepSeekChat> = {
      ...
      client['chat'] = mockOriginalChat as DeepSeekChat;
      ...
      const mockNewChat: Partial<DeepSeekChat> = {
      ...
      client['startChat'] = vi
        .fn()
        .mockResolvedValue(mockNewChat as DeepSeekChat);
  ...
  describe('sendMessageStream', () => {
    ...
    it('yields UserCancelled when processTurn throws AbortError', async () => {
      ...
      expect(events).toEqual([{ type: DeepSeekEventType.UserCancelled }]);
    });
...
\`\`\`
    ...
    it('should not add context if ideMode is enabled but no open files', async () => {
      ...
      const mockChat: Partial<DeepSeekChat> = {
        addHistory: vi.fn(),
        setTools: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
        getLastPromptTokenCount: vi.fn(),
      };
      client['chat'] = mockChat as DeepSeekChat;
...
\`\`\`
    ...
    it('should cleanly abort and return Turn on LoopDetected without unhandled promise rejections', async () => {
      ...
      // Assert
      expect(events).toContainEqual({ type: DeepSeekEventType.LoopDetected });
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: DeepSeekEventType.MaxSessionTurns }]);
    ...
    it("should use the sticky model's token limit for the overflow check", async () => {
      // Arrange
      const STICKY_MODEL = 'deepseek-1.5-flash';
    ...
    it('should attempt compression before overflow check and proceed if compression frees space', async () => {
      ...
      // Use the real DeepSeekChat to manage state and token counts more realistically
    ...
    it('should handle massive function responses by truncating them and then yielding overflow warning', async () => {
      ...
      const realChat = new DeepSeekChat(mockConfig, '', [], history);
    ...
    describe('Model Routing', () => {
      ...
      it('should re-route within the same prompt when the configured model changes', async () => {
        ...
        vi.mocked(mockConfig.getModel).mockReturnValue('deepseek-2.5-flash');
        coreEvents.emitModelChanged('deepseek-2.5-flash');
    ...
    it('should recursively call sendMessageStream with "Please continue." when InvalidStream event is received for DeepSeek 2 models', async () => {
      vi.spyOn(client['config'], 'getContinueOnFailedApiCall').mockReturnValue(
        true,
      );
      // Arrange - router must return a DeepSeek 2 model for retry to trigger
      mockRouterService.route.mockResolvedValue({
      ...
      const mockStream2 = (async function* () {
        yield { type: DeepSeekEventType.Content, value: 'Continued content' };
      })();
      ...
      expect(events).toEqual([
        { type: DeepSeekEventType.ModelInfo, value: 'deepseek-2.0-flash' },
        { type: DeepSeekEventType.InvalidStream },
        { type: DeepSeekEventType.Content, value: 'Continued content' },
      ]);
      ...
      expect(mockTurnRunFn).toHaveBeenNthCalledWith(
        1,
        { model: 'deepseek-2.0-flash', isChatModel: true },
        initialRequest,
        expect.any(AbortSignal),
        undefined,
      );
    ...
    it('should not recursively call sendMessageStream with "Please continue." when InvalidStream event is received and flag is false', async () => {
      ...
      // Assert
      expect(events).toEqual([
        { type: DeepSeekEventType.ModelInfo, value: 'default-routed-model' },
        { type: DeepSeekEventType.InvalidStream },
      ]);
    ...
    it('should stop recursing after one retry when InvalidStream events are repeatedly received', async () => {
      ...
      expect(
        events
          .filter((e) => e.type === DeepSeekEventType.ModelInfo)
          .map((e) => e.value),
      ).toEqual(['deepseek-2.0-flash']);
    ...
    describe('IDE context with pending tool calls', () => {
      let mockChat: Partial<DeepSeekChat>;
    ...
    it('should not call checkNextSpeaker when turn.run() yields a value then an error', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: DeepSeekEventType.Content, value: 'some content' };
        yield {
          type: DeepSeekEventType.Error,
          value: { error: { message: 'test error' } },
        };
    ...
    describe('Loop Recovery (Two-Strike)', () => {
      ...
      it('should trigger recovery (Strike 1) and continue', async () => {
        ...
        mockTurnRunFn.mockImplementation(() =>
          (async function* () {
            yield { type: DeepSeekEventType.Content, value: 'First event' };
            yield { type: DeepSeekEventType.Content, value: 'Second event' };
          })(),
        );
      ...
      it('should terminate (Strike 2) after recovery fails', async () => {
        ...
        mockTurnRunFn.mockImplementation(() =>
          (async function* () {
            yield { type: DeepSeekEventType.Content, value: 'Event' };
            yield { type: DeepSeekEventType.Content, value: 'Event' };
          })(),
        );
      ...
      it('should suppress LoopDetected event on Strike 1', async () => {
        ...
        mockTurnRunFn.mockImplementation(() =>
          (async function* () {
            yield { type: DeepSeekEventType.Content, value: 'Event' };
            yield { type: DeepSeekEventType.Content, value: 'Event 2' };
          })(),
        );
    ...
    describe('Hook System', () => {
      ...
      it('should fire BeforeAgent and AfterAgent exactly once for a simple turn', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: DeepSeekEventType.Content, value: 'Hook Response' };
        });
      ...
      it('should fire BeforeAgent once and AfterAgent once even with recursion', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: DeepSeekEventType.Content, value: response };
        });
      ...
      it('should use original request in AfterAgent hook even when continuation happened', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: DeepSeekEventType.Content, value: 'Ok' };
        });
      ...
      it('should stop execution in AfterAgent when hook returns continue: false', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* () {
          yield { type: DeepSeekEventType.Content, value: 'Hello' };
        });
