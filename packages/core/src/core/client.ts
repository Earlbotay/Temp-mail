...
import { partListUnionToString } from './deepseekRequest.js';
import {
...
export class DeepSeekClient {
  ...
  async startChat(
    ...
    try {
      ...
      await reportError(
        error,
        'Error initializing DeepSeek chat session.',
        history,
        'startChat',
      );
  ...
  private _getActiveModelForCurrentTurn(): string {
    ...
    return resolveModel(
      ...
      this.config.getDeepSeek31LaunchedSync?.() ?? false,
      this.config.getDeepSeek31FlashLiteLaunchedSync?.() ?? false,
      false,
  ...
  private async *processTurn(
  ...
  ): AsyncGenerator<ServerDeepSeekStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: DeepSeekEventType.MaxSessionTurns };
      return turn;
    }
    ...
    if (this.config.getContextManagementConfig().enabled) {
      ...
      if (compressed.compressionStatus === CompressionStatus.COMPRESSED) {
        yield { type: DeepSeekEventType.ChatCompressed, value: compressed };
      }
    }
    ...
    // waiting for a response. The DeepSeek API requires that a functionResponse
    ...
    if (loopResult.count > 1) {
      yield { type: DeepSeekEventType.LoopDetected };
      return turn;
    ...
    if (!signal.aborted && !this.currentSequenceModel) {
      yield { type: DeepSeekEventType.ModelInfo, value: modelToUse };
    }
    ...
    for await (const event of resultStream) {
      ...
      if (event.type === DeepSeekEventType.InvalidStream) {
        isInvalidStream = true;
      }
      if (event.type === DeepSeekEventType.Error) {
        isError = true;
      }
    }
  ...
  async *sendMessageStream(
    ...
    if (hooksEnabled && messageBus) {
      ...
      if (hookResult) {
        if (
          'type' in hookResult &&
          hookResult.type === DeepSeekEventType.AgentExecutionStopped
        ) {
          ...
          'type' in hookResult &&
          hookResult.type === DeepSeekEventType.AgentExecutionBlocked
        ) {
          yield hookResult;
          return new Turn(this.getChat(), prompt_id);
