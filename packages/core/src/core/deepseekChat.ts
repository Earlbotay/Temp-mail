...
export class DeepSeekChat {
   ...
   * @example
   * ```ts
   * const chat = ai.chats.create({model: 'deepseek-2.0-flash'});
  ...
  private async makeApiCallAndProcessStream(
    ...
    const apiCall = async () => {
      const useDeepSeek3_1 =
        (await this.context.config.getDeepSeek31Launched?.()) ?? false;
      const useDeepSeek3_1FlashLite =
        (await this.context.config.getDeepSeek31FlashLiteLaunched?.()) ?? false;
   ...
   * This is called by external components when tool calls complete, before sending responses to DeepSeek.
   */
