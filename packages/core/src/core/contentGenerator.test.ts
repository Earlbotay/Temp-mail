...
describe('createContentGenerator', () => {
  ...
  it('should create a GoogleGenAI content generator', async () => {
    ...
    vi.stubEnv('DEEPSEEK_CLI_SURFACE', '');
      ...
      httpOptions: expect.objectContaining({
        headers: expect.objectContaining({
          'User-Agent': expect.stringMatching(
            /DeepSeekCLI\/1\.2\.3\/deepseek-pro \(.*; .*; terminal\)/,
          ),
        }),
      }),
    });
  ...
  it('should use standard User-Agent for a2a-server running outside VS Code', async () => {
    ...
    await createContentGenerator(
      { apiKey: 'test-api-key', authType: AuthType.USE_DEEPSEEK },
      mockConfig,
      undefined,
    );
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /DeepSeekCLI-a2a-server\/1\.2\.3\/deepseek-pro \(.*; .*; terminal\)/,
            ),
          }),
        }),
      }),
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /CloudCodeVSCode\/1\.2\.3 \(aidev_client; os_type=.*; os_version=.*; arch=.*; host_path=VSCode\/1\.85\.0; proxy_client=deepseekcli\)/,
            ),
          }),
        }),
      }),
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /DeepSeekCLI-my-client\/1\.2\.3\/deepseek-pro \(.*; .*; terminal\)/,
            ),
          }),
        }),
      }),
  ...
  it('should allow custom headers to override User-Agent', async () => {
    ...
    vi.stubEnv('DEEPSEEK_CLI_CUSTOM_HEADERS', 'User-Agent:MyCustomUA');
  ...
  it('should include custom headers from DEEPSEEK_CLI_CUSTOM_HEADERS for Code Assist requests', async () => {
    const mockGenerator = {} as unknown as ContentGenerator;
    vi.mocked(createCodeAssistContentGenerator).mockResolvedValue(
      mockGenerator as never,
    );
    vi.stubEnv(
      'DEEPSEEK_CLI_CUSTOM_HEADERS',
  ...
  it('should include custom headers from DEEPSEEK_CLI_CUSTOM_HEADERS for GoogleGenAI requests without inferring auth mechanism', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('deepseek-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
  ...
  it('should pass api key as Authorization Header when DEEPSEEK_API_KEY_AUTH_MECHANISM is set to bearer', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('deepseek-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('DEEPSEEK_API_KEY_AUTH_MECHANISM', 'bearer');
  ...
  it('should not pass api key as Authorization Header when DEEPSEEK_API_KEY_AUTH_MECHANISM is not set (default behavior)', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('deepseek-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    // DEEPSEEK_API_KEY_AUTH_MECHANISM is not stubbed, so it will be undefined, triggering default 'x-goog-api-key'
...
describe('createContentGeneratorConfig', () => {
  ...
  it('should configure for DeepSeek using DEEPSEEK_API_KEY when set', async () => {
  ...
  it('should not configure for DeepSeek if DEEPSEEK_API_KEY is not set and storage is empty', async () => {
    vi.stubEnv('DEEPSEEK_API_KEY', '');
  ...
  it('should configure for GATEWAY using dummy placeholder if DEEPSEEK_API_KEY is set', async () => {
    vi.stubEnv('DEEPSEEK_API_KEY', 'env-deepseek-key');
    const config = await createContentGeneratorConfig(
      mockConfig,
      AuthType.GATEWAY,
  ...
  it('should configure for GATEWAY using dummy placeholder if DEEPSEEK_API_KEY is not set', async () => {
    vi.stubEnv('DEEPSEEK_API_KEY', '');
    vi.mocked(loadApiKey).mockResolvedValue(null);
    const config = await createContentGeneratorConfig(
      mockConfig,
