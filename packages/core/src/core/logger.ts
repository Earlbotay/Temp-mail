...
export class Logger {
  private deepseekDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.deepseekDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.deepseekDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.deepseekDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.deepseekDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.deepseekDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.deepseekDir) {
      debugLogger.error(
