...
describe('tokenLimit', () => {
  it('should return the correct token limit for default models', () => {
    expect(tokenLimit(DEFAULT_DEEPSEEK_MODEL)).toBe(1_048_576);
    expect(tokenLimit(DEFAULT_DEEPSEEK_FLASH_MODEL)).toBe(1_048_576);
    expect(tokenLimit(DEFAULT_DEEPSEEK_FLASH_LITE_MODEL)).toBe(1_048_576);
  });

  it('should return the correct token limit for preview models', () => {
    expect(tokenLimit(PREVIEW_DEEPSEEK_MODEL)).toBe(1_048_576);
    expect(tokenLimit(PREVIEW_DEEPSEEK_FLASH_MODEL)).toBe(1_048_576);
  });
