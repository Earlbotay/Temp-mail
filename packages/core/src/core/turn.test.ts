...
import {
  Turn,
  DeepSeekEventType,
  type ServerDeepSeekToolCallRequestEvent,
  type ServerDeepSeekErrorEvent,
} from './turn.js';
...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as DeepSeekChat, 'prompt-id-1');
  ...
  describe('run', () => {
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerDeepSeekToolCallRequestEvent;
      expect(event1.type).toBe(DeepSeekEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerDeepSeekToolCallRequestEvent;
      expect(event2.type).toBe(DeepSeekEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: DeepSeekEventType.Content, value: 'First part' },
    ...
    it('should yield InvalidStream event if sendMessageStream throws InvalidStreamError', async () => {
      ...
      expect(events).toEqual([{ type: DeepSeekEventType.InvalidStream }]);
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerDeepSeekErrorEvent;
      expect(errorEvent.type).toBe(DeepSeekEventType.Error);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerDeepSeekToolCallRequestEvent;
    ...
    it.each([
      ...
      expect(events).toEqual([
        { type: DeepSeekEventType.Content, value: contentText },
        {
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: DeepSeekEventType.Content, value: 'First part' },
        { type: DeepSeekEventType.Content, value: 'Second part' },
        {
          type: DeepSeekEventType.Finished,
    ...
    it('should yield citation and finished events when response has citationMetadata', async () => {
      ...
      expect(events).toEqual([
        { type: DeepSeekEventType.Content, value: 'Some text.' },
        {
    ...
    it('should not yield citation event if there is no finish reason', async () => {
      ...
      expect(events.some((e) => e.type === DeepSeekEventType.Citation)).toBe(
        false,
      );
    });
    ...
    it('should yield a Retry event when it receives one from the chat stream', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: DeepSeekEventType.Content, value: 'Success' },
      ]);
    });
    ...
    it('should process all parts when thought is first part in chunk', async () => {
      ...
      const thoughtEvent = events.find(
        (e) => e.type === DeepSeekEventType.Thought,
      );
      ...
      const contentEvent = events.find(
        (e) => e.type === DeepSeekEventType.Content,
      );
      ...
      const toolCallEvent = events.find(
        (e) => e.type === DeepSeekEventType.ToolCallRequest,
      );
      ...
      const citationEvent = events.find(
        (e) => e.type === DeepSeekEventType.Citation,
      );
      ...
      const finishedEvent = events.find(
        (e) => e.type === DeepSeekEventType.Finished,
      );
