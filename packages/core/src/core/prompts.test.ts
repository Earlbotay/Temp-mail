...
import { DEEPSEEK_DIR } from '../utils/paths.js';
...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('DEEPSEEK_SYSTEM_MD', undefined);
    vi.stubEnv('DEEPSEEK_WRITE_SYSTEM_MD', undefined);
    const mockRegistry = {
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempTrackerDir: vi
          .fn()
          .mockReturnValue('/mock/.deepseek/tmp/session/tracker'),
      },
      ...
      getActiveModel: vi.fn().mockReturnValue(DEFAULT_DEEPSEEK_MODEL),
      getMessageBus: vi.fn(),
  ...
  it('should include available_skills with updated verbiage for preview models', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_DEEPSEEK_MODEL);
    const skills = [
      {
        name: 'test-skill',
        description: 'A test skill description',
  ...
  it('should use chatty system prompt for preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_DEEPSEEK_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain('You are DeepSeek CLI, an interactive CLI agent'); // Check for core content
    expect(prompt).toContain('- **User Hints:**');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('# Contextual Instructions (Deepseek.md)');
    expect(prompt).toContain('<loaded_context>');
    expect(prompt).toContain(memory);
    expect(prompt).toContain('You are DeepSeek CLI, an interactive CLI agent'); // Ensure base prompt follows
  ...
  it.each([
    ...
    [
      undefined,
      'You are DeepSeek CLI, an interactive CLI agent',
  ...
  it.each([
    ...
    (toolNames, expectCodebaseInvestigator) => {
      ...
      const testConfig = {
        ...
        getActiveModel: vi.fn().mockReturnValue(PREVIEW_DEEPSEEK_MODEL),
        getPreviewFeatures: vi.fn().mockReturnValue(true),
        getAgentRegistry: vi.fn().mockReturnValue({
          getDirectoryContext: vi.fn().mockReturnValue('Mock Agent Directory'),
          getAllDefinitions: vi.fn().mockReturnValue([]),
  ...
  describe('DEEPSEEK_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should use default prompt when DEEPSEEK_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('DEEPSEEK_SYSTEM_MD', value);
        const prompt = getCoreSystemPrompt(mockConfig);
    ...
    it('should throw error if DEEPSEEK_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('DEEPSEEK_SYSTEM_MD', customPath);
    ...
    it.each(['true', '1'])(
      'should read from default path when DEEPSEEK_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(DEEPSEEK_DIR, 'system.md'));
        vi.stubEnv('DEEPSEEK_SYSTEM_MD', value);
    ...
    it('should read from custom path when DEEPSEEK_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when DEEPSEEK_SYSTEM_MD is set', () => {
  ...
  describe('DEEPSEEK_WRITE_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should not write to file when DEEPSEEK_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('DEEPSEEK_WRITE_SYSTEM_MD', value);
    ...
    it.each(['true', '1'])(
      'should write to default path when DEEPSEEK_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(DEEPSEEK_DIR, 'system.md'));
        vi.stubEnv('DEEPSEEK_WRITE_SYSTEM_MD', value);
        getCoreSystemPrompt(mockConfig);
    ...
    it('should write to custom path when DEEPSEEK_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('DEEPSEEK_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt(mockConfig);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it.each([
      ...
      'should expand tilde in custom path when DEEPSEEK_WRITE_SYSTEM_MD is "%s"',
