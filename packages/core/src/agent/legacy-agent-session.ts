...
/**
 * @fileoverview LegacyAgentSession backed by the existing DeepSeek client +
...
export class LegacyAgentProtocol implements AgentProtocol {
  ...
  private readonly _client: DeepSeekClient;
  ...
  constructor(deps: LegacyAgentSessionDeps) {
    ...
    this._client = deps.client ?? deps.config.getDeepSeekClient();
  ...
  async send(payload: AgentSend): Promise<{ streamId: string }> {
    ...
    const parts = contentPartsToDeepSeekParts(message.content);
  ...
  private async _runLoop(
    ...
    while (true) {
      ...
      for await (const event of responseStream) {
        ...
        if (event.type === DeepSeekEventType.ToolCallRequest) {
      ...
      for (const tc of completedToolCalls) {
        ...
        const content: ContentPart[] = response.error
          ...
          : deepseekPartsToContentParts(response.responseParts);
