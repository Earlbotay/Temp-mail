 ...
 * ServerDeepSeekStreamEvent objects into AgentEvent objects.
 *
...
/**
 * Translates a single ServerDeepSeekStreamEvent into zero or more AgentEvents.
 * Mutates `state` (counter, flags) as a side effect.
 */
export function translateEvent(
  event: ServerDeepSeekStreamEvent,
  state: TranslationState,
): AgentEvent[] {
  const out: AgentEvent[] = [];

  switch (event.type) {
    case DeepSeekEventType.ModelInfo:
      state.model = event.value;
      ensureStreamStart(state, out);
      out.push(makeEvent('session_update', state, { model: event.value }));
      break;

    case DeepSeekEventType.Content:
      ensureStreamStart(state, out);
      out.push(
    ...
    case DeepSeekEventType.Thought:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('message', state, {
          role: 'agent',
    ...
    case DeepSeekEventType.Citation:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('message', state, {
          role: 'agent',
    ...
    case DeepSeekEventType.Finished:
      handleFinished(event.value, state, out);
      break;

    case DeepSeekEventType.Error:
      handleError(event.value.error, state, out);
      break;

    case DeepSeekEventType.UserCancelled:
      ensureStreamStart(state, out);
      out.push(
    ...
    case DeepSeekEventType.MaxSessionTurns:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('agent_end', state, {
          reason: 'max_turns',
    ...
    case DeepSeekEventType.LoopDetected:
      ensureStreamStart(state, out);
      out.push(
    ...
    case DeepSeekEventType.ContextWindowWillOverflow:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('error', state, {
    ...
    case DeepSeekEventType.AgentExecutionStopped:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('agent_end', state, {
          reason: 'completed',
    ...
    case DeepSeekEventType.AgentExecutionBlocked:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('error', state, {
    ...
    case DeepSeekEventType.InvalidStream:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('error', state, {
          status: 'INTERNAL',
    ...
    case DeepSeekEventType.ToolCallRequest:
      ensureStreamStart(state, out);
    ...
    case DeepSeekEventType.ToolCallResponse: {
      ensureStreamStart(state, out);
      ...
      out.push(
        makeEvent('tool_response', state, {
          ...
          content: event.value.error
            ...
            : deepseekPartsToContentParts(event.value.responseParts),
    ...
    case DeepSeekEventType.ToolCallConfirmation:
      // Elicitations are handled separately by the session layer
      break;

    // Internal concerns — no AgentEvent emitted
    case DeepSeekEventType.ChatCompressed:
    case DeepSeekEventType.Retry:
      break;
...
/**
 * Maps a DeepSeek FinishReason to an AgentEnd reason.
 */
...
/**
 * Maps DeepSeek usageMetadata to Usage.
 */
