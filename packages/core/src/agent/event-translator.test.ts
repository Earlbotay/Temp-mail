...
describe('translateEvent', () => {
  ...
  describe('Content events', () => {
    it('emits agent_start + message for first content event', () => {
      const event: ServerDeepSeekStreamEvent = {
  ...
  describe('ModelInfo events', () => {
    it('emits agent_start and session_update when no stream started yet', () => {
      ...
      expect(sessionUpdate.model).toBe('deepseek-2.5-pro');
      expect(state.model).toBe('deepseek-2.5-pro');
  ...
  describe('Finished events', () => {
    it('emits usage for STOP', () => {
      ...
      expect(usage.model).toBe('deepseek-2.5-pro');
  ...
  describe('Events with no output', () => {
    it('returns empty for Retry', () => {
      const result = translateEvent({ type: DeepSeekEventType.Retry }, state);
      expect(result).toEqual([]);
    });

    it('returns empty for ChatCompressed with null', () => {
      const result = translateEvent(
        { type: DeepSeekEventType.ChatCompressed, value: null },
        state,
      );
      expect(result).toEqual([]);
    });
  ...
  describe('Event IDs', () => {
    it('generates sequential IDs', () => {
      ...
      const e1 = translateEvent(
        { type: DeepSeekEventType.Content, value: 'a' },
        state,
      );
      const e2 = translateEvent(
        { type: DeepSeekEventType.Content, value: 'b' },
        state,
      );
