...
export const DEFAULT_CONTEXT_FILENAME = 'Deepseek.md';
export const MEMORY_SECTION_HEADER = '## DeepSeek Added Memories';

// This variable will hold the currently configured filename for Deepseek.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setDeepSeekMdFilename.
let currentDeepSeekMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setDeepSeekMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentDeepSeekMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentDeepSeekMdFilename = newFilename.trim();
  }
}

export function getCurrentDeepSeekMdFilename(): string {
  if (Array.isArray(currentDeepSeekMdFilename)) {
    return currentDeepSeekMdFilename[0];
  }
  return currentDeepSeekMdFilename;
}

export function getAllDeepSeekMdFilenames(): string[] {
  if (Array.isArray(currentDeepSeekMdFilename)) {
    return currentDeepSeekMdFilename;
  }
  return [currentDeepSeekMdFilename];
...
export function getGlobalMemoryFilePath(): string {
  return path.join(Storage.getGlobalDeepSeekDir(), getCurrentDeepSeekMdFilename());
}

export function getProjectMemoryFilePath(storage: Storage): string {
  return path.join(storage.getProjectMemoryDir(), getCurrentDeepSeekMdFilename());
}
