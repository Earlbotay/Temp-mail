...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find DeepSeek CLI documentation directory.');
}
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available DeepSeek CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to DeepSeek CLI's internal documentation.
