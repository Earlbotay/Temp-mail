...
describe('ReadFileTool', () => {
  ...
  describe('execute', () => {
    ...
    describe('with .deepseekignore', () => {
      ...
      it('should throw error if path is ignored by a .deepseekignore pattern', async () => {
      ...
      it('should allow reading ignored files if respectDeepSeekIgnore is false', async () => {
        const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
        await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
  ...
  describe('getSchema', () => {
    ...
    it('should return the schema from the resolver when modelId is provided', () => {
      const modelId = 'deepseek-2.0-flash';
      const schema = tool.getSchema(modelId);
      expect(schema.name).toBe(ReadFileTool.Name);
      expect(schema.description).toMatchSnapshot();
    });

    it('should return the DeepSeek 3 schema when a DeepSeek 3 modelId is provided', () => {
      const modelId = 'deepseek-3-pro-preview';
      const schema = tool.getSchema(modelId);
      expect(schema.name).toBe(ReadFileTool.Name);
      expect(schema.description).toMatchSnapshot();
