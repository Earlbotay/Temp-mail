...
describe('RipGrepTool', () => {
  ...
  let mockConfig = {
    ...
    getFileFilteringRespectDeepSeekIgnore: () => true,
  ...
  describe('execute', () => {
    ...
    it('should filter out files based on FileDiscoveryService even if ripgrep returns them', async () => {
      // Create .deepseekignore to ignore 'ignored.txt'
      await fs.writeFile(
        path.join(tempRootDir, DEEPSEEK_IGNORE_FILE_NAME),
        'ignored.txt',
      );

      // Re-initialize tool so FileDiscoveryService loads the new .deepseekignore
  ...
  describe('advanced search options', () => {
    ...
    it('should add .deepseekignore when enabled and patterns exist', async () => {
      const deepseekIgnorePath = path.join(tempRootDir, DEEPSEEK_IGNORE_FILE_NAME);
      await fs.writeFile(deepseekIgnorePath, 'ignored.log');
      const configWithDeepSeekIgnore = {
        getTargetDir: () => tempRootDir,
        getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir),
        getDebugMode: () => false,
        getFileFilteringRespectGitIgnore: () => true,
      ...
      const deepseekIgnoreTool = new RipGrepTool(
        configWithDeepSeekIgnore,
        createMockMessageBus(),
      );
      ...
      const params: RipGrepToolParams = { pattern: 'secret' };
      const invocation = deepseekIgnoreTool.build(params);
      await invocation.execute(abortSignal);

      expect(mockSpawn).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.arrayContaining(['--ignore-file', deepseekIgnorePath]),
        expect.anything(),
      );
    });

    it('should skip .deepseekignore when disabled', async () => {
      const deepseekIgnorePath = path.join(tempRootDir, DEEPSEEK_IGNORE_FILE_NAME);
      await fs.writeFile(deepseekIgnorePath, 'ignored.log');
      const configWithoutDeepSeekIgnore = {
        getTargetDir: () => tempRootDir,
        getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir),
        getDebugMode: () => false,
        getFileFilteringRespectGitIgnore: () => true,
        getFileFilteringRespectDeepSeekIgnore: () => false,
        getFileFilteringOptions: () => ({
          respectGitIgnore: true,
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.not.arrayContaining(['--ignore-file', deepseekIgnorePath]),
        expect.anything(),
      );
    });
