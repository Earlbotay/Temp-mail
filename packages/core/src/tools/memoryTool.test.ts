...
const MEMORY_SECTION_HEADER = '## DeepSeek Added Memories';
...
describe('MemoryTool', () => {
  ...
  describe('setDeepSeekMdFilename', () => {
    it('should update currentDeepSeekMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setDeepSeekMdFilename(newName);
      expect(getCurrentDeepSeekMdFilename()).toBe(newName);
    });

    it('should not update currentDeepSeekMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentDeepSeekMdFilename();
      setDeepSeekMdFilename('  ');
      expect(getCurrentDeepSeekMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setDeepSeekMdFilename(newNames);
      expect(getCurrentDeepSeekMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllDeepSeekMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', DEEPSEEK_DIR, 'Deepseek.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(
          path.join('mock', 'home', DEEPSEEK_DIR),
        );
        expect(result.fileName).toContain('Deepseek.md');
        expect(result.fileDiff).toContain('Index: Deepseek.md');
        expect(result.fileDiff).toContain('+## DeepSeek Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## DeepSeek Added Memories');
