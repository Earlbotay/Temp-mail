...
import { partListUnionToString } from '../core/deepseekRequest.js';
...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .deepseekignore files by default', async () => {
      await fs.writeFile(
        path.join(tempRootDir, DEEPSEEK_IGNORE_FILE_NAME),
        'deepseek-ignored_test.txt',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'deepseek-ignored_test.txt'),
        'content',
      );
      await fs.writeFile(path.join(tempRootDir, 'visible_test.txt'), 'content');
      ...
      expect(result.llmContent).toContain('Found 1 file(s)');
      expect(result.llmContent).toContain('visible_test.txt');
      expect(result.llmContent).not.toContain('deepseek-ignored_test.txt');
    }, 30000);
    ...
    it('should not respect .deepseekignore when respect_deepseek_ignore is false', async () => {
      await fs.writeFile(
        path.join(tempRootDir, DEEPSEEK_IGNORE_FILE_NAME),
        'deepseek-ignored_test.txt',
      );
      ...
      const params: GlobToolParams = {
        pattern: 'deepseek-ignored_test.txt',
        respect_deepseek_ignore: false,
      };
      const invocation = globTool.build(params);
      const result = await invocation.execute(abortSignal);

      expect(result.llmContent).toContain('Found 1 file(s)');
      expect(result.llmContent).toContain('deepseek-ignored_test.txt');
    }, 30000);
  });
});
