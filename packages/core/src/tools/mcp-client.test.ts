...
describe('mcp-client', () => {
  ...
  beforeEach(() => {
    ...
    testWorkspace = fs.mkdtempSync(
      path.join(os.tmpdir(), 'deepseek-agent-test-'),
    );
  ...
  describe('createTransport', () => {
    ...
    it('sets an env variable DEEPSEEK_CLI=1 for stdio MCP servers', async () => {
      const mockedTransport = vi
        .spyOn(SdkClientStdioLib, 'StdioClientTransport')
        .mockReturnValue({} as SdkClientStdioLib.StdioClientTransport);
      ...
      expect(callArgs.env!['DEEPSEEK_CLI']).toBe('1');
    });
    ...
    it('should exclude extension settings with undefined values from environment', async () => {
      ...
      const callArgs = mockedTransport.mock.calls[0][0];
      expect(callArgs.env).toBeDefined();
      expect(callArgs.env!['DEEPSEEK_CLI_EXT_VAR']).toBeUndefined();
    });
    ...
    it('should include extension settings with defined values in environment', async () => {
      ...
      const callArgs = mockedTransport.mock.calls[0][0];
      expect(callArgs.env).toBeDefined();
      expect(callArgs.env!['DEEPSEEK_CLI_EXT_VAR']).toBe('defined-value');
    });
    ...
    it('should resolve environment variables in mcpServerConfig.env using extension settings', async () => {
      ...
      await createTransport(
        'test-server',
        {
          command: 'test-command',
          env: {
            RESOLVED_VAR: '$DEEPSEEK_CLI_EXT_VAR',
          },
          extension: {
            name: 'test-ext',
            resolvedSettings: [
      ...
      const callArgs = mockedTransport.mock.calls[0][0];
      expect(callArgs.env).toBeDefined();
      expect(callArgs.env!['DEEPSEEK_CLI_EXT_VAR']).toBe('ext-value');
    ...
    it('should expand environment variables in mcpServerConfig.env and not redact them', async () => {
      ...
      process.env = {
        ...originalEnv,
        DEEPSEEK_TEST_VAR: 'expanded-value',
      };
      ...
      try {
        await createTransport(
          'test-server',
          {
            command: 'test-command',
            env: {
              TEST_EXPANDED: 'Value is $DEEPSEEK_TEST_VAR',
