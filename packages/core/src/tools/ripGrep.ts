 ...
 *
 * NOTE:
 * - The DeepSeek CLI currently prefers a managed ripgrep binary downloaded
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    if (!no_ignore) {
      ...
      // Add .deepseekignore and custom ignore files support (if provided/mandated)
      // (ripgrep natively handles .gitignore)
      const deepseekIgnorePaths = this.fileDiscoveryService.getIgnoreFilePaths();
      for (const ignorePath of deepseekIgnorePaths) {
