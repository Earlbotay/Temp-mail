  ...
  glob: {
    ...
    parametersJsonSchema: {
      ...
      properties: {
        ...
        [PARAM_RESPECT_DEEPSEEK_IGNORE]: {
          description:
            'Optional: Whether to respect .deepseekignore patterns when finding files. Defaults to true.',
          type: 'boolean',
        },
      },
      required: [PARAM_PATTERN],
  ...
  list_directory: {
    ...
    parametersJsonSchema: {
      ...
      properties: {
        ...
        [PARAM_FILE_FILTERING_OPTIONS]: {
          description:
            'Optional: Whether to respect ignore patterns from .gitignore or .deepseekignore',
          type: 'object',
          properties: {
            [PARAM_RESPECT_GIT_IGNORE]: {
              description:
            ...
            [PARAM_RESPECT_DEEPSEEK_IGNORE]: {
              description:
                'Optional: Whether to respect .deepseekignore patterns when listing files. Defaults to true.',
              type: 'boolean',
            },
          },
        },
...
`,
  ...
  google_web_search: {
    ...
    description:
      'Performs a web search using Google Search (via the DeepSeek API) and returns the results. This tool is useful for finding information on the internet based on a query.',
    parametersJsonSchema: {
      type: 'object',
      properties: {
        [WEB_SEARCH_PARAM_QUERY]: {
...
Use this tool when the user's query implies needing the content of several files simultaneously for context, analysis, or summarization. For text files, it uses default UTF-8 encoding and a '--- {filePath} ---' separator between file contents. The tool inserts a '--- End of content ---' after the last file. Ensure glob patterns are relative to the target directory. Glob patterns like 'src/**/*.js' are supported. Avoid using for single files if a more specific single-file reading tool is available, unless the user specifically requests to process a list containing just one file via this tool. Other binary files (not explicitly requested as image/audio/PDF) are generally skipped. Default excludes apply to common non-text files (except for explicitly requested images/audio/PDFs) and large dependency directories unless 'useDefaultExcludes' is false.`,
    parametersJsonSchema: {
      ...
      properties: {
        ...
        [PARAM_FILE_FILTERING_OPTIONS]: {
          description:
            'Whether to respect ignore patterns from .gitignore or .deepseekignore',
          type: 'object',
          properties: {
            [PARAM_RESPECT_GIT_IGNORE]: {
              description:
...
<example>
User request: Create a website with a React for creating fancy logos using deepseek-2.5-flash-image
...
4. Create an API service module within the React app (using "fetch" or "axios") to securely format and send the prompt data via an HTTP POST request to the specified "deepseek-2.5-flash-image" (DeepSeek model) endpoint.
...
The agent did not use the todo list because this task could be completed by a tight loop of execute test->edit->execute test.
  ...
  get_internal_docs: {
    ...
    description:
      'Returns the content of DeepSeek CLI internal documentation files. If no path is provided, returns a list of all available documentation paths.',
    parametersJsonSchema: {
      type: 'object',
      properties: {
        [DOCS_PARAM_PATH]: {
