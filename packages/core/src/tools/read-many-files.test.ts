...
describe('ReadManyFilesTool', () => {
  ...
  beforeEach(async () => {
    ...
    fs.writeFileSync(path.join(tempRootDir, DEEPSEEK_IGNORE_FILE_NAME), 'foo.*');
  ...
  describe('execute', () => {
    ...
    it('should return error if path is ignored by a .deepseekignore pattern', async () => {
      createFile('foo.bar', '');
      createFile('bar.ts', '');
      createFile('foo.quux', '');
...
`,
  ...
  describe('JIT context discovery', () => {
    ...
    it('should discover JIT context sequentially to avoid duplicate shared parent context', async () => {
      ...
      // Simulate two subdirectories sharing a parent Deepseek.md.
