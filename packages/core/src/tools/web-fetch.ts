...
const USER_AGENT =
  'Mozilla/5.0 (compatible; Google-DeepSeek-CLI/1.0; +https://github.com/google-deepseek/deepseek-cli)';
...
> {
  ...
  private async executeFallback(
    ...
    try {
      const deepseekClient = this.context.deepseekClient;
...
`;
      const result = await deepseekClient.generateContent(
...
`;
      const response = await deepseekClient.generateContent(
        { model: 'web-fetch' },
