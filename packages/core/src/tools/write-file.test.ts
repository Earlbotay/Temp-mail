...
const rootDir = path.resolve(os.tmpdir(), 'deepseek-cli-test-root');
const plansDir = path.resolve(os.tmpdir(), 'deepseek-cli-test-plans');
...
describe('WriteFileTool', () => {
  ...
  describe('getCorrectedFileContent', () => {
    ...
    it('should set aggressiveUnescape to false for deepseek-3 models', async () => {
      const filePath = path.join(rootDir, 'deepseek3_file.txt');
      const proposedContent = 'Proposed new content.';
      const abortSignal = new AbortController().signal;

      const mockDeepSeek3Config = {
        // eslint-disable-next-line @typescript-eslint/no-misused-spread
        ...mockConfig,
        getActiveModel: () => 'deepseek-3.0-pro',
      } as unknown as Config;
