...
describe('memory commands', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      getUserMemory: vi.fn(),
      getDeepSeekMdFileCount: vi.fn(),
      getDeepSeekMdFilePaths: vi.fn(),
  ...
  describe('showMemory', () => {
    it('should show memory content if it exists', () => {
      ...
      vi.mocked(mockConfig.getDeepSeekMdFileCount).mockReturnValue(1);
    ...
    it('should show a message if memory is empty', () => {
      vi.mocked(mockConfig.getUserMemory).mockReturnValue('');
      vi.mocked(mockConfig.getDeepSeekMdFileCount).mockReturnValue(0);
  ...
  describe('listMemoryFiles', () => {
    it('should list the memory files in use', () => {
      const filePaths = ['/path/to/Deepseek.md', '/other/path/Deepseek.md'];
      vi.mocked(mockConfig.getDeepSeekMdFilePaths).mockReturnValue(filePaths);
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toContain(
          'There are 2 Deepseek.md file(s) in use:',
        );
    ...
    it('should show a message if no memory files are in use', () => {
      vi.mocked(mockConfig.getDeepSeekMdFilePaths).mockReturnValue([]);
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toBe('No Deepseek.md files in use.');
      }
    });
  ...
  describe('moveInboxSkill', () => {
    ...
    it('should move a skill to global skills directory', async () => {
      ...
      expect(result.message).toBe('Moved "my-skill" to ~/.deepseek/skills.');
