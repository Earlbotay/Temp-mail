...
export function logConsecaPolicyGeneration(
  ...
  if (clearcutLogger) {
    const data = [
      {
        deepseek_cli_key: EventMetadataKey.CONSECA_USER_PROMPT,
        value: safeJsonStringify(event.user_prompt),
      },
      {
        deepseek_cli_key: EventMetadataKey.CONSECA_TRUSTED_CONTENT,
        value: safeJsonStringify(event.trusted_content),
      },
      {
        deepseek_cli_key: EventMetadataKey.CONSECA_GENERATED_POLICY,
    ...
    if (event.error) {
      data.push({
        deepseek_cli_key: EventMetadataKey.CONSECA_ERROR,
        value: event.error,
      });
    }
...
export function logConsecaVerdict(
  ...
  if (clearcutLogger) {
    const data = [
      ...
      {
        deepseek_cli_key: EventMetadataKey.CONSECA_VERDICT_RESULT,
        value: safeJsonStringify(event.verdict),
      },
      {
        deepseek_cli_key: EventMetadataKey.CONSECA_VERDICT_RATIONALE,
