...
describe('telemetry/config helpers', () => {
  ...
  describe('resolveTelemetrySettings', () => {
    ...
    it('uses env over settings and argv over env', async () => {
      ...
      const env = {
        DEEPSEEK_TELEMETRY_ENABLED: '1',
        DEEPSEEK_TELEMETRY_TARGET: 'gcp',
        DEEPSEEK_TELEMETRY_OTLP_ENDPOINT: 'http://env:4317',
        DEEPSEEK_TELEMETRY_OTLP_PROTOCOL: 'http',
        DEEPSEEK_TELEMETRY_LOG_PROMPTS: 'true',
        DEEPSEEK_TELEMETRY_OUTFILE: 'env.log',
        DEEPSEEK_TELEMETRY_USE_COLLECTOR: 'true',
    ...
    it('resolves useCliAuth from env', async () => {
      const env = {
        DEEPSEEK_TELEMETRY_USE_CLI_AUTH: 'true',
      };
    ...
    it('falls back to OTEL_EXPORTER_OTLP_ENDPOINT when DEEPSEEK var is missing', async () => {
      const settings = {};
      const env = {
    ...
    it('throws on unknown protocol values', async () => {
      const env = { DEEPSEEK_TELEMETRY_OTLP_PROTOCOL: 'unknown' } as Record<
        string,
        string
      >;
    ...
    it('throws on unknown target values', async () => {
      const env = { DEEPSEEK_TELEMETRY_TARGET: 'unknown' } as Record<
        string,
        string
      >;
      await expect(resolveTelemetrySettings({ env })).rejects.toThrow(
