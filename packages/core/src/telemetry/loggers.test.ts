...
describe('loggers', () => {
  ...
  describe('logApiRequest', () => {
    ...
    it('should log an API request with semantic details, but without prompts when logPrompts is disabled', () => {
      ...
      expect(semanticLogCall.body).toBe(
        'GenAI operation request details from deepseek-1.0-pro.',
      );
      ...
      expect(attributes['gen_ai.request.model']).toBe('deepseek-1.0-pro');
  ...
  describe('logModelRouting', () => {
    ...
    it('should log the event to Clearcut and OTEL, and record metrics', () => {
      ...
      expect(mockLogger.emit).toHaveBeenCalledWith({
        body: 'Model routing decision. Model: deepseek-pro, Source: default',
        attributes: {
          'session.id': 'test-session-id',
          'user.email': 'test-user@example.com',
          'installation.id': 'test-installation-id',
    ...
    it('should log the event with numerical routing fields', () => {
      ...
      expect(mockLogger.emit).toHaveBeenCalledWith({
        body: 'Model routing decision. Model: deepseek-pro, Source: NumericalClassifier (Strict)',
        attributes: {
          'session.id': 'test-session-id',
          'user.email': 'test-user@example.com',
          'installation.id': 'test-installation-id',
