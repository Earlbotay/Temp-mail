...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ??
    parseBooleanEnvFlag(env['DEEPSEEK_TELEMETRY_ENABLED']) ??
    settings.enabled;

  const rawTarget =
    argv.telemetryTarget ??
    env['DEEPSEEK_TELEMETRY_TARGET'] ??
  ...
  const otlpEndpoint =
    ...
    env['DEEPSEEK_TELEMETRY_OTLP_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
    settings.otlpEndpoint;

  const rawProtocol =
    argv.telemetryOtlpProtocol ??
    env['DEEPSEEK_TELEMETRY_OTLP_PROTOCOL'] ??
  ...
  const logPrompts =
    ...
    parseBooleanEnvFlag(env['DEEPSEEK_TELEMETRY_LOG_PROMPTS']) ??
    settings.logPrompts;

  const outfile =
    argv.telemetryOutfile ??
    env['DEEPSEEK_TELEMETRY_OUTFILE'] ??
    settings.outfile;

  const useCollector =
    parseBooleanEnvFlag(env['DEEPSEEK_TELEMETRY_USE_COLLECTOR']) ??
  ...
  return {
    ...
    useCollector,
    useCliAuth:
      parseBooleanEnvFlag(env['DEEPSEEK_TELEMETRY_USE_CLI_AUTH']) ??
      settings.useCliAuth,
  };
}
