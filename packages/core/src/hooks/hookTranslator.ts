...
/**
 * Decoupled LLM request format - stable across DeepSeek CLI versions
 */
...
/**
 * Decoupled LLM response format - stable across DeepSeek CLI versions
 */
...
/**
 * Decoupled tool configuration - stable across DeepSeek CLI versions
 */
...
export class HookTranslatorGenAIv1 extends HookTranslator {
  ...
  toHookLLMRequest(sdkRequest: GenerateContentParameters): LLMRequest {
    ...
    return {
      model: sdkRequest.model || DEFAULT_DEEPSEEK_FLASH_MODEL,
      messages,
      config: {
        temperature: config?.temperature,
        maxOutputTokens: config?.maxOutputTokens,
