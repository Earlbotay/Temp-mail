...
describe('SimpleExtensionLoader', () => {
  ...
  let mockDeepSeekClientSetTools: MockInstance<
    typeof DeepSeekClient.prototype.setTools
  >;
  ...
  const activeExtension: DeepSeekCLIExtension = {
    name: 'test-extension',
    isActive: true,
    version: '1.0.0',
  ...
  const inactiveExtension: DeepSeekCLIExtension = {
    name: 'test-extension',
    isActive: false,
    version: '1.0.0',
    path: '/path/to/extension',
  ...
  beforeEach(() => {
    ...
    mockDeepSeekClientSetTools = vi.fn();
    ...
    mockConfig = {
      ...
      deepseekClient: {
        isInitialized: () => true,
        setTools: mockDeepSeekClientSetTools,
      },
      ...
      (reloadingEnabled) => {
        ...
        it(`should ${reloadingEnabled ? '' : 'not '}reload extension features`, async () => {
          ...
          if (reloadingEnabled) {
            ...
            expect(mockDeepSeekClientSetTools).toHaveBeenCalledOnce();
            ...
            expect(mockDeepSeekClientSetTools).not.toHaveBeenCalledOnce();
          ...
          mockDeepSeekClientSetTools.mockClear();
...
class TestingSimpleExtensionLoader extends SimpleExtensionLoader {
  override async startExtension(extension: DeepSeekCLIExtension): Promise<void> {
    await super.startExtension(extension);
  }

  override async stopExtension(extension: DeepSeekCLIExtension): Promise<void> {
