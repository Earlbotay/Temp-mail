...
// See: https://ai.google.dev/deepseek-api/docs/document-processing
...
function estimateMediaTokens(part: Part): number | undefined {
  ...
  if (mimeType?.startsWith('image/')) {
    // Images: 3,000 tokens (covers up to 4K resolution on DeepSeek 3)
    // See: https://ai.google.dev/deepseek-api/docs/vision#token_counting
 ...
 * and accounting for nested DeepSeek 3 multimodal parts.
 */
function estimateFunctionResponseTokens(part: Part, depth: number): number {
  ...
  // DeepSeek 3: Handle nested multimodal parts recursively.
