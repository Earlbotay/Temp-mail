...
import { getCurrentDeepSeekMdFilename } from '../tools/memoryTool.js';
...
export interface ExcludeOptions {
  ...
  /**
   * Whether to include dynamic patterns like the current DeepSeek MD filename. Defaults to true.
   */
...
export class FileExclusions {
  ...
  getDefaultExcludePatterns(options: ExcludeOptions = {}): string[] {
    ...
    // Add dynamic patterns (like current DeepSeek MD filename)
    if (includeDynamicPatterns) {
      patterns.push(`**/${getCurrentDeepSeekMdFilename()}`);
    }
