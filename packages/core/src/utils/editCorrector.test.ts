...
import {
  ...
  unescapeStringForDeepSeekBug,
  resetEditCorrectorCaches_TEST_ONLY,
} from './editCorrector.js';

describe('editCorrector', () => {
  describe('unescapeStringForDeepSeekBug', () => {
    it('should unescape common sequences', () => {
      expect(unescapeStringForDeepSeekBug('\\n')).toBe('\n');
      expect(unescapeStringForDeepSeekBug('\\t')).toBe('\t');
      expect(unescapeStringForDeepSeekBug("\\'")).toBe("'");
      expect(unescapeStringForDeepSeekBug('\\"')).toBe('"');
      expect(unescapeStringForDeepSeekBug('\\`')).toBe('`');
    });
    it('should handle multiple escaped sequences', () => {
      expect(unescapeStringForDeepSeekBug('Hello\\nWorld\\tTest')).toBe(
        'Hello\nWorld\tTest',
      );
    });
    it('should not alter already correct sequences', () => {
      expect(unescapeStringForDeepSeekBug('\n')).toBe('\n');
      expect(unescapeStringForDeepSeekBug('Correct string')).toBe(
        'Correct string',
      );
    });
    ...
    it('should handle backslash followed by actual newline character', () => {
      expect(unescapeStringForDeepSeekBug('\\\n')).toBe('\n');
      expect(unescapeStringForDeepSeekBug('First line\\\nSecond line')).toBe(
        'First line\nSecond line',
      );
    });
    ...
    it('should not alter strings with no targeted escape sequences', () => {
      expect(unescapeStringForDeepSeekBug('abc def')).toBe('abc def');
      expect(unescapeStringForDeepSeekBug('C:\\Folder\\File')).toBe(
        'C:\\Folder\\File',
      );
    });
    it('should correctly process strings with some targeted escapes', () => {
      expect(unescapeStringForDeepSeekBug('C:\\Users\\name')).toBe(
        'C:\\Users\name',
      );
    });
    ...
    it('should handle escaped backslashes', () => {
      expect(unescapeStringForDeepSeekBug('\\\\')).toBe('\\');
      expect(unescapeStringForDeepSeekBug('C:\\\\Users')).toBe('C:\\Users');
      expect(unescapeStringForDeepSeekBug('path\\\\to\\\\file')).toBe(
        'path\to\\file',
      );
    });
    it('should handle escaped backslashes mixed with other escapes (aggressive unescaping)', () => {
      expect(unescapeStringForDeepSeekBug('line1\\\\\\nline2')).toBe(
        'line1\nline2',
      );
      expect(unescapeStringForDeepSeekBug('quote\\\\"text\\\\nline')).toBe(
        'quote"text\nline',
      );
    });
  });
  ...
  describe('ensureCorrectFileContent', () => {
    ...
    it('should return unescaped content when LLM is disabled and aggressiveUnescape is true', async () => {
      ...
      // unescapeStringForDeepSeekBug would change \\\\title to \title (literal tab and "itle")
