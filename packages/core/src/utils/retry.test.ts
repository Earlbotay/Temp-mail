...
import { PREVIEW_DEEPSEEK_MODEL } from '../config/models.js';
...
describe('retryWithBackoff', () => {
  ...
  describe('Flash model fallback for OAuth users', () => {
    it('should trigger fallback for OAuth personal users on TerminalQuotaError', async () => {
      const fallbackCallback = vi.fn().mockResolvedValue('deepseek-2.5-flash');
    ...
    it.each([[AuthType.USE_DEEPSEEK], [AuthType.USE_VERTEX_AI], [undefined]])(
  ...
  it('should trigger fallback for OAuth personal users on ModelNotFoundError', async () => {
    const fallbackCallback = vi.fn().mockResolvedValue(PREVIEW_DEEPSEEK_MODEL);
