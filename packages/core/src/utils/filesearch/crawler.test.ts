...
import { createTmpDir, cleanupTmpDir } from '@google/deepseek-cli-test-utils';
...
describe('crawler', () => {
  ...
  it('should use .deepseekignore rules', async () => {
    tmpDir = await createTmpDir({
      [DEEPSEEK_IGNORE_FILE_NAME]: 'dist/',
      dist: ['ignored.js'],
      src: ['not-ignored.js'],
    });
  ...
  it('should combine .gitignore and .deepseekignore rules', async () => {
    tmpDir = await createTmpDir({
      '.git': {},
      '.gitignore': 'dist/',
      [DEEPSEEK_IGNORE_FILE_NAME]: 'build/',
      dist: ['ignored-by-git.js'],
      build: ['ignored-by-deepseek.js'],
      src: ['not-ignored.js'],
    });
