...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .deepseekignore', async () => {
    tmpDir = await createTmpDir({
      [DEEPSEEK_IGNORE_FILE_NAME]: '*.log',
    });
    const service = new FileDiscoveryService(tmpDir, {
  ...
  it('should combine rules from .gitignore and .deepseekignore', async () => {
    tmpDir = await createTmpDir({
      '.git': {},
      '.gitignore': '*.log',
      [DEEPSEEK_IGNORE_FILE_NAME]: '*.txt',
    });
    const service = new FileDiscoveryService(tmpDir, {
      respectGitIgnore: true,
      respectDeepSeekIgnore: true,
