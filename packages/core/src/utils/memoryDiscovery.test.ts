...
import {
  setDeepSeekMdFilename,
  DEFAULT_CONTEXT_FILENAME,
} from '../tools/memoryTool.js';
import { flattenMemory, type HierarchicalMemory } from '../config/memory.js';
import { FileDiscoveryService } from '../services/fileDiscoveryService.js';
import { DEEPSEEK_DIR, normalizePath, homedir as pathsHomedir } from './paths.js';
...
import { Config, type DeepSeekCLIExtension } from '../config/config.js';
...
describe('memoryDiscovery', () => {
  ...
  afterEach(async () => {
    ...
    setDeepSeekMdFilename(DEFAULT_CONTEXT_FILENAME);
  ...
  it('should load only the global context file if present and others are not (default filename)', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, DEEPSEEK_DIR, DEFAULT_CONTEXT_FILENAME),
      'default context content',
    );
...
--- End of Context from: ${defaultContextFile} ---`,
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    setDeepSeekMdFilename(customFilename);

    const customContextFile = await createTestFile(
      path.join(homedir, DEEPSEEK_DIR, customFilename),
...
--- End of Context from: ${subdirCustomFile} ---`,
  ...
  it('should load ORIGINAL_DEEPSEEK_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootDeepSeekFile = await createTestFile(
      path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
      'Project root memory',
    );
    const srcDeepSeekFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'Src directory memory',
    );
    ...
    expect(result).toEqual({
      memoryContent: `--- Project ---
--- Context from: ${projectRootDeepSeekFile} ---
Project root memory
--- End of Context from: ${projectRootDeepSeekFile} ---

--- Context from: ${srcDeepSeekFile} ---
Src directory memory
--- End of Context from: ${srcDeepSeekFile} ---`,
      fileCount: 2,
      filePaths: [projectRootDeepSeekFile, srcDeepSeekFile],
    });
  });

  it('should load ORIGINAL_DEEPSEEK_MD_FILENAME files by downward traversal from CWD', async () => {
    const subDirDeepSeekFile = await createTestFile(
      path.join(cwd, 'subdir', DEFAULT_CONTEXT_FILENAME),
      'Subdir memory',
    );
    const cwdDeepSeekFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'CWD memory',
    );
    ...
    expect(result).toEqual({
      memoryContent: `--- Project ---
--- Context from: ${cwdDeepSeekFile} ---
CWD memory
--- End of Context from: ${cwdDeepSeekFile} ---

--- Context from: ${subDirDeepSeekFile} ---
Subdir memory
--- End of Context from: ${subDirDeepSeekFile} ---`,
      fileCount: 2,
      filePaths: [cwdDeepSeekFile, subDirDeepSeekFile],
    });
  });

  it('should load and correctly order global, upward, and downward ORIGINAL_DEEPSEEK_MD_FILENAME files', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, DEEPSEEK_DIR, DEFAULT_CONTEXT_FILENAME),
      'default context content',
    );
    const rootDeepSeekFile = await createTestFile(
...
--- Project ---
--- Context from: ${rootDeepSeekFile} ---
Project parent memory
--- End of Context from: ${rootDeepSeekFile} ---
...
--- End of Context from: ${subDirDeepSeekFile} ---`,
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const regularSubDirDeepSeekFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Project ---
--- Context from: ${regularSubDirDeepSeekFile} ---
My code memory
--- End of Context from: ${regularSubDirDeepSeekFile} ---`,
      fileCount: 1,
      filePaths: [regularSubDirDeepSeekFile],
    });
  });
  ...
  it('should load extension context file paths', async () => {
    const extensionFilePath = await createTestFile(
      path.join(testRootDir, 'extensions/ext1/Deepseek.md'),
...
--- End of Context from: ${includedFile} ---`,
  ...
  it('should handle multiple directories and files in parallel correctly', async () => {
    // Create multiple test directories with Deepseek.md files
    const numDirs = 5;
  ...
  describe('getExtensionMemoryPaths', () => {
    it('should return active extension context files', async () => {
      const extFile = await createTestFile(
        path.join(testRootDir, 'ext', 'Deepseek.md'),
        'Extension content',
      );
  ...
  describe('getEnvironmentMemoryPaths', () => {
    ...
    it('should fall back to trusted root as ceiling when no .git exists', async () => {
      ...
      // notesDir has no Deepseek.md and won't traverse up to docsDir.
      const resultNotes = await getEnvironmentMemoryPaths([notesDir]);
      expect(resultNotes).toHaveLength(0);

      // docsDir has a Deepseek.md at the trusted root itself, so it's found.
    ...
    it('should keep multiple memory files from the same directory adjacent and in order', async () => {
      ...
      setDeepSeekMdFilename(['PRIMARY.md', 'SECONDARY.md']);
  ...
  describe('case-insensitive filesystem deduplication', () => {
    it('should deduplicate files that point to the same inode (same physical file)', async () => {
      const deepseekFile = await createTestFile(
        path.join(projectRoot, 'deepseek.md'),
        'Project root memory',
      );

      // create hard link to simulate case-insensitive filesystem behavior
      const deepseekFileLink = path.join(projectRoot, 'Deepseek.md');
      try {
        await fsPromises.link(deepseekFile, deepseekFileLink);
      } catch (error) {
        const errorMessage =
      ...
      const stats1 = await fsPromises.lstat(deepseekFile);
      const stats2 = await fsPromises.lstat(deepseekFileLink);
      expect(stats1.ino).toBe(stats2.ino);
      expect(stats1.dev).toBe(stats2.dev);

      setDeepSeekMdFilename(['Deepseek.md', 'deepseek.md']);
      ...
      try {
        await fsPromises.unlink(deepseekFileLink);
      } catch {
    ...
    it('should handle case where files have different inodes (different files)', async () => {
      const deepseekFileLower = await createTestFile(
        path.join(projectRoot, 'deepseek.md'),
        'Lowercase file content',
      );
      const deepseekFileUpper = await createTestFile(
        path.join(projectRoot, 'Deepseek.md'),
        'Uppercase file content',
      );

      const stats1 = await fsPromises.lstat(deepseekFileLower);
      const stats2 = await fsPromises.lstat(deepseekFileUpper);
    ...
    it("should handle files that cannot be stat'd (missing files)", async () => {
      ...
      setDeepSeekMdFilename(['deepseek.md', 'missing.md']);
    ...
    it('should deduplicate multiple paths pointing to same file (3+ duplicates)', async () => {
      ...
      const link1 = path.join(projectRoot, 'Deepseek.md');
      const link2 = path.join(projectRoot, 'DeepSeek.md');

      try {
        await fsPromises.link(deepseekFile, link1);
        await fsPromises.link(deepseekFile, link2);
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        if (
      ...
      setDeepSeekMdFilename(['deepseek.md', 'Deepseek.md', 'DeepSeek.md']);
  ...
  describe('loadJitSubdirectoryMemory', () => {
    ...
    it('should deduplicate files in JIT memory loading (same inode)', async () => {
      ...
      const deepseekFile = await createTestFile(
        path.join(subDir, 'deepseek.md'),
        'JIT memory content',
      );

      const deepseekFileLink = path.join(subDir, 'Deepseek.md');
      try {
        await fsPromises.link(deepseekFile, deepseekFileLink);
      } catch (error) {
        const errorMessage =
    ...
    it('should resolve file target to its parent directory for traversal', async () => {
      ...
      // Should find the Deepseek.md in the same directory as the file
      expect(result.files).toHaveLength(1);
      expect(result.files[0].path).toBe(subDirMemory);
    ...
    it('should fall back to trusted root as ceiling when no git root exists', async () => {
      ...
      // subDir is within the trusted root, so its Deepseek.md is found
      expect(result.files).toHaveLength(1);
      expect(result.files[0].path).toBe(subDirMemory);
  ...
  it('refreshServerHierarchicalMemory should refresh memory and update config', async () => {
    ...
    expect(config.getDeepSeekMdFileCount()).equals(refreshResult.fileCount);
    ...
    expect(config.getDeepSeekMdFilePaths()).equals(refreshResult.filePaths);
  ...
  it('should include MCP instructions in user memory', async () => {
    const mockConfig = {
      ...
      setUserMemory: vi.fn(),
      setDeepSeekMdFileCount: vi.fn(),
      setDeepSeekMdFilePaths: vi.fn(),
