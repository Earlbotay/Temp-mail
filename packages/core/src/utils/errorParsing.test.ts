...
import { DEFAULT_DEEPSEEK_FLASH_MODEL } from '../config/models.js';
import { AuthType } from '../core/contentGenerator.js';
import type { StructuredError } from '../core/turn.js';

describe('parseAndFormatApiError', () => {
  const vertexMessage = 'request a quota increase through Vertex';
  const deepseekMessage = 'request a quota increase through AI Studio';
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the deepseek-2.5-flash model',
    );
  });
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "DeepSeek 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.google.dev/deepseek-api/docs/rate-limits.",
    ...
    const result = parseAndFormatApiError(errorMessage, AuthType.USE_DEEPSEEK);
    expect(result).toContain('DeepSeek 2.5 Pro Preview');
    expect(result).toContain(deepseekMessage);
  });
