...
import { DEEPSEEK_DIR } from './paths.js';
import { DEEPSEEK_IGNORE_FILE_NAME } from 'src/config/constants.js';
...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile(DEEPSEEK_DIR, 'config.yaml');
      await createTestFile(DEEPSEEK_DIR, 'logs.json');
  ...
  describe('with deepseekignore', () => {
    it('should ignore deepseekignore files by default', async () => {
      await fsPromises.writeFile(
    ...
    it('should not ignore files if respectDeepSeekIgnore is false', async () => {
      await fsPromises.writeFile(
        path.join(testRootDir, DEEPSEEK_IGNORE_FILE_NAME),
        'ignored.txt\nnode_modules/\n.deepseek/\n!/.deepseek/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectDeepSeekIgnore: false,
          respectGitIgnore: true, // Explicitly disable deepseek ignore only
          customIgnoreFilePaths: [],
        },
      });
      expect(structure).toContain('ignored.txt');
