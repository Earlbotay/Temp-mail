...
import type { Config, DeepSeekCLIExtension } from '../config/config.js';
...
export abstract class ExtensionLoader {
   ...
   */
  abstract getExtensions(): DeepSeekCLIExtension[];

  /**
   * Fully initializes all active extensions.
   *
   * Called within `Config.initialize`, which must already have an
   * McpClientManager, PromptRegistry, and DeepSeekChat set up.
   */
   ...
   */
  protected async startExtension(extension: DeepSeekCLIExtension) {
    if (!this.config) {
    ...
    try {
      ...
      await this.maybeRefreshDeepSeekTools(extension);
  ...
  private async maybeRefreshMemories(): Promise<void> {
    if (!this.config) {
      throw new Error(
        'Cannot refresh deepseek memories prior to calling `start`.',
      );
    }
    if (
  ...
  /**
   * Refreshes the deepseek tools list if it is initialized and the extension has
   * any excludeTools settings.
   */
  private async maybeRefreshDeepSeekTools(
    extension: DeepSeekCLIExtension,
  ): Promise<void> {
    if (extension.excludeTools && extension.excludeTools.length > 0) {
      const deepseekClient = this.config?.deepseekClient;
      if (deepseekClient?.isInitialized()) {
        await deepseekClient.setTools();
      }
    }
  }
   ...
   * initialized.
   */
  protected async stopExtension(extension: DeepSeekCLIExtension) {
    if (!this.config) {
  ...
  async restartExtension(extension: DeepSeekCLIExtension): Promise<void> {
    await this.stopExtension(extension);
    await this.startExtension(extension);
  }
}
...
export class SimpleExtensionLoader extends ExtensionLoader {
  constructor(
    protected readonly extensions: DeepSeekCLIExtension[],
  ...
  async loadExtension(extension: DeepSeekCLIExtension) {
  ...
  async unloadExtension(extension: DeepSeekCLIExtension) {
