...
import { getAllDeepSeekMdFilenames } from '../tools/memoryTool.js';
...
import { DEEPSEEK_DIR, homedir, normalizePath, isSubpath } from './paths.js';
...
export interface DeepSeekFileContent {
  filePath: string;
  content: string | null;
}
...
async function getDeepSeekMdFilePathsInternal(
  currentWorkingDirectory: string,
  includeDirectoriesToReadDeepSeek: readonly string[],
  userHomePath: string,
  ...
  const dirs = new Set<string>([
    ...includeDirectoriesToReadDeepSeek,
  ...
  for (let i = 0; i < dirsArray.length; i += CONCURRENT_LIMIT) {
    ...
    const batchPromises = batch.map((dir) =>
      getDeepSeekMdFilePathsInternalForEachDir(
        dir,
        userHomePath,
        fileService,
        folderTrust,
...
async function getDeepSeekMdFilePathsInternalForEachDir(
  dir: string,
  userHomePath: string,
  fileService: FileDiscoveryService,
  folderTrust: boolean,
  ...
  const globalPaths = new Set<string>();
  const projectPaths = new Set<string>();
  const deepseekMdFilenames = getAllDeepSeekMdFilenames();

  for (const deepseekMdFilename of deepseekMdFilenames) {
    const resolvedHome = normalizePath(userHomePath);
    const globalDeepSeekDir = normalizePath(path.join(resolvedHome, DEEPSEEK_DIR));
    const globalMemoryPath = normalizePath(
      path.join(globalDeepSeekDir, deepseekMdFilename),
    );
    ...
    if (dir && folderTrust) {
      ...
      while (
        ...
        if (currentDir === globalDeepSeekDir) {
          break;
        }

        const potentialPath = normalizePath(
          path.join(currentDir, deepseekMdFilename),
        );
        try {
...
export async function readDeepSeekMdFiles(
  filePaths: string[],
  importFormat: 'flat' | 'tree' = 'tree',
  boundaryMarkers: readonly string[] = ['.git'],
): Promise<DeepSeekFileContent[]> {
  ...
  const results: DeepSeekFileContent[] = [];

  for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
    const batch = filePaths.slice(i, i + CONCURRENT_LIMIT);
    const batchPromises = batch.map(
      async (filePath): Promise<DeepSeekFileContent> => {
        try {
          ...
          if (!isTestEnv) {
            ...
            logger.warn(
              `Warning: Could not read ${getAllDeepSeekMdFilenames()} file at ${filePath}. Error: ${message}`,
            );
          }
          debugLogger.debug(
...
export function concatenateInstructions(
  instructionContents: DeepSeekFileContent[],
): string {
...
export async function getGlobalMemoryPaths(): Promise<string[]> {
  ...
  const accessChecks = deepseekMdFilenames.map(async (filename) => {
    const globalPath = normalizePath(path.join(userHome, DEEPSEEK_DIR, filename));
    try {
...
export async function getEnvironmentMemoryPaths(
  ...
  const traversalPromises = trustedRoots.map(async (root) => {
    ...
    return findUpwardDeepSeekFiles(resolvedRoot, ceiling);
  });
...
export function categorizeAndConcatenate(
  ...
  contentsMap: Map<string, DeepSeekFileContent>,
): HierarchicalMemory {
  const getConcatenated = (pList: string[]) =>
    concatenateInstructions(
      pList
        .map((p) => contentsMap.get(p))
        .filter((c): c is DeepSeekFileContent => !!c),
    );
...
/**
 * Traverses upward from startDir to stopDir, finding all Deepseek.md variants.
 *
 * Files are ordered by directory level (root to leaf), with all filename
 * variants grouped together per directory.
 */
async function findUpwardDeepSeekFiles(
  startDir: string,
  stopDir: string,
): Promise<string[]> {
  const upwardPaths: string[] = [];
  ...
  const deepseekMdFilenames = getAllDeepSeekMdFilenames();
  const globalDeepSeekDir = normalizePath(path.join(homedir(), DEEPSEEK_DIR));
...
/**
 * Loads hierarchical Deepseek.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  const [discoveryResult, extensionPaths] = await Promise.all([
    getDeepSeekMdFilePathsInternal(
      currentWorkingDirectory,
      includeDirectoriesToReadDeepSeek,
      userHomePath,
      fileService,
  ...
  if (allFilePathsStringDeduped.length === 0) {
    debugLogger.debug(
      '[DEBUG] [MemoryDiscovery] No Deepseek.md files found in hierarchy of the workspace.',
    );
    return {
  ...
  if (allFilePaths.length === 0) {
    debugLogger.debug(
      '[DEBUG] [MemoryDiscovery] No unique Deepseek.md files found after deduplication by file identity.',
    );
    return {
      memoryContent: { global: '', extension: '', project: '' },
      fileCount: 0,
  ...
  const allContents = await readDeepSeekMdFiles(
    allFilePaths,
    importFormat,
    boundaryMarkers,
  );
 ...
 *
 * Returns the result of the call to `loadHierarchicalDeepSeekMemory`.
 */
export async function refreshServerHierarchicalMemory(config: Config) {
  ...
  config.setDeepSeekMdFileCount(result.fileCount);
  config.setDeepSeekMdFilePaths(result.filePaths);
...
export async function loadJitSubdirectoryMemory(
  ...
  // path like /app/src/file.ts/Deepseek.md.
  ...
  const potentialPaths = await findUpwardDeepSeekFiles(startDir, resolvedCeiling);
  ...
  const contents = await readDeepSeekMdFiles(newPaths, 'tree', boundaryMarkers);
