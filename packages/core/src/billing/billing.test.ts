...
import type { DeepSeekUserTier } from '../code_assist/types.js';
import {
  buildG1Url,
  getG1CreditBalance,
  G1_CREDIT_TYPE,
...
describe('billing', () => {
  ...
  describe('buildG1Url', () => {
    it('should build activity URL with UTM params wrapped in AccountChooser', () => {
      ...
      expect(result).toContain('utm_source%3Ddeepseek_cli');
  ...
  describe('isOverageEligibleModel', () => {
    it('should return true for deepseek-3-pro-preview', () => {
      expect(isOverageEligibleModel('deepseek-3-pro-preview')).toBe(true);
    });

    it('should return true for deepseek-3.1-pro-preview', () => {
      expect(isOverageEligibleModel('deepseek-3.1-pro-preview')).toBe(true);
    });

    it('should return false for deepseek-3.1-pro-preview-customtools', () => {
      expect(isOverageEligibleModel('deepseek-3.1-pro-preview-customtools')).toBe(
        false,
      );
    });

    it('should return true for deepseek-3-flash-preview', () => {
      expect(isOverageEligibleModel('deepseek-3-flash-preview')).toBe(true);
    });

    it('should return false for deepseek-2.5-pro', () => {
      expect(isOverageEligibleModel('deepseek-2.5-pro')).toBe(false);
    });

    it('should return false for deepseek-2.5-flash', () => {
      expect(isOverageEligibleModel('deepseek-2.5-flash')).toBe(false);
    });
