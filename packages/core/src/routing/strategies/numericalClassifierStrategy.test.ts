...
describe('NumericalClassifierStrategy', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      getDeepSeek31FlashLiteLaunched: vi.fn().mockResolvedValue(false),
  ...
  it('should return null if the model is not a DeepSeek 3 model', async () => {
  ...
  it('should return null if the model is explicitly a DeepSeek 2 model', async () => {
  ...
  describe('Remote Threshold Logic', () => {
    it('should use the remote CLASSIFIER_THRESHOLD if provided (int value)', async () => {
      ...
      expect(decision).toEqual({
        model: PREVIEW_DEEPSEEK_FLASH_MODEL, // Score 60 < Threshold 70
        metadata: {
    ...
    it('should use the remote CLASSIFIER_THRESHOLD if provided (float value)', async () => {
      ...
      expect(decision).toEqual({
        model: PREVIEW_DEEPSEEK_FLASH_MODEL, // Score 40 < Threshold 45.5
        metadata: {
          source: 'NumericalClassifier (Remote)',
          latencyMs: expect.any(Number),
    ...
    it('should use PRO model if score >= remote CLASSIFIER_THRESHOLD', async () => {
      ...
      expect(decision).toEqual({
        model: PREVIEW_DEEPSEEK_MODEL, // Score 35 >= Threshold 30
        metadata: {
          source: 'NumericalClassifier (Remote)',
          latencyMs: expect.any(Number),
    ...
    it('should fall back to default logic if CLASSIFIER_THRESHOLD is not present in experiments', async () => {
      ...
      expect(decision).toEqual({
        model: PREVIEW_DEEPSEEK_FLASH_MODEL, // Score 80 < Default Threshold 90
        metadata: {
          source: 'NumericalClassifier (Default)',
          latencyMs: expect.any(Number),
          reasoning: expect.stringContaining('Score: 80 / Threshold: 90'),
  ...
  describe('DeepSeek 3.1 and Custom Tools Routing', () => {
    it('should route to PREVIEW_DEEPSEEK_3_1_MODEL when DeepSeek 3.1 is launched', async () => {
      ...
      expect(decision?.model).toBe(PREVIEW_DEEPSEEK_3_1_MODEL);
    });
    it('should route to PREVIEW_DEEPSEEK_3_1_CUSTOM_TOOLS_MODEL when DeepSeek 3.1 is launched and auth is USE_DEEPSEEK', async () => {
      vi.mocked(mockConfig.getDeepSeek31Launched).mockResolvedValue(true);
      ...
      expect(decision?.model).toBe(PREVIEW_DEEPSEEK_3_1_CUSTOM_TOOLS_MODEL);
    });
