...
const createMockConfig = (overrides: Partial<Config> = {}): Config =>
  ({
    ...
    getModel: vi.fn().mockReturnValue(DEFAULT_DEEPSEEK_MODEL),
    ...overrides,
...
describe('FallbackStrategy', () => {
  ...
  it('should return null if the requested model is available', async () => {
    ...
    // Should check availability of the resolved model (DEFAULT_DEEPSEEK_MODEL)
    expect(mockService.snapshot).toHaveBeenCalledWith(DEFAULT_DEEPSEEK_MODEL);
  });
  ...
  it('should return fallback decision if model is unavailable and fallback found', async () => {
    ...
    vi.mocked(selectModelForAvailability).mockReturnValue({
      selectedModel: DEFAULT_DEEPSEEK_FLASH_MODEL,
      skipped: [{ model: DEFAULT_DEEPSEEK_MODEL, reason: 'quota' }],
    });
    ...
    expect(decision?.metadata.reasoning).toContain(
      `Model ${DEFAULT_DEEPSEEK_MODEL} is unavailable`,
    );
  });
