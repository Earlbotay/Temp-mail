...
import { resolveClassifierModel, isDeepSeek3Model } from '../../config/models.js';
...
export class NumericalClassifierStrategy implements RoutingStrategy {
  ...
  async route(
    ...
    try {
      ...
      if (!isDeepSeek3Model(model, config)) {
        return null;
      }
      ...
      const [useDeepSeek3_1, useDeepSeek3_1FlashLite, useCustomToolModel] =
        await Promise.all([
          config.getDeepSeek31Launched(),
          config.getDeepSeek31FlashLiteLaunched(),
