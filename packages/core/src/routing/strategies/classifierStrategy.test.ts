...
describe('ClassifierStrategy', () => {
  ...
  it('should return null if numerical routing is enabled and model is DeepSeek 3', async () => {
  ...
  it('should NOT return null if numerical routing is enabled but model is NOT DeepSeek 3', async () => {
    vi.mocked(mockConfig.getNumericalRoutingEnabled).mockResolvedValue(true);
  ...
  it('should respect requestedModel from context in resolveClassifierModel', async () => {
    const requestedModel = DEFAULT_DEEPSEEK_MODEL; // Pro model
    const mockApiResponse = {
    ...
    expect(decision?.model).toBe(DEFAULT_DEEPSEEK_FLASH_MODEL);
  });
