...
/**
 * Compiles the DeepSeekSandbox C# helper on Windows.
...
function compileWindowsSandbox() {
  ...
  const srcHelperPath = path.resolve(
    __dirname,
    '../src/sandbox/windows/DeepSeekSandbox.exe',
  );
  const distHelperPath = path.resolve(
    __dirname,
    '../dist/src/sandbox/windows/DeepSeekSandbox.exe',
  );
  const sourcePath = path.resolve(
    __dirname,
    '../src/sandbox/windows/DeepSeekSandbox.cs',
  );
  ...
  if (result.status === 0) {
    console.log('Successfully compiled DeepSeekSandbox.exe to src');
    ...
    if (fs.existsSync(distDir)) {
      ...
      console.log('Successfully copied DeepSeekSandbox.exe to dist');
    }
  } else {
